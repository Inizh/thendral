...
void ImGui_ImplOpenGL3_UpdateTexture(ImTextureData* tex) {
  ...
  if (tex->Status == ImTextureStatus_WantCreate) {
    ...
    GL_CALL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, tex->Width, tex->Height, 0,
    ...
    for (ImTextureRect& r : tex->Updates)
      GL_CALL(glTexSubImage2D(GL_TEXTURE_2D, 0, r.x, r.y, r.w, r.h, GL_RGBA,
                              GL_UNSIGNED_BYTE, tex->GetPixelsAt(r.x, r.y)));
    GL_CALL(glPixelStorei(GL_UNPACK_ROW_LENGTH, 0));
#else
    ...
    for (ImTextureRect& r : tex->Updates) {
      ...
      GL_CALL(glTexSubImage2D(GL_TEXTURE_2D, 0, r.x, r.y, r.w, r.h, GL_RGBA,
                              GL_UNSIGNED_BYTE, bd->TempBuffer.Data));
    }
#endif
    tex->SetStatus(ImTextureStatus_OK);
