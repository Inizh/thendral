...
static void ImGui_ImplDX9_CopyTextureRegion(bool tex_use_colors,
  ...
  for (int y = 0; y < h; y++) {
    const ImU32* src_p =
        (const ImU32*)(const void*)((const unsigned char*)src + src_pitch * y);
...
void ImGui_ImplDX9_UpdateTexture(ImTextureData* tex) {
  ...
  if (tex->Status == ImTextureStatus_WantCreate) {
    ...
    HRESULT hr = bd->pd3dDevice->CreateTexture(
        ...
        bd->HasRgbaSupport ? D3DFMT_A8B8G8R8 : D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT,
        &dx_tex, nullptr);
    if (hr < 0) {
    ...
    if (dx_tex->LockRect(0, &locked_rect, nullptr, 0) == D3D_OK) {
      ImGui_ImplDX9_CopyTextureRegion(tex->UseColors, (ImU32*)tex->GetPixels(),
                                      tex->Width * 4, (ImU32*)locked_rect.pBits,
                                      (ImU32)locked_rect.Pitch, tex->Width,
                                      tex->Height);
      dx_tex->UnlockRect(0);
    }
    ...
    if (backend_tex->LockRect(0, &locked_rect, &update_rect, 0) == D3D_OK)
      for (ImTextureRect& r : tex->Updates)
        ImGui_ImplDX9_CopyTextureRegion(
            tex->UseColors, (ImU32*)tex->GetPixelsAt(r.x, r.y), tex->Width * 4,
