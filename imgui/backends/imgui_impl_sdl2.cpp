...
//  window is hovered, as the SDL_MOUSEMOTION data is reliable. Fix macOS notch
//  mouse coordinates issue in fullscreen mode + better perf on X11. (#7919,
//  #7786) 2025-09-18: Call platform_io.ClearPlatformHandlers() on shutdown.
//  2025-09-15: Content Scales are always reported as 1.0 on Wayland. (#8921)
//  2025-07-08: Made ImGui_ImplSDL2_GetContentScaleForWindow(),
...
//  facilitate making DPI-aware apps. 2025-04-09: Don't attempt to call
//  SDL_CaptureMouse() on drivers where we don't call SDL_GetGlobalMouseState().
//  (#8561) 2025-03-21: Fill gamepad inputs and set ImGuiBackendFlags_HasGamepad
...
//  Made ImGui_ImplSDL2_SetGamepadMode(ImGui_ImplSDL2_GamepadMode_Manual) accept
//  an empty array. 2024-10-24: Emscripten: from SDL 2.30.9,
//  SDL_EVENT_MOUSE_WHEEL event doesn't require dividing by 100.0f. 2024-09-09:
//  use SDL_Vulkan_GetDrawableSize() when available. (#7967, #3190) 2024-08-22:
...
bool ImGui_ImplSDL2_ProcessEvent(const SDL_Event* event) {
	...
	switch (event->type) {
		...
		case SDL_KEYDOWN:
		case SDL_KEYUP: {
			if (ImGui_ImplSDL2_GetViewportForWindowID(event->key.windowID) ==
				nullptr)
				return false;
			ImGui_ImplSDL2_UpdateKeyModifiers(
				(SDL_Keymod)event->key.keysym.mod);
			// IMGUI_DEBUG_LOG("SDL_KEY_%s : key=%d ('%s'), scancode=%d ('%s'),
			// mod=%X\n",
		...
		case SDL_WINDOWEVENT: {
			if (ImGui_ImplSDL2_GetViewportForWindowID(event->window.windowID) ==
				nullptr)
				return false;
