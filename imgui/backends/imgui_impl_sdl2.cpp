...
bool ImGui_ImplSDL2_ProcessEvent(const SDL_Event* event) {
  ...
  switch (event->type) {
    ...
    case SDL_WINDOWEVENT: {
      ...
      // LEAVE/ENTER event on every mouse move, but the final ENTER tends to be
      // right.
      ...
      if (window_event == SDL_WINDOWEVENT_FOCUS_LOST) io.AddFocusEvent(false);
      return true;
    }
...
static void ImGui_ImplSDL2_UpdateMouseData() {
  ...
  if (is_app_focused) {
    ...
    // not hovered (SDL_MOUSEMOTION already provides this when hovered or
...
static void ImGui_ImplSDL2_GetWindowSizeAndFramebufferScale(
  ...
  if (out_framebuffer_scale != nullptr)
    *out_framebuffer_scale =
        (w > 0 && h > 0) ? ImVec2((float)display_w / w, (float)display_h / h)
                         : ImVec2(1.0f, 1.0f);
}
