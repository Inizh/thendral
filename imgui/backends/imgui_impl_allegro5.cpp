...
void ImGui_ImplAllegro5_UpdateTexture(ImTextureData* tex) {
  if (tex->Status == ImTextureStatus_WantCreate) {
    ...
    // Upload pixels
    ALLEGRO_LOCKED_REGION* locked_region = al_lock_bitmap(
        cpu_bitmap, al_get_bitmap_format(cpu_bitmap), ALLEGRO_LOCK_WRITEONLY);
    ...
    ImTextureRect r =
        tex->UpdateRect;  // Bounding box encompassing all individual updates
...
void ImGui_ImplAllegro5_SetDisplay(ALLEGRO_DISPLAY* display) {
  ...
  if (bd->Display && !bd->VertexDecl) {
    ...
    // Unfortunately Allegro doesn't support 32-bits packed colors so we have to
    // convert them to 4 floats. We still use a custom declaration to use
    // 'ALLEGRO_PRIM_TEX_COORD' instead of 'ALLEGRO_PRIM_TEX_COORD_PIXEL' else
