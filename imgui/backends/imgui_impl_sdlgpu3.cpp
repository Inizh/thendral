...
void ImGui_ImplSDLGPU3_PrepareDrawData(ImDrawData* draw_data,
  ...
  SDL_UploadToGPUBuffer(copy_pass, &index_buffer_location, &index_buffer_region,
                        true);
...
void ImGui_ImplSDLGPU3_RenderDrawData(ImDrawData* draw_data,
  ...
  ImGui_ImplSDLGPU3_SetupRenderState(draw_data, &render_state, pipeline,
                                     command_buffer, render_pass, fd, fb_width,
                                     fb_height);
  ...
  for (const ImDrawList* draw_list : draw_data->CmdLists) {
    for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback != nullptr) {
        ...
        SDL_BindGPUFragmentSamplers(render_pass, 0, &texture_sampler_binding,
                                    1);

        // Draw
        // **IF YOU GET A CRASH HERE** In 1.92.2 on 2025/08/08 we have changed
        // ImTextureID to store 'SDL_GPUTexture*' instead of storing
        // 'SDL_GPUTextureSamplerBinding'. Any code loading custom texture using
        // this backend needs to be updated.
  ...
  // Note: at this point both SDL_SetGPUViewport() and SDL_SetGPUScissor() have
  // been called. Our last values will leak into user/application rendering if
  // you forgot to call SDL_SetGPUViewport() and SDL_SetGPUScissor() yourself to
  // explicitly set that state In theory we should aim to backup/restore those
  // values but I am not sure this is possible. We perform a call to
  // SDL_SetGPUScissor() to set back a full viewport which is likely to fix
  // things for 99% users but technically this is not perfect. (See github
  // #4644)
...
void ImGui_ImplSDLGPU3_UpdateTexture(ImTextureData* tex) {
  ...
  if (tex->Status == ImTextureStatus_WantCreate ||
    ...
    {
      void* texture_ptr =
          SDL_MapGPUTransferBuffer(v->Device, bd->TexTransferBuffer, true);
