...
// CHANGELOG
//  2025-11-26: macOS version can use MSL shaders in order to support
//  macOS 10.14+ (vs Metallib shaders requiring macOS 14+). Requires calling
//  SDL_CreateGPUDevice() with SDL_GPU_SHADERFORMAT_MSL. 2025-09-18: Call
//  platform_io.ClearRendererHandlers() on shutdown. 2025-08-20: Added
//  ImGui_ImplSDLGPU3_InitInfo::SwapchainComposition and
	...
	if (tex->Status == ImTextureStatus_WantCreate ||
		...
		if (bd->TexTransferBufferSize < upload_size) {
			...
			IM_ASSERT(
				...
				"Failed to create transfer buffer, call SDL_GetError() for "
				"more information");
		}
...
static void ImGui_ImplSDLGPU3_CreateGraphicsPipeline() {
	...
	IM_ASSERT(
		bd->Pipeline != nullptr &&
		"Failed to create graphics pipeline, call SDL_GetError() for more "
		"information");
}
