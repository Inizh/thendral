  ...
  switch (event_type) {
    case AINPUT_EVENT_TYPE_KEY: {
      ...
      switch (event_action) {
        ...
        // as soon as a touch pointer goes up from a key. We use a simple key
        // event queue/ and process one event per key per frame in
        // ImGui_ImplAndroid_NewFrame()...or consider using IO queue, if
        // suitable: https://github.com/ocornut/imgui/issues/2787
    ...
    case AINPUT_EVENT_TYPE_MOTION: {
      ...
      switch (event_action) {
        ...
        case AMOTION_EVENT_ACTION_UP: {
          ...
          // process them separately to identify the actual button pressed. This
          // is done below via AMOTION_EVENT_ACTION_BUTTON_PRESS/_RELEASE. Here,
          // we only process "FINGER" input (and "UNKNOWN", as a fallback).
          int tool_type =
