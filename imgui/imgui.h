...
struct ImTextureRef {
  ...
  ImTextureData* _TexData;  //      A texture, generally owned by a ImFontAtlas.
                            //      Will convert to ImTextureID during render
                            //      loop, after texture has been uploaded.
  ImTextureID _TexID;  // _OR_ Low-level backend texture identifier, if already
                       // uploaded or created by user/app. Generally provided to
                       // e.g. ImGui::Image() calls.
};
IM_MSVC_RUNTIME_CHECKS_RESTORE
...
enum ImGuiChildFlags_ {
  ...
  ImGuiChildFlags_NavFlattened =
      1 << 8,  // [BETA] Share focus scope, allow keyboard/gamepad navigation to
...
enum ImGuiInputTextFlags_ {
  ...
  // - Wrapping width needs to always account for the possibility of a vertical
  // scrollbar.
...
enum ImGuiTreeNodeFlags_ {
  ...
  ImGuiTreeNodeFlags_OpenOnArrow =
      1 << 7,  // Open when clicking on the arrow part (default for multi-select
               // unless any _OpenOnXXX behavior is set explicitly). Both
               // behaviors may be combined.
  ...
  // [EXPERIMENTAL] Draw lines connecting TreeNode hierarchy. Discuss in GitHub
  // issue #2920. Default value is pulled from style.TreeLinesFlags. May be
  // overridden in TreeNode calls.
  ImGuiTreeNodeFlags_DrawLinesNone = 1 << 18,  // No lines drawn
  ImGuiTreeNodeFlags_DrawLinesFull =
      1 << 19,  // Horizontal lines to child nodes. Vertical line drawn down to
                // TreePop() position: cover full contents. Faster (for large
                // trees).
  ImGuiTreeNodeFlags_DrawLinesToNodes =
      1 << 20,  // Horizontal lines to child nodes. Vertical line drawn down to
...
enum ImGuiPopupFlags_ {
  ...
  ImGuiPopupFlags_MouseButtonLeft =
      1 << 2,  // For BeginPopupContext*(): open on Left Mouse release. Only one
               // button allowed!
  ImGuiPopupFlags_MouseButtonRight =
  ...
  ImGuiPopupFlags_InvalidMask_ =
      0x03,  // [Internal] Reserve legacy bits 0-1 to detect incorrectly passing
             // 1 or 2 to the function.
};
...
enum ImGuiHoveredFlags_ {
  ...
  ImGuiHoveredFlags_NoSharedDelay =
      1 << 17,  // IsItemHovered() only: Disable shared delay system where
                // moving from one item to the next keeps the previous timer for
                // a short time (standard for tooltips with long delays)
};
...
enum ImGuiKey : int {
  ...
  ImGuiKey_LeftShift,
  ImGuiKey_LeftAlt,
  ImGuiKey_LeftSuper,  // Also see ImGuiMod_Ctrl, ImGuiMod_Shift, ImGuiMod_Alt,
                       // ImGuiMod_Super below!
  ImGuiKey_RightCtrl,
  ImGuiKey_RightShift,
  ImGuiKey_RightAlt,
  ImGuiKey_RightSuper,
  ...
  ImGuiKey_GamepadLStickRight,  //             |         |          | [Analog]
                                //             Move Window (in Windowing mode)
  ImGuiKey_GamepadLStickUp,  //             |         |          | [Analog] Move
...
enum ImGuiInputFlags_ {
  ...
  ImGuiInputFlags_RouteOverActive =
      1
      << 15,  // Option: global route: higher priority than active item.
              // Unlikely you need to use that: will interfere with every active
              // items, e.g. Ctrl+A registered by InputText will be overridden
              // by this. May not be fully honored as user/internal code is
...
enum ImGuiCol_ {
  ...
  ImGuiCol_NavCursor,  // Color of keyboard/gamepad navigation cursor/rectangle,
                       // when visible
...
enum ImGuiColorEditFlags_ {
  ...
  ImGuiColorEditFlags_NoColorMarkers =
      1 << 11,  //              // ColorEdit: disable rendering R/G/B/A color
                //              marker. May also be disabled globally by setting
                //              style.ColorMarkerSize = 0.
  ...
  ImGuiColorEditFlags_AlphaNoBg =
      1 << 13,  //              // ColorEdit, ColorPicker, ColorButton: disable
                //              rendering a checkerboard background behind
                //              transparent color.
  ImGuiColorEditFlags_AlphaPreviewHalf =
      1 << 14,  //              // ColorEdit, ColorPicker, ColorButton: display
...
enum ImGuiSliderFlags_ {
  ...
  ImGuiSliderFlags_ClampZeroRange =
      1 << 10,  // Clamp even if min==max==0.0f. Otherwise due to legacy reason
  ...
  ImGuiSliderFlags_ColorMarkers =
      1 << 12,  // DragScalarN(), SliderScalarN(): Draw R/G/B/A color markers on
                // each component.
  ...
  ImGuiSliderFlags_InvalidMask_ =
      0x7000000F,  // [Internal] We treat using those bits as being potentially
                   // a 'float power' argument from legacy API (obsoleted
                   // 2020-08) that has got miscast to this enum, and will
...
enum ImGuiMouseCursor_ {
  ...
  ImGuiMouseCursor_Progress,  // When waiting for something to process/load, but
                              // application is still interactive.
...
enum ImGuiTableColumnFlags_ {
  ...
  ImGuiTableColumnFlags_NoHeaderLabel =
      1 << 12,  // TableHeadersRow() will submit an empty label for this column.
...
struct ImGuiStyle {
  ...
  float FontSizeBase;   // Current base font size before external global factors
  ...
  float WindowBorderHoverPadding;  // Hit-testing extent outside/inside resizing
  ...
  float TabCloseButtonMinWidthUnselected;  // -1: always visible. 0.0f: visible
                                           // when hovered. >0.0f: visible when
                                           // hovered if minimum width. FLT_MAX:
                                           // never show close button when
                                           // unselected.
  ...
  float DragDropTargetBorderSize;  // Thickness of the drag and drop target
                                   // border.
  float DragDropTargetPadding;  // Size to expand the drag and drop target from
                                // actual target item size.
  float ColorMarkerSize;  // Size of R/G/B/A color markers for ColorEdit4() and
                          // for Drags/Sliders when using
                          // ImGuiSliderFlags_ColorMarkers.
  ImGuiDir
  ...
  bool
      AntiAliasedLinesUseTex;  // Enable anti-aliased lines/borders using
  ...
  // [Internal]
  float
      _MainScale;  // FIXME-WIP: Reference scale, as applied by ScaleAllSizes().
...
struct ImGuiIO {
  ...
  ImVec2
      DisplayFramebufferScale;  // = (1, 1)         // Main display density. For
                                // retina display where window coordinates are
                                // different from framebuffer coordinates. This
                                // will affect font density + will end up in
  ...
  bool
      ConfigNavSwapGamepadButtons;  // = false          // Swap Activate<>Cancel
                                    // (A<>B) buttons, matching typical
                                    // "Nintendo/Japanese style" gamepad layout.
  bool
      ConfigNavMoveSetMousePos;  // = false          // Directional/tabbing
                                 // navigation teleports the mouse cursor. May
                                 // be useful on TV/console systems where moving
                                 // a virtual mouse is difficult. Will update
                                 // io.MousePos and set io.WantSetMousePos=true.
  bool ConfigNavCaptureKeyboard;  // = true           // Sets
                                  // io.WantCaptureKeyboard when io.NavActive is
                                  // set.
  bool ConfigNavEscapeClearFocusItem;  // = true           // Pressing Escape
                                       // can clear focused item + navigation
                                       // id/highlight. Set to false if you want
                                       // to always keep highlight on.
  ...
  bool ConfigWindowsCopyContentsWithCtrlC;  // = false      // [EXPERIMENTAL]
                                            ...
                                            // quality varies (3) text output is
                                            // in submission order rather than
                                            // spatial order.
  bool
      ConfigScrollbarScrollByPage;  // = true           // Enable scrolling page
                                    // by page when clicking outside the
                                    // scrollbar grab. When disabled, always
                                    // scroll to clicked location. When enabled,
  ...
  //    - Recovery after a programming error (native code or scripting language
  ...
  bool ConfigErrorRecovery;  // = true       // Enable error recovery support.
                             // Some errors won't be detected and lead to direct
                             // crashes if recovery is disabled.
  ...
  IMGUI_API void AddKeyEvent(
      ...
      bool down);  // Queue a new key down/up event. Key should be "translated"
                   // (as in, generally ImGuiKey_A matches the key end-user
                   // would use to emit an 'A' character)
  ...
  bool WantTextInput;  // Mobile/console: when set, you may display an on-screen
                       ...
                       // textual keyboard input to happen (e.g. when a
  ...
  ImGuiKeyData
      KeysData[ImGuiKey_NamedKey_COUNT];  // Key state for all known keys. MUST
                                          // use 'key - ImGuiKey_NamedKey_BEGIN'
                                          // as index. Use IsKeyXXX() functions
                                          // to access this.
  ...
  bool MouseWheelRequestAxisSwap;          // On a non-Mac system, holding Shift
                                   ...
                                   // of a WheelX event. On a Mac system this is
  ...
  // indices into the KeysDown[512] entries array which represent your "native"
  // keyboard state. The first 512 are now unused and should be kept zero.
  // Legacy backend will write into KeyMap[] using ImGuiKey_ indices which are
  // always >512. bool      KeysDown[ImGuiKey_COUNT];           // [LEGACY]
  // Input: Keyboard keys that are pressed (ideally left in the "native" order
  // your engine has access to keyboard keys, so you can use your own
  // defines/enums for keys). This used to be [512] sized. It is now
  ...
  // ImGuiKey_GamepadXXX enums. void*     ImeWindowHandle;                    //
  // [Obsoleted in 1.87] Set ImGuiViewport::PlatformHandleRaw instead. Set this
  // to your HWND to get automatic IME cursor positioning.
  ...
  // ambiguous/misleading and generally incorrect to use with the existence of a
  // higher-level input queue.
#endif
...
struct ImGuiInputTextCallbackData {
  ...
  ImWchar EventChar;  // Character input                      // Read-write   //
                      // [CharFilter] Replace character with another one, or set
                      // to zero to drop. return 1 is equivalent to setting
                      // EventChar=0;
...
struct ImGuiListClipper {
  ...
  double StartSeekOffsetY;  // [Internal] Account for frozen rows in a table and
  ...
  // which case the cursor won't be advanced in the final step, and you can call
  // SeekCursorForItem() manually if you need) items_height: Use -1.0f to be
  // calculated automatically on first step. Otherwise pass in the distance
  ...
  // IncludeItemsByIndex(item_begin, item_end); } // [renamed in 1.89.9] inline
...
enum ImGuiMultiSelectFlags_ {
  ...
  ImGuiMultiSelectFlags_SingleSelect =
      1
      << 0,  // Disable selecting more than one item. This is available to allow
             // single-selection code to share same code/logic if desired. It
             // essentially disables the main purpose of BeginMultiSelect() tho!
  ...
  ImGuiMultiSelectFlags_NoAutoClear =
      1 << 4,  // Disable clearing selection when navigating or selecting
               ...
               // ImGuiMultiSelectFlags_NoAutoSelect. useful for e.g. supporting
  ...
  ImGuiMultiSelectFlags_BoxSelect2d =
      1 << 7,  // Enable box-selection with varying width or varying x pos items
  ...
  ImGuiMultiSelectFlags_ScopeWindow =
      1 << 11,  // Scope for _BoxSelect and _ClearOnClickVoid is whole window
                // (Default). Use if BeginMultiSelect() covers a whole window or
                // used a single time in same window.
  ...
  ImGuiMultiSelectFlags_NavWrapX =
      1 << 16,  // [Temporary] Enable navigation wrapping on X axis. Provided as
                // a convenience because we don't have a design for the general
                // Nav API for this yet. When the more general feature be public
                // we may obsolete this flag in favor of new one.
  ImGuiMultiSelectFlags_NoSelectOnRightClick =
      1 << 17,  // Disable default right-click processing, which selects item on
                // mouse down, and is designed for context-menus.
};
...
struct ImGuiMultiSelectIO {
  ...
  ImGuiSelectionUserData
      RangeSrcItem;  //  ms:w  app:r     /                // (If using clipper)
                     //  Begin: Source item (often the first selected item) must
                     //  never be clipped: use clipper.IncludeItemByIndex() to
...
enum ImGuiSelectionRequestType {
  ...
  ImGuiSelectionRequestType_SetAll,  // Request app to clear selection (if
                                     // Selected==false) or select all items (if
                                     // Selected==true). We cannot set
...
struct ImGuiSelectionRequest {
  ...
  ImGuiSelectionUserData
      RangeLastItem;  //                  /  ms:w, app:r   // Parameter for
                      //                  SetRange request (this is generally ==
                      //                  RangeSrcItem when shift selecting from
...
struct ImGuiSelectionBasicStorage {
  ...
  IMGUI_API bool GetNextSelectedItem(
      void** opaque_it,
      ImGuiID*
          out_id);  // Iterate selection with 'void* it = NULL; ImGuiID id;
...
struct ImGuiSelectionExternalStorage {
  ...
  void (*AdapterSetItemSelected)(
      ...
      bool selected);  // e.g. AdapterSetItemSelected =
                       // [](ImGuiSelectionExternalStorage* self, int idx, bool
                       // selected) { ((MyItems**)self->UserData)[idx]->Selected
                       // = selected; }
...
struct ImDrawCmd {
  ...
  // for an upcoming feature) Since 1.92: removed ImDrawCmd::TextureId field,
  // the getter function must be used!
  inline ImTextureID GetTexID()
...
struct ImDrawList {
  ...
  // ImGui::GetDrawListSharedData(). (advanced: you may create and use your own
  // ImDrawListSharedData so you can use ImDrawList without ImGui, but that's
  // more involved)
  ...
  // complexity. Provided as a convenience for the user but not used by the main
  // library.
  ...
  // - Use special ImDrawCallback_ResetRenderState callback to instruct backend
  // to reset its render state to the default.
  ...
  // honoring this.
  // - For some backends, the callback may access selected render-states exposed
  // by the backend in a ImGui_ImplXXXX_RenderState structure pointed to by
  // platform_io.Renderer_RenderState.
  // - IMPORTANT: please be mindful of the different level of indirection
  // between using size==0 (copying argument) and using size>0 (copying pointed
  // data into a buffer).
  //   - If userdata_size == 0: we copy/store the 'userdata' argument as-is. It
  //   will be available unmodified in ImDrawCmd::UserCallbackData during
  //   render.
  ...
  //   ImDrawCmd::UserCallbackData will point inside that buffer so you have to
  //   retrieve data from there. Your callback may need to use
...
struct ImDrawData {
  ...
  ImVector<ImTextureData*>*
      Textures;  // List of textures to update. Most of the times the list is
                 // shared by all ImDrawData, has only 1 texture and it doesn't
                 // need any update. This almost always points to
                 // ImGui::GetPlatformIO().Textures[]. May be overridden or set
...
enum ImTextureStatus {
  ...
  ImTextureStatus_WantCreate,   // Requesting backend to create the texture. Set
                                // status OK when done.
  ImTextureStatus_WantUpdates,  // Requesting backend to update specific blocks
                                // of pixels (write to texture portions which
                                // have never been used before). Set status OK
                                // when done.
...
struct ImTextureData {
  ...
  ImTextureID TexID;  // r    w   // Backend-specific texture identifier. Always
                      // use SetTexID() to modify! The identifier will stored in
                      // ImDrawCmd::GetTexID() and passed to backend's
                      // RenderDrawData function.
  ...
  ImTextureRect UsedRect;  // w    r   // Bounding box encompassing all past and
                           // queued Updates[].
  ImTextureRect UpdateRect;  // w    r   // Bounding box encompassing all queued
                             // Updates[].
  ...
  int UnusedFrames;  // w    r   // In order to facilitate handling
                     ...
                     // successive frames where the texture was not used. Always
                     // >0 when Status==WantDestroy.
  unsigned short RefCount;  // w    r   // Number of contexts using this
                            // texture. Used during backend shutdown.
  bool UseColors;  // w    r   // Tell whether our texture data is known to use
                   // colors (rather than just white + alpha).
  ...
  // destroy, which is fine (e.g. freeing resources), but we immediately set the
  // texture back in _WantCreate mode.
...
struct ImFontConfig {
  // Data Source
  char Name[40];  // <auto>   // Name (strictly to ease debugging, hence limited
                  // size buffer)
  void* FontData;             //          // TTF/OTF data
  int FontDataSize;           //          // TTF/OTF data size
  bool FontDataOwnedByAtlas;  // true     // TTF/OTF data ownership taken by the
                              // owner ImFontAtlas (will delete memory itself).
                              // SINCE 1.92, THE DATA NEEDS TO PERSIST FOR WHOLE
                              // DURATION OF ATLAS.
  ...
  const ImWchar*
      GlyphExcludeRanges;  // NULL     // Pointer to a small user-provided list
                           // of Unicode ranges (2 value per range, values are
                           // inclusive, zero-terminated list). This is very
                           // close to GlyphRanges[] but designed to exclude
                           // ranges from a font source, when merging fonts with
                           // overlapping glyphs. Use "Input Glyphs Overlap
                           // Detection Tool" to find about your overlapping
                           // ranges.
  // ImVec2        GlyphExtraSpacing;      // 0, 0     // (REMOVED AT IT SEEMS
  // LARGELY OBSOLETE. PLEASE REPORT IF YOU WERE USING THIS). Extra spacing (in
  // pixels) between glyphs when rendered: essentially add to glyph->AdvanceX.
  ...
  float GlyphMinAdvanceX;  // 0        // Minimum AdvanceX for glyphs, set Min
                           ...
                           // mono-space font. Absolute value for default size,
                           // other sizes will scale this value.
  float GlyphMaxAdvanceX;  // FLT_MAX  // Maximum AdvanceX for glyphs
  float GlyphExtraAdvanceX;  // 0        // Extra spacing (in pixels) between
  ...
  // unsigned int  FontBuilderFlags;       // --       // [Renamed in 1.92] Use
  // FontLoaderFlags.
  float
      RasterizerMultiply;  // 1.0f     // Linearly brighten (>1.0f) or darken
                           ...
                           // This is a silly thing we may remove in the future.
  float RasterizerDensity;  // 1.0f     // [LEGACY: this only makes sense when
                            ...
                            // is expected that you increase/decrease font scale
                            // roughly to the inverse of this, otherwise quality
                            // may look lowered.
  ...
  const ImFontLoader*
      FontLoader;  // Custom font backend for this source (default source is the
                   // one stored in ImFontAtlas)
...
struct ImFontGlyph {
  ...
  float U0, V0, U1,
      V1;  // Texture coordinates for the current value of ImFontAtlas->TexRef.
           // Cached equivalent of calling GetCustomRect() with PackId.
  int PackId;  // [Internal] ImFontAtlasRectId value (FIXME: Cold data, could be
               // moved elsewhere?)
...
struct ImFontAtlas {
  ...
  IMGUI_API ImFont* AddFontDefaultBitmap(
      const ImFontConfig* font_cfg =
          NULL);  // Embedded classic pixel-clean font. Recommended at Size 13px
                  // with no scaling.
  ...
  // those soon:
  IMGUI_API void
  ClearInputData();  // [OBSOLETE] Clear input data (all ImFontConfig structures
                     // including sizes, TTF data, glyph ranges, etc.) = all the
                     // data used to build the texture and fonts.
  IMGUI_API void
  ClearFonts();  // [OBSOLETE] Clear input+output font data (same as
                 // ClearInputData() + glyphs storage, UV coordinates).
  IMGUI_API void
  ClearTexData();  // [OBSOLETE] Clear CPU-side copy of the texture data. Saves
                   // RAM once the texture has been copied to graphics memory.
  ...
  //   'atlas->TexRef'. Both TexRef and UV coordinates are typically changed at
  //   the same time.
  // - If you render colored output into your custom rectangles: set
  // 'atlas->TexPixelsUseColors = true' as this may help some backends decide of
  // preferred texture format.
  // - Read docs/FONTS.md for more details about using colorful icons.
  // - Note: this API may be reworked further in order to facilitate supporting
  // e.g. multi-monitor, varying DPI settings.
  ...
  IMGUI_API ImFontAtlasRectId AddCustomRect(
      ...
      ImFontAtlasRect* out_r = NULL);  // Register a rectangle. Return -1
                                       // (ImFontAtlasRectId_Invalid) on error.
  IMGUI_API void RemoveCustomRect(
      ImFontAtlasRectId
          id);  // Unregister a rectangle. Existing pixels will stay in texture
                // until resized / garbage collected.
  ...
  int TexMinWidth;   // Minimum desired texture width. Must be a power of two.
                     // Default to 512.
  int TexMinHeight;  // Minimum desired texture height. Must be a power of two.
                     // Default to 128.
  int TexMaxWidth;   // Maximum desired texture width. Must be a power of two.
                     // Default to 8192.
  int TexMaxHeight;  // Maximum desired texture height. Must be a power of two.
                     // Default to 8192.
  ...
  union {
    ImTextureRef TexRef;
    ImTextureRef TexID;
  };  // Latest texture identifier == TexData->GetTexRef(). // RENAMED TexID to
      // TexRef in 1.92.0.
#endif
  ...
  bool TexPixelsUseColors;  // Tell whether our texture data is known to use
                            ...
                            // to help backend select a format or conversion
                            // process.
  ...
  const ImFontLoader*
      FontLoader;  // Font loader opaque interface (default to use FreeType when
                   // IMGUI_ENABLE_FREETYPE is defined, otherwise default to use
                   // stb_truetype). Use SetFontLoader() to change this at
                   // runtime.
  const char* FontLoaderName;  // Font loader name (for display e.g. in About
                               // box) == FontLoader->Name
  void* FontLoaderData;        // Font backend opaque storage
  unsigned int
      FontLoaderFlags;  // Shared flags (for all fonts) for font loader. THIS IS
                        // BUILD IMPLEMENTATION DEPENDENT (e.g. Per-font
                        // override is also available in ImFontConfig).
  int RefCount;         // Number of contexts using this atlas
  ImGuiContext* OwnerContext;  // Context which own the atlas will be in charge
                               // of updating and destroying it.

  // [Obsolete]
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  // Legacy: You can request your rectangles to be mapped as font glyph (given a
  // font + Unicode point), so you can render e.g. custom colorful icons and use
  // them as regular glyphs. --> Prefer using a custom ImFontLoader.
  ...
  IMGUI_API ImFontAtlasRectId AddCustomRectFontGlyph(
      ...
      const ImVec2& offset = ImVec2(
          0,
          0));  // OBSOLETED in 1.92.0: Use custom ImFontLoader in ImFontConfig
  ...
  // OBSOLETED in 1.92.0: Force texture width before calling Build(). Must be a
  // power-of-two. If have many glyphs your graphics API have texture size
...
struct ImFontBaked {
  ...
  float Size;  // 4     // in  // Height of characters/line, set during loading
  ...
  float Ascent, Descent;  // 4+4   // out // Ascent: distance from top to bottom
                          // of e.g. 'A' [0..FontSize] (unscaled)
  unsigned int
      MetricsTotalSurface : 26;  // 3  // out // Total surface in pixels to get
                                 // an idea of the font rasterization/texture
                                 // cost (not exact, we approximate the cost of
                                 // padding between glyphs)
  unsigned int WantDestroy : 1;  // 0  //     // Queued for destroy
  unsigned int LoadNoFallback : 1;  // 0  //     // Disable loading fallback in
                                    // lower-level calls.
  unsigned int
      LoadNoRenderOnLayout : 1;  // 0  //     // Enable a two-steps mode where
                                 ...
                                 // unlikely to actually be rendered, otherwise
                                 // it is slower because we'd do one query on
                                 // the first CalcTextSize and one query on the
                                 // first Draw.
...
enum ImFontFlags_ {
  ...
  ImFontFlags_LockBakedSizes =
      1 << 3,  // [Internal] Disable loading new baked sizes, disable garbage
               // collecting current ones. e.g. if you want to lock a font to a
               // single size. Important: if you use this to preload given
               // sizes, consider the possibility of multiple font density used
               // on Retina display.
};
...
struct ImFont {
  ...
  float LegacySize;  // 4     // in  // Font size passed to AddFont(). Use for
                     // old code calling PushFont() expecting to use that size.
                     // (use ImGui::GetFontBaked() to get font baked at current
                     // bound size).
  ImVector<ImFontConfig*> Sources;  // 16    // in  // List of sources. Pointers
                                    // within OwnerAtlas->Sources[]
  ...
  ImU8 Used8kPagesMap[(IM_UNICODE_CODEPOINT_MAX + 1) / 8192 /
                      8];  // 1 bytes if ImWchar=ImWchar16, 16 bytes if
                           ...
                           // used codepoints.
  bool EllipsisAutoBake;   // 1     //     // Mark when the "..." glyph needs to
                           // be generated.
  ImGuiStorage RemapPairs;  // 16    //     // Remapping pairs when using
                            // AddRemapChar(), otherwise empty.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  float Scale;  // 4     // in  // Legacy base font scale (~1.0f), multiplied by
...
struct ImGuiViewport {
  ...
  void* PlatformHandleRaw;  // void* to hold lower-level, platform-native window
                            // handle (under Win32 this is expected to be a
                            // HWND, unused for other platforms)
...
struct ImGuiPlatformIO {
  ...
  const char* (*Platform_GetClipboardTextFn)(
      ImGuiContext* ctx);  // Should return NULL on failure (e.g. clipboard data
                           // is not text).
  void (*Platform_SetClipboardTextFn)(ImGuiContext* ctx, const char* text);
  void* Platform_ClipboardUserData;

  // Optional: Open link/folder/file in OS Shell
  // (default to use ShellExecuteW() on Windows, system() on Linux/Mac. expected
  // to return false on failure, but some platforms may always return true)
  ...
  // languages (e.g. German), generally pulled from *localeconv()->decimal_point
  ...
  // Optional: Maximum texture size supported by renderer (used to adjust how we
  // size textures). 0 if not known.
  ...
  // ImGui::Render) The ImGui_ImplXXXX_RenderDrawData() function of each backend
  // generally access this via ImDrawData::Textures which points to this. The
  // array is available here mostly because backends will want to destroy
  // textures on shutdown.
  ImVector<ImTextureData*>
      Textures;  // List of textures used by Dear ImGui (most often 1) +
                 // contents of external texture list is automatically appended
                 // into this.
