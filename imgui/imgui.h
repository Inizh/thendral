...
struct ImGuiTextFilter;	 // Helper to parse and apply text filters (e.g.
						 // "aaaaa[,bbbbb][,ccccc]")
struct ImGuiViewport;	 // A Platform Window (always only one in 'master'
					   // branch), in the future may represent Platform Monitor
...
struct ImTextureRef {
	...
	ImTextureData*
		_TexData;		 //      A texture, generally owned by a ImFontAtlas.
						 //      Will convert to ImTextureID during render
						 //      loop, after texture has been uploaded.
...
GetBackgroundDrawList();  // this draw list will be the first rendered one.
						  // Useful to quickly draw shapes/text behind dear
						  // imgui contents.
IMGUI_API ImDrawList*
GetForegroundDrawList();  // this draw list will be the last rendered one.
						  // Useful to quickly draw shapes/text over dear imgui
						  // contents.
...
enum ImGuiInputTextFlags_ {
	...
	ImGuiInputTextFlags_EnterReturnsTrue =
		1
		<< 6,  // Return 'true' when Enter is pressed (as opposed to every time
	...
	ImGuiInputTextFlags_DisplayEmptyRefVal =
		1
		<< 14,	// InputFloat(), InputInt(), InputScalar() etc. only: when value
...
enum ImGuiPopupFlags_ {
	...
	ImGuiPopupFlags_AnyPopupId =
		1 << 10,  // For IsPopupOpen(): ignore the ImGuiID
...
enum ImGuiTabItemFlags_ {
	...
	ImGuiTabItemFlags_NoReorder =
		1 << 5,	 // Disable reordering this tab or having
...
enum ImGuiHoveredFlags_ {
	...
	// IsWindowHovered() only: Consider docking hierarchy (treat dockspace host
	...
	ImGuiHoveredFlags_Stationary =
		1
		<< 13,	// Require mouse to be stationary for style.HoverStationaryDelay
...
enum ImGuiKey : int {
	...
	//   interfere
	//   and break that equivalence. The safer decision is to relay that
	//   ambiguity
	...
	ImGuiMod_Shortcut =
		ImGuiMod_Ctrl,	// Removed in 1.90.7, you can now simply use
						// ImGuiMod_Ctrl ImGuiKey_ModCtrl = ImGuiMod_Ctrl,
						// ImGuiKey_ModShift = ImGuiMod_Shift, ImGuiKey_ModAlt =
						// ImGuiMod_Alt, ImGuiKey_ModSuper = ImGuiMod_Super, //
						// Renamed in 1.89 ImGuiKey_KeyPadEnter =
...
enum ImGuiInputFlags_ {
	...
	ImGuiInputFlags_RouteGlobal =
		1 << 12,  // Global route (unless a focused window
...
enum ImGuiSliderFlags_ {
	...
	ImGuiSliderFlags_NoInput =
		1 << 7,	 // Disable Ctrl+Click or Enter key allowing
...
enum ImGuiTableFlags_ {
	...
	ImGuiTableFlags_Sortable =
		1
		<< 3,  // Enable sorting. Call TableGetSortSpecs() to obtain sort specs.
	...
	ImGuiTableFlags_NoHostExtendX =
		1
		<< 16,	// Make outer width auto-fit to columns, overriding outer_size.x
	...
	ImGuiTableFlags_NoKeepColumnsVisible =
		1
		<< 18,	// Disable keeping column always minimally visible when ScrollX
	...
	// if BordersOuterV is on, single inner padding if BordersOuterV
	// is off). Scrolling
	ImGuiTableFlags_ScrollX = 1 << 24,	// Enable horizontal scrolling. Require
										// 'outer_size' parameter of
	...
	ImGuiTableFlags_ScrollY =
		1
		<< 25,	// Enable vertical scrolling. Require 'outer_size' parameter of
	...
	ImGuiTableFlags_SortTristate = 1
								   << 27,  // Allow no sorting, disable default
										   // sorting. TableGetSortSpecs()
...
enum ImGuiTableColumnFlags_ {
	...
	ImGuiTableColumnFlags_WidthFixed =
		1
		<< 4,  // Column will not stretch. Preferable with horizontal scrolling
			   // enabled (default if table sizing policy is _SizingFixedFit and
			   // table is resizable).
	ImGuiTableColumnFlags_NoResize = 1 << 5,  // Disable manual resizing.
	ImGuiTableColumnFlags_NoReorder =
		1
		<< 6,  // Disable manual reordering this column, this will also prevent
	...
	ImGuiTableColumnFlags_NoHeaderLabel =
		1
		<< 12,	// TableHeadersRow() will submit an empty label for this column.
				// Convenient for some small columns. Name will still appear in
				// context menu or in angled headers. You may append into this
				// cell by calling TableSetColumnIndex() right after the
	...
	ImGuiTableColumnFlags_PreferSortAscending =
		1
		<< 14,	// Make the initial sort direction Ascending when first sorting
				// on this column (default).
	ImGuiTableColumnFlags_PreferSortDescending =
		1
		<< 15,	// Make the initial sort direction Descending when first sorting
				// on this column.
	...
	ImGuiTableColumnFlags_IsEnabled =
		1
		<< 24,	// Status: is enabled == not hidden by user/api (referred to as
...
struct ImGuiIO {
	...
	//    language
	//    - the latter tends to facilitate iterating on code while running).
	...
	ImGuiKeyData
		KeysData[ImGuiKey_NamedKey_COUNT];	// Key state for all known keys.
											// MUST use 'key -
											// ImGuiKey_NamedKey_BEGIN' as
											// index. Use IsKeyXXX() functions
											// to access this.
	...
	float
		MouseDownDuration[5];  // Duration the mouse button has been down (0.0f
							   // == just clicked)
...
struct ImGuiSelectionBasicStorage {
	...
	int _SelectionOrder;  // [Internal] Increasing counter to store selection
						  // order
	ImGuiStorage
		_Storage;  // [Internal] Selection set. Think of this as similar
...
struct ImDrawList {
	...
	ImDrawCmdHeader
		_CmdHeader;	 // [Internal] template of active commands. Fields
...
struct ImFontConfig {
	...
	float
		RasterizerMultiply;	 // 1.0f     // Linearly brighten (>1.0f) or darken
							 // (<1.0f) font output. Brightening small fonts may
							 // be a good workaround to make them more readable.
							 // This is a silly thing we may remove in the
							 // future.
...
struct ImFontGlyph {
	unsigned int
		Colored : 1;  // Flag to indicate glyph is colored and should
...
enum ImGuiViewportFlags_ {
	...
	ImGuiViewportFlags_OwnedByApp =
		1 << 2,	 // Platform Window: Is created/managed by the application
...
struct ImGuiViewport {
	...
	void* PlatformHandleRaw;  // void* to hold lower-level, platform-native
							  ...
							  // be a HWND, unused for other platforms)
