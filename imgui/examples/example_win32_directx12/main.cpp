...
struct ExampleDescriptorHeapAllocator {
	...
	void Alloc(D3D12_CPU_DESCRIPTOR_HANDLE* out_cpu_desc_handle,
		...
		out_cpu_desc_handle->ptr =
			HeapStartCpu.ptr + (idx * HeapHandleIncrement);
		out_gpu_desc_handle->ptr =
			HeapStartGpu.ptr + (idx * HeapHandleIncrement);
	}
...
static D3D12_CPU_DESCRIPTOR_HANDLE
	g_mainRenderTargetDescriptor[APP_NUM_BACK_BUFFERS] = {};
...
int main(int, char**) {
	...
	// ImGui_ImplDX12_Init(g_pd3dDevice, APP_NUM_FRAMES_IN_FLIGHT,
	...
	while (!done) {
		...
		// HRESULT hr = g_pSwapChain->Present(0, g_SwapChainTearingSupport ?
...
bool CreateDeviceD3D(HWND hWnd) {
	// Setup swap chain
	// This is a basic setup. Optimally could handle fullscreen mode
	...
	if (pdx12Debug != nullptr) {
		...
		// Disable breaking on this warning because of a suspected bug in the
		// D3D12 SDK layer, see #9084 for details.
		...
		case WM_SIZE:
			if (g_pd3dDevice != nullptr && wParam != SIZE_MINIMIZED) {
				...
				HRESULT result = g_pSwapChain->ResizeBuffers(
					0, (UINT)LOWORD(lParam), (UINT)HIWORD(lParam), desc.Format,
					desc.Flags);
