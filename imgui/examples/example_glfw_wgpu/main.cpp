...
static WGPUSurface CreateWGPUSurface(const WGPUInstance& instance,
									 GLFWwindow* window);
...
int main(int, char**) {
	...
	GLFWwindow* window =
		glfwCreateWindow(wgpu_surface_width, wgpu_surface_height,
...
#endif
	{
		...
		if (ImGui_ImplWGPU_IsSurfaceStatusSubOptimal(surface_texture.status)) {
			...
			if (width > 0 && height > 0) ResizeSurface(width, height);
			continue;
		}
...
static WGPUAdapter RequestAdapter(wgpu::Instance& instance) {
	...
	wgpu::Future waitAdapterFunc{instance.RequestAdapter(
		&adapter_options, wgpu::CallbackMode::WaitAnyOnly, onRequestAdapter)};
	wgpu::WaitStatus waitStatusAdapter =
		instance.WaitAny(waitAdapterFunc, UINT64_MAX);
	IM_ASSERT(acquired_adapter != nullptr &&
			  waitStatusAdapter == wgpu::WaitStatus::Success &&
			  "Error on Adapter request");
	return acquired_adapter.MoveToCHandle();
}

static WGPUDevice RequestDevice(wgpu::Instance& instance,
								wgpu::Adapter& adapter) {
		...
		[](const wgpu::Device&, wgpu::DeviceLostReason type,
		   wgpu::StringView msg) {
			fprintf(stderr, "%s error: %s\n",
					ImGui_ImplWGPU_GetDeviceLostReasonName(
	...
	wgpu::Future waitDeviceFunc{adapter.RequestDevice(
		&device_desc, wgpu::CallbackMode::WaitAnyOnly, onRequestDevice)};
	wgpu::WaitStatus waitStatusDevice =
		instance.WaitAny(waitDeviceFunc, UINT64_MAX);
	IM_ASSERT(acquired_device != nullptr &&
			  waitStatusDevice == wgpu::WaitStatus::Success &&
...
static bool InitWGPU(GLFWwindow* window) {
	...
	wgpuSurfaceGetCapabilities(wgpu_surface, adapter.Get(),
	...
	preferred_fmt =
		wgpuSurfaceGetPreferredFormat(wgpu_surface, {} /* adapter */);
#else	// __EMSCRIPTEN__
	wgpuSetLogCallback(
...
// GLFW helper to create a WebGPU surface, used only in WGPU-Native. DAWN-Native
// already has a built-in function As of today (2025/10) there is no "official"
// support in GLFW to create a surface for WebGPU backend This stub uses "low
// level" GLFW calls to acquire information from a specific Window Manager.
// Currently supported platforms: Windows / Linux (X11 and Wayland) / MacOS. Not
// necessary nor available with EMSCRIPTEN.
#ifndef __EMSCRIPTEN__
...
#undef Status  // X11 headers are leaking this and also 'Success', 'Always',
			   // 'None', all used in DAWN api. Add #undef if necessary.
