	...
	while (true) {
		...
		while (ALooper_pollOnce(g_Initialized ? 0 : -1, nullptr, &out_events,
			...
			if (app->destroyRequested != 0) {
				// shutdown() should have been called already while processing
				// the app command APP_CMD_TERM_WINDOW. But we play save here
...
void Init(struct android_app* app) {
	...
	// This is mostly boilerplate code for EGL...
	{
		...
		eglChooseConfig(g_EglDisplay, egl_attributes, &egl_config, 1,
						&num_configs);
		EGLint egl_format;
	...
	// (android/app/src/main/assets), we use our GetAssetData() helper to
	// retrieve them. style.FontSizeBase = 20.0f;
...
static int PollUnicodeChars() {
	...
	while ((unicode_character = java_env->CallIntMethod(g_App->activity->clazz,
														method_id)) != 0)
	...
	if (asset_descriptor) {
		...
		*outData = IM_ALLOC(num_bytes);
		int64_t num_bytes_read =
			AAsset_read(asset_descriptor, *outData, num_bytes);
