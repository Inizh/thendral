...
bool ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count,
  ...
  // - instance numbers varying/unstable. may not be a direct problem for users,
  // but could make outside access broken or confusing, e.g. TestEngine.
  // - can't implement support for ImGuiChildFlags_ResizeY as we need to somehow
  // pull the height data from somewhere. this also needs stable instance
  // numbers. The side-effects of accessing table data on coarse clip would be:
  // - always reserving the pooled ImGuiTable data ahead for a fully clipped
  // table (minor IMHO). Also the 'outer_window_is_measuring_size' criteria may
  // already be defeating this in some situations.
  ...
  const ImVec2 actual_outer_size = ImTrunc(CalcItemSize(
      outer_size, ImMax(avail_size.x, IMGUI_WINDOW_HARD_MIN_SIZE),
      use_child_window ? ImMax(avail_size.y, IMGUI_WINDOW_HARD_MIN_SIZE)
                       : 0.0f));
  ...
  if (use_child_window) {
    ...
    BeginChildEx(name, instance_id, outer_rect.GetSize(), child_child_flags,
                 child_window_flags);
    ...
    table->InnerWindow->DC
        .TreeDepth++;  // This is designed to always linking
                       // ImGuiTreeNodeFlags_DrawLines linking across a table
  }
  ...
  // TableMergeDrawChannels() expanding non-clipped column toward the limits of
  // that rectangle, in order for ImDrawListSplitter::Merge() to merge the draw
  // commands. However since the overlap problem only affect scrolling tables in
  // this case we can get away with doing it without extra cost).
  if (inner_window != outer_window) {
    // FIXME: Because inner_window's Scrollbar doesn't know about border size,
    // since it's not encoded in window->WindowBorderSize, it already overlaps
    // it and doesn't need an extra offset. Ideally we should be able to pass
    // custom border size with different x/y values to BeginChild().
  ...
  if (old_columns_count != 0 && old_columns_count != columns_count) {
    // Attempt to preserve width and other settings on column count/specs change
    // (#4046)
  ...
  if (table->IsInitializing) {
    ...
    table->IsSettingsDirty = true;  // Records itself into .ini file even when
                                    // in default state (#7934)
...
void ImGui::TableUpdateLayout(ImGuiTable* table) {
  ...
  for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
    ...
    column->NavLayerCurrent =
        (ImS8)(table->FreezeRowsCount > 0 ? ImGuiNavLayer_Menu
  ...
  // keep a column visible. Else if give no chance to a clipper-savvy user to
  ...
  // - To modify or replace this: set table->DisableDefaultContextMenu = true,
  // then call TableBeginContextMenuPopup()/.../EndPopup().
...
void ImGui::EndTable() {
  ...
  const float inner_content_max_y = ImCeil(
      table->RowPosY2);  // Rounding final position is important as we currently
                         // don't round row height ('Demo->Tables->Outer Size'
                         // demo uses non-integer heights)
  ...
  // impossible to do we'd rather update both axes together.
  ...
  if (table->Flags & ImGuiTableFlags_NoHostExtendX) {
    ...
    const float inner_content_max_x =
        table->OuterRect.Min.x +
        table->ColumnsAutoFitWidth;  // Slightly misleading name but used for
  ...
  if (temp_data->UserOuterSize.y <= 0.0f) {
    ...
    outer_window->DC.CursorMaxPos.y = ImMax(
  ...
  temp_data = (--g.TablesTempDataStacked > 0)
                  ? &g.TablesTempData[g.TablesTempDataStacked - 1]
                  : NULL;
  g.CurrentTable = temp_data && (temp_data->WindowID == outer_window->ID)
                       ? g.Tables.GetByIndex(temp_data->TableIndex)
                       : NULL;
  if (g.CurrentTable) {
...
// Called in TableSetupColumn() when initializing and in TableLoadSettings() for
// defaults before applying stored settings. 'init_mask' specify which fields to
// initialize.
static void TableInitColumnDefaults(ImGuiTable* table, ImGuiTableColumn* column,
  ...
  if (init_mask & ImGuiTableFlags_Resizable) {
    ...
    column->WidthRequest = ((flags & ImGuiTableColumnFlags_WidthFixed) &&
    ...
    column->StretchWeight = (init_width_or_weight > 0.0f &&
                             (flags & ImGuiTableColumnFlags_WidthStretch))
                                ? init_width_or_weight
                                : -1.0f;
    if (init_width_or_weight > 0.0f)  // Disable auto-fit if an explicit
                                      // width/weight has been specified
      column->AutoFitQueue = 0x00;
  }
  if (init_mask & ImGuiTableFlags_Reorderable)
    column->DisplayOrder =
        (ImGuiTableColumnIdx)table->Columns.index_from_ptr(column);
  if (init_mask & ImGuiTableFlags_Hideable)
    column->IsUserEnabled = column->IsUserEnabledNextFrame =
        (flags & ImGuiTableColumnFlags_DefaultHide) ? 0 : 1;
  if (init_mask & ImGuiTableFlags_Sortable) {
    // Multiple columns using _DefaultSort will be reassigned unique SortOrder
...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags,
  ...
  ImGuiTable* table = g.CurrentTable;
  IM_ASSERT_USER_ERROR_RET(
      table != NULL, "Call should only be done while in BeginTable() scope!");
  ...
  if (table->IsDefaultSizingPolicy &&
    ...
    IM_ASSERT_USER_ERROR_RET(
        init_width_or_weight <= 0.0f,
        "TableSetupColumn(): can only specify width/weight if sizing policy is "
        "set explicitly in either Table or Column.");
...
void ImGui::TableEndRow(ImGuiTable* table) {
  ...
  // and alter clipping rectangle)
  // - We need to do that in TableEndRow() instead of TableBeginRow() so the
  // list clipper can mark
...
bool ImGui::TableSetColumnIndex(int column_n) {
  ...
  if (table->CurrentColumn != column_n) {
    ...
    IM_ASSERT_USER_ERROR_RETV(column_n >= 0 && column_n < table->ColumnsCount,
                              false,
...
// FIXME: This could be abstracted and merged with PushColumnsBackground(), by
// creating a generic struct with storage for backup cliprect + backup channel +
// storage for splitter pointer, new clip rect. This would slightly simplify
// caller code.
...
void ImGui::TablePopColumnChannel() {
  ...
  if ((table->Flags & ImGuiTableFlags_NoClip) ||
      (table->CurrentColumn ==
       -1))  // Calling TreePop() after TableNextRow() is supported.
    return;
  ImGuiWindow* window = g.CurrentWindow;
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table) {
  ...
  if (merge_group_mask != 0) {
    ...
    for (int merge_group_n = 0; merge_group_n < IM_COUNTOF(merge_groups);
         merge_group_n++) {
...
void ImGui::TableDrawBorders(ImGuiTable* table) {
  ...
  if (table->Flags & ImGuiTableFlags_BordersOuter) {
    ...
    if ((table->Flags & ImGuiTableFlags_BordersOuter) ==
      ...
      inner_drawlist->AddRect(outer_border.Min, outer_border.Max, outer_col,
                              0.0f, 0, border_size);
...
void ImGui::TableHeadersRow() {
  ...
  for (int column_n = 0; column_n < columns_count; column_n++) {
    ...
    // Push an id to allow empty/unnamed headers. This is also idiomatic as it
    // ensure there is a consistent ID path to access columns (for e.g.
    // automation)
    const char* name =
...
void ImGui::TableHeader(const char* label) {
  ...
  if ((table->Flags & ImGuiTableFlags_Sortable) &&
    ...
    if (column->SortOrder > 0) {
      ImFormatString(sort_order_suf, IM_COUNTOF(sort_order_suf), "%d",
                     column->SortOrder + 1);
  ...
  RenderTextEllipsis(window->DrawList, label_pos,
                     ImVec2(ellipsis_max, bb.Max.y), ellipsis_max, label,
...
void ImGui::TableAngledHeadersRowEx(ImGuiID row_id, float angle,
  ...
  const float ascent_scaled =
      g.FontBaked->Ascent *
      g.FontBakedScale;  // FIXME: Standardize those scaling factors better
    ...
    for (int order_n = 0; order_n < data_count; order_n++) {
      ...
      if (pass == 0) {
        ...
        float line_off_for_align_x =
            ImFloor(ImMax((((column->MaxX - column->MinX) - padding.x * 2.0f) -
        ...
        while (label_name < label_name_end) {
          ...
          RenderTextEllipsis(draw_list, clip_r.Min, clip_r.Max, clip_r.Max.x,
                             label_name, label_name_eol, &label_size);
          PopStyleColor();
...
void ImGui::TableDrawDefaultContextMenu(
  ...
  if (flags_for_section_to_display & ImGuiTableFlags_Resizable) {
    ...
    if (table->ColumnsEnabledFixedCount == table->ColumnsEnabledCount &&
      ...
      size_all_desc =
          LocalizeGetMsg(ImGuiLocKey_TableSizeAllDefault);  // "###SizeAll" All
                                                            // stretch or mixed
...
void ImGui::TableLoadSettings(ImGuiTable* table) {
  ...
  for (int data_n = 0; data_n < settings->ColumnsCount;
    ...
    if ((settings->SaveFlags & ImGuiTableFlags_Hideable) &&
...
struct ImGuiTableFixDisplayOrderColumnData {
  ...
  ImGuiTable*
      Table;  // This is unfortunate but we don't have userdata in qsort api.
};

// Sort by DisplayOrder and then Index
static int IMGUI_CDECL TableFixDisplayOrderComparer(const void* lhs,
                                                    const void* rhs) {
  const ImGuiTable* table =
      ((const ImGuiTableFixDisplayOrderColumnData*)lhs)->Table;
  const ImGuiTableColumnIdx lhs_idx =
      ((const ImGuiTableFixDisplayOrderColumnData*)lhs)->Idx;
  const ImGuiTableColumnIdx rhs_idx =
      ((const ImGuiTableFixDisplayOrderColumnData*)rhs)->Idx;
  const int order_delta = (table->Columns[lhs_idx].DisplayOrder -
                           table->Columns[rhs_idx].DisplayOrder);
  return (order_delta > 0)     ? +1
         : (order_delta < 0)   ? -1
         : (lhs_idx > rhs_idx) ? +1
                               : -1;
}

// Fix invalid display order data: compact values (0,1,3 -> 0,1,2); preserve
// relative order (0,3,1 -> 0,2,1); deduplicate (0,4,1,1 -> 0,3,1,2)
void ImGui::TableFixDisplayOrder(ImGuiTable* table) {
  ImGuiContext& g = *GImGui;
  g.TempBuffer.reserve((int)(sizeof(ImGuiTableFixDisplayOrderColumnData) *
                             table->ColumnsCount));  // FIXME: Maybe wrap those
                                                     // two lines as a helper.
  ImGuiTableFixDisplayOrderColumnData* fdo_columns =
      (ImGuiTableFixDisplayOrderColumnData*)(void*)g.TempBuffer.Data;
  for (int n = 0; n < table->ColumnsCount; n++) {
    fdo_columns[n].Idx = (ImGuiTableColumnIdx)n;
    fdo_columns[n].Table = table;
  }
  ImQsort(fdo_columns, (size_t)table->ColumnsCount,
          sizeof(ImGuiTableFixDisplayOrderColumnData),
...
static void TableSettingsHandler_WriteAll(ImGuiContext* ctx,
  ...
  for (ImGuiTableSettings* settings = g.SettingsTables.begin();
    ...
    // We need to save the [Table] entry even if all the bools are false, since
    // this records a table with "default settings".
...
void ImGui::DebugNodeTable(ImGuiTable* table) {
  ...
  if (IsItemHovered())
    GetForegroundDrawList(table->OuterWindow)
        ->AddRect(table->OuterRect.Min, table->OuterRect.Max,
  ...
  for (int n = 0; n < table->ColumnsCount; n++) {
    ...
    if (IsItemHovered()) {
      ImRect r(column->MinX, table->OuterRect.Min.y, column->MaxX,
               table->OuterRect.Max.y);
      GetForegroundDrawList(table->OuterWindow)
          ->AddRect(r.Min, r.Max, IM_COL32(255, 255, 0, 255));
    }
  }
