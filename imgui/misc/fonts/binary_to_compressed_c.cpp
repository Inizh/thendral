...
// in binary. Endianness dependent, need swapping on big-endian CPU.
// - As char: ~12 bytes of source code for 4 bytes of input data. 4 bytes stored
// in binary. Not endianness dependent. Load compressed TTF fonts with
...
int main(int argc, char **argv) {
	if (argc < 3) {
		printf(
			"Syntax: %s [-u8|-u32|-base85] [-nocompress] [-nostatic] "
			"<inputfile> "
			"<symbolname>\n",
			argv[0]);
		printf("Source encoding types:\n");
		printf(
			" -u8     = ~12 bytes of source per 4 bytes of data. 4 bytes in "
			"binary.\n");
		printf(
			" -u32    = ~11 bytes of source per 4 bytes of data. 4 bytes in "
			"binary. Need endianness swapping on big-endian.\n");
		printf(
			" -base85 =  ~5 bytes of source per 4 bytes of data. 5 bytes in "
			"binary. Need decoder.\n");
		return 0;
	}
	...
	bool ret =
		binary_to_compressed_c(argv[argn], argv[argn + 1], source_encoding,
...
bool binary_to_compressed_c(const char *filename, const char *symbol,
	...
	if (source_encoding == SourceEncoding_Base85) {
		fprintf(
			out,
			"// Exported using binary_to_compressed_c.exe -base85 \"%s\" %s\n",
			filename, symbol);
		...
		for (int src_i = 0; src_i < compressed_sz; src_i += 4) {
			// This is made a little more complicated by the fact that ??X
			// sequences are interpreted as trigraphs by old C/C++ compilers. So
			// we need to escape pairs of ??.
		...
		fprintf(out,
				"// Exported using binary_to_compressed_c.exe -u8 \"%s\" %s\n",
				filename, symbol);
		...
		// As integers
		fprintf(out,
				"// Exported using binary_to_compressed_c.exe -u32 \"%s\" %s\n",
				filename, symbol);
		fprintf(out, "%sconst unsigned int %s_%ssize = %d;\n", static_str,
				symbol, compressed_str, (int)compressed_sz);
