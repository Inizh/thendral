...
int main(int argc, char **argv) {
  if (argc < 3) {
    printf(
        "Syntax: %s [-u8|-u32|-base85] [-nocompress] [-nostatic] <inputfile> "
        "<symbolname>\n",
        argv[0]);
  ...
  bool ret = binary_to_compressed_c(argv[argn], argv[argn + 1], source_encoding,
...
bool binary_to_compressed_c(const char *filename, const char *symbol,
  ...
  int compressed_sz = use_compression ? stb_compress((stb_uchar *)compressed,
                                                     (stb_uchar *)data, data_sz)
                                      : data_sz;
  if (use_compression)
