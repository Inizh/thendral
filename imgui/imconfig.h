...
// - Compiling with NDEBUG will usually strip out assert() to nothing, which is
// NOT recommended because we use asserts to notify of programmer mistakes.
...
// fonts (ProggyClean/ProggyVector), remove ~9 KB + ~17 KB from output binary.
// AddFontDefaultXXX() functions will assert. #define IMGUI_DISABLE_SSE //
...
// repository) + the FreeType library (not provided). Note that
// imgui_freetype.cpp may be used _without_ this define, if you manually call
// ImFontAtlas::SetFontLoader(). The define is simply a convenience. On Windows
// you may use vcpkg with 'vcpkg install freetype --triplet=x64-windows' +
