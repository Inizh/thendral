...
enum ImGuiItemFlagsPrivate_ {
	...
	ImGuiItemFlags_Inputable =
		1
		<< 20,	// false     // [WIP] Auto-activate input mode when tab focused.
	...
	// // Can't have a redirect as we inverted the behavior
};
...
enum ImGuiButtonFlagsPrivate_ {
	...
	ImGuiButtonFlags_NoFocus =
		1 << 22,  // [EXPERIMENTAL: Not very well specced].
				  // Don't focus parent window when clicking.
...
struct IMGUI_API ImGuiInputTextState {
	...
	ImGuiInputTextFlags
		Flags;			  // copy of InputText() flags. may be used to check
...
enum ImGuiInputFlagsPrivate_ {
	...
	ImGuiInputFlags_RepeatUntilRelease =
		1
		<< 4,  // Stop repeating when released (default for all functions except
...
struct ImGuiViewportP : public ImGuiViewport {
	...
	ImDrawList*
		BgFgDrawLists[2];  // Convenience background (0) and foreground
						   // (1) draw lists. We use them to draw software
...
struct ImGuiContext {
	...
	float
		FontSize;  // Currently bound font size == line height (== FontSizeBase
				   // + externals scales applied in the UpdateCurrentFontSize()
				   // function).
	...
	ImGuiWindow* MovingWindow;	// Track the window we clicked on (in order to
								// preserve focus). The actual window that is
	...
	ImGuiNavItemData
		NavMoveResultLocalVisible;	// Best move request candidate within
...
struct IMGUI_API ImGuiWindow {
   ...
   public:
	...
	// g.FontSizeBase * FontWindowScale * FontWindowScaleParents;
};
...
enum ImGuiTabItemFlagsPrivate_ {
	...
	// ImGuiTabItemFlags_Unsorted                = 1 << 23,  // [Docking]
	// Trailing tabs with the _Unsorted flag will be sorted based on the
	// DockOrder of their Window.
};
...
struct ImGuiTableColumn {
	...
	bool
		IsEnabled;	// IsUserEnabled && (Flags & ImGuiTableColumnFlags_Disabled)
					// == 0
...
struct IMGUI_API ImGuiTable {
	...
	ImGuiTableColumnIdx ColumnsEnabledFixedCount;  // Number of enabled columns
												   // using fixed width
												   // (<= ColumnsCount)
...
}  // For code consistency. This is public API in docking branch.
...
struct ImFontAtlasBuilder {
	stbrp_context_opaque
		PackContext;  // Actually 'stbrp_context' but we don't
					  // want to define this in the header file.
