...
enum ImDrawTextFlags_ {
  ...
  ImDrawTextFlags_CpuFineClip =
      1 << 0,  // Must be == 1/true for legacy with 'bool cpu_fine_clip' arg to
               // RenderText()
...
struct IMGUI_API ImDrawListSharedData {
  ...
  float
      FontSize;  // Current font size (used for for simplified AddText overload)
  ...
  ImGuiContext*
      Context;  // [OPTIONAL] Link to Dear ImGui context. 99% of
                // ImDrawList/ImFontAtlas can function without an ImGui context,
                // but this facilitate handling one legacy edge case.
...
enum ImGuiItemFlagsPrivate_ {
  ...
  ImGuiItemFlags_Inputable =
      1 << 20,  // false     // [WIP] Auto-activate input mode when tab focused.
  ...
  // Obsolete
  // ImGuiItemFlags_SelectableDontClosePopup = !ImGuiItemFlags_AutoClosePopups,
...
enum ImGuiButtonFlagsPrivate_ {
  ...
  // ImGuiButtonFlags_DontClosePopups      = 1 << 13,  // disable automatically
  // closing parent popup on press ImGuiButtonFlags_Disabled             = 1 <<
  ...
  ImGuiButtonFlags_NoTestKeyOwner =
      1 << 21,  // don't test key/input owner when polling the key (note: mouse
                // buttons are keys! often, the key in question will be
                // ImGuiKey_MouseLeft!)
  ImGuiButtonFlags_NoFocus = 1
                             << 22,  // [EXPERIMENTAL: Not very well specced].
...
struct IMGUI_API ImGuiInputTextState {
  ...
  const char* TextSrc;  // == TextA.Data unless read-only, in which case == buf
                        // passed to InputText(). For _ReadOnly fields, pointer
                        // will be null outside the InputText() call.
  ImVector<char>
      TextA;  // main UTF8 buffer. TextA.Size is a buffer size! Should always be
  ...
  bool CursorCenterY;  // set when we want scrolling to be centered over the
                       // cursor position (while resizing a word-wrapping field)
...
struct ImGuiPopupData {
  ...
  int ParentNavLayer;  // Resolved on BeginPopup(). Actually a ImGuiNavLayer
                       // type (declared down below), initialized to -1 which is
                       // not part of an enum, but serves well-enough as "not
                       // any of layers" value
...
struct ImGuiBoxSelectState {
  ...
  bool UnclipMode;  // (Temp/Transient, here in hot area). Set/cleared by the
                    ...
                    // box-select.
  ImRect
      UnclipRect;  // Rectangle where ItemAdd() clipping may be temporarily
                   // disabled. Need support by multi-select supporting widgets.
...
struct IMGUI_API ImGuiMultiSelectTempData {
  ImGuiMultiSelectIO IO;  // MUST BE FIRST FIELD. Requests are set and returned
                          // by BeginMultiSelect()/EndMultiSelect() + written to
                          // by user during the loop.
  ...
  bool IsFocused;  // Set if currently focusing the selection scope (any item of
                   // the selection). May be used if you have custom shortcut
                   // associated to selection.
...
struct IMGUI_API ImGuiMultiSelectState {
  ...
  ImGuiSelectionUserData NavIdItem;  // SetNextItemSelectionUserData() value for
...
struct ImGuiViewportP : public ImGuiViewport {
  ...
  ImDrawList* BgFgDrawLists[2];   // Convenience background (0) and foreground
...
struct ImGuiDebugItemPathQuery {
  ...
  bool Active;  // Used to disambiguate the case when ID == 0 and e.g. some code
                // calls PushOverrideID(0).
  bool Complete;  // All sub-queries are finished (some may have failed).
  ImS8 Step;  // -1: query stack + init Results, >= 0: filling individual stack
              // level.
...
struct ImGuiContext {
  ...
  ImVector<ImFontAtlas*>
      FontAtlases;  // List of font atlases used by the context (generally only
  ...
  float FontSize;  // Currently bound font size == line height (== FontSizeBase
  ...
  float
      FontBakedScale;  // == FontBaked->Size / FontSize. Scale factor over baked
                       // size. Rarely used nowadays, very often == 1.0f.
  ...
  float WindowsBorderHoverPadding;  // Padding around resizable windows for
  ...
  ImGuiID DebugDrawIdConflictsId;  // Set when we detect multiple items with the
                                   // same identifier
  ...
  int HoveredIdPreviousFrameItemCount;  // Count numbers of items using the same
                                        // ID as last frame's hovered id
  ...
  bool ActiveIdHasBeenPressedBefore;  // Track whether the active id led to a
                                      ...
                                      // between PressOnClick and PressOnRelease
                                      // without pressing twice). Used by
                                      // range_select branch.
  ...
  ImGuiID
      ActiveIdDisabledId;  // When clicking a disabled item we set
                           // ActiveId=window->MoveId to avoid interference with
                           // widget code. Actual item ID is stored here.
  ImVec2
  ...
  // Shared stacks
  ImGuiCol DebugFlashStyleColorIdx;  // (Keep close to ColorStack to share cache
                                     // line)
  ...
  bool NavHighlightItemUnderNav;  // Disable mouse hovering highlight. Highlight
                                  // navigation focused item instead of mouse
                                  // hovered item.
  ...
  // // Old name for g.NavHighlightItemUnderNav before 1.91.1 (2024/10/18) this
  // was called When user starts using keyboard/gamepad, we hide mouse hovering
  ...
  ImGuiNavLayer NavLayer;  // Focused layer (main scrolling layer, or menu/title
                           // bar layer)
  ImGuiID
      NavActivateId;  // ~~ (g.ActiveId == 0) && (IsKeyPressed(ImGuiKey_Space)
                      ...
                      // also set when calling ActivateItemByID()
  ImGuiID
  ...
  ImGuiID
      NavNextActivateId;  // Set by ActivateItemByID(), queued until next frame.
  ...
  // g.NavActivateFlags |= ImGuiActivateFlags_PreferInput. See commit c9a53aa74,
  // issue #5606.
  ...
  bool
      NavInitRequest;  // Init request for appearing window to select first item
  ...
  bool ConfigNavWindowingWithGamepad;  // = true. Enable Ctrl+Tab by holding
                                       ...
                                       // ImGuiKey_NavGamepadMenu). When false,
                                       // the button may still be used to toggle
                                       // Menu layer.
  ImGuiKeyChord
  ...
  bool NavWindowingToggleLayer;  // Set while Alt or GamepadMenu is held, may be
                                 // cleared by other operations, and processed
                                 // when releasing the key.
  ImGuiKey NavWindowingToggleKey;  // Keyboard/gamepad key used when toggling to
                                   // menu layer.
  ...
  short ScrollbarSeekMode;  // 0: scroll to clicked location, -1/+1: prev/next
                            // page.
  float
      ScrollbarClickDeltaToGrabCenter;  // When scrolling to mouse location:
                                        // distance between mouse and center of
  ...
  ImGuiPlatformImeData
      PlatformImeData;  // Data updated by current frame. Will be applied at end
                        // of the frame. For some backends, this is required to
                        // have WantVisible=true in order to receive text
                        // message.
  ImGuiPlatformImeData
  ...
  ImVector<ImTextureData*>
      UserTextures;  // List of textures created/managed by user or third-party
                     // extension. Automatically appended into
  ...
  int WantTextInputNextFrame;        // Copied in EndFrame() from
                               ...
                               // set for some backends (SDL3) to emit character
                               // inputs.
...
struct IMGUI_API ImGuiWindowTempData {
  ...
  ImVec1 ColumnsOffset;  // Offset to the current column (if ColumnsCurrent >
                         ...
                         // allow use cases like Tree->Column->Tree. Need revamp
                         // columns API.
  ImVec1 GroupOffset;
  ...
  ImU32
      TreeRecordsClippedNodesY2Mask;  // Store whether we should keep recording
                                      // Y2. Cleared when passing clip max.
                                      // Equivalent TreeHasStackDataDepthMask
...
struct IMGUI_API ImGuiWindow {
  ...
  bool
      ScrollbarXStabilizeEnabled;  // Was ScrollbarX previously auto-stabilized?
  ...
  ImVector<ImGuiID> IDStack;  // ID stack. ID are hashes seeded with the value
                              // at the top of the stack. (In theory this should
                              // be in the TempData structure)
  ...
  ImRect
      ContentRegionRect;  // FIXME: This is currently confusing/misleading. It
  ...
  float FontRefSize;  // This is a copy of window->CalcFontSize() at the time of
                      // Begin(), trying to phase out CalcFontSize() especially
                      // as it may be called on non-current window.
  ...
  ImGuiWindow*
      ParentWindow;  // If we are a child _or_ popup _or_ docked window, this is
  ...
  ImGuiWindow*
      ParentWindowForFocusRoute;  // Set to manual link a window to its logical
                                  // parent so that Shortcut() chain are honored
  ...
  int MemoryDrawListIdxCapacity;  // Backup of last idx/vtx count, so when
                                  // waking up the window we can preallocate and
...
enum ImGuiTabItemFlagsPrivate_ {
  ...
  // ImGuiTabItemFlags_Unsorted                = 1 << 23,  // [Docking] Trailing
  // tabs with the _Unsorted flag will be sorted based on the DockOrder of their
  // Window.
};
...
struct ImGuiTabItem {
  ...
  float ContentWidth;  // Width of label + padding, stored during BeginTabItem()
                       // call (misnamed as "Content" would normally imply width
                       // of label only)
  float
...
struct ImGuiTableColumn {
  ...
  bool IsEnabled;  // IsUserEnabled && (Flags & ImGuiTableColumnFlags_Disabled)
                   // == 0
  bool
      IsUserEnabled;  // Is the column not marked Hidden by the user? (unrelated
...
struct IMGUI_API ImGuiTable {
  ...
  ImS16
      InstanceCurrent;  // Count of BeginTable() calls with same ID in the same
                        ...
                        // BeginCount for a window, but multiple tables with the
                        // same ID are multiple tables, they are just synced.
  ...
  bool IsSettingsDirty;  // Set when table settings have changed and needs to be
                         // reported into ImGuiTableSettings data.
...
struct ImFontLoader {
  ...
  // FIXME: At this point the two other types of buffers may be managed by core
  // to be consistent?
...
struct ImFontAtlasRectEntry {
  int TargetIndex : 20;  // When Used: ImFontAtlasRectId -> into Rects[]. When
                         ...
                         // consume free-list.
  unsigned int Generation : 10;  // Increased each time the entry is reused for
                                 // a new rectangle.
...
struct ImFontAtlasBuilder {
  stbrp_context_opaque PackContext;  // Actually 'stbrp_context' but we don't
  ...
  bool
      PreloadedAllGlyphsRanges;  // Set when missing
                                 // ImGuiBackendFlags_RendererHasTextures
                                 // features forces atlas to preload everything.
