...
struct ImGui_ImplVulkan_Data {
  ...
  ImVector<VkFormat>
      PipelineRenderingCreateInfoColorAttachmentFormats;  // Deep copy of format
                                                          // array
...
static void ImGui_ImplVulkan_SetupRenderState(
  ...
  if (draw_data->TotalVtxCount > 0) {
    ...
    vkCmdBindIndexBuffer(
        command_buffer, rb->IndexBuffer, 0,
...
void ImGui_ImplVulkan_UpdateTexture(ImTextureData* tex) {
  ...
  if (tex->Status == ImTextureStatus_WantCreate ||
    ...
    // Update full texture or selected blocks. We only ever write to textures
    // regions which have never been used before! This backend choose to use
    // tex->UpdateRect but you can use tex->Updates[] to upload individual
    // regions. We could use the smaller rect on _WantCreate but using the full
    // rect allows us to clear the texture.
    const int upload_x =
        ...
        AlignBufferSize(upload_h * upload_pitch, bd->NonCoherentAtomSize);
    {
      ...
      buffer_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
      err =
          vkCreateBuffer(v->Device, &buffer_info, v->Allocator, &upload_buffer);
      check_vk_result(err);
      VkMemoryRequirements req;
      ...
      check_vk_result(err);
      err =
          vkBindBufferMemory(v->Device, upload_buffer, upload_buffer_memory, 0);
      check_vk_result(err);
    }
    ...
    // Copy to Image:
    {
      ...
      vkCmdPipelineBarrier(
          ...
          VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 1, upload_barrier, 1,
          copy_barrier);
      ...
      vkCmdCopyBufferToImage(bd->TexCommandBuffer, upload_buffer,
                             backend_tex->Image,
...
#endif
  VkPipeline pipeline;
  VkResult err = vkCreateGraphicsPipelines(device, pipelineCache, 1,
                                           &create_info, allocator, &pipeline);
  check_vk_result(err);
  return pipeline;
}
...
bool ImGui_ImplVulkan_CreateDeviceObjects() {
  ...
  if (!bd->TexSamplerLinear) {
    ...
    err =
        vkCreateSampler(v->Device, &info, v->Allocator, &bd->TexSamplerLinear);
    check_vk_result(err);
  }
...
static void ImGui_ImplVulkan_LoadDynamicRenderingFunctions(
  ...
  // - Try loading core (non-KHR) versions first (this will work for Vulkan 1.3+
  // and the device supports dynamic rendering)
...
bool ImGui_ImplVulkan_LoadFunctions(
  ...
  //      function_name, void*) { return vkGetInstanceProcAddr(your_vk_isntance,
  //      function_name); });
