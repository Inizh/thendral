...
bool ImGui_ImplFreeType_FontSrcData::InitFont(
  ...
  FT_Error error = FT_New_Memory_Face(ft_library, (const FT_Byte*)src->FontData,
                                      (FT_Long)src->FontDataSize,
...
static const FT_Glyph_Metrics* ImGui_ImplFreeType_LoadGlyph(
  ...
  // Apply convenience transform (this is not picking from real "Bold"/"Italic"
  // fonts! Merely applying FreeType helper transform. Oblique == Slanting)
...
static void ImGui_ImplFreeType_BlitGlyph(const FT_Bitmap* ft_bitmap,
  ...
  switch (ft_bitmap->pixel_mode) {
    ...
    case FT_PIXEL_MODE_MONO:  // Monochrome image, 1 bit per pixel. The bits in
                              // each byte are ordered from MSB to LSB.
    {
      for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch) {
        uint8_t bits = 0;
...
#define DE_MULTIPLY(color, alpha) \
  ImMin((ImU32)(255.0f * (float)color / (float)(alpha + FLT_MIN) + 0.5f), 255u)
      for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch)
        for (uint32_t x = 0; x < w; x++) {
          uint8_t r = src[x * 4 + 2], g = src[x * 4 + 1], b = src[x * 4],
                  a = src[x * 4 + 3];
...
static void* FreeType_Realloc(FT_Memory /*memory*/, long cur_size,
  ...
  if (new_size > cur_size) {
    void* new_block = GImGuiFreeTypeAllocFunc((size_t)new_size,
                                              GImGuiFreeTypeAllocatorUserData);
...
static bool ImGui_ImplFreeType_FontSrcInit(ImFontAtlas* atlas,
  ...
  if (!bd_font_data->InitFont(
          bd->Library, src, (ImGuiFreeTypeLoaderFlags)atlas->FontLoaderFlags)) {
...
static bool ImGui_ImplFreeType_FontBakedInit(ImFontAtlas* atlas,
  ...
  // Vuhdo 2017: "I'm not sure how to deal with font sizes properly. As far as I
  // understand, currently ImGui assumes that the 'pixel_height' is a maximum
  // height of an any given glyph, i.e. it's the sum of font's ascender and
  // descender. Seems strange to me. FT_Set_Pixel_Sizes() doesn't seem to get us
  // the same result." (FT_Set_Pixel_Sizes() essentially calls FT_Request_Size()
  ...
  if (src->MergeMode == false) {
    ...
    baked->Ascent = (float)FT_CEIL(metrics.ascender) *
                    scale;  // The pixel extents above the baseline in pixels
                            // (typically positive).
    baked->Descent = (float)FT_CEIL(metrics.descender) *
                     scale;  // The extents below the baseline in pixels
                             // (typically negative).
    // LineSpacing     = (float)FT_CEIL(metrics.height) * scale;         // The
    // baseline-to-baseline distance. Note that it usually is larger than the
    ...
    // finds appropriate. LineGap         = (float)FT_CEIL(metrics.height -
    // metrics.ascender + metrics.descender) * scale; // The spacing in pixels
    // between one row's descent and the next row's ascent. MaxAdvanceWidth =
    // (float)FT_CEIL(metrics.max_advance) * scale;    // This field gives the
...
static FT_Error ImGuiLunasvgPortRender(FT_GlyphSlot slot, FT_Pointer* _state) {
  ...
  state->svg->render(
      bitmap, state->matrix);  // state->matrix is already scaled and translated
#else
  state->svg->setMatrix(
      state->svg->matrix()
