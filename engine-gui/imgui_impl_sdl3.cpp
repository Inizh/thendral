...
static void ImGui_ImplSDL3_UpdateIme() {
  ...
  if ((!bd->ImeDirty && bd->ImeWindow == window) || (window == nullptr)) return;
...
bool ImGui_ImplSDL3_ProcessEvent(const SDL_Event* event) {
  ...
  switch (event->type) {
    ...
    case SDL_EVENT_KEY_UP: {
      ...
      // IMGUI_DEBUG_LOG("SDL_EVENT_KEY_%s : key=%d ('%s'), scancode=%d ('%s'),
      // mod=%X\n",
      ...
      //     event->key.mod);
      ImGuiKey key = ImGui_ImplSDL3_KeyEventToImGuiKey(event->key.key,
                                                       event->key.scancode);
      io.AddKeyEvent(key, (event->type == SDL_EVENT_KEY_DOWN));
      io.SetKeyEventNativeData(
          key, (int)event->key.key, (int)event->key.scancode,
          (int)event->key.scancode);  // To support legacy indexing (<1.87 user
...
static bool ImGui_ImplSDL3_Init(SDL_Window* window, SDL_Renderer* renderer,
  ...
  const char* capture_and_global_state_whitelist[] = {"windows", "cocoa", "x11",
                                                      "DIVE", "VMAN"};
...
static void ImGui_ImplSDL3_UpdateMouseData() {
  ...
  if (bd->MouseCanUseCapture) {
    ...
    for (int button_n = 0; button_n < ImGuiMouseButton_COUNT && !want_capture;
         button_n++)
  ...
  if (is_app_focused) {
    ...
    // (Optional) Fallback to provide unclamped mouse position when focused but
    // not hovered (SDL_EVENT_MOUSE_MOTION already provides this when hovered or
    // captured) Note that SDL_GetGlobalMouseState() is in theory slow on X11,
    // but this only runs on rather specific cases. If a problem we may provide
    // a way to opt-out this feature.
...
static void ImGui_ImplSDL3_UpdateGamepads() {
  ...
  ImGui_ImplSDL3_UpdateGamepadAnalog(
      bd, io, ImGuiKey_GamepadR2, SDL_GAMEPAD_AXIS_RIGHT_TRIGGER, 0.0f, 32767);
  ...
  ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadRStickLeft,
                                     SDL_GAMEPAD_AXIS_RIGHTX, -thumb_dead_zone,
                                     -32768);
  ...
  ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadRStickUp,
                                     SDL_GAMEPAD_AXIS_RIGHTY, -thumb_dead_zone,
                                     -32768);
