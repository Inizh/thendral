...
bool ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count,
	...
	if (use_child_window) {
		...
		table->InnerWindow->DC
			.TreeDepth++;  // This is designed to always linking
						   // ImGuiTreeNodeFlags_DrawLines linking across a
						   // table
	}
	...
	if (old_columns_count != 0 && old_columns_count != columns_count) {
		// Attempt to preserve width and other settings on column count/specs
		// change
		// (#4046)
	...
	if (table->IsInitializing) {
		...
		table->IsSettingsDirty = true;	// Records itself into .ini file even
										// when in default state (#7934)
...
void ImGui::TableUpdateLayout(ImGuiTable* table) {
	...
	float stretch_sum_weights =
		0.0f;  // Sum of all weights for stretch columns.
	...
	for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
		...
		// artifacts.) if (column->Flags & ImGuiTableColumnFlags_AlignRight)
	...
	// we keep a column visible. Else if give no chance to a clipper-savvy user
...
void ImGui::EndTable() {
	...
	const float inner_content_max_y =
		ImCeil(table->RowPosY2);  // Rounding final position is important as we
								  // currently don't round row height
								  // ('Demo->Tables->Outer Size' demo uses
								  // non-integer heights)
...
static void TableInitColumnDefaults(ImGuiTable* table, ImGuiTableColumn* column,
	...
	if (init_mask & ImGuiTableFlags_Sortable) {
		// Multiple columns using _DefaultSort will be reassigned unique
		// SortOrder values when building the sort specs.
		column->SortOrder =
			(flags & ImGuiTableColumnFlags_DefaultSort) ? 0 : -1;
...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags,
	...
	if (table->IsDefaultSizingPolicy &&
		...
		IM_ASSERT_USER_ERROR_RET(init_width_or_weight <= 0.0f,
								 "TableSetupColumn(): can only specify "
								 "width/weight if sizing policy is "
...
void ImGui::TableEndRow(ImGuiTable* table) {
	...
	// cursor and alter clipping rectangle)
...
bool ImGui::TableSetColumnIndex(int column_n) {
	...
	if (table->CurrentColumn != column_n) {
		...
		IM_ASSERT_USER_ERROR_RETV(
			column_n >= 0 && column_n < table->ColumnsCount, false,
...
void ImGui::TableBeginCell(ImGuiTable* table, int column_n) {
	...
	window->DC.CursorPosPrevLine.x =
		window->DC.CursorPos
			.x;	 // PrevLine.y is preserved. This allows users to call
				 // SameLine() to share LineSize between columns.
...
void ImGui::TableOpenContextMenu(int column_n) {
	...
	if (column_n ==
		table
			->ColumnsCount)	 // To facilitate using with TableGetHoveredColumn()
		column_n = -1;
...
void ImGui::TableFixDisplayOrder(ImGuiTable* table) {
	ImGuiContext& g = *GImGui;
	g.TempBuffer.reserve(
		(int)(sizeof(ImGuiTableFixDisplayOrderColumnData) *
	...
	for (int n = 0; n < table->ColumnsCount; n++)
		table->Columns[fdo_columns[n].Idx].DisplayOrder =
			(ImGuiTableColumnIdx)n;
}
...
static void TableSettingsHandler_WriteAll(ImGuiContext* ctx,
	...
	for (ImGuiTableSettings* settings = g.SettingsTables.begin();
		...
		// We need to save the [Table] entry even if all the bools are false,
		// since this records a table with "default settings".
