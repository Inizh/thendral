...
// FIXME-WIP: Works but API is likely to be reworked. This is designed for 1
// item on the line. (#7024)
void ImGui::TextAlignedV(float align_x, float size_x, const char* fmt,
                         va_list args) {
  ImGuiWindow* window = GetCurrentWindow();
  if (window->SkipItems) return;
  ...
  window->DC.CursorMaxPos.x =
      ImMax(window->DC.CursorMaxPos.x, pos.x + text_size.x);
  window->DC.IdealMaxPos.x =
      ImMax(window->DC.IdealMaxPos.x, pos.x + text_size.x);
  if (align_x > 0.0f && text_size.x < size_x)
    pos.x += ImTrunc((size_x - text_size.x) * align_x);
  RenderTextEllipsis(window->DrawList, pos, pos_max, pos_max.x, text, text_end,
                     &text_size);
...
//   using multiple overlaid ButtonBehavior() with same ID and different
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered,
  ...
  // where holding the button pressed for a long time while drag a payload item
  // triggers the button.
  if (g.DragDropActive) {
    ...
    if (g.DragDropAcceptIdPrev == id &&
        (g.DragDropAcceptFlagsPrev & ImGuiDragDropFlags_AcceptDrawAsHovered))
      hovered = true;
  }
    ...
    if (mods_ok) {
      if (mouse_button_clicked != -1 && g.ActiveId != id) {
        ...
        if (flags & (ImGuiButtonFlags_PressedOnClickRelease |
          ...
          if (!(flags & ImGuiButtonFlags_NoNavFocus)) {
            ...
            FocusWindow(
                window,
                ImGuiFocusRequestFlags_RestoreFocusedChild);  // Still need to
                                                              // focus and bring
                                                              // to front, but
                                                              // try to avoid
                                                              // losing NavId
      ...
      if (flags & ImGuiButtonFlags_PressedOnRelease) {
        if (mouse_button_released != -1) {
          ...
          if (!(flags & ImGuiButtonFlags_NoNavFocus))
            SetFocusID(id,
                       window);  // FIXME: Lack of FocusWindow() call here is
                                 // inconsistent with other paths. Research why.
          ClearActiveID();
        }
      }
...
bool ImGui::InvisibleButton(const char* str_id, const ImVec2& size_arg,
  ...
  ImVec2 size = CalcItemSize(ImVec2(size_arg.x != 0.0f ? size_arg.x : -FLT_MIN,
                                    size_arg.y != 0.0f ? size_arg.y : -FLT_MIN),
                             0.0f, 0.0f);
  ...
  if (!ItemAdd(bb, id, NULL,
               (flags & ImGuiButtonFlags_EnableNav) ? ImGuiItemFlags_None
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis,
  ...
  ImRect bb = bb_frame;
  float padding = IM_TRUNC(ImMin(
      style.ScrollbarPadding, ImMin(bb_frame_width, bb_frame_height) * 0.5f));
  bb.Expand(-padding);
  ...
  if (scrollbar_size_v < 1.0f) return false;
  ...
  if (held && allow_interaction && grab_h_norm < 1.0f) {
    ...
    const int held_dir = (clicked_v_norm < grab_v_norm)                 ? -1
                         : (clicked_v_norm > grab_v_norm + grab_h_norm) ? +1
                                                                        : 0;
    if (g.ActiveIdIsJustActivated) {
      // On initial click when held_dir == 0 (clicked over grab): calculate the
      // distance between mouse and the center of the grab
  ...
  window->DrawList->AddRectFilled(bb_frame.Min, bb_frame.Max, bg_col,
                                  window->WindowRounding, draw_rounding_flags);
  ImRect grab_rect;
  if (axis == ImGuiAxis_X)
...
// above.
void ImGui::ImageWithBg(ImTextureRef tex_ref, const ImVec2& image_size,
  ...
  if (bg_col.w > 0.0f)
    window->DrawList->AddRectFilled(bb.Min + padding, bb.Max - padding,
                                    GetColorU32(bg_col), rounding);
  if (rounding > 0.0f)
    window->DrawList->AddImageRounded(tex_ref, bb.Min + padding,
                                      bb.Max - padding, uv0, uv1,
                                      GetColorU32(tint_col), rounding);
  else
    window->DrawList->AddImage(tex_ref, bb.Min + padding, bb.Max - padding, uv0,
...
void ImGui::Image(ImTextureRef tex_ref, const ImVec2& image_size,
                  const ImVec2& uv0, const ImVec2& uv1) {
  ImageWithBg(tex_ref, image_size, uv0, uv1);
}

// 1.91.9 (February 2025) removed 'tint_col' and 'border_col' parameters, made
// border size not depend on color value. (#8131, #8238)
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureRef tex_ref,
  ...
  float image_rounding =
      ImMax(g.Style.FrameRounding - ImMax(padding.x, padding.y),
            g.Style.ImageRounding);
  if (image_rounding > 0.0f)
    window->DrawList->AddImageRounded(tex_ref, bb.Min + padding,
                                      bb.Max - padding, uv0, uv1,
                                      GetColorU32(tint_col), image_rounding);
  else
    window->DrawList->AddImage(tex_ref, bb.Min + padding, bb.Max - padding, uv0,
                               uv1, GetColorU32(tint_col));
...
// not the best design?
bool ImGui::ImageButton(const char* str_id, ImTextureRef tex_ref,
                        const ImVec2& image_size, const ImVec2& uv0,
                        const ImVec2& uv1, const ImVec4& bg_col,
  ...
  return ImageButtonEx(window->GetID(str_id), tex_ref, image_size, uv0, uv1,
                       bg_col, tint_col);
}
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg,
  ...
  if (fill_x0 < fill_x1)
    RenderRectFilledInRangeH(window->DrawList, bb,
                             GetColorU32(ImGuiCol_PlotHistogram), fill_x0,
                             fill_x1, style.FrameRounding);
  ...
  if (!is_indeterminate || overlay != NULL) {
    if (!overlay) {
      ImFormatString(overlay_buf, IM_COUNTOF(overlay_buf), "%.0f%%",
                     fraction * 100 + 0.01f);
      overlay = overlay_buf;
    }
...
bool ImGui::TextLink(const char* label) {
  ...
  {
    // FIXME-STYLE: Read comments above. This widget is NOT written in the same
    // style as some earlier widgets, as we are currently experimenting/planning
  ...
  float line_y =
      bb.Max.y + ImFloor(g.FontBaked->Descent * g.FontBakedScale * 0.20f);
...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem* items, int count,
  ...
  while (width_excess > 0.001f && count_same_width < count) {
    ...
    max_width_to_remove_per_item =
        ImMin(items[0].Width - width_min, max_width_to_remove_per_item);
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb,
  ...
  char name[16];
  ImFormatString(name, IM_COUNTOF(name), "##Combo_%02d",
  ...
  if (!ret) {
    ...
    if (!g.IO.ConfigDebugBeginReturnValueOnce &&
        !g.IO.ConfigDebugBeginReturnValueLoop)  // Begin may only return false
                                                // with those debug tools
                                                // activated.
      IM_ASSERT(
...
void ImGui::EndCombo() {
  ...
  char name[16];
  ImFormatString(name, IM_COUNTOF(name), "##Combo_%02d",
                 g.BeginComboDepth);  // FIXME: Move those to helpers?
...
static const ImU32 GDefaultRgbaColorMarkers[4] = {
    IM_COL32(240, 20, 20, 255), IM_COL32(20, 240, 20, 255),
...
static float GetMinimumStepAtDecimalPrecision(int decimal_precision) {
  ...
  return (decimal_precision < IM_COUNTOF(min_steps))
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data,
  ...
  RenderFrame(frame_bb.Min, frame_bb.Max, frame_col, false,
              style.FrameRounding);
  if (color_marker != 0 && style.ColorMarkerSize > 0.0f)
    RenderColorComponentMarker(frame_bb, GetColorU32(color_marker),
                               style.FrameRounding);
  ...
  const char* value_buf_end =
      value_buf + DataTypeFormatString(value_buf, IM_COUNTOF(value_buf),
    ...
    if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
      ...
      if (input_delta != 0.0f) {
        ...
        if (decimal_precision > 0) {
          input_delta /=
              100.0f;  // Keyboard/Gamepad tweak speeds in % of slider bounds
...
bool ImGui::TempInputScalar(const ImRect& bb, ImGuiID id, const char* label,
  ...
  DataTypeFormatString(data_buf, IM_COUNTOF(data_buf), data_type, p_data,
                       format);
  ...
  g.LastItemData.ItemFlags |=
      ImGuiItemFlags_NoMarkEdited;  // Because TempInputText() uses
                                    // ImGuiInputTextFlags_MergedItem it doesn't
                                    // submit a new item, so we poke
                                    // LastItemData.
  bool value_changed = false;
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type,
  ...
  IM_ASSERT((flags & ImGuiInputTextFlags_EnterReturnsTrue) ==
            0);  // Not supported by InputScalar(). Please open an issue if you
                 // this would be useful to you. Otherwise use
  ...
  if (p_step == NULL) {
    ...
    if (InputText("", buf, IM_COUNTOF(buf),
...
bool ImGui::InputFloat(const char* label, float* v, float step, float step_fast,
  ...
  return InputScalar(
      label, ImGuiDataType_Float, (void*)v, (void*)(step > 0.0f ? &step : NULL),
...
bool ImGui::InputDouble(const char* label, double* v, double step,
                        ...
                        ImGuiInputTextFlags flags) {
  return InputScalar(
      label, ImGuiDataType_Double, (void*)v, (void*)(step > 0.0 ? &step : NULL),
...
// If you want to use InputText() with std::string or any custom dynamic string
// type, use the wrapper in misc/cpp/imgui_stdlib.h/.cpp!
...
static ImVec2 InputTextCalcTextSize(ImGuiContext* ctx, const char* text_begin,
  ...
  return ImFontCalcTextSizeEx(g.Font, g.FontSize, FLT_MAX, obj->WrapWidth,
                              text_begin, text_end_display, text_end,
...
static void STB_TEXTEDIT_LAYOUTROW(StbTexteditRow* r, ImGuiInputTextState* obj,
  ...
  const ImVec2 size = InputTextCalcTextSize(
      obj->Ctx, text + line_start_idx, text + obj->TextLen, text + obj->TextLen,
      &text_remaining, NULL,
...
// Reimplementation of
// stb_textedit_move_line_start()/stb_textedit_move_line_end() which supports
// word-wrapping.
static int STB_TEXTEDIT_MOVELINESTART_IMPL(ImGuiInputTextState* obj,
                                           ImStb::STB_TexteditState* state,
                                           int cursor) {
  ...
  if (obj->WrapWidth > 0.0f) {
    ...
    const char* p = p_bol;
    const char* text_end =
        obj->TextSrc + obj->TextLen;  // End of line would be enough
    while (p >= p_bol) {
      const char* p_eol = ImFontCalcWordWrapPositionEx(
          g.Font, g.FontSize, p, text_end, obj->WrapWidth,
          ImDrawTextFlags_WrapKeepBlanks);
      if (p == p_cursor)  // If we are already on a visible beginning-of-line,
                          // return real beginning-of-line (would be same as
                          // regular handler below)
...
static int STB_TEXTEDIT_MOVELINEEND_IMPL(ImGuiInputTextState* obj,
                                         ImStb::STB_TexteditState* state,
                                         int cursor) {
    ...
    while (p < text_end) {
      ...
      if (p_eol == p_cursor &&
          obj->LastMoveDirectionLR !=
              ImGuiDir_Left)  // If we are already on a visible end-of-line,
  ...
  while (cursor < n &&
         STB_TEXTEDIT_GETCHAR(obj, cursor) != STB_TEXTEDIT_NEWLINE)
...
static int STB_TEXTEDIT_INSERTCHARS(ImGuiInputTextState* obj, int pos,
                                    const char* new_text, int new_text_len) {
  ...
  if (!is_resizable && new_text_len > avail)
    new_text_len =
        (int)(ImTextFindValidUtf8CodepointEnd(new_text, new_text + new_text_len,
                                              new_text + avail) -
              new_text);  // Truncate to closest UTF-8 codepoint. Alternative:
                          // return 0 to cancel insertion.
  if (new_text_len == 0) return 0;

  // Grow internal buffer if needed
  IM_ASSERT(obj->TextSrc == obj->TextA.Data);
  if (text_len + new_text_len + 1 > obj->TextA.Size && is_resizable) {
    obj->TextA.resize(text_len +
                      ImClamp(new_text_len, 32, ImMax(256, new_text_len)) + 1);
...
static void stb_textedit_replace(ImGuiInputTextState* str,
  ...
  int text_len_inserted =
      ImStb::STB_TEXTEDIT_INSERTCHARS(str, 0, text, text_len);
...
void ImGuiInputTextCallbackData::InsertChars(int pos, const char* new_text,
  ...
  int new_text_len =
      new_text_end ? (int)(new_text_end - new_text) : (int)ImStrlen(new_text);
  ...
  // Grow internal buffer if needed
  if (new_text_len + BufTextLen + 1 > obj->TextA.Size && is_resizable &&
      !is_readonly) {
...
static bool InputTextFilterCharacter(ImGuiContext* ctx,
  ...
  if (flags & ImGuiInputTextFlags_CallbackCharFilter) {
    ...
    callback_data.EventActivated =
        (g.ActiveId == state->ID && g.ActiveIdIsJustActivated);
...
static void InputTextReconcileUndoState(ImGuiInputTextState* state,
                                        const char* old_buf, int old_length,
...
// FIXME-WORDWRAP: Bundle some of this into ImGuiTextIndex and/or extract as a
// different tool? 'max_output_buffer_size' happens to be a meaningful
// optimization to avoid writing the full line_index when not necessarily needed
// (e.g. very large buffer, scrolled up, inactive)
static int InputTextLineIndexBuild(ImGuiInputTextFlags flags,
                                   ImGuiTextIndex* line_index, const char* buf,
                                   const char* buf_end, float wrap_width,
  ...
  if (flags & ImGuiInputTextFlags_WordWrap) {
    for (s = buf; s < buf_end; s = (*s == '\n') ? s + 1 : s) {
      ...
      s = ImFontCalcWordWrapPositionEx(g.Font, g.FontSize, s, buf_end,
                                       wrap_width,
                                       ImDrawTextFlags_WrapKeepBlanks);
    }
...
static ImVec2 InputTextLineIndexGetPosOffset(
    ImGuiContext& g, ImGuiInputTextState* state, ImGuiTextIndex* line_index,
  ...
  if (it > it_begin)
    if (it == it_end || *it != cursor_n ||
        (state != NULL && state->WrapWidth > 0.0f &&
         state->LastMoveDirectionLR == ImGuiDir_Right &&
         cursor_ptr[-1] != '\n' && cursor_ptr[-1] != 0))
      it--;

  const int line_no =
      (it == it_begin) ? 0 : line_index->Offsets.index_from_ptr(it);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf,
  ...
  IM_ASSERT(
      !((flags & ImGuiInputTextFlags_ElideLeft) &&
        (flags & ImGuiInputTextFlags_Multiline)));  // Multiline does not not
                                                    // work with left-trimming
  ...
  if (is_multiline) {
    ...
    // Prevent NavActivation from explicit Tabbing when our widget accepts Tab
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding,
                 ImVec2(0, 0));  // Ensure no clip rect so mouse hover can reach
                                 // FramePadding edges
    bool child_visible =
        BeginChildEx(label, id, frame_bb.GetSize(), ImGuiChildFlags_Borders,
    ...
    draw_window->DC.NavLayersActiveMaskNext |=
        (1 << draw_window->DC
                  .NavLayerCurrent);  // This is to ensure that EndChild() will
                                      // display a navigation highlight so we
                                      // can "enter" into it.
  ...
  // Word-wrapping: enforcing a fixed width not altered by vertical scrollbar
  // makes things easier, notably to track cursor reliably and avoid one-frame
  // glitches. Instead of using ImGuiWindowFlags_AlwaysVerticalScrollbar we
  // account for that space if the scrollbar is not visible.
  const bool is_wordwrap = (flags & ImGuiInputTextFlags_WordWrap) != 0;
  float wrap_width = 0.0f;
  if (is_wordwrap)
    wrap_width = ImMax(
        1.0f, GetContentRegionAvail().x +
  ...
  if (init_reload_from_user_buf) {
    ...
    InputTextReconcileUndoState(state, state->TextA.Data, state->TextLen, buf,
                                new_len);
    ...
    state->Stb->cursor = state->Stb->select_end =
        state->ReloadSelectionEnd;  // will be clamped to bounds below
    ...
    if (recycle_state && (state->TextLen != buf_len ||
    ...
    if (flags & ImGuiInputTextFlags_ElideLeft)
      state->Scroll.x += ImMax(0.0f, CalcTextSize(buf).x - frame_size.x +
  ...
  if (g.ActiveId != id && init_make_active) {
    ...
    SetFocusID(id, window);
    FocusWindow(window);
    if (input_requested_by_nav) SetNavCursorVisibleAfterMove();
  }
  if (g.ActiveId == id) {
    // Declare some inputs, the other are registered and polled via Shortcut()
    // routing system.
    // FIXME: The reason we don't use Shortcut() is we would need a routing flag
    // to specify multiple mods, or to all mods combination into individual
    // shortcuts.
    ...
    // Read-only mode always ever read from source buffer. Refresh TextLen when
    // active.
    if (is_readonly && state != NULL) state->TextLen = (int)ImStrlen(buf);
    if (state != NULL) state->CursorClamp();
    // if (is_readonly && state != NULL)
    //     state->TextA.clear(); // Uncomment to facilitate debugging, but we
    //     otherwise prefer to keep/amortize th allocation.
  }
  ...
  if (is_wordwrap && state != NULL && state->ID == id &&
  ...
  if (g.ActiveId == id) {
    ...
    if (select_all) {
      ...
      if ((multiclick_count % 2) == 0) {
        ...
        state->WrapWidth =
            0.0f;  // Temporarily disable wrapping so we use real line start.
    ...
    if ((flags & ImGuiInputTextFlags_AllowTabInput) && !is_readonly) {
      if (Shortcut(ImGuiKey_Tab, ImGuiInputFlags_Repeat, id)) {
        ...
        if (InputTextFilterCharacter(&g, state, &c, callback,
                                     callback_user_data))
  ...
  if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
    ...
    // use Ctrl+A and Ctrl+B: former would be handled by InputText) Otherwise we
    ...
    const bool is_enter = Shortcut(ImGuiKey_Enter, f_repeat, id) ||
                          Shortcut(ImGuiKey_KeypadEnter, f_repeat, id);
    const bool is_ctrl_enter =
        Shortcut(ImGuiMod_Ctrl | ImGuiKey_Enter, f_repeat, id) ||
    ...
    if (IsKeyPressed(ImGuiKey_LeftArrow)) {
      ...
      if (!is_multiline || is_gamepad_validate ||
          (ctrl_enter_for_new_line != is_ctrl_enter)) {
        validated = true;
      ...
      if (g.PlatformIO.Platform_SetClipboardTextFn != NULL) {
        ...
        // state->TextSrc may point to read-only user buffer, so we need to make
        // a copy.
      ...
      if (const char* clipboard = GetClipboardText()) {
        ...
        for (const char* s = clipboard; *s != 0;) {
          ...
          memcpy(clipboard_filtered.Data + clipboard_filtered.Size - out_len,
                 c_utf8, out_len);
        }
  ...
  if (g.ActiveId == id) {
    ...
    // callbacks. If we do that, need to ensure that as special case, 'validated
    // == true' also writes back. This also allows the user to use InputText()
    // without maintaining any user-side storage. (please note that if you use
    // this property along ImGuiInputTextFlags_CallbackResize you can end up
    // with your temporary string object unnecessarily allocating once a frame,
    ...
    if (apply_edit_back_to_user_buffer) {
      ...
      if ((flags & (ImGuiInputTextFlags_CallbackCompletion |
        ...
        if (event_flag) {
          ...
          // FIXME-OPT: Undo stack reconcile needs a backup of the data until we
          // rework API, see #7925
          ...
          callback_buf =
              is_readonly
                  ? buf
                  : state->TextA.Data;  // Pointer may have been invalidated by
                                        // a resize callback
          ...
          if (callback_data.BufDirty ||
            ...
            state->CursorFollow = true;
          state->Stb->cursor =
              ImClamp(callback_data.CursorPos, 0, callback_data.BufTextLen);
          state->Stb->select_start = ImClamp(callback_data.SelectionStart, 0,
                                             callback_data.BufTextLen);
          state->Stb->select_end =
              ImClamp(callback_data.SelectionEnd, 0, callback_data.BufTextLen);
          if (callback_data.BufDirty) {
            // Callback may update buffer and thus set buf_dirty even in
            // read-only mode.
            IM_ASSERT(
                callback_data.BufTextLen ==
                (int)ImStrlen(
  ...
  // At this point we need to handle the possibility that a callback could have
  // modified the underlying buffer (#8368)
  ...
  if (is_displaying_hint) {
    ...
    if (render_cursor || render_selection || g.ActiveId == id)
      ...
      buf_display_end =
          NULL;  // Inactive multi-line: end of buffer will be output by
  ...
  if (is_multiline) {
    ...
    // FIXME-OPT: Could append to index when new value of line_visible_n1
    // becomes bigger, see second call to CalcClipRectVisibleItemsY() below.
    bool will_scroll_y =
        state &&
        ((state->CursorFollow && render_cursor) ||
  ...
  // GetForegroundDrawList()->AddRect(draw_pos + ImVec2(0, line_visible_n0 *
  // g.FontSize), draw_pos + ImVec2(frame_size.x, line_visible_n1 * g.FontSize),
  ...
  const ImU32 text_col =
      GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
  if (render_cursor || render_selection) {
    ...
    if (state->CursorCenterY) {
      if (is_multiline)
        new_scroll_y = cursor_offset.y - g.FontSize -
                       (inner_size.y * 0.5f - style.FramePadding.y);
    ...
    if (new_scroll_y != scroll_y) {
      ...
      CalcClipRectVisibleItemsY(clip_rect, draw_pos, g.FontSize,
    ...
    if (render_selection) {
      ...
      const float bg_eol_width =
          IM_TRUNC(g.FontBaked->GetCharAdvance((ImWchar)' ') *
      ...
      for (int line_n = line_visible_n0; line_n < line_visible_n1; line_n++) {
        ...
        const char* line_selected_begin =
            (text_selected_begin > p) ? text_selected_begin : p;
        const char* line_selected_end =
            (text_selected_end < p_eol) ? text_selected_end : p_eol;
        ...
        if (text_selected_begin <= p_eol && text_selected_end > p_eol &&
            !p_eol_is_wrap)
        ...
        ImRect rect;
        rect.Min.x =
            draw_pos.x - draw_scroll.x + CalcTextSize(p, line_selected_begin).x;
  ...
  if (g.ActiveId != id && flags & ImGuiInputTextFlags_ElideLeft)
    draw_pos.x =
        ImMin(draw_pos.x, frame_bb.Max.x - CalcTextSize(buf_display, NULL).x -
  ...
  if (render_cursor) {
    ...
    if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
      draw_window->DrawList->AddLine(
          cursor_screen_rect.Min, cursor_screen_rect.GetBL(),
    ...
    // Windows IME can cover our cursor. Bit of an extra nicety.) This is
    // required for some backends (SDL3) to start emitting character/text
    // inputs. As per #6341, make sure we don't set that on the deactivating
    // frame.
  ...
  if (is_multiline) {
    ...
    // ItemAdd() call specifies the ImGuiItemFlags_Inputable (see #4761,
    // #7870)...
    Dummy(ImVec2(text_size.x, text_size.y + style.FramePadding.y));
    g.NextItemData.ItemFlags |=
        (ImGuiItemFlags)ImGuiItemFlags_Inputable | ImGuiItemFlags_NoTabStop;
    EndChild();
  ...
  if (state && is_readonly) state->TextSrc = NULL;
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* state) {
  ...
  Text("TextLen: %d, Cursor: %d%s, Selection: %d..%d", state->TextLen,
       stb_state->cursor,
                     ...
                     ImGuiChildFlags_ResizeY))  // Visualize undo state
  {
    ...
    for (int n = 0; n < IMSTB_TEXTEDIT_UNDOSTATECOUNT; n++) {
      ...
      Text(
          "%c [%02d] where %03d, insert %03d, delete %03d, char_storage %03d "
          "\"%.*s\"",
          undo_rec_type, n, undo_rec->where, undo_rec->insert_length,
          undo_rec->delete_length, undo_rec->char_storage, buf_preview_len,
          buf_preview_str);
...
bool ImGui::ColorEdit4(const char* label, float col[4],
  ...
  if ((flags & (ImGuiColorEditFlags_DisplayRGB |
    ...
    const bool draw_color_marker =
        (flags & (ImGuiColorEditFlags_DisplayHSV |
                  ImGuiColorEditFlags_NoColorMarkers)) == 0;
    ...
    const int fmt_idx = hide_prefix                                ? 0
                        : (flags & ImGuiColorEditFlags_DisplayHSV) ? 2
                                                                   : 1;
    const ImGuiSliderFlags drag_flags = draw_color_marker
    ...
    for (int n = 0; n < components; n++) {
      ...
      if (flags & ImGuiColorEditFlags_Float) {
        ...
        value_changed |= DragInt(ids[n], &i[n], 1.0f, 0, hdr ? 0 : 255,
                                 fmt_table_int[fmt_idx][n], drag_flags);
      }
      if (!(flags & ImGuiColorEditFlags_NoOptions))
    ...
    char buf[64];
    if (alpha)
      ImFormatString(buf, IM_COUNTOF(buf), "#%02X%02X%02X%02X",
                     ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255),
                     ImClamp(i[2], 0, 255), ImClamp(i[3], 0, 255));
    else
      ImFormatString(buf, IM_COUNTOF(buf), "#%02X%02X%02X",
                     ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255),
                     ImClamp(i[2], 0, 255));
    SetNextItemWidth(w_inputs);
    if (InputText("##Text", buf, IM_COUNTOF(buf),
...
bool ImGui::ColorPicker4(const char* label, float col[4],
  ...
  if (flags & ImGuiColorEditFlags_PickerHueWheel) {
    ...
    if (IsItemActive() && !is_readonly) {
      ...
      ColorEditRestoreH(
          col, &H);  // Greatly reduces hue jitter and reset to 0 when hue ==
  ...
  if ((flags & ImGuiColorEditFlags_NoBorder) == 0) {
    if (g.Style.FrameBorderSize > 0.0f)
      ...
      window->DrawList->AddRect(
          ...
          rounding);  // Color buttons are often in need of some sort of border
                      // // FIXME-DPI
  }
  ...
  if (BeginPopup("Copy")) {
    int cr = IM_F32_TO_INT8_SAT(col[0]), cg = IM_F32_TO_INT8_SAT(col[1]),
        ...
        ca = (flags & ImGuiColorEditFlags_NoAlpha) ? 255
                                                   : IM_F32_TO_INT8_SAT(col[3]);
    char buf[64];
    ImFormatString(buf, IM_COUNTOF(buf), "(%.3ff, %.3ff, %.3ff, %.3ff)", col[0],
                   col[1], col[2],
                   (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : col[3]);
...
static void TreeNodeStoreStackData(ImGuiTreeNodeFlags flags, float x1) {
  ...
  // Initially I tried to latch value for GetColorU32(ImGuiCol_TreeLines) but
  // it's not a good trade-off for very large trees.
  const bool draw_lines = (flags & (ImGuiTreeNodeFlags_DrawLinesFull |
                                    ImGuiTreeNodeFlags_DrawLinesToNodes)) != 0;
  ...
  tree_node_data->DrawLinesTableColumn =
      (draw_lines && g.CurrentTable)
          ? (ImGuiTableColumnIdx)g.CurrentTable->CurrentColumn
          : -1;
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags,
  ...
  // When not framed, we vertically increase height up to typical framed widget
  // height
  const bool display_frame = (flags & ImGuiTreeNodeFlags_Framed) != 0;
  const bool use_frame_padding =
  ...
  frame_bb.Max.y =
      window->DC.CursorPos.y + (text_offset_y - padding.y) + frame_height;
  if (display_frame) {
  ...
  // ImGuiTreeNodeFlags_NavLeftJumpsToParent enabled: Store data for the current
  ...
  // ImGuiTreeNodeFlags_NavLeftJumpsToParent by default or move it to
  // ImGuiStyle.
    ...
    if ((flags & ImGuiTreeNodeFlags_DrawLinesToNodes) &&
      ...
      parent_data->DrawLinesToNodesY2 =
          ImMax(parent_data->DrawLinesToNodesY2,
                window->DC.CursorPos.y);  // Don't need to aim to mid Y position
                                          // as we are clipped anyway.
      if (frame_bb.Min.y >= window->ClipRect.Max.y)
        window->DC.TreeRecordsClippedNodesY2Mask &=
            ~(1 << (window->DC.TreeDepth - 1));  // Done
    }
  ...
  // Render
  {
    ...
    if (draw_tree_lines)
      TreeNodeDrawLineToChildNode(ImVec2(text_pos.x - text_offset_x + padding.x,
                                         text_pos.y + g.FontSize * 0.5f));
  ...
  if (is_open && store_tree_node_stack_data)
    TreeNodeStoreStackData(
        flags, text_pos.x - text_offset_x);  // Call before TreePushOverrideID()
  if (is_open && !(flags & ImGuiTreeNodeFlags_NoTreePushOnOpen))
    TreePushOverrideID(
...
// This is only called for visible child nodes so we are not too fussy anymore
// about performances
void ImGui::TreeNodeDrawLineToChildNode(const ImVec2& target_pos) {
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
  if (window->DC.TreeDepth == 0 || (window->DC.TreeHasStackDataDepthMask &
                                    (1 << (window->DC.TreeDepth - 1))) == 0)
    return;
  ...
  float rounding = (g.Style.TreeLinesRounding > 0.0f)
                       ? ImMin(x2 - x1, g.Style.TreeLinesRounding)
                       : 0.0f;
  parent_data->DrawLinesToNodesY2 =
      ImMax(parent_data->DrawLinesToNodesY2, y - rounding);
  if (x1 >= x2) return;
  if (rounding > 0.0f) {
    x1 += 0.5f + rounding;
    window->DrawList->PathArcToFast(ImVec2(x1, y - rounding), rounding, 6, 3);
    if (x1 < x2) window->DrawList->PathLineTo(ImVec2(x2, y));
    window->DrawList->PathStroke(GetColorU32(ImGuiCol_TreeLines),
                                 ImDrawFlags_None, g.Style.TreeLinesSize);
  } else {
    window->DrawList->AddLine(ImVec2(x1, y), ImVec2(x2, y),
                              GetColorU32(ImGuiCol_TreeLines),
...
void ImGui::TreeNodeDrawLineToTreePop(const ImGuiTreeNodeStackData* data) {
  ...
  if (data->TreeFlags & ImGuiTreeNodeFlags_DrawLinesFull) {
    ...
    if (y2 + (g.Style.ItemSpacing.y + g.Style.TreeLinesRounding) <
        y2_full)  // FIXME: threshold to use ToNodes Y2 instead of Full Y2 when
                  // close by ItemSpacing.y
      y2 = y2_full;
  }
  ...
  window->DrawList->AddLine(ImVec2(x, y1), ImVec2(x, y2),
                            GetColorU32(ImGuiCol_TreeLines),
                            g.Style.TreeLinesSize);
...
void ImGui::TreePop() {
  ...
  if (window->DC.TreeHasStackDataDepthMask & tree_depth_mask) {
    ...
    // ImGuiTreeNodeFlags_NavLeftJumpsToParent is enabled)
...
bool ImGui::Selectable(const char* label, bool selected,
  ...
  if (is_visible) {
    ...
    if (highlighted || selected) {
      // Between 1.91.0 and 1.91.4 we made selected Selectable use an arbitrary
      // lerp between _Header and _HeaderHovered. Removed that now. (#8106)
  ...
  if (is_visible)
    RenderTextClipped(
        pos,
        ImVec2(ImMin(pos.x + size.x, window->WorkRect.Max.x), pos.y + size.y),
  ...
  // if you need that state updated (e.g. for rendering purpose) before reaching
  // EndMultiSelect().
  IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
...
int ImGui::TypingSelectFindNextSingleCharMatch(
  ...
  for (int idx = 0; idx < items_count; idx++) {
    ...
    if (first_match_idx == -1 &&
        nav_item_idx == -1)  // Return first match immediately if we don't have
                             // a nav_item_idx value.
      return idx;
...
static void BoxSelectScrollWithMouseDrag(ImGuiBoxSelectState* bs,
  ...
  for (int n = 0; n < 2; n++)  // each axis
  {
    ...
    const float scroll_step =
        g.FontSize * 35.0f * speed_multiplier * ImSign(dist) * g.IO.DeltaTime;
...
bool ImGui::BeginBoxSelect(const ImRect& scope_rect, ImGuiWindow* window,
  ...
  if (ms_flags & ImGuiMultiSelectFlags_BoxSelect2d)
    if (bs->BoxSelectRectPrev.Min.x != bs->BoxSelectRectCurr.Min.x ||
      ...
      bs->UnclipMode = true;
      bs->UnclipRect =
          bs->BoxSelectRectPrev;  // FIXME-OPT: UnclipRect x coordinates could
                                  // be intersection of Prev and Curr rect on X
                                  // axis.
...
void ImGui::EndBoxSelect(const ImRect& scope_rect,
  ...
  if (enable_scroll) {
    ...
    // GetForegroundDrawList()->AddRect(scroll_r.Min, scroll_r.Max, IM_COL32(0,
    // 255, 0, 255));
...
ImGuiMultiSelectIO* ImGui::BeginMultiSelect(ImGuiMultiSelectFlags flags,
  ...
  if (ImGuiTable* table = g.CurrentTable)
    if (table->CurrentColumn != -1)
      TableEndCell(table);  // This is currently safe to call multiple time. If
  ...
  // (we compare FocusScopeId so it possible to use multiple selections inside a
  // same window)
  ...
  if (ms->IsFocused) {
    ...
    // - Only claim shortcut if selection is not empty, allowing further presses
    // on Escape to e.g. leave current child window.
...
ImGuiMultiSelectIO* ImGui::EndMultiSelect() {
  ...
  if (ms->IsFocused) {
    // We currently don't allow user code to modify RangeSrcItem by writing to
    // BeginIO's version, but that would be an easy change here.
    if (ms->IO.RangeSrcReset ||
        (ms->RangeSrcPassedBy == false &&
         ms->IO.RangeSrcItem !=
             ImGuiSelectionUserData_Invalid))  // Can't read
                                               // storage->RangeSrcItem here ->
                                               // we want the state at beginning
                                               // of the scope (see tests for
                                               // easy failure)
    {
      IMGUI_DEBUG_LOG_SELECTION(
        ...
        if (ms->Flags & ImGuiMultiSelectFlags_ScopeRect)
          SetNavID(
              ...
              ImRect(g.IO.MousePos,
                     g.IO.MousePos));  // Automatically switch FocusScope for
  ...
  if (ms->Flags & ImGuiMultiSelectFlags_NavWrapX) {
    IM_ASSERT(
        ms->Flags &
        ImGuiMultiSelectFlags_ScopeWindow);  // Only supported at window scope
...
void ImGui::SetNextItemSelectionUserData(
  ...
  if (ImGuiMultiSelectTempData* ms = g.CurrentMultiSelect) {
    // Auto updating RangeSrcPassedBy for cases were clipper is not used (done
    // before ItemAdd() clipping)
    g.NextItemData.ItemFlags |=
...
void ImGui::MultiSelectItemHeader(ImGuiID id, bool* p_selected,
  ...
  if (ms->IsFocused) {
    ...
    // This is only useful if the user hasn't processed them already, and this
    // only works if the user isn't using the clipper. If you are using a
    // clipper you need to process the SetAll request after calling
    // BeginMultiSelect()
    if (ms->LoopRequestSetAll != -1) selected = (ms->LoopRequestSetAll == 1);

    // When using Shift+Nav: because it can incur scrolling we cannot afford a
    // frame of lag with the selection highlight (otherwise scrolling would
    // happen before selection) For this to work, we need someone to set
    // 'RangeSrcPassedBy = true' at some point (either clipper either
    ...
    if (ms->IsKeyboardSetRange) {
      ...
      if (is_range_dst &&
          storage->RangeSrcItem ==
              ImGuiSelectionUserData_Invalid)  // If we don't have RangeSrc,
  ...
  // selection on click. Enabling this test makes actions using Ctrl+Shift delay
  // their effect on MouseUp which is annoying, but it allows drag and drop of
  // multiple items.
...
void ImGui::MultiSelectItemFooter(ImGuiID id, bool* p_selected,
  ...
  if (g.NavJustMovedToId == id) {
    if ((flags & ImGuiMultiSelectFlags_NoAutoSelect) == 0) {
      ...
      if (is_shift)
        ...
        apply_to_range_src = true;  // Since if (pressed) {} main block is not
                                    // running we update this
    }
  }
    ...
    if (ImGuiBoxSelectState* bs = GetBoxSelectState(ms->BoxSelectId)) {
      ...
      if ((rect_overlap_curr && !rect_overlap_prev && !selected) ||
        ...
        if (storage->LastSelectionSize <= 0 && bs->IsStartedSetNavIdOnce) {
          pressed = true;  // First item act as a pressed: code below will emit
                           // selection request and set NavId (whatever we emit
                           // here will be overridden anyway)
  ...
  // Right-click handling.
  // FIXME-MULTISELECT: Maybe should be moved to Selectable()? Also see #5816,
  // #8200, #9015
  if (hovered && IsMouseClicked(1) &&
      (flags & (ImGuiMultiSelectFlags_NoAutoSelect |
                ImGuiMultiSelectFlags_NoSelectOnRightClick)) == 0) {
  ...
  // unless current item is not selected. The later, "unless current item is not
  // select", may become optional? It seems like a better default if Enter
  // doesn't necessarily open something (unlike e.g. Windows explorer). For use
  // case where Enter always open something, we might decide to make this
  // optional?
  ...
  if (pressed && (!enter_pressed || !selected)) {
    ...
    //----------------------------------------------------------------------------------------
    // Keys Navigated:             | Clear  | Src=item, Sel=1 SetRange 1 Keys
    // Navigated: Ctrl        | n/a    | n/a Keys Navigated:      Shift  | n/a
    // | Dst=item, Sel=1,   => Clear + SetRange 1 Keys Navigated: Ctrl+Shift  |
    // n/a    | Dst=item, Sel=Src  => Clear + SetRange Src-Dst Keys Activated:
    // | n/a    | Src=item, Sel=1    => Clear + SetRange 1 Keys Activated: Ctrl
    // | n/a    | Src=item, Sel=!Sel =>         SetSange 1 Keys Activated: Shift
    ...
    // SetRange 1 Mouse Pressed:       Shift  | n/a    | Dst=item, Sel=1,   =>
    // Clear + SetRange 1 Mouse Pressed:  Ctrl+Shift  | n/a    | Dst=item,
    // Sel=!Sel =>         SetRange Src-Dst
    //----------------------------------------------------------------------------------------
    ...
    if ((flags & ImGuiMultiSelectFlags_NoAutoClear) == 0) {
      ...
      if (is_singleselect)
        request_clear = true;
                ...
                g.NavActivateId == id) &&
               !is_ctrl)
        request_clear = (flags & ImGuiMultiSelectFlags_NoAutoClearOnReselect)
                            ? !selected
                            : true;
      else if ((input_source == ImGuiInputSource_Keyboard ||
                input_source == ImGuiInputSource_Gamepad) &&
               is_shift && !is_ctrl)
        request_clear = true;  // With is_shift==false the RequestClear was done
    ...
    if (is_shift && !is_singleselect) {
      ...
      if ((flags & ImGuiMultiSelectFlags_NoAutoSelect) == 0) {
        ...
        // Ctrl+Shift+Arrow copy source selection state (already stored by
        // BeginMultiSelect() in storage->RangeSelected)
...
void ImGui::MultiSelectAddSetRange(ImGuiMultiSelectTempData* ms, bool selected,
  ...
  // Merge contiguous spans into same request (unless NoRangeSelect is set which
  // guarantees single-item ranges)
...
void ImGuiSelectionBasicStorage::ApplyRequests(ImGuiMultiSelectIO* ms_io) {
  // For convenience we obtain ItemsCount as passed to BeginMultiSelect(), which
  // is optional. It makes sense when using ImGuiSelectionBasicStorage to simply
  // pass your items count to BeginMultiSelect(). Other scheme may handle SetAll
  // differently.
  ...
  for (ImGuiSelectionRequest& req : ms_io->Requests) {
    if (req.Type == ImGuiSelectionRequestType_SetAll) {
      ...
      // ImGuiContext& g = *GImGui; IMGUI_DEBUG_LOG_SELECTION("Req %d/%d: set %d
      // to %d\n", ms_io->Requests.index_from_ptr(&req), ms_io->Requests.Size,
      // selection_changes, req.Selected);
      if (selection_changes == 1 || (selection_changes < Size / 100)) {
        // Multiple sorted insertion + copy likely to be faster.
        // Technically we could do a single copy with a little more work (sort
        // sequential SetRange requests)
        ...
        // Use req.RangeDirection to set order field so that Shift+Clicking from
        // 1 to 5 is different than Shift+Clicking from 5 to 1
        const int size_before_amends = _Storage.Data.Size;
        int selection_order =
            _SelectionOrder +
...
bool ImGui::ListBox(const char* label, int* current_item,
  ...
  clipper.Begin(
      ...
      GetTextLineHeightWithSpacing());  // We know exactly our line height here
                                        ...
                                        // optimization, but generally you don't
                                        // need to.
  clipper.IncludeItemByIndex(*current_item);
  while (clipper.Step())
    for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++) {
...
bool ImGui::BeginMenuBar() {
  ...
  const float border_top =
      ImMax(window->WindowBorderSize * 0.5f - window->TitleBarHeight, 0.0f);
  ...
  if (NavMoveRequestButNoResultYet() &&
    ...
    if (nav_earliest_child->ParentWindow == window &&
      ...
      if (g.NavCursorVisible) {
        g.NavCursorVisible =
            false;  // Hide nav cursor for the current frame so we don't see the
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled) {
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    RenderArrow(
        window->DrawList,
        ImVec2(text_pos.x + offsets->OffsetMark + extra_w + g.FontSize * 0.30f,
               text_pos.y),
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    if (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible) {
      ...
      if (shortcut_w > 0.0f) {
        ...
        LogSetNextTextDecoration("(", ")");
        RenderText(text_pos + ImVec2(offsets->OffsetShortcut + stretch_w, 0.0f),
                   shortcut, NULL, false);
        PopStyleColor();
      }
      if (selected)
        RenderCheckMark(window->DrawList,
                        text_pos + ImVec2(offsets->OffsetMark + stretch_w +
                                              g.FontSize * 0.40f,
...
bool ImGui::BeginTabBar(const char* str_id, ImGuiTabBarFlags flags) {
  ...
  tab_bar->SeparatorMinX =
      tab_bar_bb.Min.x - IM_TRUNC(window->WindowPadding.x * 0.5f);
  tab_bar->SeparatorMaxX =
      tab_bar_bb.Max.x + IM_TRUNC(window->WindowPadding.x * 0.5f);
...
void ImGui::EndTabBar() {
  ...
  IM_ASSERT_USER_ERROR_RET(tab_bar != NULL,
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar) {
  ...
  const bool scroll_to_selected_tab =
  ...
  sections[0].Spacing = sections[0].TabCount > 0 && (sections[1].TabCount +
                            ...
                            : 0.0f;
  sections[1].Spacing =
      sections[1].TabCount > 0 && sections[2].TabCount > 0 ? tab_spacing : 0.0f;
  ...
  for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++) {
    ...
    section->Width +=
        tab->ContentWidth + (section_n == curr_section_n ? tab_spacing : 0.0f);
    section->WidthAfterShrinkMinWidth +=
        ImMin(tab->ContentWidth, shrink_min_width) +
        (section_n == curr_section_n ? tab_spacing : 0.0f);
  ...
  for (int section_n = 0; section_n < 3; section_n++) {
    ...
    width_all_tabs_after_min_width_shrink +=
        sections[section_n].WidthAfterShrinkMinWidth +
  ...
  tab_bar->ScrollButtonEnabled =
      ((width_all_tabs_to_use_for_scroll > tab_bar->BarRect.GetWidth() &&
        tab_bar->Tabs.Size > 1) &&
  ...
  const bool can_shrink =
      (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyShrink) ||
      (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyMixed);
  if (width_excess >= 1.0f && (can_shrink || !central_section_is_visible)) {
    ...
    ShrinkWidths(g.ShrinkWidthBuffer.Data + shrink_data_offset,
                 shrink_data_count, width_excess, shrink_min_width);
...
void ImGui::TabBarQueueReorderFromMousePos(ImGuiTabBar* tab_bar,
  ...
  for (int i = src_idx; i >= 0 && i < tab_bar->Tabs.Size; i += dir) {
    ...
    const float x2 =
        bar_offset + dst_tab->Offset + dst_tab->Width + tab_spacing;
...
void ImGui::TabItemSpacing(const char* str_id, ImGuiTabItemFlags flags,
                           float width) {
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
  if (window->SkipItems) return;
  ...
  if (is_visible) {
    ...
    if (tab_contents_visible &&
      ...
      if (style.TabRounding > 0.0f) {
        ...
        display_draw_list->PathArcToFast(tl + ImVec2(+rounding, +rounding),
                                         rounding, 7, 9);
        display_draw_list->PathArcToFast(tr + ImVec2(-rounding, +rounding),
                                         rounding, 9, 11);
        display_draw_list->PathStroke(overline_col, 0,
                                      style.TabBarOverlineSize);
      } else {
        display_draw_list->AddLine(tl - ImVec2(0.5f, 0.5f),
                                   tr - ImVec2(0.5f, 0.5f), overline_col,
                                   style.TabBarOverlineSize);
      }
    }
    RenderNavCursor(bb, id);
...
void ImGui::TabItemBackground(ImDrawList* draw_list, const ImRect& bb,
  ...
  const float rounding = ImMax(
      0.0f, ImMin((flags & ImGuiTabItemFlags_Button) ? g.Style.FrameRounding
                                                     : g.Style.TabRounding,
                  width * 0.5f - 1.0f));
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb,
  ...
  ImRect text_ellipsis_clip_bb(bb.Min.x + frame_padding.x,
  ...
  bool is_hovered =
      g.HoveredId == tab_id || g.HoveredId == close_button_id ||
      g.ActiveId == tab_id ||
    ...
    if (is_contents_visible)
      close_button_visible =
          (g.Style.TabCloseButtonMinWidthSelected < 0.0f)
              ...
              : (is_hovered &&
                 bb.GetWidth() >=
                     ImMax(button_sz, g.Style.TabCloseButtonMinWidthSelected));
    else
      close_button_visible =
  ...
  // When tabs/document is unsaved, the unsaved marker takes priority over the
  // close button.
  ...
  RenderTextEllipsis(draw_list, text_ellipsis_clip_bb.Min,
                     text_ellipsis_clip_bb.Max, ellipsis_max_x, label, NULL,
                     &label_size);
