...
void ImGui::TextAlignedV(float align_x, float size_x, const char* fmt,
	...
	RenderTextEllipsis(window->DrawList, pos, pos_max, pos_max.x, text,
					   text_end, &text_size);
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered,
	...
	// where holding the button pressed for a long time while drag a payload
	// item triggers the button.
	if (g.DragDropActive) {
	...
	if ((item_flags & ImGuiItemFlags_Disabled) == 0) {
		...
		if (g.NavActivateDownId == id) {
			...
			if (!nav_activated_by_inputs &&
				(item_flags & ImGuiItemFlags_ButtonRepeat)) {
				// Avoid pressing multiple keys from triggering excessive amount
				// of repeat events
			...
			if (nav_activated_by_code || nav_activated_by_inputs) {
				...
				// IsItemActive(), equivalent of holding the mouse button.
				pressed = true;
				SetActiveID(id, window);
...
bool ImGui::InvisibleButton(const char* str_id, const ImVec2& size_arg,
	...
	ImVec2 size =
		CalcItemSize(ImVec2(size_arg.x != 0.0f ? size_arg.x : -FLT_MIN,
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis,
	...
	if ((axis == ImGuiAxis_Y) && bb_frame_height < bb_frame_width)
		alpha =
			ImSaturate(bb_frame_height / ImMax(bb_frame_width * 2.0f, 1.0f));
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureRef tex_ref,
	...
	if (image_rounding > 0.0f)
		window->DrawList->AddImageRounded(
			tex_ref, bb.Min + padding, bb.Max - padding, uv0, uv1,
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness) {
	...
	IM_ASSERT(ImIsPowerOfTwo(
		flags & (ImGuiSeparatorFlags_Horizontal |
				 ImGuiSeparatorFlags_Vertical)));  // Check that only 1 option
												   // is selected
...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem* items, int count,
	...
	while (width_excess > 0.001f && count_same_width < count) {
		...
		for (int item_n = 0; item_n < count_same_width; item_n++) {
			float width_to_remove_for_this_item = ImMin(
				base_width_to_remove_per_item, items[item_n].Width - width_min);
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb,
	...
	if (!ret) {
		...
		if (!g.IO.ConfigDebugBeginReturnValueOnce &&
			!g.IO.ConfigDebugBeginReturnValueLoop)	// Begin may only return
													// false with those debug
													// tools activated.
...
void ImGui::DataTypeApplyOp(ImGuiDataType data_type, int op, void* output,
	...
	switch (data_type) {
		case ImGuiDataType_S8:
			if (op == '+') {
				*(ImS8*)output =
					ImAddClampOverflow(*(const ImS8*)arg1, *(const ImS8*)arg2,
									   IM_S8_MIN, IM_S8_MAX);
			}
			if (op == '-') {
				*(ImS8*)output =
					ImSubClampOverflow(*(const ImS8*)arg1, *(const ImS8*)arg2,
									   IM_S8_MIN, IM_S8_MAX);
			}
			return;
		case ImGuiDataType_U8:
			if (op == '+') {
				*(ImU8*)output =
					ImAddClampOverflow(*(const ImU8*)arg1, *(const ImU8*)arg2,
									   IM_U8_MIN, IM_U8_MAX);
			}
			if (op == '-') {
				*(ImU8*)output =
					ImSubClampOverflow(*(const ImU8*)arg1, *(const ImU8*)arg2,
									   IM_U8_MIN, IM_U8_MAX);
			}
			return;
...
TYPE ImGui::RoundScalarWithFormatT(const char* format, ImGuiDataType data_type,
	...
	if (fmt_start[0] != '%' ||
		fmt_start[1] == '%')  // Don't apply if the value is not visible in the
							  // format string
		return v;
				...
				if (decimal_precision > 0) {
					...
					if ((v_range_f >= -100.0f && v_range_f <= 100.0f &&
						...
						input_delta = ((input_delta < 0.0f) ? -1.0f : +1.0f) /
									  v_range_f;  // Keyboard/Gamepad tweak
...
const char* ImParseFormatTrimDecorations(const char* fmt, char* buf,
	...
	if (fmt_end[0] == 0)  // If we only have leading decoration, we don't need
						  // to copy the data.
		return fmt_start;
...
static int STB_TEXTEDIT_MOVELINESTART_IMPL(ImGuiInputTextState* obj,
	...
	if (obj->WrapWidth > 0.0f) {
		...
		while (p >= p_bol) {
			...
			if (p ==
				p_cursor)  // If we are already on a visible beginning-of-line,
		...
		while (p < text_end) {
			...
			if (p_eol == p_cursor &&
				obj->LastMoveDirectionLR !=
					ImGuiDir_Left)	// If we are already on a visible
									// end-of-line, switch to regular handle
				break;
...
static int STB_TEXTEDIT_INSERTCHARS(ImGuiInputTextState* obj, int pos,
	...
	if (!is_resizable && new_text_len > avail)
		new_text_len =
			(int)(ImTextFindValidUtf8CodepointEnd(new_text,
												  new_text + new_text_len,
												  new_text + avail) -
				  new_text);  // Truncate to closest UTF-8 codepoint.
							  // Alternative: return 0 to cancel insertion.
	...
	if (text_len + new_text_len + 1 > obj->TextA.Size && is_resizable) {
		obj->TextA.resize(
			text_len + ImClamp(new_text_len, 32, ImMax(256, new_text_len)) + 1);
...
void ImGuiInputTextState::OnKeyPressed(int key) {
	...
	if (key_u == STB_TEXTEDIT_K_LEFT || key_u == STB_TEXTEDIT_K_LINESTART ||
		key_u == STB_TEXTEDIT_K_TEXTSTART ||
		...
		LastMoveDirectionLR = ImGuiDir_Left;
			 ...
			 key_u == STB_TEXTEDIT_K_TEXTEND ||
			 key_u == STB_TEXTEDIT_K_DELETE ||
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf,
	...
	if (is_multiline) {
		...
		// Prevent NavActivation from explicit Tabbing when our widget accepts
		// Tab inputs: this allows cycling through widgets without stopping.
	...
	if (init_reload_from_user_buf) {
		...
		IM_ASSERT(
			((buf_len + 1 <= buf_size) || (buf_len == 0 && buf_size == 0)) &&
			"Is your input buffer properly zero-terminated?");
	...
	if (g.ActiveId == id) {
		...
		// flag to specify multiple mods, or to all mods combination into
		...
		if (select_all) {
			...
			if ((multiclick_count % 2) == 0) {
				...
				const bool is_eol = ImStb::STB_TEXTEDIT_GETCHAR(
										state, state->Stb->cursor) == '\n';
				state->WrapWidth = 0.0f;  // Temporarily disable wrapping so we
										  // use real line start.
	...
	if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
		...
		// trying to use Ctrl+A and Ctrl+B: former would be handled by
		...
		const bool is_redo =
			(Shortcut(ImGuiMod_Ctrl | ImGuiKey_Y, f_repeat, id) ||
			 Shortcut(ImGuiMod_Ctrl | ImGuiMod_Shift | ImGuiKey_Z, f_repeat,
					  id)) &&
			!is_readonly && is_undoable;
	...
	if (g.ActiveId == id) {
		...
		if (revert_edit && !is_readonly) {
			...
			} else if (strcmp(state->TextA.Data, state->TextToRevertTo.Data) !=
					   0) {
		...
		// 'validated
		// == true' also writes back. This also allows the user to use
		// InputText() without maintaining any user-side storage. (please note
		// that if you use this property along
			...
			if ((flags & (ImGuiInputTextFlags_CallbackCompletion |
				...
				if (event_flag) {
					...
					if (callback_data.BufDirty ||
						callback_data.CursorPos != state->Stb->cursor)
						state->CursorFollow = true;
					state->Stb->cursor = ImClamp(callback_data.CursorPos, 0,
												 callback_data.BufTextLen);
					state->Stb->select_start =
						ImClamp(callback_data.SelectionStart, 0,
								callback_data.BufTextLen);
					state->Stb->select_end =
						ImClamp(callback_data.SelectionEnd, 0,
								callback_data.BufTextLen);
					if (callback_data.BufDirty) {
	...
	// At this point we need to handle the possibility that a callback could
	// have modified the underlying buffer (#8368)
	...
	// Build line index for easy data access (makes code below simpler and
	// faster)
	...
	// g.FontSize), draw_pos + ImVec2(frame_size.x, line_visible_n1 *
	// g.FontSize), IM_COL32(255, 0, 0, 255));

	// Calculate blinking cursor position
	const ImVec2 cursor_offset = render_cursor && state
									 ? InputTextLineIndexGetPosOffset(
										   g, state, line_index, buf_display,
	...
	if (render_cursor || render_selection) {
		...
		if (render_cursor && state->CursorFollow) {
			...
			if (!(flags & ImGuiInputTextFlags_NoHorizontalScroll)) {
				...
				if (cursor_offset.x < state->Scroll.x)
					...
					state->Scroll.x = IM_TRUNC(cursor_offset.x - visible_width +
											   scroll_increment_x);
			} else {
				state->Scroll.x = 0.0f;
			}
			...
			if (is_multiline) {
				...
				if (cursor_offset.y - g.FontSize < scroll_y)
					new_scroll_y = ImMax(0.0f, cursor_offset.y - g.FontSize);
						 ...
						 scroll_y)
					new_scroll_y = cursor_offset.y - inner_size.y +
		...
		if (render_selection) {
			...
			for (int line_n = line_visible_n0; line_n < line_visible_n1;
				...
				const bool p_eol_is_wrap =
					(p_eol < buf_display_end && *p_eol != '\n');
				...
				if (line_selected_begin < line_selected_end)
					rect_width +=
						CalcTextSize(line_selected_begin, line_selected_end).x;
				if (text_selected_begin <= p_eol && text_selected_end > p_eol &&
					!p_eol_is_wrap)
					rect_width +=
						bg_eol_width;  // So we can see selected empty lines
	...
	if (g.ActiveId != id && flags & ImGuiInputTextFlags_ElideLeft)
		draw_pos.x = ImMin(draw_pos.x, frame_bb.Max.x -
	...
	if (render_cursor) {
		...
		// that Windows IME can cover our cursor. Bit of an extra nicety.) This
		// is required for some backends (SDL3) to start emitting character/text
		...
		if (!is_readonly && g.ActiveId == id) {
			...
			ime_data->InputPos = ImVec2(cursor_screen_pos.x - 1.0f,
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* state) {
	...
	Text("BufCapacity: %d, LineCount: %d", state->BufCapacity,
		 state->LineCount);
...
bool ImGui::ColorEdit4(const char* label, float col[4],
	...
	if ((flags & (ImGuiColorEditFlags_DisplayRGB |
		...
		const bool hide_prefix =
			draw_color_marker ||
			(w_per_component <= CalcTextSize((flags & ImGuiColorEditFlags_Float)
												 ? "M:0.000"
												 : "M:000")
									.x);
		...
		for (int n = 0; n < components; n++) {
			...
			if (flags & ImGuiColorEditFlags_Float) {
				value_changed |= DragFloat(
					ids[n], &f[n], 1.0f / 255.0f, 0.0f, hdr ? 0.0f : 1.0f,
	...
	if ((flags & ImGuiColorEditFlags_NoBorder) == 0) {
		if (g.Style.FrameBorderSize > 0.0f)
			...
			window->DrawList->AddRect(bb.Min, bb.Max,
									  ...
									  rounding);  // Color buttons are often in
	...
	if (BeginPopup("Copy")) {
		int cr = IM_F32_TO_INT8_SAT(col[0]), cg = IM_F32_TO_INT8_SAT(col[1]),
			...
			ca = (flags & ImGuiColorEditFlags_NoAlpha)
					 ...
					 : IM_F32_TO_INT8_SAT(col[3]);
		char buf[64];
		ImFormatString(buf, IM_COUNTOF(buf), "(%.3ff, %.3ff, %.3ff, %.3ff)",
					   col[0], col[1], col[2],
					   (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : col[3]);
		...
		if (Selectable(buf)) SetClipboardText(buf);
		if (!(flags & ImGuiColorEditFlags_NoAlpha)) {
			ImFormatString(buf, IM_COUNTOF(buf), "#%02X%02X%02X%02X", cr, cg,
						   cb, ca);
			if (Selectable(buf)) SetClipboardText(buf);
		}
		EndPopup();
...
static void TreeNodeStoreStackData(ImGuiTreeNodeFlags flags, float x1) {
	...
	tree_node_data->DrawLinesX1 =
		draw_lines ? (x1 + g.FontSize * 0.5f + g.Style.FramePadding.x)
				   : +FLT_MAX;
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags,
	...
	// When not framed, we vertically increase height up to typical framed
	// widget height
	...
	// ImGuiTreeNodeFlags_NavLeftJumpsToParent enabled: Store data for the
	...
	if (!(flags & ImGuiTreeNodeFlags_NoTreePushOnOpen)) {
		...
		if ((flags & ImGuiTreeNodeFlags_NavLeftJumpsToParent) &&
			!g.NavIdIsAlive)
	...
	if (!is_visible) {
		...
		if (is_open && store_tree_node_stack_data)
			TreeNodeStoreStackData(
				flags,
				text_pos.x -
					text_offset_x);	 // Call before TreePushOverrideID()
...
void ImGui::TreeNodeDrawLineToChildNode(const ImVec2& target_pos) {
	...
	if (rounding > 0.0f) {
		x1 += 0.5f + rounding;
		window->DrawList->PathArcToFast(ImVec2(x1, y - rounding), rounding, 6,
										3);
...
void ImGui::TreeNodeDrawLineToTreePop(const ImGuiTreeNodeStackData* data) {
	...
	if (data->TreeFlags & ImGuiTreeNodeFlags_DrawLinesFull) {
		...
		if (y2 + (g.Style.ItemSpacing.y + g.Style.TreeLinesRounding) <
			y2_full)  // FIXME: threshold to use ToNodes Y2 instead of Full Y2
					  // when close by ItemSpacing.y
			y2 = y2_full;
	}
...
bool ImGui::Selectable(const char* label, bool selected,
	...
	if (is_visible) {
		...
		if (g.NavId == id) {
			...
			if (is_multi_select)
				nav_render_cursor_flags |=
					ImGuiNavRenderCursorFlags_AlwaysDraw;  // Always show the
														   // nav rectangle
	...
	if (is_visible)
		RenderTextClipped(pos,
						  ImVec2(ImMin(pos.x + size.x, window->WorkRect.Max.x),
								 pos.y + size.y),
						  label, NULL, &label_size, style.SelectableTextAlign,
						  &bb);
		...
		if (bs->BoxSelectRectPrev.Min.x != bs->BoxSelectRectCurr.Min.x ||
			...
			bs->UnclipRect =
				bs->BoxSelectRectPrev;	// FIXME-OPT: UnclipRect x coordinates
										// could be intersection of Prev and
										// Curr rect on X axis.
...
void ImGui::EndBoxSelect(const ImRect& scope_rect,
	...
	window->DrawList->AddRect(
		box_select_r.Min, box_select_r.Max,
		GetColorU32(
			ImGuiCol_NavCursor));  // FIXME-MULTISELECT FIXME-DPI: Styling
...
void ImGui::MultiSelectItemFooter(ImGuiID id, bool* p_selected,
	...
	if (pressed && (!enter_pressed || !selected)) {
		...
		// Keys Navigated: Ctrl        | n/a    | n/a Keys Navigated:      Shift
		// | n/a | Dst=item, Sel=1,   => Clear + SetRange 1 Keys Navigated:
		// Ctrl+Shift  | n/a    | Dst=item, Sel=Src  => Clear + SetRange Src-Dst
		// Keys Activated: | n/a    | Src=item, Sel=1    => Clear + SetRange 1
		// Keys Activated: Ctrl | n/a    | Src=item, Sel=!Sel => SetSange 1 Keys
		// Activated: Shift | n/a    | Dst=item, Sel=1    => Clear + SetSange 1
		//----------------------------------------------------------------------------------------
		...
		// => Clear + SetRange 1 Mouse Pressed:  Ctrl+Shift  | n/a    |
		// Dst=item, Sel=!Sel =>         SetRange Src-Dst
		//----------------------------------------------------------------------------------------
...
void ImGui::EndListBox() {
	...
	EndChild();
	EndGroup();	 // This is only required to be able to do IsItemXXX query on
				 // the whole ListBox including label
}
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled) {
	...
	if (g.MenusIdSubmittedThisFrame.contains(id)) {
		if (menu_is_open)
			...
			g.NextWindowData.ClearFlags();	// we behave like Begin() and need
	...
	if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
		...
		LogSetNextTextDecoration("", ">");
		RenderText(ImVec2(text_pos.x + offsets->OffsetLabel, text_pos.y),
				   label);
		if (icon_w > 0.0f)
			RenderText(ImVec2(text_pos.x + offsets->OffsetIcon, text_pos.y),
					   icon);
		RenderArrow(window->DrawList,
					ImVec2(text_pos.x + offsets->OffsetMark + extra_w +
							   g.FontSize * 0.30f,
						   text_pos.y),
	...
	if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
		...
		if (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible) {
			...
			if (shortcut_w > 0.0f) {
				...
				RenderText(
					text_pos +
						ImVec2(offsets->OffsetShortcut + stretch_w, 0.0f),
					shortcut, NULL, false);
				PopStyleColor();
			}
			if (selected)
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar) {
	...
	for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++) {
		...
		if ((tab->Flags & ImGuiTabItemFlags_Button) == 0)
			tab->ContentWidth =
				ImMax(tab->ContentWidth, g.Style.TabMinWidthBase);
...
static void ImGui::TabBarScrollToTab(ImGuiTabBar* tab_bar, ImGuiID tab_id,
	...
	ImGuiContext& g = *GImGui;
	float margin =
		g.FontSize * 1.0f;	// When to scroll to make Tab N+1 visible always
							// make a bit of N visible to suggest more scrolling
							// area (since we don't have a scrollbar)
	...
	if (is_visible) {
		...
		if (text_clipped && g.HoveredId == id && !held)
			if (!(tab_bar->Flags & ImGuiTabBarFlags_NoTooltip) &&
				...
				SetItemTooltip(
					"%.*s", (int)(FindRenderedTextEnd(label) - label), label);
	}
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb,
	...
	if (unsaved_marker_visible) {
		...
		RenderBullet(draw_list, bullet_pos,
