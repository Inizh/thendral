...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags,
	...
	if (table->IsDefaultSizingPolicy &&
		...
		IM_ASSERT_USER_ERROR_RET(
			init_width_or_weight <= 0.0f,
			"TableSetupColumn(): can only specify width/weight if sizing "
			"policy is set explicitly in either Table or Column.");
...
void ImGui::TableFixDisplayOrder(ImGuiTable* table) {
	...
	g.TempBuffer.reserve(
		(int)(sizeof(ImGuiTableFixDisplayOrderColumnData) *
			  table->ColumnsCount));  // FIXME: Maybe wrap those two lines as a
									  // helper.
