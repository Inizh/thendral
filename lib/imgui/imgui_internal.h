...
#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable : 4251)	 // class 'xxx' needs to have dll-interface to
								 // be used by clients of struct 'xxx' // when
...
// ImSpanAllocator<>, ImStableVector<>, ImPool<>, ImChunkStream<>)
...
struct ImFontAtlasBuilder;	  // Internal storage for incrementally packing and
							  // building a ImFontAtlas
struct ImFontAtlasPostProcessData;	// Data available to potential texture
...
struct ImGuiDockNodeSettings;	 // Storage for a dock node in .ini file (we
								 // preserve those even if the associated dock
...
struct ImGuiNavItemData;  // Result of a keyboard/gamepad directional navigation
						  // move query result
struct ImGuiMetricsConfig;	 // Storage for ShowMetricsWindow() and
							 // DebugNodeXXX() functions
...
struct ImGuiSettingsHandler;  // Storage for one type registered in the .ini
							  // file
struct ImGuiStyleMod;  // Stacked style modifier, backup of modified data so we
					   // can restore it
struct ImGuiStyleVarInfo;  // Style variable information (e.g. to access style
						   // variables from an enum)
...
struct ImGuiWindowTempData;	  // Temporary storage for one window (that's the
							  // data which in theory we could ditch at the end
							  // of the frame, in practice we currently keep it
							  // for each window)
...
typedef int ImGuiDataAuthority;	 // -> enum ImGuiDataAuthority_      // Enum:
								 // for storing the source authority (dock node
								 // vs window) of a field
...
typedef int
	ImGuiNavRenderCursorFlags;	// -> enum ImGuiNavRenderCursorFlags_//Flags:
								// for RenderNavCursor()
typedef int ImGuiNavMoveFlags;	// -> enum ImGuiNavMoveFlags_       // Flags:
								// for navigation requests
typedef int ImGuiNextItemDataFlags;	 // -> enum ImGuiNextItemDataFlags_  //
									 // Flags: for SetNextItemXXX() functions
typedef int
	ImGuiNextWindowDataFlags;  // -> enum ImGuiNextWindowDataFlags_// Flags: for
...
typedef int ImGuiTooltipFlags;	// -> enum ImGuiTooltipFlags_       // Flags:
								// for BeginTooltipEx()
typedef int ImGuiTypingSelectFlags;	  // -> enum ImGuiTypingSelectFlags_  //
									  // Flags: for GetTypingSelectRequest()
typedef int ImGuiWindowBgClickFlags;  // -> enum ImGuiWindowBgClickFlags_ //
									  // Flags: for overriding behavior of
									  // clicking on window background/void.
...
#define IMGUI_DEBUG_LOG_FONT(...)                                   \
	do {                                                            \
		...
		if (g2 && g2->DebugLogFlags & ImGuiDebugLogFlags_EventFont) \
			IMGUI_DEBUG_LOG(__VA_ARGS__);                           \
	} while (0)	 // Called from ImFontAtlas function which may operate without a
				 // context.
...
#define IM_TRUNC(_VAL) \
	((float)(int)(_VAL))  // Positive values only! ImTrunc() is not inlined in
						  // MSVC debug builds
#define IM_ROUND(_VAL) ((float)(int)((_VAL) + 0.5f))  // Positive values only!
// #define IM_FLOOR IM_TRUNC             // [OBSOLETE] Renamed in 1.90.0 (Sept
// 2023)
...
IMGUI_API void* ImMemdup(const void* src,
						 size_t size);	// Duplicate a chunk of memory.
...
IMGUI_API const char* ImStristr(
	const char* haystack, const char* haystack_end, const char* needle,
...
IMGUI_API const char* ImStrbol(
	const char* buf_mid_line, const char* buf_begin);  // Find beginning-of-line
...
// Helpers: UTF-8 <> wchar conversions
IMGUI_API int ImTextCharToUtf8(
	char out_buf[5], unsigned int c);  // return output UTF-8 bytes count
IMGUI_API int ImTextStrToUtf8(
	char* out_buf, int out_buf_size, const ImWchar* in_text,
...
IMGUI_API const char* ImTextFindPreviousUtf8Codepoint(
	...
	const char* in_p);	// return previous UTF-8 code-point.
IMGUI_API const char* ImTextFindValidUtf8CodepointEnd(
	const char* in_text_start, const char* in_text_end,
	const char* in_p);	// return previous UTF-8 code-point if 'in_p' is not the
						// end of a valid one.
...
enum ImDrawTextFlags_ {
	...
	ImDrawTextFlags_CpuFineClip =
		1 << 0,	 // Must be == 1/true for legacy with 'bool cpu_fine_clip' arg
				 // to RenderText()
	ImDrawTextFlags_WrapKeepBlanks = 1 << 1,
	ImDrawTextFlags_StopOnNewLine = 1 << 2,
};
IMGUI_API ImVec2 ImFontCalcTextSizeEx(
...
IMGUI_API const char* ImFontCalcWordWrapPositionEx(ImFont* font, float size,
												   const char* text,
												   const char* text_end,
												   float wrap_width,
...
IMGUI_API void ImTextClassifierClear(ImU32* bits, unsigned int codepoint_min,
									 unsigned int codepoint_end,
									 ImWcharClass char_class);
IMGUI_API void ImTextClassifierSetCharClass(ImU32* bits,
											unsigned int codepoint_min,
											unsigned int codepoint_end,
											ImWcharClass char_class,
											unsigned int c);
IMGUI_API void ImTextClassifierSetCharClassFromStr(ImU32* bits,
												   unsigned int codepoint_min,
												   unsigned int codepoint_end,
												   ImWcharClass char_class,
												   const char* s);
...
inline float ImLinearRemapClamp(float s0, float s1, float d0, float d1,
								float x) {
...
#define IM_BITARRAY_CLEARBIT(_ARRAY, _N) \
	((_ARRAY[(_N) >> 5] &=               \
	  ~((ImU32)1 << ((_N) &              \
					 31))))	 // Macro version of ImBitArrayClearBit(): ensure
...
// Helper: ImStableVector<>
// Allocating chunks of BLOCKSIZE items. Objects pointers are never invalidated
// when growing, only by clear(). Important: does not destruct anything!
...
struct ImStableVector {
	...
	inline T* push_back(const T& v) {
		...
		void* ptr = &Blocks[i / BLOCKSIZE][i % BLOCKSIZE];
...
struct IMGUI_API ImDrawListSharedData {
	ImVec2 TexUvWhitePixel;	   // UV of white pixel in the atlas (==
							   // FontAtlas->TexUvWhitePixel)
	const ImVec4* TexUvLines;  // UV of anti-aliased lines in the atlas (==
							   // FontAtlas->TexUvLines)
	...
	ImGuiContext*
		Context;  // [OPTIONAL] Link to Dear ImGui context. 99% of
				  // ImDrawList/ImFontAtlas can function without an ImGui
				  // context, but this facilitate handling one legacy edge case.
	...
	ImU8 CircleSegmentCounts[64];  // Precomputed segment count for given radius
								   // before we calculate it dynamically (to
...
enum ImGuiItemFlagsPrivate_ {
	...
	ImGuiItemFlags_NoNavDisableMouseHover =
		1
		<< 15,	// false     // Nav keyboard/gamepad mode doesn't disable hover
	...
	ImGuiItemFlags_NoFocus =
		1 << 17,  // false     // [EXPERIMENTAL: Not very well specced] Clicking
				  // doesn't take focus. Automatically sets
				  // ImGuiButtonFlags_NoFocus + ImGuiButtonFlags_NoNavFocus in
				  // ButtonBehavior().
...
enum ImGuiItemStatusFlags_ {
	...
	ImGuiItemStatusFlags_ToggledSelection =
		1 << 3,	 // Set when Selectable(), TreeNode() reports toggling a
				 // selection. We can't report "Selected", only state changes,
				 // in order to easily handle clipping with less issues.
	...
	ImGuiItemStatusFlags_HasShortcut =
		1 << 10,  // g.LastItemData.Shortcut valid. Set by SetNextItemShortcut()
				  // -> ItemAdd().
// ImGuiItemStatusFlags_FocusedByTabbing = 1 << 8,   // Removed IN 1.90.1 (Dec
// 2023). The trigger is part of g.NavActivateId. See commit 54c1bdeceb.
...
enum ImGuiButtonFlagsPrivate_ {
	...
	ImGuiButtonFlags_AlignTextBaseLine =
		1 << 15,  // vertically align button to match text baseline - ButtonEx()
				  // only // FIXME: Should be removed and handled by
				  // SmallButton(), not possible currently because of
	...
	ImGuiButtonFlags_NoNavFocus =
		1 << 18,  // don't override navigation focus when activated (FIXME: this
				  // is essentially used every time an item uses
				  // ImGuiItemFlags_NoNav, but because legacy specs don't
				  // requires LastItemData to be set ButtonBehavior(), we can't
				  // poll g.LastItemData.ItemFlags)
	...
	ImGuiButtonFlags_NoTestKeyOwner =
		1 << 21,  // don't test key/input owner when polling the key (note:
				  // mouse buttons are keys! often, the key in question will be
				  // ImGuiKey_MouseLeft!)
	ImGuiButtonFlags_NoFocus =
		1 << 22,  // [EXPERIMENTAL: Not very well specced]. Don't focus parent
	...
	// ImGuiButtonFlags_NoKeyModsAllowed, // Renamed in 1.91.4
};
...
enum ImGuiTreeNodeFlagsPrivate_ {
	ImGuiTreeNodeFlags_NoNavFocus =
		1
		<< 27,	// Don't claim nav focus when interacting with this item (#8551)
...
struct IMGUI_API ImGuiInputTextState {
	...
	const char* TextSrc;  // == TextA.Data unless read-only, in which case ==
						  // buf passed to InputText(). For _ReadOnly fields,
						  // pointer will be null outside the InputText() call.
	ImVector<char>
	...
	ImVector<char> TextToRevertTo;	// value to revert to when pressing Escape =
									// backup of end-user buffer at the time of
	...
	bool CursorCenterY;	 // set when we want scrolling to be centered over the
						 // cursor position (while resizing a word-wrapping
						 // field)
	...
	ImS8 LastMoveDirectionLR;  // ImGuiDir_Left or ImGuiDir_Right. track last
							   // movement direction so when cursor cross over a
							   // word-wrapping boundaries we can display it on
							   // either line depending on last move.s
...
enum ImGuiWindowBgClickFlags_ {
	...
	ImGuiWindowBgClickFlags_Move =
		1 << 0,	 // Click on bg/void + drag to move window. Cleared by default
				 // when using io.ConfigWindowsMoveFromTitleBarOnly.
};
...
struct ImGuiNextItemData {
	...
	ImGuiItemFlags ItemFlags;  // Currently only tested/used for
							   // ImGuiItemFlags_AllowOverlap and
							   // ImGuiItemFlags_HasSelectionUserData.

	// Members below are NOT cleared by ItemAdd() meaning they are still valid
	// during e.g. NavProcessItem(). Always rely on HasFlags.
	...
	ImU32 ColorMarker;	// Set by SetNextItemColorMarker(). Not exposed yet,
						// supported by DragScalar,SliderScalar and for
...
// - To implement ImGuiTreeNodeFlags_NavLeftJumpsToParent: store the minimum
// amount of data
...
struct ImGuiInputEvent {
	...
	ImU32 EventId;	// Unique, sequential increasing integer to identify an
...
#define ImGuiKeyOwner_Any \
	((ImGuiID)0)  // Accept key that have an owner, UNLESS a call to
				  // SetKeyOwner() explicitly used ImGuiInputFlags_LockThisFrame
...
struct ImGuiKeyOwnerData {
	...
	bool LockUntilRelease;	// Reading this key requires explicit owner id
							// (until key is released). Set by
...
enum ImGuiInputFlagsPrivate_ {
	...
	ImGuiInputFlags_LockThisFrame =
		1 << 20,  // Further accesses to key data will require EXPLICIT owner ID
				  // (ImGuiKeyOwner_Any/0 will NOT accepted for polling).
				  // Cleared at end of frame.
	ImGuiInputFlags_LockUntilRelease =
		1 << 21,  // Further accesses to key data will require EXPLICIT owner ID
				  // (ImGuiKeyOwner_Any/0 will NOT accepted for polling).
				  // Cleared when the key is released or at end of each frame if
				  // key is released.
...
enum ImGuiActivateFlags_ {
	...
	ImGuiActivateFlags_FromFocusApi =
		1 << 5,	 // Activation requested by an api request
...
enum ImGuiScrollFlags_ {
	...
	ImGuiScrollFlags_KeepVisibleCenterX =
		1 << 2,	 // If item is not visible: scroll to make the item centered on
				 // X axis [rarely used]
	ImGuiScrollFlags_KeepVisibleCenterY =
		1 << 3,	 // If item is not visible: scroll to make the item centered on
				 // Y axis
...
// ImGuiNavHighlightFlags_TypeThin       = ImGuiNavRenderCursorFlags_Compact, //
// Renamed in 1.90.2
#endif
};
...
enum ImGuiNavMoveFlags_ {
	...
	ImGuiNavMoveFlags_AllowCurrentNavId =
		1 << 4,	 // Allow scoring and considering the current NavId as a move
				 ...
				 // offset (e.g. pressing PageDown actually needs to send a Up
				 // move request, if we are pressing PageDown from the
				 // bottom-most item we need to stay in place)
	ImGuiNavMoveFlags_AlsoScoreVisibleSet =
		1 << 5,	 // Store alternate result in NavMoveResultLocalVisible that
				 // only comprise elements that are already fully visible (used
				 // by PageUp/PageDown)
	ImGuiNavMoveFlags_ScrollToEdgeY =
		1 << 6,	 // Force scrolling to min/max (used by Home/End) // FIXME-NAV:
				 // Aim to remove or reword as ImGuiScrollFlags
	ImGuiNavMoveFlags_Forwarded = 1 << 7,
	ImGuiNavMoveFlags_DebugNoResult =
		1 << 8,	 // Dummy scoring for debug purpose, don't apply result
	ImGuiNavMoveFlags_FocusApi =
		1 << 9,	 // Requests from focus API can land/focus/activate items even
				 // if they are marked with _NoTabStop (see
				 // NavProcessItemForTabbingRequest() for details)
	ImGuiNavMoveFlags_IsTabbing = 1
								  << 10,  // == Focus + Activate if item is
										  // Inputable + DontChangeNavHighlight
	ImGuiNavMoveFlags_IsPageMove =
		1 << 11,  // Identify a PageDown/PageUp request.
...
struct ImGuiNavItemData {
	...
	float DistBox;	   //      Move    // Best candidate box distance to current
					   //      NavId
...
struct IMGUI_API ImGuiMultiSelectTempData {
	ImGuiMultiSelectIO IO;	// MUST BE FIRST FIELD. Requests are set and
							// returned by BeginMultiSelect()/EndMultiSelect() +
	...
	bool RangeDstPassedBy;	// Set by the item that matches NavJustMovedToId
							// when IsSetRange is set.
...
enum ImGuiDockNodeFlagsPrivate_ {
	...
	ImGuiDockNodeFlags_HiddenTabBar =
		1 << 13,  // Saved // Tab bar is hidden, with a triangle in the corner
				  // to show it again (NB: actual tab-bar instance may be
				  // destroyed as this is only used for single-window tab bar)
	...
	ImGuiDockNodeFlags_NoDockingSplitOther =
		1 << 19,  //       // Disable this node from splitting other
				  //       windows/nodes.
...
struct IMGUI_API ImGuiDockNode {
	...
	ImGuiDockNodeFlags
		MergedFlags;  // (Read)  Effective flags (== SharedFlags |
	...
	ImGuiWindow* VisibleWindow;	 // Generally point to window which is ID is ==
								 // SelectedTabID, but when CTRL+Tabbing this
...
struct ImGuiViewportP : public ImGuiViewport {
	...
	int LastFrameActive;  // Last frame number this viewport was activated by a
						  // window
	int LastFocusedStampCount;	// Last stamp number from when a window hosted
								// by this viewport was focused (by comparing
								// this value between two viewport we have an
	...
	ImDrawList*
		BgFgDrawLists[2];  // Convenience background (0) and foreground (1) draw
						   // lists. We use them to draw software mouser cursor
						   // when io.MouseDrawCursor is set and to draw most
						   // debug overlays.
...
struct ImGuiWindowSettings {
	...
	bool IsChild;
	bool
		WantApply;	// Set when loaded from .ini data (to enable merging/loading
...
typedef void (*ImGuiErrorCallback)(
	ImGuiContext* ctx, void* user_data,
...
enum ImGuiDebugLogFlags_ {
	...
	ImGuiDebugLogFlags_OutputToDebugger =
		1 << 21,  // Also send output to Debugger Console [Windows only]
	ImGuiDebugLogFlags_OutputToTestEngine =
		1 << 22,  // Also send output to Dear ImGui Test Engine
};
...
struct ImGuiContext {
	...
	float FontSize;			 // Currently bound font size == line height (==
							 // FontSizeBase + externals scales applied in the
							 // UpdateCurrentFontSize() function).
	float FontSizeBase;	   // Font size before scaling == style.FontSizeBase ==
						   // value passed to PushFont() when specified.
	float FontBakedScale;  // == FontBaked->Size / FontSize. Scale factor over
						   // baked size. Rarely used nowadays, very often
						   // == 1.0f.
	float FontRasterizerDensity;  // Current font density. Used by all calls to
								  // GetFontBaked().
	...
	ImVector<ImGuiInputEvent>
		InputEventsQueue;  // Input events which will be trickled/written into
						   // IO structure.
	ImVector<ImGuiInputEvent>
	...
	float
		WindowsBorderHoverPadding;	// Padding around resizable windows for
									// which hovering on counts as hovering the
									// window == ImMax(style.TouchExtraPadding,
									// style.WindowBorderHoverPadding). This
	...
	ImGuiWindow* HoveredWindowUnderMovingWindow;  // Hovered window ignoring
												  // MovingWindow. Only set if
												  // MovingWindow is set.
	ImGuiWindow*
		HoveredWindowBeforeClear;  // Window the mouse is hovering. Filled even
								   // with _NoMouse. This is currently useful
								   // for multi-context compositors.
	ImGuiWindow*
	...
	ImGuiWindow*
		WheelingWindow;	 // Track the window we started mouse-wheeling on. Until
						 // a timer elapse or mouse has moved, generally keep
						 // scrolling the same window even if during the course
						 // of scrolling the mouse ends up hovering a child
						 // window.
	...
	ImGuiID DebugDrawIdConflictsId;	 // Set when we detect multiple items with
									 // the same identifier
	ImGuiID DebugHookIdInfoId;	// Will call core hooks: DebugHookIdInfo() from
	...
	ImGuiID
		ActiveIdDisabledId;	 // When clicking a disabled item we set
							 // ActiveId=window->MoveId to avoid interference
	...
	ImGuiID LastActiveId;  // Store the last non-zero ActiveId, useful for
						   // animation.
	float
		LastActiveIdTimer;	// Store the last non-zero ActiveId timer since the
	...
	double
		LastKeyboardKeyPressTime;  // Record the last time a keyboard key
	...
	ImGuiID DebugLocateId;	// Storage for DebugLocateItemOnHover() feature:
							// this is read by ItemAdd() so we keep it in a
							// hot/cached location
	...
	ImVector<ImGuiColorMod>
		ColorStack;	 // Stack for PushStyleColor()/PopStyleColor() - inherited
					 // by Begin()
	ImVector<ImGuiStyleMod>
		StyleVarStack;	// Stack for PushStyleVar()/PopStyleVar() - inherited by
						// Begin()
	ImVector<ImFontStackData>
	...
	ImVector<ImGuiItemFlags>
		ItemFlagsStack;	 // Stack for PushItemFlag()/PopItemFlag() - inherited
						 // by Begin()
	...
	ImVector<ImGuiViewportP*>
		Viewports;	// Active viewports (always 1+, and generally 1 unless
					...
					// copy of ImDrawData.
	ImGuiViewportP*
		CurrentViewport;  // We track changes of viewport (happening in Begin)
						  // so we can call Platform_OnChangedViewport()
	ImGuiViewportP* MouseViewport;
	ImGuiViewportP*
		MouseLastHoveredViewport;  // Last known viewport that was hovered by
								   // mouse (even if we are not hovering any
								   // viewport any more) + honoring the
								   // _NoInputs flag.
	...
	bool NavMousePosDirty;	// When set we will update mouse position if
							// io.ConfigNavMoveSetMousePos is set (not enabled
							// by default)
	...
	ImGuiID
		NavActivateId;	// ~~ (g.ActiveId == 0) && (IsKeyPressed(ImGuiKey_Space)
						...
						// 0, also set when calling ActivateItemByID()
	ImGuiID NavActivateDownId;	   // ~~ IsKeyDown(ImGuiKey_Space) ||
								   // IsKeyDown(ImGuiKey_Enter) ||
								   // IsKeyDown(ImGuiKey_NavGamepadActivate) ?
								   // NavId : 0
	...
	ImVector<ImGuiFocusScopeData>
		NavFocusRoute;	// Reversed copy focus scope stack for NavId (should
						// contains NavFocusScopeId). This essentially follow
						// the window->ParentWindowForFocusRoute chain.
	ImGuiID NavHighlightActivatedId;
	float NavHighlightActivatedTimer;
	ImGuiID NavNextActivateId;	// Set by ActivateItemByID(), queued until next
								// frame.
	ImGuiActivateFlags NavNextActivateFlags;
	ImGuiInputSource NavInputSource;  // Keyboard or Gamepad mode? THIS CAN ONLY
									  // BE ImGuiInputSource_Keyboard or
									  // ImGuiInputSource_Gamepad
	ImGuiSelectionUserData
		NavLastValidSelectionUserData;	// Last valid data passed to
										// SetNextItemSelectionUser(), or -1.
										// For current window. Not reset when
										// focusing an item that doesn't have
										// selection data.
	ImS8 NavCursorHideFrames;
	// ImGuiID               NavActivateInputId;                 // Removed
	// in 1.89.4 (July 2023). This is now part of g.NavActivateId and sets
	// g.NavActivateFlags |= ImGuiActivateFlags_PreferInput. See commit
	...
	ImGuiID NavJustMovedFromFocusScopeId;  // Just navigated from this focus
										   // scope id (result of a successfully
										   // MoveRequest).
	...
	bool ConfigNavWindowingWithGamepad;	 // = true. Enable Ctrl+Tab by holding
										 // ImGuiKey_GamepadFaceLeft (==
										 // ImGuiKey_NavGamepadMenu). When
										 // false, the button may still be used
										 // to toggle Menu layer.
	ImGuiKeyChord
	...
	ImGuiWindow*
		NavWindowingTargetAnim;	 // Record of last valid NavWindowingTarget
								 ...
								 // NavWindowingHighlightAlpha becomes 0.0f, so
								 // the fade-out can stay on it.
	...
	bool NavWindowingToggleLayer;  // Set while Alt or GamepadMenu is held, may
								   // be cleared by other operations, and
								   // processed when releasing the key.
	ImGuiKey NavWindowingToggleKey;	 // Keyboard/gamepad key used when toggling
									 // to menu layer.
	...
	int MultiSelectTempDataStacked;	 // Temporary multi-select data size
									 // (because we leave previous instances
									 // undestructed, we generally don't use
	...
	ImGuiID HoverWindowUnlockedStationaryId;  // Mouse has once been stationary
											  // on this window. Only reset
											  // after departing the window.

	// Mouse state
	ImGuiMouseCursor MouseCursor;
	float MouseStationaryTimer;	 // Time the mouse has been stationary (with
								 // some loose heuristic)
	...
	ImGuiID TempInputId;  // Temporary text input when using Ctrl+Click on a
						  // slider, etc.
	...
	ImVector<ImGuiID> MenusIdSubmittedThisFrame;  // A list of menu IDs that
	...
	ImGuiPlatformImeData
		PlatformImeData;  // Data updated by current frame. Will be applied at
						  // end of the frame. For some backends, this is
						  // required to have WantVisible=true in order to
	...
	ImVector<ImTextureData*>
		UserTextures;  // List of textures created/managed by user or
					   // third-party extension. Automatically appended into
	...
	const char* LogNextPrefix;	// See comment in LogSetNextTextDecoration():
								// doesn't copy underlying data, use carefully!
	...
	// etc.)
	int DebugDrawIdConflictsCount;	// Locked count (preserved when holding
									// Ctrl)
	...
	int WantTextInputNextFrame;		   // Copied in EndFrame() from
								 // g.PlatformImeData.WantTextInput. Needs to be
								 // set for some backends (SDL3) to emit
								 // character inputs.
...
struct IMGUI_API ImGuiWindowTempData {
	...
	ImVec2 CursorStartPosLossyness;	 // Record the loss of precision of
									 ...
									 // scrolling amount. This is used by
									 // clipper to compensate and fix the most
	...
	bool
		NavIsScrollPushableX;  // Set when current work location may be scrolled
	...
	ImU32 TreeRecordsClippedNodesY2Mask;  // Store whether we should keep
										  // recording Y2. Cleared when passing
										  // clip max. Equivalent
										  // TreeHasStackDataDepthMask value
...
struct IMGUI_API ImGuiWindow {
	...
	ImVec2 ViewportPos;	 // We backup the viewport position (since the viewport
						 // may disappear or never be created if the window is
						 // inactive)
	int ViewportAllowPlatformMonitorExtend;	 // Reset to -1 every frame (index
											 // is guaranteed to be valid
											 // between NewFrame..EndFrame),
											 // only used in the Appearing frame
											 // of a tooltip/popup to enforce
											 // clamping to a given monitor
	ImVec2 Pos;		  // Position (always rounded-up to nearest pixel)
	ImVec2 Size;	  // Current size (==SizeFull or collapsed title bar size)
	ImVec2 SizeFull;  // Size when non collapsed
	ImVec2 ContentSize;	 // Size of contents/scrollable client area (calculated
						 // from the extents reach of the cursor) from previous
						 // frame. Does not include window decoration or window
						 // padding.
	...
	float TitleBarHeight,
		MenuBarHeight;	// Note that those used to be function before
						// 2024/05/28. If you have old code calling
						// TitleBarHeight() you can change it to TitleBarHeight.
	...
	float DecoOuterSizeX2,
		DecoOuterSizeY2;  // Right/Down offsets (X2 generally ==
						  // ScrollbarSize.x, Y2 == ScrollbarSizes.y).
	float DecoInnerSizeX1,
		DecoInnerSizeY1;  // Applied AFTER/OVER InnerRect. Specialized for
	...
	bool ScrollbarXStabilizeEnabled;		 // Was ScrollbarX previously
											 // auto-stabilized?
	ImU8 ScrollbarXStabilizeToggledHistory;	 // Used to stabilize scrollbar
											 // visibility in case of feedback
											 // loops
	bool ViewportOwned;
	...
	signed char ResizeBorderHeld;  // Current border being held for resize (-1:
								   // none, otherwise 0-3)
	short BeginCount;  // Number of Begin() during the current frame (generally
					   // 0 or 1, 1+ if appending via multiple Begin/End pairs)
	short
		BeginCountPreviousFrame;  // Number of Begin() during the previous frame
	short BeginOrderWithinParent;  // Begin() order within immediate parent
								   // window, if we are a child window.
								   // Otherwise 0.
	short
		BeginOrderWithinContext;  // Begin() order within entire imgui context.
	...
	ImGuiCond SetWindowDockAllowFlags : 8;	// store acceptable condition flags
											// for SetNextWindowDock() use.
	...
	ImRect
		WorkRect;  // Initially covers the whole scrolling region. Reduced by
	...
	ImRect
		ParentWorkRect;	 // Backup of WorkRect before entering a container such
	...
	ImGuiWindow* RootWindowPopupTree;  // Point to ourself or first ancestor
									   // that is not a child window. Cross
									   // through popups parent<>child.
	ImGuiWindow*
	...
	ImGuiWindow* ParentWindowForFocusRoute;	 // Set to manual link a window to
											 // its logical parent so that
											 // Shortcut() chain are honored
	...
	ImGuiWindow*
		NavLastChildNavWindow;	// When going to the menu bar, we remember the
								...
								// probably be made implicit if we kept
								// g.Windows sorted by last focused including
								// child window.)
	...
	// Docking
	bool DockIsActive : 1;	// When docking artifacts are actually visible. When
							// this is set, DockNode is guaranteed to be !=
							// NULL. ~~ (DockNode != NULL) &&
	...
	ImGuiDockNode*
		DockNode;  // Which node are we docked into. Important: Prefer testing
				   // DockIsActive in many cases as this will still be set when
				   // the dock node is hidden.
	ImGuiDockNode*
   ...
   public:
	...
	// [OBSOLETE] ImGuiWindow::CalcFontSize() was removed in 1.92.0 because
	// error-prone/misleading. You can use window->FontRefSize for a copy of
	// g.FontSize at the time of the last Begin() call for this window.
	// float     CalcFontSize() const    { ImGuiContext& g = *Ctx; return
	// g.FontSizeBase * FontWindowScale * FontDpiScale * FontWindowScaleParents;
};
...
enum ImGuiTabItemFlagsPrivate_ {
	...
	ImGuiTabItemFlags_Button = 1 << 21,	 // Used by TabItemButton, change the
...
struct ImGuiTabItem {
	...
	ImGuiWindow* Window;  // When TabItem is part of a DockNode's TabBar, we
						  // hold on to a window.
...
struct IMGUI_API ImGuiTabBar {
	...
	ImRect BarRect;
	float BarRectPrevWidth;	 // Backup of previous width. When width change we
							 // enforce keep horizontal scroll on focused tab.
...
#define IMGUI_TABLE_MAX_COLUMNS \
	512	 // Arbitrary "safety" maximum, may be lifted in the future if needed.
		 // Must fit in ImGuiTableColumnIdx/ImGuiTableDrawChannelIdx.
...
struct ImGuiTableColumn {
	...
	float
		ItemWidth;	// Current item width for the column, preserved across rows
	float ContentMaxXFrozen;  // Contents maximum position for frozen rows
							  // (apart from headers), from which we can infer
							  // content width.
	...
	ImGuiTableColumnIdx SortOrder;	// Index of this column within sort specs,
									// -1 if not sorting on this column, 0 for
	...
	bool IsEnabled;			  // IsUserEnabled && (Flags &
							  // ImGuiTableColumnFlags_Disabled) == 0
...
struct ImGuiTableInstanceData {
	...
	float LastTopHeadersRowHeight;	// Height of first consecutive header rows
...
struct IMGUI_API ImGuiTable {
	...
	ImS16 InstanceCurrent;	// Count of BeginTable() calls with same ID in the
							...
							// tables with the same ID are multiple tables, they
							// are just synced.
	...
	float
		ResizeLockMinContentsX2;  // Lock minimum contents width while resizing
	...
	float AngledHeadersSlope;	// Set by TableAngledHeadersRow(), used in
								// TableUpdateLayout()
	ImRect OuterRect;  // Note: for non-scrolling table, OuterRect.Max.y is
					   // often FLT_MAX until EndTable(), unless a height has
					   // been specified in BeginTable().
	...
	ImRect Bg2ClipRectForDrawCmd;  // Actual ImDrawCmd clip rect for BG2
								   // channel. This tends to be a correct,
								   // tight-fit, because output to BG2 are done
								   // by widgets relying on regular ClipRect.
	ImRect HostClipRect;  // This is used to check if we can eventually merge
						  // our columns draw calls into the current draw call
	...
	ImVector<ImGuiTableColumnSortSpecs>
		SortSpecsMulti;	 // FIXME-OPT: Using a small-vector pattern would be
						 // good.
	ImGuiTableSortSpecs SortSpecs;	// Public facing sorts specs, this is what
									// we return in TableGetSortSpecs()
	ImGuiTableColumnIdx SortSpecsCount;
	ImGuiTableColumnIdx
		ColumnsEnabledCount;  // Number of enabled columns (<= ColumnsCount)
	ImGuiTableColumnIdx
		ColumnsEnabledFixedCount;  // Number of enabled columns using fixed
								   // width (<= ColumnsCount)
	ImGuiTableColumnIdx DeclColumnsCount;  // Count calls to TableSetupColumn()
	ImGuiTableColumnIdx
		AngledHeadersCount;	 // Count columns with angled headers
	ImGuiTableColumnIdx
	...
	bool IsContextPopupOpen;  // Set when default context menu is open (also
							  // see: ContextPopupColumn, InstanceInteracted).
	bool DisableDefaultContextMenu;	 // Disable default context menu. You may
									 // submit your own using
									 // TableBeginContextMenuPopup()/EndPopup()
	bool IsSettingsRequestLoad;
	bool IsSettingsDirty;  // Set when table settings have changed and needs to
						   // be reported into ImGuiTableSettings data.
	...
	bool HasScrollbarYCurr;	 // Whether ANY instance of this table had a
							 // vertical scrollbar during the current frame.
	bool HasScrollbarYPrev;	 // Whether ANY instance of this table had a
							 // vertical scrollbar during the previous.
...
struct IMGUI_API ImGuiTableTempData {
	...
	ImRect HostBackupWorkRect;	// Backup of InnerWindow->WorkRect at the end of
								// BeginTable()
	ImRect HostBackupParentWorkRect;  // Backup of InnerWindow->ParentWorkRect
...
struct ImGuiTableSettings {
	...
	ImGuiTableColumnIdx
		ColumnsCountMax;  // Maximum number of columns this settings instance
						  // can store, we can recycle a settings instance with
						  // lower number of columns but not higher
	bool
		WantApply;	// Set when loaded from .ini data (to enable merging/loading
					// .ini data into an already running context)
...
IMGUI_API void RegisterUserTexture(
	ImTextureData*
		tex);  // Register external texture. EXPERIMENTAL: DO NOT USE YET.
...
IMGUI_API ImGuiID AddContextHook(ImGuiContext* ctx,
...
IMGUI_API void ShrinkWidths(ImGuiShrinkWidthItem* items, int count,
							float width_excess, float width_min);
IMGUI_API void CalcClipRectVisibleItemsY(const ImRect& clip_rect,
...
IMGUI_API bool BeginPopupMenuEx(ImGuiID id, const char* label,
...
IMGUI_API void ActivateItemByID(
	ImGuiID id);  // Activate an item by ID (button, checkbox, tree node etc.).
				  ...
				  // the item is encountered again. Was called 'ActivateItem()'
				  // before 1.89.7.
...
IMGUI_API bool TestKeyOwner(
	ImGuiKey key, ImGuiID owner_id);  // Test that key is either not owned,
...
IMGUI_API bool IsKeyPressed(
	...
	ImGuiID owner_id =
		0);	 // Important: when transitioning from old to new IsKeyPressed():
			 ...
			 // New API requires explicit ImGuiInputFlags_Repeat.
...
IMGUI_API void DockContextProcessUndockWindow(
	ImGuiContext* ctx, ImGuiWindow* window,
...
IMGUI_API void DockBuilderRemoveNodeDockedWindows(
	ImGuiID node_id, bool clear_settings_refs = true);
...
IMGUI_API bool BeginDragDropTargetViewport(ImGuiViewport* viewport,
										   const ImRect* p_bb = NULL);
IMGUI_API void ClearDragDrop();
IMGUI_API bool IsDragDropPayloadBeingAccepted();
IMGUI_API void RenderDragDropTargetRectForItem(const ImRect& bb);
IMGUI_API void RenderDragDropTargetRectEx(ImDrawList* draw_list,
										  const ImRect& bb);
...
// Box-Select API
IMGUI_API bool BeginBoxSelect(const ImRect& scope_rect, ImGuiWindow* window,
...
IMGUI_API void MultiSelectItemFooter(ImGuiID id, bool* p_selected,
									 bool* p_pressed);
...
IMGUI_API void TabItemSpacing(const char* str_id, ImGuiTabItemFlags flags,
							  float width);
...
IMGUI_API void TabItemLabelAndCloseButton(
	ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags,
...
IMGUI_API void RenderFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col,
						   bool borders = true, float rounding = 0.0f);
IMGUI_API void RenderFrameBorder(ImVec2 p_min, ImVec2 p_max,
								 float rounding = 0.0f);
IMGUI_API void RenderColorComponentMarker(const ImRect& bb, ImU32 col,
										  float rounding);
...
IMGUI_API void RenderArrowDockMenu(ImDrawList* draw_list, ImVec2 p_min,
								   float sz, ImU32 col);
IMGUI_API void RenderRectFilledInRangeH(ImDrawList* draw_list,
										const ImRect& rect, ImU32 col,
										float fill_x0, float fill_x1,
										float rounding);
...
IMGUI_API void TextAlignedV(float align_x, float size_x, const char* fmt,
							va_list args);
...
IMGUI_API bool ArrowButtonEx(const char* str_id, ImGuiDir dir, ImVec2 size_arg,
							 ImGuiButtonFlags flags = 0);
IMGUI_API bool ImageButtonEx(ImGuiID id, ImTextureRef tex_ref,
...
}  // This may be useful to apply workaround that a based on distinguish
   // whenever an item is active as a text input field.
...
IMGUI_API void ColorPickerOptionsPopup(const float* ref_col,
									   ImGuiColorEditFlags flags);
inline void SetNextItemColorMarker(ImU32 col) {
	ImGuiContext& g = *GImGui;
	g.NextItemData.HasFlags |= ImGuiNextItemDataFlags_HasColorMarker;
...
IMGUI_API void DebugAllocHook(
	ImGuiDebugAllocInfo* info, int frame_count, void* ptr,
...
IMGUI_API void DebugNodeFontGlyphesForSrcMask(ImFont* font, ImFontBaked* baked,
											  int src_mask);
...
// updates. Conceptually this could be public, but API is still going to be
// evolve.
struct ImFontLoader {
	const char* Name;
	...
	bool (*FontSrcContainsGlyph)(ImFontAtlas* atlas, ImFontConfig* src,
								 ImWchar codepoint);
	bool (*FontBakedInit)(ImFontAtlas* atlas, ImFontConfig* src,
						  ImFontBaked* baked, void* loader_data_for_baked_src);
	void (*FontBakedDestroy)(ImFontAtlas* atlas, ImFontConfig* src,
							 ImFontBaked* baked,
							 void* loader_data_for_baked_src);
	bool (*FontBakedLoadGlyph)(ImFontAtlas* atlas, ImFontConfig* src,
							   ImFontBaked* baked,
							   void* loader_data_for_baked_src,
							   ImWchar codepoint, ImFontGlyph* out_glyph,
							   float* out_advance_x);

	// Size of backend data, Per Baked * Per Source. Buffers are managed by core
	// to avoid excessive allocations.
	// FIXME: At this point the two other types of buffers may be managed by
	// core to be consistent?
...
typedef ImFontLoader
	ImFontBuilderIO;  // [renamed/changed in 1.92.0] The types are not actually
					  // compatible but we provide this as a compile-time error
					  // report helper.
#endif
...
#define ImFontAtlasRectId_GenerationMask_ \
	(0x3FF00000)  // 10-bits: entry generation, so each ID is unique and get can
				  // safely detected old identifiers.
...
inline ImFontAtlasRectId ImFontAtlasRectId_Make(int index_idx, int gen_idx) {
	IM_ASSERT(index_idx >= 0 && index_idx <= ImFontAtlasRectId_IndexMask_ &&
			  gen_idx <= (ImFontAtlasRectId_GenerationMask_ >>
						  ImFontAtlasRectId_GenerationShift_));
	return (ImFontAtlasRectId)(index_idx |
							   (gen_idx << ImFontAtlasRectId_GenerationShift_));
}

// Packed rectangle lookup entry (we need an indirection to allow
// removing/reordering rectangles) User are returned ImFontAtlasRectId values
// which are meant to be persistent. We handle this with an indirection. While
// Rects[] may be in theory shuffled, compacted etc., RectsIndex[] cannot it is
// keyed by ImFontAtlasRectId. RectsIndex[] is used both as an index into
// Rects[] and an index into itself. This is basically a free-list. See
// ImFontAtlasBuildAllocRectIndexEntry() code. Having this also makes it easier
// to e.g. sort rectangles during repack.
struct ImFontAtlasRectEntry {
	int TargetIndex : 20;  // When Used: ImFontAtlasRectId -> into Rects[]. When
						   // unused: index to next unused RectsIndex[] slot to
						   // consume free-list.
	unsigned int Generation : 10;  // Increased each time the entry is reused
								   // for a new rectangle.
...
// We avoid dragging imstb_rectpack.h into public header (partly because binding
// generators are having issues with it)
...
struct ImFontAtlasBuilder {
	stbrp_context_opaque
		PackContext;  // Actually 'stbrp_context' but we don't want to define
					  // this in the header file.
	ImVector<stbrp_node_im> PackNodes;
	ImVector<ImTextureRect> Rects;
	ImVector<ImFontAtlasRectEntry>
		RectsIndex;	 // ImFontAtlasRectId -> index into Rects[]
	...
	int RectsPackedSurface;	 // Number of packed pixels. Used when compacting to
							 // heuristically find the ideal texture size.
	int RectsDiscardedCount;
	int RectsDiscardedSurface;
	int FrameCount;			 // Current frame count
	ImVec2i MaxRectSize;	 // Largest rectangle to pack (de-facto used as a
							 // "minimum texture size")
	ImVec2i MaxRectBounds;	 // Bottom-right most used pixels
	bool LockDisableResize;	 // Disable resizing texture
	bool PreloadedAllGlyphsRanges;	// Set when missing
									// ImGuiBackendFlags_RendererHasTextures
									// features forces atlas to preload
									// everything.
	...
	ImFontAtlasRectId
		PackIdMouseCursors;	 // White pixel + mouse cursors. Also happen to be
							 // fallback in case of packing failure.
...
IMGUI_API void ImFontAtlasBuildSetupFontLoader(ImFontAtlas* atlas,
											   const ImFontLoader* font_loader);
IMGUI_API void ImFontAtlasBuildNotifySetFont(ImFontAtlas* atlas,
											 ImFont* old_font,
											 ImFont* new_font);
IMGUI_API void ImFontAtlasBuildUpdatePointers(ImFontAtlas* atlas);
IMGUI_API void ImFontAtlasBuildRenderBitmapFromString(ImFontAtlas* atlas, int x,
													  int y, int w, int h,
													  const char* in_str,
													  char in_marker_char);
IMGUI_API void ImFontAtlasBuildClear(
	ImFontAtlas* atlas);  // Clear output and custom rects

IMGUI_API ImTextureData* ImFontAtlasTextureAdd(ImFontAtlas* atlas, int w,
											   int h);
...
IMGUI_API void ImFontAtlasTextureGrow(ImFontAtlas* atlas, int old_w = -1,
									  int old_h = -1);
...
IMGUI_API void ImFontAtlasBuildSetupFontSpecialGlyphs(ImFontAtlas* atlas,
													  ImFont* font,
													  ImFontConfig* src);
IMGUI_API void ImFontAtlasBuildLegacyPreloadAllGlyphRanges(
	ImFontAtlas* atlas);  // Legacy
IMGUI_API void ImFontAtlasBuildGetOversampleFactors(ImFontConfig* src,
													ImFontBaked* baked,
													int* out_oversample_h,
													int* out_oversample_v);
IMGUI_API void ImFontAtlasBuildDiscardBakes(ImFontAtlas* atlas,
											int unused_frames);

IMGUI_API bool ImFontAtlasFontSourceInit(ImFontAtlas* atlas, ImFontConfig* src);
IMGUI_API void ImFontAtlasFontSourceAddToFont(ImFontAtlas* atlas, ImFont* font,
											  ImFontConfig* src);
IMGUI_API void ImFontAtlasFontDestroySourceData(ImFontAtlas* atlas,
												ImFontConfig* src);
IMGUI_API bool ImFontAtlasFontInitOutput(
	ImFontAtlas* atlas,
	ImFont* font);	// Using FontDestroyOutput/FontInitOutput sequence useful
...
IMGUI_API void ImFontAtlasFontDiscardBakes(ImFontAtlas* atlas, ImFont* font,
										   int unused_frames);

IMGUI_API ImGuiID ImFontAtlasBakedGetId(ImGuiID font_id, float baked_size,
										float rasterizer_density);
IMGUI_API ImFontBaked* ImFontAtlasBakedGetOrAdd(ImFontAtlas* atlas,
												ImFont* font, float font_size,
												float font_rasterizer_density);
IMGUI_API ImFontBaked* ImFontAtlasBakedGetClosestMatch(
	ImFontAtlas* atlas, ImFont* font, float font_size,
	float font_rasterizer_density);
IMGUI_API ImFontBaked* ImFontAtlasBakedAdd(ImFontAtlas* atlas, ImFont* font,
										   float font_size,
										   float font_rasterizer_density,
										   ImGuiID baked_id);
IMGUI_API void ImFontAtlasBakedDiscard(ImFontAtlas* atlas, ImFont* font,
									   ImFontBaked* baked);
IMGUI_API ImFontGlyph* ImFontAtlasBakedAddFontGlyph(
	ImFontAtlas* atlas, ImFontBaked* baked, ImFontConfig* src,
	const ImFontGlyph* in_glyph);
IMGUI_API void ImFontAtlasBakedAddFontGlyphAdvancedX(ImFontAtlas* atlas,
													 ImFontBaked* baked,
													 ImFontConfig* src,
													 ImWchar codepoint,
													 float advance_x);
IMGUI_API void ImFontAtlasBakedDiscardFontGlyph(ImFontAtlas* atlas,
												ImFont* font,
												ImFontBaked* baked,
												ImFontGlyph* glyph);
...
ImFontAtlasPackAddRect(ImFontAtlas* atlas, int w, int h,
					   ImFontAtlasRectEntry* overwrite_entry = NULL);
IMGUI_API ImTextureRect* ImFontAtlasPackGetRect(ImFontAtlas* atlas,
												ImFontAtlasRectId id);
IMGUI_API ImTextureRect* ImFontAtlasPackGetRectSafe(ImFontAtlas* atlas,
													ImFontAtlasRectId id);
IMGUI_API void ImFontAtlasPackDiscardRect(ImFontAtlas* atlas,
										  ImFontAtlasRectId id);

IMGUI_API void ImFontAtlasUpdateNewFrame(ImFontAtlas* atlas, int frame_count,
										 bool renderer_has_textures);
IMGUI_API void ImFontAtlasAddDrawListSharedData(ImFontAtlas* atlas,
												ImDrawListSharedData* data);
IMGUI_API void ImFontAtlasRemoveDrawListSharedData(ImFontAtlas* atlas,
												   ImDrawListSharedData* data);
IMGUI_API void ImFontAtlasUpdateDrawListsTextures(ImFontAtlas* atlas,
...
IMGUI_API void ImFontAtlasTextureBlockConvert(const unsigned char* src_pixels,
											  ...
											  ImTextureFormat dst_fmt,
											  int dst_pitch, int w, int h);
IMGUI_API void ImFontAtlasTextureBlockPostProcess(
	ImFontAtlasPostProcessData* data);
IMGUI_API void ImFontAtlasTextureBlockPostProcessMultiply(
	ImFontAtlasPostProcessData* data, float multiply_factor);
IMGUI_API void ImFontAtlasTextureBlockFill(ImTextureData* dst_tex, int dst_x,
										   int dst_y, int w, int h, ImU32 col);
IMGUI_API void ImFontAtlasTextureBlockCopy(ImTextureData* src_tex, int src_x,
										   int src_y, ImTextureData* dst_tex,
										   int dst_x, int dst_y, int w, int h);
IMGUI_API void ImFontAtlasTextureBlockQueueUpload(ImFontAtlas* atlas,
												  ImTextureData* tex, int x,
												  int y, int w, int h);
...
IMGUI_API bool ImFontAtlasGetMouseCursorTexData(
	ImFontAtlas* atlas, ImGuiMouseCursor cursor_type, ImVec2* out_offset,
