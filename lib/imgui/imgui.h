...
#define IMGUI_HAS_TEXTURES	// Added ImGuiBackendFlags_RendererHasTextures -
							// from IMGUI_VERSION_NUM >= 19198
...
ImDrawData)
// [SECTION] Texture API (ImTextureFormat, ImTextureStatus, ImTextureRect,
ImTextureData)
// [SECTION] Font API (ImFontConfig, ImFontGlyph, ImFontGlyphRangesBuilder,
ImFontAtlasFlags, ImFontAtlas, ImFontBaked, ImFont)
...
// Helper Macros
// (note: compiling with NDEBUG will usually strip out assert() to nothing,
// which is NOT recommended because we use asserts to notify of programmer
// mistakes.)
#ifndef IM_ASSERT
#include <assert.h>
...
#define IM_UNUSED(_VAR) \
	((void)(_VAR))	// Used to silence "unused variable warnings". Often useful
					// as asserts may be stripped out from final builds.
#define IM_STRINGIFY_HELPER(_EXPR) #_EXPR
#define IM_STRINGIFY(_EXPR) \
	IM_STRINGIFY_HELPER(    \
		_EXPR)	// Preprocessor idiom to stringify e.g. an integer or a macro.
...
// Annotations but it requires the macros in a different
//  location. e.g. #include <sal.h> + void myprintf(_Printf_format_string_ const
//  char* format, ...), and only works when using Code Analysis, rather than
//  just normal compiling).
...
struct ImDrawChannel;  // Temporary storage to output draw commands out of
					   // order, used by ImDrawListSplitter and
					   // ImDrawList::ChannelsSplit()
struct ImDrawCmd;	   // A single draw command within a parent ImDrawList
					   // (generally maps to 1 GPU draw call, unless it is a
					   // callback)
...
struct ImFontAtlasRect;		// Output of ImFontAtlas::GetCustomRect() when using
							// custom rectangles.
...
struct ImFontGlyphRangesBuilder;  // Helper to build glyph ranges from
								  // text/string data
struct ImFontLoader;			  // Opaque interface to a font loading backend
								  // (stb_truetype, FreeType etc.).
struct ImTextureData;  // Specs and pixel storage for a texture used by Dear
					   // ImGui.
...
struct ImGuiIO;		  // Main configuration and I/O between your application and
					  // ImGui (also see: ImGuiPlatformIO)
...
struct ImGuiPlatformImeData;  // Platform IME data for io.PlatformSetImeDataFn()
							  // function.
struct ImGuiPlatformMonitor;  // Multi-viewport support: user-provided bounds
							  // for each connected monitor/display. Used when
...
typedef int ImGuiCond;	// -> enum ImGuiCond_            // Enum: A condition
						// for many Set*() functions
...
typedef int ImDrawListFlags;  // -> enum ImDrawListFlags_      // Flags: for
							  // ImDrawList instance
typedef int
	ImDrawTextFlags;  // -> enum ImDrawTextFlags_      // Internal, do not use!
...
typedef int ImGuiButtonFlags;	  // -> enum ImGuiButtonFlags_     // Flags: for
								  // InvisibleButton()
typedef int ImGuiChildFlags;	  // -> enum ImGuiChildFlags_      // Flags: for
								  // BeginChild()
...
typedef int ImGuiListClipperFlags;	// -> enum ImGuiListClipperFlags_// Flags:
...
typedef int ImGuiTabItemFlags;	// -> enum ImGuiTabItemFlags_    // Flags: for
								// BeginTabItem()
typedef int ImGuiTableFlags;	// -> enum ImGuiTableFlags_      // Flags: For
								// BeginTable()
...
// ImTextureID = backend specific, low-level identifier for a texture uploaded
// in GPU/graphics system. [Compile-time configurable type]
// - When a Rendered Backend creates a texture, it store its native identifier
// into a ImTextureID value.
...
//         etc.).
// - User may submit their own textures to e.g. ImGui::Image() function by
// passing this value.
// - During the rendering loop, the Renderer Backend retrieve the ImTextureID,
// which stored inside a
//   ImTextureRef, which is stored inside a ImDrawCmd.
// - Compile-time type configuration:
//   - To use something other than a 64-bit value: add '#define ImTextureID
//   MyTextureType*' in your imconfig.h file.
//   - This can be whatever to you want it to be! read the FAQ entry about
//   textures for details.
//   - You may decide to store a higher-level structure containing texture,
//   sampler, shader etc. with various
...
// 'void*' to 'ImU64'. This allowed backends requiring 64-bit worth of data to
// build on 32-bit architectures. Use intermediary intptr_t cast and read FAQ if
// you have casting warnings.
// - In v1.92.0 (2025/06/11): added ImTextureRef which carry either a
// ImTextureID either a pointer to internal texture atlas. All user facing
// functions taking ImTextureID changed to ImTextureRef
#ifndef ImTextureID
typedef ImU64
	ImTextureID;  // Default: store up to 64-bits (any pointer or integer). A
...
// ImTextureRef = higher-level identifier for a texture. Store a ImTextureID
// _or_ a ImTextureData*. The identifier is valid even before the texture has
// been uploaded to the GPU/graphics system. This is what gets passed to
// functions such as `ImGui::Image()`, `ImDrawList::AddImage()`. This is what
// gets stored in draw commands (`ImDrawCmd`) to identify a texture during
// rendering.
// - When a texture is created by user code (e.g. custom images), we directly
// store the low-level ImTextureID.
//   Because of this, when displaying your own texture you are likely to ever
//   only manage ImTextureID values on your side.
...
//   happened.
// - To create a ImTextureRef from a ImTextureData you can use
// ImTextureData::GetTexRef().
//   We intentionally do not provide an ImTextureRef constructor for this: we
//   don't expect this to be frequently useful to the end-user, and it would be
//   erroneously called by many legacy code.
...
//      ImTextureRef tex_ref = { ._TexData = NULL, .TexID = tex_id }; return
//      tex_ref; }
// In 1.92 we changed most drawing functions using ImTextureID to use
// ImTextureRef. We intentionally do not provide an implicit ImTextureRef ->
// ImTextureID cast operator because it is technically lossy to convert
// ImTextureRef to ImTextureID before rendering.
IM_MSVC_RUNTIME_CHECKS_OFF
struct ImTextureRef {
	ImTextureRef() {
		_TexData = NULL;
	...
	inline ImTextureID GetTexID()
		const;	// == (_TexData ? _TexData->TexID : _TexID) // Implemented below
				// in the file.

	// Members (either are set, never both!)
	ImTextureData*
		_TexData;  //      A texture, generally owned by a ImFontAtlas. Will
				   //      convert to ImTextureID during render loop, after
				   //      texture has been uploaded.
	ImTextureID _TexID;	 // _OR_ Low-level backend texture identifier, if
						 // already uploaded or created by user/app. Generally
						 // provided to e.g. ImGui::Image() calls.
};
IM_MSVC_RUNTIME_CHECKS_RESTORE
...
GetPlatformIO();  // access the ImGuiPlatformIO structure (mostly
				  // hooks/functions to connect to platform/renderer and OS
				  // Clipboard, IME etc.)
IMGUI_API ImGuiStyle&
GetStyle();	 // access the Style structure (colors, sizes). Always use
			 // PushStyleColor(), PushStyleVar() to modify style mid-frame!
IMGUI_API void
NewFrame();	 // start a new Dear ImGui frame, you can submit any command from
...
GetDrawData();	// valid after Render() and until the next call to NewFrame().
				// Call ImGui_ImplXXXX_RenderDrawData() function in your
				// Renderer Backend to render.
...
IMGUI_API ImVec2
GetWindowSize();  // get current window size (IT IS UNLIKELY YOU EVER NEED TO
				  // USE THIS. Consider always using GetCursorScreenPos() and
...
IMGUI_API float
GetWindowHeight();	// get current window height (IT IS UNLIKELY YOU EVER NEED
					// TO USE THIS). Shortcut for GetWindowSize().y.
IMGUI_API ImGuiViewport*
GetWindowViewport();  // get viewport currently associated to the current
					  // window.
...
IMGUI_API float
GetScrollMaxX();  // get maximum scrolling amount ~~ ContentSize.x -
				  // WindowSize.x - DecorationsSize.x
IMGUI_API float
GetScrollMaxY();  // get maximum scrolling amount ~~ ContentSize.y -
...
IMGUI_API void SetScrollFromPosX(
	float local_x, float center_x_ratio =
					   0.5f);  // adjust scrolling amount to make given position
							   // visible. Generally GetCursorStartPos() +
...
//  - PushFont(font, 0.0f)                       // Change font and keep current
//  size
//  - PushFont(NULL, 20.0f)                      // Keep font and change current
//  size
//  - PushFont(font, 20.0f)                      // Change font and set size
//  to 20.0f
//  - PushFont(font, style.FontSizeBase * 2.0f)  // Change font and set size to
//  be twice bigger than current size.
//  - PushFont(font, font->LegacySize)           // Change font and set size to
//  size passed to AddFontXXX() function. Same as pre-1.92 behavior.
// *IMPORTANT* before 1.92, fonts had a single size. They can now be dynamically
// be adjusted.
//  - In 1.92 we have REMOVED the single parameter version of PushFont() because
//  it seems like the easiest way to provide an error-proof transition.
//  - PushFont(font) before 1.92 = PushFont(font, font->LegacySize) after 1.92
//  // Use default font size as passed to AddFontXXX() function.
// *IMPORTANT* global scale factors are applied over the provided size.
//  - Global scale factors are: 'style.FontScaleMain', 'style.FontScaleDpi' and
//  maybe more.
// -  If you want to apply a factor to the _current_ font size:
//  - CORRECT:   PushFont(NULL, style.FontSizeBase)         // use current
//  unscaled size    == does nothing
//  - CORRECT:   PushFont(NULL, style.FontSizeBase * 2.0f)  // use current
//  unscaled size x2 == make text twice bigger
//  - INCORRECT: PushFont(NULL, GetFontSize())              // INCORRECT! using
//  size after global factors already applied == GLOBAL SCALING FACTORS WILL
//  APPLY TWICE!
//  - INCORRECT: PushFont(NULL, GetFontSize() * 2.0f)       // INCORRECT! using
//  size after global factors already applied == GLOBAL SCALING FACTORS WILL
//  APPLY TWICE!
...
IMGUI_API float
GetFontSize();	// get current scaled font size (= height in pixels). AFTER
				// global scale factors applied. *IMPORTANT* DO NOT PASS THIS
				// VALUE TO PushFont()! Use ImGui::GetStyle().FontSizeBase to
				// get value before global scale factors.
IMGUI_API ImFontBaked*
GetFontBaked();	 // get current font bound at current size // ==
				 // GetFont()->GetFontBaked(GetFontSize())

// Parameters stacks (shared)
IMGUI_API void PushStyleColor(
	ImGuiCol idx, ImU32 col);  // modify a style color. always use this if you
...
IMGUI_API void PushStyleVar(
	ImGuiStyleVar idx, const ImVec2& val);	// modify a style ImVec2 variable. "
...
IMGUI_API ImVec2
GetFontTexUvWhitePixel();  // get UV coordinate for a white pixel, useful to
...
IMGUI_API void SetCursorScreenPos(
	const ImVec2& pos);	 // cursor position, absolute coordinates. THIS IS YOUR
						 // BEST FRIEND.
IMGUI_API ImVec2
GetContentRegionAvail();  // available space from current position. THIS IS YOUR
						  // BEST FRIEND.
IMGUI_API ImVec2
...
// above.
// - Image() pads adds style.ImageBorderSize on each side, ImageButton() adds
// style.FramePadding on each side.
// - ImageButton() draws a background based on regular Button() color +
// optionally an inner background if specified.
// - An obsolete version of Image(), before 1.91.9 (March 2025), had a
// 'tint_col' parameter which is now supported by the ImageWithBg() function.
...
IMGUI_API bool TreeNode(const char* str_id, const char* fmt, ...) IM_FMTARGS(
	2);	 // helper variation to easily decorrelate the id from the displayed
...
// This is analogous to how Combos are created.
...
IMGUI_API bool BeginItemTooltip();	// begin/append a tooltip window if
									// preceding item was hovered.
...
IMGUI_API void OpenPopupOnItemClick(
	...
	ImGuiPopupFlags popup_flags =
		0);	 // helper to open popup when clicked on last item. Default to
...
//  - IMPORTANT: If you ever used the left mouse button with
//  BeginPopupContextXXX() helpers before 1.92.6:
//    - Before this version, OpenPopupOnItemClick(), BeginPopupContextItem(),
//    BeginPopupContextWindow(), BeginPopupContextVoid() had 'a ImGuiPopupFlags
//    popup_flags = 1' default value in their function signature.
//    - Before: Explicitly passing a literal 0 meant
//    ImGuiPopupFlags_MouseButtonLeft. The default = 1 meant
...
//    (if legacy behavior are enabled) or will assert (if legacy behavior are
//    disabled).
//    - TL;DR: if you don't want to use right mouse button for popups, always
//    specify it explicitly using a named ImGuiPopupFlags_MouseButtonXXXX value.
//    - Read "API BREAKING CHANGES" 2026/01/07 (1.92.6) entry in imgui.cpp or
//    GitHub topic #9157 for all details.
IMGUI_API bool BeginPopupContextItem(
	const char* str_id = NULL,
	ImGuiPopupFlags popup_flags =
		0);	 // open+begin popup when clicked on last item. Use str_id==NULL to
...
IMGUI_API bool BeginPopupContextWindow(
	const char* str_id = NULL,
	ImGuiPopupFlags popup_flags =
		0);	 // open+begin popup when clicked on current window.
IMGUI_API bool BeginPopupContextVoid(
	const char* str_id = NULL,
	ImGuiPopupFlags popup_flags = 0);  // open+begin popup when clicked in void
...
IMGUI_API void TableNextRow(
	...
	float min_row_height =
		0.0f);	// append into the first cell of a new row. 'min_row_height'
				// include the minimum top and bottom padding aka CellPadding.y
				// * 2.0f.
IMGUI_API bool
...
IMGUI_API int
TableGetRowIndex();	 // return current row index (header rows are accounted for)
...
TableGetHoveredColumn();  // return hovered column. return -1 when table is not
						  // hovered. return columns_count if the unused space
						  // at the right of visible columns is hovered. Can
						  // also use (TableGetColumnFlags() &
...
//   - This is a helper to create an invisible window covering a viewport, then
//   submit a DockSpace() into it.
//   - Most applications can simply call DockSpaceOverViewport() once to allow
//   docking windows into e.g. the edge of your screen.
//     e.g. ImGui::NewFrame(); ImGui::DockSpaceOverViewport(); // Create a
//     dockspace in main viewport.
//      or: ImGui::NewFrame(); ImGui::DockSpaceOverViewport(0, nullptr,
//      ImGuiDockNodeFlags_PassthruCentralNode); // Create a dockspace in main
//      viewport, central node is transparent.
// - Dockspaces:
...
//   host. Submit them early in your frame!
//   - IMPORTANT: Dockspaces need to be kept alive if hidden, otherwise windows
//   docked into it will be undocked.
//     If you have e.g. multiple tabs with a dockspace inside each tab: submit
//     the non-visible dockspaces with ImGuiDockNodeFlags_KeepAliveOnly.
//   - See 'Demo->Examples->Dockspace' or 'Demo->Examples->Documents' for more
//   detailed demos.
// - Programmatic docking:
//   - There is no public API yet other than the very limited
//   SetNextWindowDockID() function. Sorry for that!
//   - Read https://github.com/ocornut/imgui/wiki/Docking for examples of how to
//   use current internal API.
...
IMGUI_API ImGuiID DockSpaceOverViewport(
	ImGuiID dockspace_id = 0, const ImGuiViewport* viewport = NULL,
	ImGuiDockNodeFlags flags = 0, const ImGuiWindowClass* window_class = NULL);
IMGUI_API void SetNextWindowDockID(
	ImGuiID dock_id, ImGuiCond cond = 0);  // set next window dock id
IMGUI_API void SetNextWindowClass(
	const ImGuiWindowClass*
		window_class);	// set next window class (control docking compatibility
						...
						// relationship)
IMGUI_API ImGuiID GetWindowDockID();  // get dock id of current window, or 0 if
									  // not associated to any docking node.
IMGUI_API bool
...
// everything disabled)
// - Tooltips windows are automatically opted out of disabling. Note that
// IsItemHovered() by default returns false on disabled items, unless using
...
// Focus, Activation
IMGUI_API void SetItemDefaultFocus();  // make last item the default focused
									   // item of a newly appearing window.
...
IsItemDeactivatedAfterEdit();  // was the last item just made inactive and made
							   ...
							   // item).
IMGUI_API bool IsItemToggledOpen();	 // was the last item open state toggled?
									 // set by TreeNode().
...
// - Consider using the Shortcut() function instead of
// IsKeyPressed()/IsKeyChordPressed()! Shortcut() is easier to use and better
// featured (can do focus routing check).
...
// - (legacy: before v1.87 (2022-02), we used ImGuiKey < 512 values to carry
// native/user indices as defined by each backends. This was obsoleted in 1.87
// (2022-02) and completely removed in 1.91.5 (2024-11). See
// https://github.com/ocornut/imgui/issues/4921)
IMGUI_API bool IsKeyDown(ImGuiKey key);	 // is key being held.
IMGUI_API bool IsKeyPressed(
	ImGuiKey key,
	bool repeat = true);  // was key pressed (went from !Down to Down)? Repeat
						  // rate uses io.KeyRepeatDelay / KeyRepeatRate.
...
IMGUI_API const char* GetKeyName(
	ImGuiKey key);	// [DEBUG] returns English name of the key. Those names are
...
// ... }'.
// - Flags: Default route use ImGuiInputFlags_RouteFocused, but see
// ImGuiInputFlags_RouteGlobal and other options in ImGuiInputFlags_!
...
//   routed it calls IsKeyChordPressed()
...
IMGUI_API bool
IsAnyMouseDown();  // [WILL OBSOLETE] is any mouse button held? This was
				   // designed for backends, but prefer having backend maintain
				   // a mask of held mouse buttons, because upcoming input queue
...
IMGUI_API void SetNextFrameWantCaptureMouse(
	bool want_capture_mouse);  // Override io.WantCaptureMouse flag next frame
							   ...
							   // handle, typical when true it instructs your
							   // app to ignore inputs). This is equivalent to
...
// - Interactive tools are all accessible from the 'Dear ImGui Demo->Tools'
// menu.
// - Read https://github.com/ocornut/imgui/wiki/Debug-Tools for a description of
// all available debug tools.
...
DestroyPlatformWindows();  // call DestroyWindow platform functions for all
						   // viewports. call from backend Shutdown() if you
						   // need to close platform windows before imgui
						   // shutdown. otherwise will be called by
						   // DestroyContext().
IMGUI_API ImGuiViewport* FindViewportByID(
	ImGuiID viewport_id);  // this is a helper for backends.
...
enum ImGuiWindowFlags_ {
	...
	ImGuiWindowFlags_UnsavedDocument =
		1
		<< 18,	// Display a dot next to the title. When used in a tab/docking
...
// ImGuiWindowFlags_NavFlattened           = 1 << 29,  // Obsoleted in 1.90.9:
// moved to ImGuiChildFlags. BeginChild(name, size, 0,
// ImGuiWindowFlags_NavFlattened)           --> BeginChild(name, size,
// ImGuiChildFlags_NavFlattened, 0) ImGuiWindowFlags_AlwaysUseWindowPadding = 1
// << 30,  // Obsoleted in 1.90.0: moved to ImGuiChildFlags. BeginChild(name,
// size, 0, ImGuiWindowFlags_AlwaysUseWindowPadding) --> BeginChild(name, size,
...
// in 1.91.1 (August 2024) for consistency.
#endif
};
...
enum ImGuiItemFlags_ {
	...
	ImGuiItemFlags_ButtonRepeat =
		1 << 3,	 // false    // Any button-like behavior will have repeat mode
				 ...
				 // values). Note that you can also call IsItemActive() after
				 // any button to tell if it is being held.
	ImGuiItemFlags_AutoClosePopups =
		1 << 4,	 // true     // MenuItem()/Selectable() automatically close
				 // their parent popup window.
	ImGuiItemFlags_AllowDuplicateId =
		1
		<< 5,  // false    // Allow submitting an item with the same identifier
	...
	ImGuiItemFlags_Disabled =
		1 << 6,	 // false    // [Internal] Disable interactions. DOES NOT affect
				 // visuals. This is used by BeginDisabled()/EndDisabled() and
				 // only provided here so you can read back via GetItemFlags().
};
...
enum ImGuiInputTextFlags_ {
	...
	ImGuiInputTextFlags_CallbackEdit =
		1 << 23,  // Callback on any edit. Note that InputText() already returns
				  ...
				  // callback is useful to manipulate the underlying buffer
				  // while focus is active.

	// Multi-line Word-Wrapping [BETA]
	// - Not well tested yet. Please report any incorrect cursor movement,
	// selection behavior etc. bug to
	// https://github.com/ocornut/imgui/issues/3237.
	// - Wrapping style is not ideal. Wrapping of long words/sections (e.g.
	// words larger than total available width) may be particularly unpleasing.
	// - Wrapping width needs to always account for the possibility of a
	// vertical scrollbar.
	// - It is much slower than regular text fields.
	//   Ballpark estimate of cost on my 2019 desktop PC: for a 100 KB text
	//   buffer: +~0.3 ms (Optimized) / +~1.0 ms (Debug build). The CPU cost is
	//   very roughly proportional to text length, so a 10 KB buffer should cost
	//   about ten times less.
	ImGuiInputTextFlags_WordWrap =
		1 << 24,  // InputTextMultiline(): word-wrap lines that are too long.
...
enum ImGuiTreeNodeFlags_ {
	...
	ImGuiTreeNodeFlags_NavLeftJumpsToParent =
		1 << 17,  // Nav: left arrow moves back to parent. This is processed in
				  // TreePop() when there's an unfulfilled Left nav request
				  // remaining.
	ImGuiTreeNodeFlags_CollapsingHeader = ImGuiTreeNodeFlags_Framed |
										  ImGuiTreeNodeFlags_NoTreePushOnOpen |
										  ImGuiTreeNodeFlags_NoAutoOpenOnLog,

	// [EXPERIMENTAL] Draw lines connecting TreeNode hierarchy. Discuss in
	// GitHub issue #2920. Default value is pulled from style.TreeLinesFlags.
	// May be overridden in TreeNode calls.
	ImGuiTreeNodeFlags_DrawLinesNone = 1 << 18,	 // No lines drawn
	ImGuiTreeNodeFlags_DrawLinesFull =
		1 << 19,  // Horizontal lines to child nodes. Vertical line drawn down
				  // to TreePop() position: cover full contents. Faster (for
				  // large trees).
	ImGuiTreeNodeFlags_DrawLinesToNodes =
		1 << 20,  // Horizontal lines to child nodes. Vertical line drawn down
				  // to bottom-most child node. Slower (for large trees).
...
// - IMPORTANT: If you ever used the left mouse button with
// BeginPopupContextXXX() helpers before 1.92.6: Read "API BREAKING CHANGES"
// 2026/01/07 (1.92.6) entry in imgui.cpp or GitHub topic #9157.
// - Multiple buttons currently cannot be combined/or-ed in those functions (we
// could allow it later).
enum ImGuiPopupFlags_ {
	ImGuiPopupFlags_None = 0,
	ImGuiPopupFlags_MouseButtonLeft =
		1 << 2,	 // For BeginPopupContext*(): open on Left Mouse release. Only
				 // one button allowed!
	ImGuiPopupFlags_MouseButtonRight =
		2 << 2,	 // For BeginPopupContext*(): open on Right Mouse release. Only
				 // one button allowed! (default)
	ImGuiPopupFlags_MouseButtonMiddle =
		3 << 2,	 // For BeginPopupContext*(): open on Middle Mouse release. Only
				 // one button allowed!
	...
	ImGuiPopupFlags_NoOpenOverExistingPopup =
		1 << 7,	 // For OpenPopup*(), BeginPopupContext*(): don't open if
				 // there's already a popup at the same level of the popup stack
	ImGuiPopupFlags_NoOpenOverItems =
		1 << 8,	 // For BeginPopupContextWindow(): don't return true when
				 // hovering items, only when hovering empty space
	ImGuiPopupFlags_AnyPopupId =
		1 << 10,  // For IsPopupOpen(): ignore the ImGuiID parameter and test
				  // for any popup.
	...
	ImGuiPopupFlags_InvalidMask_ =
		0x03,  // [Internal] Reserve legacy bits 0-1 to detect incorrectly
			   // passing 1 or 2 to the function.
};
...
enum ImGuiSelectableFlags_ {
	...
	ImGuiSelectableFlags_SpanAllColumns =
		1 << 1,	 // Frame will span all columns of its container table (text
				 // will still fit in current column)
	...
	ImGuiSelectableFlags_SelectOnNav =
		1 << 6,	 // Auto-select when moved into, unless Ctrl is held. Automatic
				 // when in a BeginMultiSelect() block.
...
enum ImGuiComboFlags_ {
	...
	ImGuiComboFlags_HeightSmall =
		1 << 1,	 // Max ~4 items visible. Tip: If you want your combo popup to
				 // be a specific size you can use
				 // SetNextWindowSizeConstraints() prior to calling BeginCombo()
	ImGuiComboFlags_HeightRegular = 1 << 2,	 // Max ~8 items visible (default)
	ImGuiComboFlags_HeightLarge = 1 << 3,	 // Max ~20 items visible
	ImGuiComboFlags_HeightLargest = 1
									<< 4,  // As many fitting items as possible
...
enum ImGuiTabBarFlags_ {
	...
	ImGuiTabBarFlags_NoTooltip = 1
								 << 5,	// Disable tooltips when hovering a tab
	ImGuiTabBarFlags_DrawSelectedOverline =
		1 << 6,	 // Draw selected overline markers over selected tab

	// Fitting/Resize policy
	ImGuiTabBarFlags_FittingPolicyMixed =
		1 << 7,	 // Shrink down tabs when they don't fit, until width is
				 // style.TabMinWidthShrink, then enable scrolling buttons.
	ImGuiTabBarFlags_FittingPolicyShrink =
		1 << 8,	 // Shrink down tabs when they don't fit
	ImGuiTabBarFlags_FittingPolicyScroll =
		1 << 9,	 // Enable scrolling buttons when tabs don't fit
...
enum ImGuiTabItemFlags_ {
	...
	ImGuiTabItemFlags_NoReorder =
		1 << 5,	 // Disable reordering this tab or having another tab cross over
				 // this tab
...
enum ImGuiFocusedFlags_ {
	...
	ImGuiFocusedFlags_DockHierarchy =
		1
		<< 4,  // Consider docking hierarchy (treat dockspace host as parent of
...
enum ImGuiHoveredFlags_ {
	...
	ImGuiHoveredFlags_AllowWhenBlockedByActiveItem =
		1 << 7,	 // Return true even if an active item is blocking access to
	...
	ImGuiHoveredFlags_AllowWhenDisabled =
		1 << 10,  // IsItemHovered() only: Return true even if the item is
				  // disabled
	...
	// behavior.
	//   e.g. 'HoverFlagsForTooltipMouse' defaults to
	//   'ImGuiHoveredFlags_Stationary | ImGuiHoveredFlags_DelayShort |
	//   ImGuiHoveredFlags_AllowWhenDisabled'.
	...
	ImGuiHoveredFlags_DelayShort =
		1 << 15,  // IsItemHovered() only: Return true after
				  // style.HoverDelayShort elapsed (~0.15 sec) (shared between
				  // items) + requires mouse to be stationary for
				  // style.HoverStationaryDelay (once per item).
	ImGuiHoveredFlags_DelayNormal =
		1 << 16,  // IsItemHovered() only: Return true after
				  // style.HoverDelayNormal elapsed (~0.40 sec) (shared between
				  // items) + requires mouse to be stationary for
				  // style.HoverStationaryDelay (once per item).
...
enum ImGuiDockNodeFlags_ {
	...
	ImGuiDockNodeFlags_NoDockingOverCentralNode =
		1 << 2,	 //       // Disable docking over the Central Node, which will
				 //       be always kept empty.
	ImGuiDockNodeFlags_PassthruCentralNode =
		...
		<< 3,  //       // Enable passthru dockspace: 1) DockSpace() will render
			   ...
			   //       prior to Begin() when using this. 2) When Central Node
			   //       is empty: let inputs pass-through + won't display a
	...
	ImGuiDockNodeFlags_NoResize =
		1
		<< 5,  // Saved // Disable resizing node using the splitter/separators.
...
enum ImGuiDragDropFlags_ {
	...
	ImGuiDragDropFlags_SourceNoDisableHover =
		1 << 1,	 // By default, when dragging we clear data so that
				 ...
				 // code submitting tooltips. This flag disables this behavior
	...
	ImGuiDragDropFlags_AcceptDrawAsHovered =
		1 << 13,  // Accepting item will render as if hovered. Useful for e.g. a
...
// physical keys and are not the same concept as input "Characters", the latter
...
enum ImGuiKey : int {
	...
	ImGuiKey_LeftShift,
	ImGuiKey_LeftAlt,
	ImGuiKey_LeftSuper,	 // Also see ImGuiMod_Ctrl, ImGuiMod_Shift,
						 // ImGuiMod_Alt, ImGuiMod_Super below!
	ImGuiKey_RightCtrl,
	ImGuiKey_RightShift,
	ImGuiKey_RightAlt,
	ImGuiKey_RightSuper,
	...
	// calls)
	// - Any functions taking a ImGuiKeyChord parameter can binary-or those with
	// regular keys, e.g. Shortcut(ImGuiMod_Ctrl | ImGuiKey_S).
	// - Those are written back into io.KeyCtrl, io.KeyShift, io.KeyAlt,
	// io.KeySuper for convenience,
	//   but may be accessed via standard key API such as IsKeyPressed(),
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
	ImGuiKey_COUNT =
		ImGuiKey_NamedKey_END,	// Obsoleted in 1.91.5 because it was misleading
								// (since named keys don't start at 0 anymore)
	ImGuiMod_Shortcut =
		ImGuiMod_Ctrl,	// Removed in 1.90.7, you can now simply use
						// ImGuiMod_Ctrl
						// ImGuiKey_ModCtrl = ImGuiMod_Ctrl, ImGuiKey_ModShift =
						// ImGuiMod_Shift, ImGuiKey_ModAlt = ImGuiMod_Alt,
						// ImGuiKey_ModSuper = ImGuiMod_Super, // Renamed
						// in 1.89 ImGuiKey_KeyPadEnter = ImGuiKey_KeypadEnter,
						// // Renamed in 1.87
#endif
};
...
enum ImGuiInputFlags_ {
	...
	ImGuiInputFlags_Repeat =
		1 << 0,	 // Enable repeat. Return true on successive repeats. Default
				 // for legacy IsKeyPressed(). NOT Default for legacy
...
enum ImGuiConfigFlags_ {
	...
	ImGuiConfigFlags_NoMouse =
		1
		<< 4,  // Instruct dear imgui to disable mouse inputs and interactions.
	ImGuiConfigFlags_NoMouseCursorChange =
		1
		<< 5,  // Instruct backend to not alter mouse cursor shape and
	...
	ImGuiConfigFlags_ViewportsEnable =
		1 << 10,  // Viewport enable flags (require both
				  ...
				  // ImGuiBackendFlags_RendererHasViewports set by the
				  // respective backends)
	...
	ImGuiConfigFlags_DpiEnableScaleFonts =
		1
		<< 14,	// [moved/renamed in 1.92.0] -> use bool io.ConfigDpiScaleFonts
	ImGuiConfigFlags_DpiEnableScaleViewports =
		1 << 15,  // [moved/renamed in 1.92.0] -> use bool
...
enum ImGuiBackendFlags_ {
	...
	ImGuiBackendFlags_RendererHasViewports =
		1 << 10,  // Backend Renderer supports multiple viewports.
	ImGuiBackendFlags_PlatformHasViewports =
		1 << 11,  // Backend Platform supports multiple viewports.
	ImGuiBackendFlags_HasMouseHoveredViewport =
		1
		<< 12,	// Backend Platform supports calling io.AddMouseViewportEvent()
				...
				// If this cannot be done, Dear ImGui needs to use a flawed
				// heuristic to find the viewport under.
	ImGuiBackendFlags_HasParentViewport =
		1 << 13,  // Backend Platform supports honoring
				  // viewport->ParentViewport/ParentViewportId value, by
				  // applying the corresponding parent/child relation at the
				  // Platform level.
};
...
enum ImGuiCol_ {
	...
	ImGuiCol_ResizeGrip,  // Resize grip in lower-right and lower-left corners
						  // of windows.
	ImGuiCol_ResizeGripHovered,
	ImGuiCol_ResizeGripActive,
	ImGuiCol_InputTextCursor,  // InputText cursor/caret
	ImGuiCol_TabHovered,	   // Tab background, when hovered
	ImGuiCol_Tab,		   // Tab background, when tab-bar is focused & tab is
						   // unselected
	...
	ImGuiCol_TreeLines,			 // Tree node hierarchy outlines when using
								 // ImGuiTreeNodeFlags_DrawLines
	ImGuiCol_DragDropTarget,	 // Rectangle border highlighting a drop target
	ImGuiCol_DragDropTargetBg,	 // Rectangle background highlighting a drop
								 // target
...
enum ImGuiButtonFlags_ {
	...
	ImGuiButtonFlags_MouseButtonMiddle = 1
										 << 2,	// React on center mouse button
...
enum ImGuiColorEditFlags_ {
	...
	ImGuiColorEditFlags_NoSidePreview =
		1 << 8,	 //              // ColorPicker: disable bigger color preview on
				 //              right side of the picker, use small color
	...
	ImGuiColorEditFlags_NoColorMarkers =
		1 << 11,  //              // ColorEdit: disable rendering R/G/B/A color
				  //              marker. May also be disabled globally by
				  //              setting style.ColorMarkerSize = 0.
	...
	ImGuiColorEditFlags_AlphaOpaque =
		1 << 12,  //              // ColorEdit, ColorPicker, ColorButton:
				  //              disable alpha in the preview,. Contrary to
				  //              _NoAlpha it may still be edited when calling
				  //              ColorEdit4()/ColorPicker4(). For ColorButton()
				  //              this does the same as _NoAlpha.
	ImGuiColorEditFlags_AlphaNoBg =
		1 << 13,  //              // ColorEdit, ColorPicker, ColorButton:
				  //              disable rendering a checkerboard background
				  //              behind transparent color.
	ImGuiColorEditFlags_AlphaPreviewHalf =
		1
		<< 14,	//              // ColorEdit, ColorPicker, ColorButton: display
				//              half opaque / half transparent preview.

	// User Options (right-click on widget to change some of them).
	ImGuiColorEditFlags_AlphaBar =
		1 << 18,  //              // ColorEdit, ColorPicker: show vertical alpha
...
enum ImGuiSliderFlags_ {
	...
	ImGuiSliderFlags_ClampOnInput =
		1
		<< 9,  // Clamp value to min/max bounds when input manually with
	...
	ImGuiSliderFlags_NoSpeedTweaks =
		1 << 11,  // Disable keyboard modifiers altering tweak speed. Useful if
				  // you want to alter tweak speed yourself based on your own
				  // logic.
	ImGuiSliderFlags_ColorMarkers =
		1 << 12,  // DragScalarN(), SliderScalarN(): Draw R/G/B/A color markers
				  // on each component.
	...
	ImGuiSliderFlags_InvalidMask_ =
		0x7000000F,	 // [Internal] We treat using those bits as being
					 // potentially a 'float power' argument from legacy API
					 // (obsoleted 2020-08) that has got miscast to this enum,
					 // and will trigger an assert if needed.
};
...
enum ImGuiMouseCursor_ {
	...
	ImGuiMouseCursor_ResizeEW,	  // When hovering over a vertical border or a
								  // column
	ImGuiMouseCursor_ResizeNESW,  // When hovering over the bottom-left corner
								  // of a window
	...
	ImGuiMouseCursor_Progress,	// When waiting for something to process/load,
								// but application is still interactive.
...
enum ImGuiMouseSource : int {
	...
	ImGuiMouseSource_TouchScreen,  // Input is coming from a touch screen (no
								   ...
								   // wheeling possible).
	ImGuiMouseSource_Pen,  // Input is coming from a pressure/magnetic pen
						   // (often used in conjunction with high-sampling
						   // rates).
...
enum ImGuiTableFlags_ {
	...
	ImGuiTableFlags_SizingStretchSame =
		4 << 13,  // Columns default to _WidthStretch with default weights all
				  // equal, unless overridden by TableSetupColumn(). Sizing
				  // Extra Options
	...
	ImGuiTableFlags_NoKeepColumnsVisible =
		1 << 18,  // Disable keeping column always minimally visible when
				  // ScrollX is off and table gets too small. Not recommended if
	...
	ImGuiTableFlags_ScrollX =
		1 << 24,  // Enable horizontal scrolling. Require 'outer_size' parameter
				  // of BeginTable() to specify the container size. Changes
				  // default sizing policy. Because this creates a child window,
				  // ScrollY is currently generally recommended when using
				  // ScrollX.
	ImGuiTableFlags_ScrollY =
		1 << 25,  // Enable vertical scrolling. Require 'outer_size' parameter
				  // of BeginTable() to specify the container size. Sorting
	ImGuiTableFlags_SortMulti =
		1
		<< 26,	// Hold shift when clicking headers to sort on multiple column.
				// TableGetSortSpecs() may return specs where (SpecsCount > 1).
	ImGuiTableFlags_SortTristate =
		1 << 27,  // Allow no sorting, disable default sorting.
				  // TableGetSortSpecs() may return specs where (SpecsCount ==
				  // 0). Miscellaneous
...
enum ImGuiTableColumnFlags_ {
	...
	ImGuiTableColumnFlags_PreferSortAscending =
		1 << 14,  // Make the initial sort direction Ascending when first
	...
	ImGuiTableColumnFlags_IsEnabled =
		1 << 24,  // Status: is enabled == not hidden by user/api (referred to
				  // as "Hide" in _DefaultHide and _NoHide) flags.
	ImGuiTableColumnFlags_IsVisible =
		1 << 25,  // Status: is visible == is enabled AND not clipped by
				  // scrolling.
...
enum ImGuiTableBgTarget_ {
	...
	ImGuiTableBgTarget_CellBg =
		3,	// Set cell background color (top-most color)
};
...
struct ImGuiStyle {
	// Font scaling
	// - recap: ImGui::GetFontSize() == FontSizeBase * (FontScaleMain *
	// FontScaleDpi * other_scaling_factors)
	float
		FontSizeBase;  // Current base font size before external global factors
					   // are applied. Use PushFont(NULL, size) to modify. Use
					   // ImGui::GetFontSize() to obtain scaled value.
	float FontScaleMain;  // Main global scale factor. May be set by application
						  // once, or exposed to end-user.
	float FontScaleDpi;	 // Additional global scale factor from viewport/monitor
						 // contents scale. In docking branch: when
	...
	float
		WindowBorderHoverPadding;  // Hit-testing extent outside/inside resizing
								   // border. Also extend determination of
								   // hovered window. Generally meaningfully
								   // larger than WindowBorderSize to make it
	...
	ImVec2 ItemInnerSpacing;  // Horizontal and vertical spacing between within
							  // elements of a composed widget (e.g. a slider
							  // and its label).
	...
	float ScrollbarPadding;	 // Padding of scrollbar grab within its frame (same
							 // for both axes).
	...
	float TabMinWidthBase;	// Minimum tab width, to make tabs larger than their
	...
	float TabCloseButtonMinWidthSelected;  // -1: always visible. 0.0f: visible
										   // when hovered. >0.0f: visible when
										   // hovered if minimum width.
	float TabCloseButtonMinWidthUnselected;	 // -1: always visible. 0.0f:
											 // visible when hovered. >0.0f:
	...
	ImGuiTreeNodeFlags
		TreeLinesFlags;		  // Default way to draw lines connecting TreeNode
							  // hierarchy. ImGuiTreeNodeFlags_DrawLinesNone or
							  // ImGuiTreeNodeFlags_DrawLinesFull or
							  // ImGuiTreeNodeFlags_DrawLinesToNodes.
	float TreeLinesSize;	  // Thickness of outlines when using
							  // ImGuiTreeNodeFlags_DrawLines.
	float TreeLinesRounding;  // Radius of lines connecting child nodes to the
							  // vertical line.
	...
	float ColorMarkerSize;	// Size of R/G/B/A color markers for ColorEdit4()
							...
							// ImGuiSliderFlags_ColorMarkers.
	ImGuiDir ColorButtonPosition;  // Side of the color button in the ColorEdit4
								   // widget (left/right). Defaults to
								   // ImGuiDir_Right.
	ImVec2 ButtonTextAlign;	 // Alignment of button text when button is larger
							 // than text. Defaults to (0.5f, 0.5f) (centered).
	ImVec2
		SelectableTextAlign;  // Alignment of selectable text. Defaults to
	...
	ImVec2 SeparatorTextPadding;  // Horizontal offset of text from each edge of
								  ...
								  // Generally small values. .y is recommended
	...
	ImVec2
		DisplaySafeAreaPadding;	 // Apply to every windows, menus, popups,
	...
	bool DockingNodeHasCloseButton;	 // Docking node has their own CloseButton()
									 // to close all docked windows.
	...
	// [Internal]
	float _MainScale;			   // FIXME-WIP: Reference scale, as applied by
								   // ScaleAllSizes().
	float _NextFrameFontSizeBase;  // FIXME: Temporary hack until we finish
								   // remaining work.

	// Functions
	IMGUI_API ImGuiStyle();
	IMGUI_API void ScaleAllSizes(
		float scale_factor);  // Scale all spacing/padding/thickness values. Do
							  // not scale fonts.

	// Obsolete names
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
	// TabMinWidthForCloseButton = TabCloseButtonMinWidthUnselected // Renamed
	// in 1.91.9.
#endif
};
...
struct ImGuiIO {
	...
	ImGuiBackendFlags
		BackendFlags;  // = 0              // See ImGuiBackendFlags_ enum. Set
					   // by backend (imgui_impl_xxx files or custom backend) to
	...
	ImVec2
		DisplayFramebufferScale;  // = (1, 1)         // Main display density.
								  // For retina display where window coordinates
								  // are different from framebuffer coordinates.
								  // This will affect font density + will end up
	...
	const char*
		IniFilename;  // = "imgui.ini"    // Path to .ini file (important:
					  ...
					  // dir!). Set NULL to disable automatic .ini
					  // loading/saving or if you want to manually call
	...
	bool ConfigNavMoveSetMousePos;	// = false          // Directional/tabbing
									// navigation teleports the mouse cursor.
									// May be useful on TV/console systems where
	...
	bool ConfigNavEscapeClearFocusWindow;  // = false          // Pressing
										   // Escape can clear focused window as
										   // well (super set of
										   // io.ConfigNavEscapeClearFocusItem).
	bool ConfigNavCursorVisibleAuto;  // = true           // Using directional
									  // navigation key makes the cursor
									  // visible. Mouse click hides the cursor.
	...
	bool ConfigDockingNoSplit;	// = false          // Simplified docking mode:
								...
								// into tab-bars.
	bool ConfigDockingNoDockingOver;  // = false          // Simplified docking
									  // mode: disable window merging into a
									  // same tab-bar, so docking is limited to
									  // splitting windows.
	...
	bool ConfigDockingTransparentPayload;  // = false          // [BETA] Make
										   // window or viewport transparent
										   // when docking and only display
										   // docking boxes on the target
										   // viewport. Useful if rendering of
										   // multiple viewport cannot be
										   // synced. Best used with
	...
	bool ConfigViewportsNoAutoMerge;  // = false;         // Set to make all
									  ...
									  // merged into the main host viewports
									  // when overlapping it. May also set
									  // ImGuiViewportFlags_NoAutoMerge on
									  // individual viewport.
	bool ConfigViewportsNoTaskBarIcon;	// = false          // Disable default
										// OS task bar icon flag for secondary
	...
	bool ConfigViewportsNoDecoration;  // = true           // Disable default OS
									   ...
									   // viewports. When a viewport doesn't
									   // want window decorations,
									   // ImGuiViewportFlags_NoDecoration will
									   // be set on it. Enabling decoration can
									   // create subsequent issues at OS levels
									   // (e.g. minimum window size).
	bool ConfigViewportsNoDefaultParent;  // = true           // When false: set
										  // secondary viewports'
										  // ParentViewportId to main viewport
										  // ID by default. Expects the platform
										  // backend to setup a parent/child
										  // relationship between the OS windows
										  // based on this value. Some backend
										  // may ignore this. Set to true if you
										  // want viewports to automatically be
										  // parent of main viewport, otherwise
										  // all viewports will be top-level OS
										  // windows.
	bool
	...
	bool ConfigDpiScaleFonts;  // = false          // [EXPERIMENTAL]
							   // Automatically overwrite style.FontScaleDpi
							   // when Monitor DPI changes. This will scale
							   // fonts but _NOT_ scale sizes/padding for now.
	bool ConfigDpiScaleViewports;  // = false          // [EXPERIMENTAL] Scale
								   // Dear ImGui and Platform Windows when
								   // Monitor DPI changes.
	...
	bool
		ConfigMacOSXBehaviors;	// = defined(__APPLE__) // Swap Cmd<>Ctrl keys +
	...
	bool ConfigInputTrickleEventQueue;	// = true           // Enable input
										// queue trickling: some types of events
	...
	bool ConfigWindowsResizeFromEdges;	// = true           // Enable resizing
										// of windows from their edges and from
										// the lower-left corner. This requires
										...
										// flag)
	bool ConfigWindowsMoveFromTitleBarOnly;	 // = false      // Enable allowing
											 // to move windows only when
											 // clicking on their title bar.
											 // Does not apply to windows
											 // without a title bar.
	bool
		ConfigWindowsCopyContentsWithCtrlC;	 // = false      // [EXPERIMENTAL]
	...
	float KeyRepeatDelay;	   // = 0.275f         // When holding a key/button,
	...
	//    language - the latter tends to facilitate iterating on code while
	//    running).
	...
	bool ConfigErrorRecoveryEnableTooltip;	// = true       // Enable tooltip on
											...
											// include a way to enable asserts
	...
	IMGUI_API void AddMouseButtonEvent(
		int button, bool down);	 // Queue a mouse button change
	...
	ImVec2 MousePos;  // Mouse position, in pixels. Set to ImVec2(-FLT_MAX,
					  ...
					  // etc.)
	bool
		MouseDown[5];  // Mouse buttons: 0=left, 1=right, 2=middle + extras
					   // (ImGuiMouseButton_COUNT == 5). Dear ImGui mostly uses
					   // left and right buttons. Other buttons allow us to
	...
	float MouseWheelH;	// Mouse wheel Horizontal. >0 scrolls Left, <0 scrolls
						// Right. Most users don't have a mouse with a
	...
	ImGuiID
		MouseHoveredViewport;  // (Optional) Modify using
							   ...
							   // with the ImGuiViewportFlags_NoInputs flag is
	...
	bool KeySuper;	// Keyboard modifier down: Windows/Super (non-macOS), Ctrl
					// (macOS)
	...
	ImGuiKeyData KeysData
		[ImGuiKey_NamedKey_COUNT];	// Key state for all known keys. MUST use
									// 'key - ImGuiKey_NamedKey_BEGIN' as index.
									// Use IsKeyXXX() functions to access this.
	...
	ImU16 MouseClickedCount[5];	 // == 0 (not clicked), == 1 (same as
								 ...
								 // (triple-clicked) etc. when going from !Down
								 // to Down
	...
	double
		MouseReleasedTime[5];  // Time of last released (rarely used! but useful
							   // to handle delayed single-click when trying to
							   // disambiguate them from double-click).
	...
	bool
		MouseWheelRequestAxisSwap;	// On a non-Mac system, holding Shift
	...
	bool MouseCtrlLeftAsRightClick;	 // (OSX) Set to true when the current click
									 // was a Ctrl+Click that spawned a
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
	float FontGlobalScale;	// Moved io.FontGlobalScale to style.FontScaleMain
							// in 1.92 (June 2025)
	...
	// void ClearInputCharacters() { InputQueueCharacters.resize(0); } //
	// [Obsoleted in 1.89.8] Clear the current frame text input buffer. Now
	// included within ClearInputKeys(). Removed this as it is
	// ambiguous/misleading and generally incorrect to use with the existence of
...
struct ImGuiInputTextCallbackData {
	...
	ImWchar EventChar;	// Character input                      // Read-write //
						...
						// set to zero to drop. return 1 is equivalent to
						// setting EventChar=0;
	bool EventActivated;  // Input field just got activated       // Read-only
						  // // [Always]
	...
	int BufSize;	 // Buffer size (in bytes) = capacity+1  // Read-only    //
				  ...
				  // storage. In C land: == ARRAYSIZE(my_char_array), in C++
				  // land: string.capacity()+1
	...
	void SetSelection(int s, int e) {
		IM_ASSERT(s >= 0 && s <= BufTextLen);
		IM_ASSERT(e >= 0 && e <= BufTextLen);
		SelectionStart = s;
...
struct ImGuiWindowClass {
	...
	ImGuiViewportFlags
		ViewportFlagsOverrideSet;  // Viewport flags to set when a window of
	...
	bool
		DockingAllowUnclassed;	// Set to true to allow windows of this class to
...
// Flags for ImGuiListClipper (currently not fully exposed in function calls: a
// future refactor will likely add this to ImGuiListClipper::Begin function
// equivalent)
enum ImGuiListClipperFlags_ {
	ImGuiListClipperFlags_None = 0,
	ImGuiListClipperFlags_NoSetTableRowCounters =
		1 << 0,	 // [Internal] Disabled modifying table row counters. Avoid
				 // assumption that 1 clipper item == 1 table row.
};
...
struct ImGuiListClipper {
	...
	double StartPosY;	// [Internal] Cursor position at the time of Begin() or
	...
	ImGuiListClipperFlags
		Flags;	// [Internal] Flags, currently not yet well exposed.
	...
	IMGUI_API void IncludeItemsByIndex(
		int item_begin, int item_end);	// item_end is exclusive e.g. use (42,
...
enum ImGuiMultiSelectFlags_ {
	...
	ImGuiMultiSelectFlags_NoAutoClearOnReselect =
		1 << 5,	 // Disable clearing selection when clicking/selecting an
				 // already selected item.
	ImGuiMultiSelectFlags_BoxSelect1d =
		1 << 6,	 // Enable box-selection with same width and same x pos items
				 // (e.g. full row Selectable()). Box-selection works better
				 // with little bit of spacing between items hit-box in order to
				 // be able to aim at empty space.
	...
	ImGuiMultiSelectFlags_SelectOnClickRelease =
		1 << 14,  // Apply selection on mouse release when clicking an
				  // unselected item. Allow dragging an unselected item without
				  // altering selection.
	...
	ImGuiMultiSelectFlags_NoSelectOnRightClick =
		1 << 17,  // Disable default right-click processing, which selects item
...
struct ImGuiMultiSelectIO {
	...
	ImGuiSelectionUserData
		RangeSrcItem;  //  ms:w  app:r     /                // (If using
					   //  clipper) Begin: Source item (often the first selected
					   //  item) must never be clipped: use
					   //  clipper.IncludeItemByIndex() to ensure it is
					   //  submitted.
	ImGuiSelectionUserData
		NavIdItem;	//  ms:w, app:r     /                // (If using deletion)
					//  Last known SetNextItemSelectionUserData() value for
					//  NavId (if part of submitted items).
	bool NavIdSelected;	 //  ms:w, app:r     /        app:r   // (If using
						 //  deletion) Last known selection state for NavId (if
						 //  part of submitted items).
	bool
		RangeSrcReset;	//        app:w     /  ms:r          // (If using
...
enum ImGuiSelectionRequestType {
	...
	ImGuiSelectionRequestType_SetRange,	 // Request app to select/unselect
										 ...
										 // request this, app code can read
										 // after BeginMultiSelect() and it will
										 // always be false.
};
...
struct ImGuiSelectionRequest {
	...
	bool Selected;	//  ms:w, app:r     /  ms:w, app:r   // Parameter for
					//  SetAll/SetRange requests (true = select, false =
					//  unselect)
	ImS8
		RangeDirection;	 //                  /  ms:w  app:r   // Parameter for
...
struct ImGuiSelectionBasicStorage {
	...
	int _SelectionOrder;  // [Internal] Increasing counter to store selection
						  // order
	ImGuiStorage
		_Storage;  // [Internal] Selection set. Think of this as similar to e.g.
				   // std::set<ImGuiID>. Prefer not accessing directly: iterate
				   // with GetNextSelectedItem().
...
//   VtxOffset fields.
// - The ClipRect/TexRef/VtxOffset fields must be contiguous as we memcmp() them
// together (this is asserted for).
struct ImDrawCmd {
	ImVec4 ClipRect;  // 4*4  // Clipping rectangle (x1, y1, x2, y2). Subtract
					  // ImDrawData->DisplayPos to get clipping rectangle in
					  // "viewport" coordinates
	ImTextureRef TexRef;  // 16   // Reference to a font/texture atlas (where
						  // backend called ImTextureData::SetTexID()) or to a
						  // user-provided texture ID (via e.g. ImGui::Image()
						  // calls). Both will lead to a ImTextureID value.
	unsigned int VtxOffset;	 // 4    // Start offset in vertex buffer.
							 // ImGuiBackendFlags_RendererHasVtxOffset: always
							 // 0, otherwise may be >0 to support meshes larger
	...
	ImDrawCallback
		UserCallback;  // 4-8  // If != NULL, call the function instead of
					   // rendering the vertices. clip_rect and texture_id will
					   // be set normally.
	...
	// function for an upcoming feature) Since 1.92: removed
	// ImDrawCmd::TextureId field, the getter function must be used!
	inline ImTextureID GetTexID()
...
enum ImDrawFlags_ {
	...
	ImDrawFlags_RoundCornersTopLeft =
		1 << 4,	 // AddRect(), AddRectFilled(), PathRect(): enable rounding
				 // top-left corner only (when rounding > 0.0f, we default to
				 // all corners). Was 0x01.
...
enum ImDrawListFlags_ {
	...
	ImDrawListFlags_AntiAliasedLines =
		1 << 0,	 // Enable anti-aliased lines/borders (*2 the number of
...
struct ImDrawList {
	...
	float _FringeScale;	 // [Internal] anti-alias fringe is scaled by this
						 // value, this helps to keep things sharp while zooming
						 // at vertex buffer content
	...
	IMGUI_API void AddText(const ImVec2& pos, ImU32 col, const char* text_begin,
						   const char* text_end = NULL);
	IMGUI_API void AddText(ImFont* font, float font_size, const ImVec2& pos,
	...
	// complexity. Provided as a convenience for the user but not used by the
	// main library.
	...
	IMGUI_API void AddImage(ImTextureRef tex_ref, const ImVec2& p_min,
							const ImVec2& p_max,
							const ImVec2& uv_min = ImVec2(0, 0),
							const ImVec2& uv_max = ImVec2(1, 1),
							ImU32 col = IM_COL32_WHITE);
	IMGUI_API void AddImageQuad(
		ImTextureRef tex_ref, const ImVec2& p1, const ImVec2& p2,
	...
	IMGUI_API void AddImageRounded(ImTextureRef tex_ref, const ImVec2& p_min,
	...
	IMGUI_API void AddCallback(ImDrawCallback callback, void* userdata,
	...
	IMGUI_API ImDrawList* CloneOutput()
		const;	// Create a clone of the CmdBuffer/IdxBuffer/VtxBuffer. For
				// multi-threaded rendering, consider using
				// `imgui_threaded_rendering` from
...
struct ImDrawData {
	bool Valid;	 // Only valid after Render() is called and before the next
				 // NewFrame() is called.
	int CmdListsCount;	// == CmdLists.Size. (OBSOLETE: exists for legacy
						// reasons). Number of ImDrawList* to render.
	...
	ImVec2 FramebufferScale;  // Amount of pixels for each unit of DisplaySize.
							  // Copied from viewport->FramebufferScale (==
							  // io.DisplayFramebufferScale for main viewport).
							  // Generally (1,1) on normal display, (2,2) on OSX
							  // with Retina display.
	ImGuiViewport*
		OwnerViewport;	// Viewport carrying the ImDrawData instance, might be
						// of use to the renderer (generally not).
	ImVector<ImTextureData*>*
		Textures;  // List of textures to update. Most of the times the list is
				   // shared by all ImDrawData, has only 1 texture and it
				   // doesn't need any update. This almost always points to
				   // ImGui::GetPlatformIO().Textures[]. May be overridden or
				   // set to NULL if you want to manually update textures.
...
// ImTextureData)
//-----------------------------------------------------------------------------
// In principle, the only data types that user/application code should care
// about are 'ImTextureRef' and 'ImTextureID'. They are defined above in this
// header file. Read their description to the difference between ImTextureRef
// and ImTextureID. FOR ALL OTHER ImTextureXXXX TYPES: ONLY CORE LIBRARY AND
// RENDERER BACKENDS NEED TO KNOW AND CARE ABOUT THEM.
//-----------------------------------------------------------------------------

#undef Status  // X11 headers are leaking this.

// We intentionally support a limited amount of texture formats to limit burden
// on CPU-side code and extension. Most standard backends only support RGBA32
// but we provide a single channel option for low-resource/embedded systems.
enum ImTextureFormat {
	ImTextureFormat_RGBA32,	 // 4 components per pixel, each is unsigned 8-bit.
							 // Total size = TexWidth * TexHeight * 4
	ImTextureFormat_Alpha8,	 // 1 component per pixel, each is unsigned 8-bit.
...
enum ImTextureStatus {
	...
	ImTextureStatus_WantDestroy,  // Requesting backend to destroy the texture.
								  // Set status to Destroyed when done.
};

// Coordinates of a rectangle within a texture.
// When a texture is in ImTextureStatus_WantUpdates state, we provide a list of
// individual rectangles to copy to the graphics system. You may use
// ImTextureData::Updates[] for the list, or ImTextureData::UpdateBox for a
// single bounding box.
...
// This is only useful for (1) core library and (2) backends.
// End-user/applications do not need to care about this. Renderer Backends will
// create a GPU-side version of this. Why does we store two identifiers: TexID
// and BackendUserData?
// - ImTextureID    TexID           = lower-level identifier stored in
// ImDrawCmd. ImDrawCmd can refer to textures not created by the backend, and
// for which there's no ImTextureData.
// - void*          BackendUserData = higher-level opaque storage for backend
// own book-keeping. Some backends may have enough with TexID and not need both.
// In columns below: who reads/writes each fields? 'r'=read, 'w'=write,
...
struct ImTextureData {
	...
	ImTextureID TexID;		// r    w   // Backend-specific texture identifier.
						// Always use SetTexID() to modify! The identifier will
						// stored in ImDrawCmd::GetTexID() and passed to
						// backend's RenderDrawData function.
	ImTextureFormat Format;	 // w    r   // ImTextureFormat_RGBA32 (default) or
	...
	ImVector<ImTextureRect>
		Updates;	   // w    r   // Array of individual updates.
	int UnusedFrames;  // w    r   // In order to facilitate handling
					   // Status==WantDestroy in some backend: this is a count
					   // successive frames where the texture was not used.
					   // Always >0 when Status==WantDestroy.
	unsigned short RefCount;  // w    r   // Number of contexts using this
							  // texture. Used during backend shutdown.
	bool UseColors;	 // w    r   // Tell whether our texture data is known to
					 // use colors (rather than just white + alpha).
	bool WantDestroyNextFrame;	// rw   -   // [Internal] Queued to set
								// ImTextureStatus_WantDestroy next frame. May
	...
	// - Call SetTexID() and SetStatus() after honoring texture requests. Never
	// modify TexID and Status directly!
	// - A backend may decide to destroy a texture that we did not request to
	// destroy, which is fine (e.g. freeing resources), but we immediately set
	// the texture back in _WantCreate mode.
...
struct ImFontConfig {
	// Data Source
	char Name[40];	   // <auto>   // Name (strictly to ease debugging, hence
					   // limited size buffer)
	void* FontData;	   //          // TTF/OTF data
	int FontDataSize;  //          // TTF/OTF data size
	bool FontDataOwnedByAtlas;	// true     // TTF/OTF data ownership taken by
								// the owner ImFontAtlas (will delete memory
								// itself). SINCE 1.92, THE DATA NEEDS TO
								// PERSIST FOR WHOLE DURATION OF ATLAS.
	...
	ImS8
		OversampleH;  // 0 (2)    // Rasterize at higher quality for sub-pixel
					  // positioning. 0 == auto == 1 or 2 depending on size.
					  // Note the difference between 2 and 3 is minimal. You can
	...
	ImWchar EllipsisChar;  // 0        // Explicitly specify Unicode codepoint
						   // of ellipsis character. When fonts are being merged
						   // first specified ellipsis will be used.
	float SizePixels;  //          // Output size in pixels for rasterizer (more
					   //          or less maps to the resulting font height).
	const ImWchar*
		GlyphRanges;  // NULL     // *LEGACY* THE ARRAY DATA NEEDS TO PERSIST AS
					  // LONG AS THE FONT IS ALIVE. Pointer to a user-provided
					  // list of Unicode range (2 value per range, values are
					  // inclusive, zero-terminated list).
	const ImWchar*
		GlyphExcludeRanges;	 // NULL     // Pointer to a small user-provided
							 // list of Unicode ranges (2 value per range,
							 // values are inclusive, zero-terminated list).
							 // This is very close to GlyphRanges[] but designed
							 // to exclude ranges from a font source, when
							 // merging fonts with overlapping glyphs. Use
							 // "Input Glyphs Overlap Detection Tool" to find
							 // about your overlapping ranges.
	// ImVec2        GlyphExtraSpacing;      // 0, 0     // (REMOVED AT IT SEEMS
	// LARGELY OBSOLETE. PLEASE REPORT IF YOU WERE USING THIS). Extra spacing
	// (in pixels) between glyphs when rendered: essentially add to
	// glyph->AdvanceX. Only X axis is supported for now.
	ImVec2 GlyphOffset;	 // 0, 0     // Offset (in pixels) all glyphs from this
						 // font input. Absolute value for default size, other
						 // sizes will scale this value.
	float GlyphMinAdvanceX;	 // 0        // Minimum AdvanceX for glyphs, set Min
							 // to align font icons, set both Min/Max to enforce
							 // mono-space font. Absolute value for default
							 // size, other sizes will scale this value.
	float GlyphMaxAdvanceX;	 // FLT_MAX  // Maximum AdvanceX for glyphs
	float
		GlyphExtraAdvanceX;	 // 0        // Extra spacing (in pixels) between
							 // glyphs. Please contact us if you are using this.
							 // // FIXME-NEWATLAS: Intentionally unscaled
	ImU32 FontNo;			 // 0        // Index of font within TTF/OTF file
	unsigned int FontLoaderFlags;  // 0        // Settings for custom font
								   // builder. THIS IS BUILDER IMPLEMENTATION
								   // DEPENDENT. Leave as zero if unsure.
	// unsigned int  FontBuilderFlags;       // --       // [Renamed in 1.92]
	// Use FontLoaderFlags.
	float RasterizerMultiply;  // 1.0f     // Linearly brighten (>1.0f) or
							   // darken (<1.0f) font output. Brightening small
							   // fonts may be a good workaround to make them
							   // more readable. This is a silly thing we may
							   // remove in the future.
	float RasterizerDensity;  // 1.0f     // [LEGACY: this only makes sense when
							  // ImGuiBackendFlags_RendererHasTextures is not
							  // supported] DPI scale multiplier for
							  // rasterization. Not altering other font metrics:
							  // makes it easy to swap between e.g. a 100% and a
							  // 400% fonts for a zooming display, or handle
							  // Retina screen. IMPORTANT: If you change this it
							  // is expected that you increase/decrease font
							  // scale roughly to the inverse of this, otherwise
							  // quality may look lowered.
	float
		ExtraSizeScale;	 // 1.0f     // Extra rasterizer scale over SizePixels.

	// [Internal]
	ImFontFlags Flags;	// Font flags (don't use just yet, will be exposed in
						// upcoming 1.92.X updates)
	ImFont* DstFont;  // Target font (as we merging fonts, multiple ImFontConfig
					  // may target the same font)
	const ImFontLoader*
		FontLoader;	 // Custom font backend for this source (default source is
					 // the one stored in ImFontAtlas)
	void* FontLoaderData;  // Font loader opaque storage (per font config)

#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
	bool PixelSnapV;  // true    // [Obsoleted in 1.91.6] Align Scaled
					  // GlyphOffset.y to pixel boundaries.
#endif
	IMGUI_API ImFontConfig();
};

// Hold rendering data for one glyph.
// (Note: some language parsers may fail to convert the bitfield members, in
// this case maybe drop store a single u32 or we can rework this)
struct ImFontGlyph {
	unsigned int
		Colored : 1;  // Flag to indicate glyph is colored and should generally
					  // ignore tinting (make it usable with no shift on
					  // little-endian as this is used in loops)
	...
	float U0, V0, U1, V1;  // Texture coordinates for the current value of
...
// The rectangle may move and UV may be invalidated, use GetCustomRect() to
// retrieve it.
...
// Those values may not be cached/stored as they are only valid for the current
// value of atlas->TexRef (this is in theory derived from ImTextureRect but we
// use separate structures for reasons)
...
enum ImFontAtlasFlags_ {
	...
	ImFontAtlasFlags_NoBakedLines =
		1
		<< 2,  // Don't build thick line textures into the atlas (save a little
...
//  - If you don't call any AddFont*** functions, the default font embedded in
//  the code will be loaded for you.
// It is the rendering backend responsibility to upload texture into your
// graphics API:
//  - ImGui_ImplXXXX_RenderDrawData() functions generally iterate
//  platform_io->Textures[] to create/update/destroy each ImTextureData
//  instance.
//  - Backend then set ImTextureData's TexID and BackendUserData.
//  - Texture id are passed back to you during rendering to identify the
//  texture. Read FAQ entry about ImTextureID/ImTextureRef for more details.
// Legacy path:
//  - Call Build() + GetTexDataAsAlpha8() or GetTexDataAsRGBA32() to build and
//  retrieve pixels data.
...
struct ImFontAtlas {
	...
	IMGUI_API ImFont* AddFontDefault(
		const ImFontConfig* font_cfg =
			NULL);	// Selects between AddFontDefaultVector() and
					// AddFontDefaultBitmap().
	IMGUI_API ImFont* AddFontDefaultVector(
		const ImFontConfig* font_cfg =
			NULL);	// Embedded scalable font. Recommended at any higher size.
	IMGUI_API ImFont* AddFontDefaultBitmap(
		const ImFontConfig* font_cfg =
			NULL);	// Embedded classic pixel-clean font. Recommended at Size
					// 13px with no scaling.
	...
	IMGUI_API void
	Clear();  // Clear everything (input fonts, output glyphs/textures).
	IMGUI_API void CompactCache();	// Compact cached glyphs and texture.
	IMGUI_API void SetFontLoader(
		const ImFontLoader* font_loader);  // Change font loader at runtime.

	// As we are transitioning toward a new font system, we expect to obsolete
	// those soon:
	IMGUI_API void
	ClearInputData();  // [OBSOLETE] Clear input data (all ImFontConfig
					   // structures including sizes, TTF data, glyph ranges,
					   // etc.) = all the data used to build the texture and
					   // fonts.
	IMGUI_API void
	ClearFonts();  // [OBSOLETE] Clear input+output font data (same as
				   // ClearInputData() + glyphs storage, UV coordinates).
	IMGUI_API void ClearTexData();	// [OBSOLETE] Clear CPU-side copy of the
									// texture data. Saves RAM once the texture
	...
	// create a texture with your engine). Then store your texture handle with
	// SetTexID().
	// - The pitch is always = Width * BytesPerPixels (1 or 4)
	// - Building in RGBA32 format is provided for convenience and
	// compatibility, but note that unless you manually manipulate or copy color
	// data into
	//   the texture (e.g. when using the AddCustomRect*** api), then the RGB
	//   pixels emitted will always be white (~75% of memory/bandwidth waste).
	// - From 1.92 with backends supporting
	// ImGuiBackendFlags_RendererHasTextures:
	//   - Calling Build(), GetTexDataAsAlpha8(), GetTexDataAsRGBA32() is not
	//   needed.
	...
	// Since 1.92: specifying glyph ranges is only useful/necessary if your
	// backend doesn't support ImGuiBackendFlags_RendererHasTextures!
	...
	// your own purpose.
	// - Since 1.92.0, packing is done immediately in the function call
	// (previously packing was done during the Build call)
	// - You can render your pixels into the texture right after calling the
	// AddCustomRect() functions.
	// - VERY IMPORTANT:
	//   - Texture may be created/resized at any time when calling ImGui or
	//   ImFontAtlas functions.
	//   - IT WILL INVALIDATE RECTANGLE DATA SUCH AS UV COORDINATES. Always use
	//   latest values from GetCustomRect().
	//   - UV coordinates are associated to the current texture identifier aka
	//   'atlas->TexRef'. Both TexRef and UV coordinates are typically changed
	//   at the same time.
	// - If you render colored output into your custom rectangles: set
	// 'atlas->TexPixelsUseColors = true' as this may help some backends decide
	// of preferred texture format.
	// - Read docs/FONTS.md for more details about using colorful icons.
	// - Note: this API may be reworked further in order to facilitate
	// supporting e.g. multi-monitor, varying DPI settings.
	// - (Pre-1.92 names) ------------> (1.92 names)
	//   - GetCustomRectByIndex()   --> Use GetCustomRect()
	//   - CalcCustomRectUV()       --> Use GetCustomRect() and read uv0, uv1
	//   fields.
	//   - AddCustomRectRegular()   --> Renamed to AddCustomRect()
	//   - AddCustomRectFontGlyph() --> Prefer using custom ImFontLoader inside
	//   ImFontConfig
	...
	AddCustomRect(int width, int height,
				  ImFontAtlasRect* out_r =
					  NULL);  // Register a rectangle. Return -1
							  // (ImFontAtlasRectId_Invalid) on error.
	IMGUI_API void RemoveCustomRect(
		ImFontAtlasRectId
			id);  // Unregister a rectangle. Existing pixels will stay in
				  // texture until resized / garbage collected.
	IMGUI_API bool GetCustomRect(ImFontAtlasRectId id, ImFontAtlasRect* out_r)
		const;	// Get rectangle coordinates for current texture. Valid
	...
	ImTextureFormat TexDesiredFormat;  // Desired texture format (default to
									   // ImTextureFormat_RGBA32 but may be
									   // changed to ImTextureFormat_Alpha8).
	...
	int TexMinWidth;   // Minimum desired texture width. Must be a power of two.
					   // Default to 512.
	int TexMinHeight;  // Minimum desired texture height. Must be a power of
					   // two. Default to 128.
	int TexMaxWidth;   // Maximum desired texture width. Must be a power of two.
					   // Default to 8192.
	int TexMaxHeight;  // Maximum desired texture height. Must be a power of
					   // two. Default to 8192.
	void* UserData;	 // Store your own atlas related user-data (if e.g. you have
					 // multiple font atlas).

	// Output
	// - Because textures are dynamically created/resized, the current texture
	// identifier may changed at *ANY TIME* during the frame.
	// - This should not affect you as you can always use the latest value. But
	// note that any precomputed UV coordinates are only valid for the current
	// TexRef.
	...
	union {
		ImTextureRef TexRef;
		ImTextureRef TexID;
	};	// Latest texture identifier == TexData->GetTexRef(). // RENAMED TexID
		// to TexRef in 1.92.0.
#endif
	...
	// [Internal]
	ImVector<ImTextureData*>
		TexList;  // Texture list (most often TexList.Size == 1). TexData is
				  // always == TexList.back(). DO NOT USE DIRECTLY, USE
				  // GetDrawData().Textures[]/GetPlatformIO().Textures[]
				  // instead!
	bool Locked;  // Marked as locked during ImGui::NewFrame()..EndFrame() scope
				  // if TexUpdates are not supported. Any attempt to modify the
				  // atlas will assert.
	bool RendererHasTextures;  // Copy of (BackendFlags &
							   // ImGuiBackendFlags_RendererHasTextures) from
							   // supporting context.
	bool TexIsBuilt;  // Set when texture was built matching current font input.
					  // Mostly useful for legacy IsBuilt() call.
	bool TexPixelsUseColors;  // Tell whether our texture data is known to use
							  // colors (rather than just alpha channel), in
							  // order to help backend select a format or
							  // conversion process.
	ImVec2 TexUvScale;	// = (1.0f/TexData->TexWidth, 1.0f/TexData->TexHeight).
						// May change as new texture gets created.
	ImVec2 TexUvWhitePixel;	 // Texture coordinates to a white pixel. May change
							 // as new texture gets created.
	ImVector<ImFont*>
	...
	ImVector<ImDrawListSharedData*>
		DrawListSharedDatas;  // List of users for this atlas. Typically one per
							  // Dear ImGui context.
	ImFontAtlasBuilder*
		Builder;  // Opaque interface to our data that doesn't need to be public
				  // and may be discarded when rebuilding.
	const ImFontLoader*
		FontLoader;	 // Font loader opaque interface (default to use FreeType
					 // when IMGUI_ENABLE_FREETYPE is defined, otherwise default
					 // to use stb_truetype). Use SetFontLoader() to change this
					 // at runtime.
	const char* FontLoaderName;	 // Font loader name (for display e.g. in About
								 // box) == FontLoader->Name
	...
	// [Obsolete]
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
	// Legacy: You can request your rectangles to be mapped as font glyph (given
	// a font + Unicode point), so you can render e.g. custom colorful icons and
	// use them as regular glyphs. --> Prefer using a custom ImFontLoader.
	...
	IMGUI_API ImFontAtlasRectId AddCustomRectFontGlyph(
		...
		const ImVec2& offset = ImVec2(0,
									  0));	// OBSOLETED in 1.92.0: Use custom
											// ImFontLoader in ImFontConfig
	IMGUI_API ImFontAtlasRectId AddCustomRectFontGlyphForSize(
		ImFont* font, float font_size, ImWchar codepoint, int w, int h,
		float advance_x,
		const ImVec2& offset = ImVec2(0, 0));  // ADDED AND OBSOLETED in 1.92.0
#endif
	// unsigned int                      FontBuilderFlags;        // OBSOLETED
	// in 1.92.0: Renamed to FontLoaderFlags. int TexDesiredWidth;         //
	// OBSOLETED in 1.92.0: Force texture width before calling Build(). Must be
	// a power-of-two. If have many glyphs your graphics API have texture size
	// restrictions you may want to increase texture width to decrease height.
	// typedef ImFontAtlasRect           ImFontAtlasCustomRect;   // OBSOLETED
	// in 1.92.0 typedef ImFontAtlasCustomRect     CustomRect;              //
	// OBSOLETED in 1.72+ typedef ImFontGlyphRangesBuilder  GlyphRangesBuilder;
	// // OBSOLETED in 1.67+
};
...
struct ImFontBaked {
	...
	ImVector<float>
		IndexAdvanceX;	// 12-16 // out // Sparse. Glyphs->AdvanceX in a
						// directly indexable way (cache-friendly for
						// CalcTextSize functions which only this info, and are
						// often bottleneck in large UI).
	float
		FallbackAdvanceX;  // 4     // out // FindGlyph(FallbackChar)->AdvanceX
	float Size;	 // 4     // in  // Height of characters/line, set during
				 // loading (doesn't change after loading)
	...
	unsigned int MetricsTotalSurface : 26;	// 3  // out // Total surface in
	...
	unsigned int
		LoadNoRenderOnLayout : 1;  // 0  //     // Enable a two-steps mode where
								   // CalcTextSize() calls will load AdvanceX
								   // *without* rendering/packing glyphs. Only
								   // advantagous if you know that the glyph is
								   // unlikely to actually be rendered,
								   // otherwise it is slower because we'd do one
								   // query on the first CalcTextSize and one
								   // query on the first Draw.
	...
	void* FontLoaderDatas;	// 4-8   //     // Font loader opaque storage (per
							// baked font * sources): single contiguous buffer
							// allocated by imgui, passed to loader.
	...
	IMGUI_API ImFontGlyph* FindGlyphNoFallback(
		ImWchar c);	 // Return NULL if glyph doesn't exist
	IMGUI_API float GetCharAdvance(ImWchar c);
	IMGUI_API bool IsGlyphLoaded(ImWchar c);
};

// Font flags
// (in future versions as we redesign font loading API, this will become more
// important and better documented. for now please consider this as
// internal/advanced use)
enum ImFontFlags_ {
	ImFontFlags_None = 0,
	ImFontFlags_NoLoadError =
		1 << 1,	 // Disable throwing an error/assert when calling AddFontXXX()
				 // with missing file/data. Calling code is expected to check
				 // AddFontXXX() return value.
	ImFontFlags_NoLoadGlyphs = 1
							   << 2,  // [Internal] Disable loading new glyphs.
	ImFontFlags_LockBakedSizes =
		1 << 3,	 // [Internal] Disable loading new baked sizes, disable garbage
				 // collecting current ones. e.g. if you want to lock a font to
				 // a single size. Important: if you use this to preload given
				 // sizes, consider the possibility of multiple font density
				 // used on Retina display.
};

// Font runtime data and rendering
// - ImFontAtlas automatically loads a default embedded font for you if you
// didn't load one manually.
// - Since 1.92.0 a font may be rendered as any size! Therefore a font doesn't
// have one specific size.
// - Use 'font->GetFontBaked(size)' to retrieve the ImFontBaked* corresponding
// to a given size.
// - If you used g.Font + g.FontSize (which is frequent from the ImGui layer),
// you can use g.FontBaked as a shortcut, as g.FontBaked ==
// g.Font->GetFontBaked(g.FontSize).
struct ImFont {
	// [Internal] Members: Hot ~12-20 bytes
	ImFontBaked* LastBaked;	  // 4-8   // Cache last bound baked. NEVER USE
							  // DIRECTLY. Use GetFontBaked().
	ImFontAtlas* OwnerAtlas;  // 4-8   // What we have been loaded into.
	ImFontFlags Flags;		  // 4     // Font flags.
	float CurrentRasterizerDensity;	 // Current rasterizer density. This is a
									 // varying state of the font.

	// [Internal] Members: Cold ~24-52 bytes
	// Conceptually Sources[] is the list of font sources merged to create this
	// font.
	ImGuiID FontId;	   // Unique identifier for the font
	float LegacySize;  // 4     // in  // Font size passed to AddFont(). Use for
					   // old code calling PushFont() expecting to use that
					   // size. (use ImGui::GetFontBaked() to get font baked at
					   // current bound size).
	ImVector<ImFontConfig*> Sources;  // 16    // in  // List of sources.
									  // Pointers within OwnerAtlas->Sources[]
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
	float Scale;  // 4     // in  // Legacy base font scale (~1.0f), multiplied
				  // by the per-window font scale which you can adjust with
				  // SetWindowFontScale()
#endif
	...
	IMGUI_API ImVec2 CalcTextSizeA(float size, float max_width,
								   float wrap_width, const char* text_begin,
								   const char* text_end = NULL,
								   const char** out_remaining = NULL);
	IMGUI_API const char* CalcWordWrapPosition(float size, const char* text,
											   const char* text_end,
											   float wrap_width);
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
	inline const char* CalcWordWrapPositionA(float scale, const char* text,
											 const char* text_end,
											 float wrap_width) {
		return CalcWordWrapPosition(LegacySize * scale, text, text_end,
									wrap_width);
	}
#endif
	...
	IMGUI_API void AddRemapChar(
		...
		ImWchar to_codepoint);	// Makes 'from_codepoint' character points to
								// 'to_codepoint' glyph.
...
// Using an indirection to avoid patching ImDrawCmd after a SetTexID() call (but
...
inline ImTextureID ImDrawCmd::GetTexID() const {
	...
	// ImGuiBackendFlags_RendererHasTextures (1.92) must iterate and handle
	// ImTextureData requests stored in ImDrawData::Textures[].
	ImTextureID tex_id = TexRef._TexData
							 ? TexRef._TexData->TexID
	...
	if (TexRef._TexData != NULL)
		IM_ASSERT(
			...
			"graphics system. Backend must call ImTextureData::SetTexID() "
			"after handling ImTextureStatus_WantCreate request!");
	return tex_id;
}
...
struct ImGuiViewport {
	...
	void* PlatformHandle;  // void* to hold higher-level, platform window handle
						   // (e.g. HWND for Win32 backend, Uint32 WindowID for
						   // SDL, GLFWWindow* for GLFW), for
						   // FindViewportByPlatformHandle().
	void* PlatformHandleRaw;  // void* to hold lower-level, platform-native
							  // window handle (always HWND on Win32 platform,
	...
	bool PlatformRequestMove;	 // Platform window requested move (e.g. window
								 // was moved by the OS / host window manager,
								 // authoritative position will be OS window
								 // position)
	bool
		PlatformRequestResize;	// Platform window requested resize (e.g. window
								// was resized by the OS / host window manager,
								// authoritative size will be OS window size)
	bool PlatformRequestClose;	// Platform window requested closure (e.g.
								// window was moved by the OS / host window
...
struct ImGuiPlatformIO {
	...
	// architectures)
	const char* (*Platform_GetClipboardTextFn)(
		ImGuiContext* ctx);	 // Should return NULL on failure (e.g. clipboard
							 // data is not text).
	...
	// expected to return false on failure, but some platforms may always return
	// true)
	...
	// Optional: Maximum texture size supported by renderer (used to adjust how
	// we size textures). 0 if not known.
	...
	//------------------------------------------------------------------
	// Input - Interface with Platform & Renderer backends for Multi-Viewport
	// support
	//------------------------------------------------------------------
	...
	ImVec2 (*Platform_GetWindowFramebufferScale)(
		ImGuiViewport* vp);	 // N . . . .  // Return viewport density. Always
							 // 1,1 on Windows, often 2,2 on Retina display on
							 // macOS/iOS. MUST BE INTEGER VALUES.
	...
	int (*Platform_CreateVkSurface)(
		ImGuiViewport* vp, ImU64 vk_inst, const void* vk_allocators,
	...
	// Textures list (the list is updated by calling ImGui::EndFrame or
	// ImGui::Render) The ImGui_ImplXXXX_RenderDrawData() function of each
	// backend generally access this via ImDrawData::Textures which points to
	// this. The array is available here mostly because backends will want to
	// destroy textures on shutdown.
	ImVector<ImTextureData*>
		Textures;  // List of textures used by Dear ImGui (most often 1) +
				   // contents of external texture list is automatically
				   // appended into this.
	...
	IMGUI_API void
	ClearPlatformHandlers();  // Clear all Platform_XXX fields. Typically called
							  // on Platform Backend shutdown.
	IMGUI_API void
	ClearRendererHandlers();  // Clear all Renderer_XXX fields. Typically called
							  // on Renderer Backend shutdown.
};
...
struct ImGuiPlatformMonitor {
	...
	ImVec2 WorkPos,
		WorkSize;  // Coordinates without task bars / side bars / menu bars.
				   // Used to avoid positioning popups/tooltips inside this
				   // region. If you don't have this info, please copy the value
...
// platform_io.Platform_SetImeDataFn() function. Handler is called during
// EndFrame().
struct ImGuiPlatformImeData {
	bool WantVisible;	 // A widget wants the IME to be visible.
	bool WantTextInput;	 // A widget wants text input, not necessarily IME to be
						 // visible. This is automatically set to the upcoming
						 // value of io.WantTextInput.
...
IMGUI_API void SetWindowFontScale(
	float scale);  // Set font scale factor for current window. Prefer using
				   // PushFont(NULL, style.FontSizeBase * factor) or use
				   // style.FontScaleMain to scale all windows.
// OBSOLETED in 1.91.9 (from February 2025)
IMGUI_API void Image(
	ImTextureRef tex_ref, const ImVec2& image_size, const ImVec2& uv0,
	const ImVec2& uv1, const ImVec4& tint_col,
	const ImVec4& border_col);	// <-- 'border_col' was removed in favor of
								// ImGuiCol_ImageBorder. If you use 'tint_col',
...
GetWindowContentRegionMax();  // Content boundaries max for the window (roughly
							  // (0,0)+Size-Scroll), in window-local
							  // coordinates. You should never need this. Always
							  // use GetCursorScreenPos() and
							  // GetContentRegionAvail()!
...
// so they are not reported in IDE) OBSOLETED in 1.90.0 (from September 2023)
// inline bool         BeginChild(const char* str_id, const ImVec2& size_arg,
...
// window_flags); } // Unnecessary as true == ImGuiChildFlags_Borders inline
// bool         BeginChild(ImGuiID id, const ImVec2& size_arg, bool borders,
...
// Unnecessary as true == ImGuiChildFlags_Borders inline bool
// BeginChildFrame(ImGuiID id, const ImVec2& size, ImGuiWindowFlags flags = 0) {
// return BeginChild(id, size, ImGuiChildFlags_FrameStyle, flags); } inline void
// EndChildFrame()                                                             {
// EndChild(); } inline void         ShowStackToolWindow(bool* p_open = NULL) {
// ShowIDStackToolWindow(p_open); }
// OBSOLETED in 1.89.7 (from June 2023)
// IMGUI_API void      SetItemAllowOverlap(); // Use SetNextItemAllowOverlap()
// _before_ item.
//-- OBSOLETED in 1.89.4 (from March 2023)
// static inline void  PushAllowKeyboardFocus(bool tab_stop) {
// PushItemFlag(ImGuiItemFlags_NoTabStop, !tab_stop); } static inline void
...
// - ImFontAtlasCustomRect::GlyphColored --> if you need to write to this,
// instead you can write to 'font->Glyphs.back()->Colored' after calling
// AddCustomRectFontGlyph() We could make ImTextureRect an union to use old
// names, but 1) this would be confusing 2) the fix is easy 3)
// ImFontAtlasCustomRect was always a rather esoteric api.
typedef ImFontAtlasRect ImFontAtlasCustomRect;
/*struct ImFontAtlasCustomRect
{
	unsigned short  X, Y;           // Output   // Packed position in Atlas
	unsigned short  Width, Height;  // Input    // [Internal] Desired rectangle
dimension unsigned int    GlyphID:31;     // Input    // [Internal] For custom
font glyphs only (ID < 0x110000) unsigned int    GlyphColored:1; // Input    //
[Internal] For custom font glyphs only: glyph is colored, removed tinting. float
GlyphAdvanceX;  // Input    // [Internal] For custom font glyphs only: glyph
xadvance ImVec2          GlyphOffset;    // Input    // [Internal] For custom
font glyphs only: glyph display offset ImFont*         Font;           // Input
// [Internal] For custom font glyphs only: target font ImFontAtlasCustomRect()
{ X = Y = 0xFFFF; Width = Height = 0; GlyphID = 0; GlyphColored = 0;
GlyphAdvanceX = 0.0f; GlyphOffset = ImVec2(0, 0); Font = NULL; } bool IsPacked()
const           { return X != 0xFFFF; }
};*/
...
// in 1.88 (from April 2022). Exceptionally commented out ahead of obsolescence
...
// #define IM_OFFSETOF(_TYPE,_MEMBER)  offsetof(_TYPE, _MEMBER)  // OBSOLETED
// IN 1.90 (now using C++11 standard version)

#endif	// #ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS

#define IM_ARRAYSIZE \
	IM_COUNTOF	// RENAMED IN 1.92.6: IM_ARRAYSIZE -> IM_COUNTOF
