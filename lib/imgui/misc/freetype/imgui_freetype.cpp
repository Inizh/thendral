// dear imgui: FreeType font builder (used as a replacement for the stb_truetype
// builder) (code)

// Get the latest version at
// https://github.com/ocornut/imgui/tree/master/misc/freetype Original code by
// @vuhdo (Aleksei Skriabin) in 2017, with improvements by @mikesart. Maintained
// since 2019 by @ocornut.

// CHANGELOG
// (minor and older changes stripped away, please see git history for details)
//  2025/06/11: refactored for the new ImFontLoader architecture, and
//  ImGuiBackendFlags_RendererHasTextures support. 2024/10/17: added plutosvg
//  support for SVG Fonts (seems faster/better than lunasvg). Enable by using
//  '#define IMGUI_ENABLE_FREETYPE_PLUTOSVG'. (#7927) 2023/11/13: added support
//  for ImFontConfig::RasterizationDensity field for scaling render density
//  without scaling metrics. 2023/08/01: added support for SVG fonts, enable by
//  using '#define IMGUI_ENABLE_FREETYPE_LUNASVG'. (#6591) 2023/01/04: fixed a
//  packing issue which in some occurrences would prevent large amount of glyphs
//  from being packed correctly. 2021/08/23: fixed crash when FT_Render_Glyph()
//  fails to render a glyph and returns nullptr. 2021/03/05: added
//  ImGuiFreeTypeBuilderFlags_Bitmap to load bitmap glyphs. 2021/03/02: set
//  'atlas->TexPixelsUseColors = true' to help some backends with deciding of a
//  preferred texture format. 2021/01/28: added support for color-layered glyphs
//  via ImGuiFreeTypeBuilderFlags_LoadColor (require Freetype 2.10+).
...
//  consistency with other API. removed ImGuiFreeType::BuildFontAtlas().
...
//  for ImFontConfig::GlyphMinAdvanceX, GlyphMaxAdvanceX. 2018/02/04: moved to
//  main imgui repository (away from http://www.github.com/ocornut/imgui_club)
...
//  ImFontConfig::RasterizerFlags, ImFontConfig::RasterizerMultiply. 2017/08/16:
//  imported from https://github.com/Vuhdo/imgui_freetype into
//  http://www.github.com/ocornut/imgui_club, updated for latest changes in
//  ImFontAtlas, minor tweaks.
...
// FIXME: cfg.OversampleH, OversampleV are not supported, but generally not
// necessary with this rasterizer because Hinting makes everything look better.
...
#error \
	"Cannot enable both IMGUI_ENABLE_FREETYPE_LUNASVG and IMGUI_ENABLE_FREETYPE_PLUTOSVG"
#endif
...
#ifndef __clang__
#pragma GCC diagnostic ignored \
	"-Wsubobject-linkage"  // warning: 'xxxx' has a field 'xxxx' whose type uses
...
static void* (*GImGuiFreeTypeAllocFunc)(size_t size, void* user_data) =
	ImGuiFreeTypeDefaultAllocFunc;
static void (*GImGuiFreeTypeFreeFunc)(void* ptr, void* user_data) =
...
static FT_Error ImGuiLunasvgPortPresetSlot(FT_GlyphSlot slot, FT_Bool cache,
										   FT_Pointer* _state);
#endif
...
#define FT_CEIL(X)      \
	(((X + 63) & -64) / \
	 64)  // From SDL_ttf: Handy routines for converting from fixed point
...
struct ImGui_ImplFreeType_FontSrcData {
	// Initialize from an external data buffer. Doesn't copy data, and you must
	// ensure it stays valid up to this object lifetime.
	bool InitFont(FT_Library ft_library, const ImFontConfig* src,
...
// Stored in ImFontBaked::FontLoaderDatas: pointer to SourcesCount instances of
// this. ALLOCATED BY CORE.
...
bool ImGui_ImplFreeType_FontSrcData::InitFont(
	FT_Library ft_library, const ImFontConfig* src,
	ImGuiFreeTypeLoaderFlags extra_font_loader_flags) {
	FT_Error error = FT_New_Memory_Face(
		ft_library, (const FT_Byte*)src->FontData, (FT_Long)src->FontDataSize,
	...
	UserFlags = (ImGuiFreeTypeLoaderFlags)(src->FontLoaderFlags |
										   extra_font_loader_flags);
...
static const FT_Glyph_Metrics* ImGui_ImplFreeType_LoadGlyph(
	...
	FT_Error error =
		FT_Load_Glyph(src_data->FtFace, glyph_index, src_data->LoadFlags);
	...
	IM_ASSERT(
		slot->format != FT_GLYPH_FORMAT_SVG &&
		"The font contains SVG glyphs, you'll need to enable "
		"IMGUI_ENABLE_FREETYPE_PLUTOSVG or IMGUI_ENABLE_FREETYPE_LUNASVG in "
		"imconfig.h and install required libraries in order to use this font");
#endif
	IM_ASSERT(slot->format == FT_GLYPH_FORMAT_OUTLINE ||
			  slot->format == FT_GLYPH_FORMAT_BITMAP);
#endif	// IMGUI_ENABLE_FREETYPE_LUNASVG

	// Apply convenience transform (this is not picking from real
	// "Bold"/"Italic" fonts! Merely applying FreeType helper transform. Oblique
	// == Slanting)
...
static void ImGui_ImplFreeType_BlitGlyph(const FT_Bitmap* ft_bitmap,
	...
	switch (ft_bitmap->pixel_mode) {
		...
		case FT_PIXEL_MODE_MONO:  // Monochrome image, 1 bit per pixel. The bits
								  // in each byte are ordered from MSB to LSB.
		{
...
#define DE_MULTIPLY(color, alpha)                                           \
	ImMin((ImU32)(255.0f * (float)color / (float)(alpha + FLT_MIN) + 0.5f), \
		  255u)
			for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch)
				for (uint32_t x = 0; x < w; x++) {
					uint8_t r = src[x * 4 + 2], g = src[x * 4 + 1],
							b = src[x * 4], a = src[x * 4 + 3];
					dst[x] = IM_COL32(DE_MULTIPLY(r, a), DE_MULTIPLY(g, a),
									  DE_MULTIPLY(b, a), a);
				}
#undef DE_MULTIPLY
			break;
...
static void* FreeType_Alloc(FT_Memory /*memory*/, long size) {
	return GImGuiFreeTypeAllocFunc((size_t)size,
...
static void* FreeType_Realloc(FT_Memory /*memory*/, long cur_size,
							  long new_size, void* block) {
	// Implement realloc() as we don't ask user to provide it.
	if (block == nullptr)
		return GImGuiFreeTypeAllocFunc((size_t)new_size,
									   GImGuiFreeTypeAllocatorUserData);
	...
	if (new_size > cur_size) {
		void* new_block = GImGuiFreeTypeAllocFunc(
...
static bool ImGui_ImplFreeType_LoaderInit(ImFontAtlas* atlas) {
	...
	// If you don't call FT_Add_Default_Modules() the rest of code may work, but
	// FreeType won't use our custom allocator.
	...
	SVG_RendererHooks hooks = {ImGuiLunasvgPortInit, ImGuiLunasvgPortFree,
	...
	FT_Property_Set(bd->Library, "ot-svg", "svg-hooks",
...
static void ImGui_ImplFreeType_LoaderShutdown(ImFontAtlas* atlas) {
	ImGui_ImplFreeType_Data* bd =
		(ImGui_ImplFreeType_Data*)atlas->FontLoaderData;
...
static bool ImGui_ImplFreeType_FontSrcInit(ImFontAtlas* atlas,
	...
	ImGui_ImplFreeType_FontSrcData* bd_font_data =
		IM_NEW(ImGui_ImplFreeType_FontSrcData);
	IM_ASSERT(src->FontLoaderData == nullptr);
	src->FontLoaderData = bd_font_data;

	if (!bd_font_data->InitFont(
			bd->Library, src,
			(ImGuiFreeTypeLoaderFlags)atlas->FontLoaderFlags)) {
...
static bool ImGui_ImplFreeType_FontBakedInit(ImFontAtlas* atlas,
	...
	// Vuhdo 2017: "I'm not sure how to deal with font sizes properly. As far as
	// I understand, currently ImGui assumes that the 'pixel_height' is a
	// maximum height of an any given glyph, i.e. it's the sum of font's
	// ascender and descender. Seems strange to me. FT_Set_Pixel_Sizes() doesn't
	// seem to get us the same result." (FT_Set_Pixel_Sizes() essentially calls
	...
	if (src->MergeMode == false) {
		...
		baked->Ascent = (float)FT_CEIL(metrics.ascender) *
						scale;	// The pixel extents above the baseline in
								// pixels (typically positive).
		baked->Descent = (float)FT_CEIL(metrics.descender) *
						 scale;	 // The extents below the baseline in pixels
								 // (typically negative).
		// LineSpacing     = (float)FT_CEIL(metrics.height) * scale;         //
		// The baseline-to-baseline distance. Note that it usually is larger
		// than the sum of the ascender and descender taken as absolute values.
		// There is also no guarantee that no glyphs extend above or below
		// subsequent baselines when using this distance. Think of it as a value
		// the designer of the font finds appropriate. LineGap         =
		// (float)FT_CEIL(metrics.height - metrics.ascender + metrics.descender)
		// * scale; // The spacing in pixels between one row's descent and the
		// next row's ascent. MaxAdvanceWidth =
		// (float)FT_CEIL(metrics.max_advance) * scale;    // This field gives
		// the maximum horizontal cursor advance for all glyphs in the font.
	}
	return true;
}

static void ImGui_ImplFreeType_FontBakedDestroy(
	ImFontAtlas* atlas, ImFontConfig* src, ImFontBaked* baked,
	void* loader_data_for_baked_src) {
	IM_UNUSED(atlas);
	IM_UNUSED(baked);
	IM_UNUSED(src);
	...
	bd_baked_data
		->~ImGui_ImplFreeType_FontSrcBakedData();  // ~IM_PLACEMENT_DELETE()
}

static bool ImGui_ImplFreeType_FontBakedLoadGlyph(
	ImFontAtlas* atlas, ImFontConfig* src, ImFontBaked* baked,
	...
	FT_Render_Mode render_mode =
		(bd_font_data->UserFlags & ImGuiFreeTypeLoaderFlags_Monochrome)
			? FT_RENDER_MODE_MONO
...
const ImFontLoader* ImGuiFreeType::GetFontLoader() {
	...
	loader.FontBakedSrcLoaderDataSize =
		sizeof(ImGui_ImplFreeType_FontSrcBakedData);
	return &loader;
}
...
bool ImGuiFreeType::DebugEditFontLoaderFlags(
	unsigned int* p_font_loader_flags) {
	bool edited = false;
	edited |= ImGui::CheckboxFlags("NoHinting", p_font_loader_flags,
								   ImGuiFreeTypeLoaderFlags_NoHinting);
	edited |= ImGui::CheckboxFlags("NoAutoHint", p_font_loader_flags,
	...
	edited |= ImGui::CheckboxFlags("Bold", p_font_loader_flags,
								   ImGuiFreeTypeLoaderFlags_Bold);
	edited |= ImGui::CheckboxFlags("Oblique", p_font_loader_flags,
								   ImGuiFreeTypeLoaderFlags_Oblique);
	edited |= ImGui::CheckboxFlags("Monochrome", p_font_loader_flags,
								   ImGuiFreeTypeLoaderFlags_Monochrome);
	edited |= ImGui::CheckboxFlags("LoadColor", p_font_loader_flags,
								   ImGuiFreeTypeLoaderFlags_LoadColor);
	edited |= ImGui::CheckboxFlags("Bitmap", p_font_loader_flags,
...
// The original code from the demo is licensed under CeCILL-C Free Software
// License Agreement
...
static FT_Error ImGuiLunasvgPortRender(FT_GlyphSlot slot, FT_Pointer* _state) {
	...
	// ImGuiLunasvgPortPresetSlot(), the renderer hook still get called, so just
	// returns the error.
	if (state->err != FT_Err_Ok) return state->err;

	// rows is height, pitch (or stride) equals to width * sizeof(int32)
	lunasvg::Bitmap bitmap((uint8_t*)slot->bitmap.buffer, slot->bitmap.width,
						   slot->bitmap.rows, slot->bitmap.pitch);
#if LUNASVG_VERSION_MAJOR >= 3
	state->svg->render(
		bitmap,
		state->matrix);	 // state->matrix is already scaled and translated
#else
	state->svg->setMatrix(
		state->svg->matrix()
			.identity());  // Reset the svg matrix to the default value
	state->svg->render(
		bitmap,
		state->matrix);	 // state->matrix is already scaled and translated
#endif
...
static FT_Error ImGuiLunasvgPortPresetSlot(FT_GlyphSlot slot, FT_Bool cache,
	...
	// This function is called twice, once in the FT_Load_Glyph() and another
	// right before ImGuiLunasvgPortRender(). If it's the latter, don't do
	// anything because it's // already done in the former.
	...
	double vertBearingY =
		(slot->metrics.vertAdvance / 64.0 - slot->metrics.height / 64.0) / 2.0;
	slot->metrics.width = FT_Pos(IM_ROUND(
		metrics_width *
		64.0));	 // Using IM_ROUND() assume width and height are positive
