...
void ImGui::Image(ImTextureRef tex_ref, const ImVec2& image_size,
	...
	PushStyleVar(
		...
		(border_col.w > 0.0f)
			...
			: 0.0f);  // Preserve legacy behavior where border is always visible
					  // when border_col's Alpha is >0.0f
	...
	if (!is_resizable && new_text_len > avail)
		new_text_len =
			(int)(ImTextFindValidUtf8CodepointEnd(
					  new_text, new_text + new_text_len, new_text + avail) -
	...
	if (c < 0x20) {
		...
		pass |= (c == '\n') &&
				(flags & ImGuiInputTextFlags_Multiline) !=
					0;	// Note that an Enter KEY will emit \r and be ignored
						// (we poll for KEY in InputText() code)
	...
	if ((flags & ImGuiColorEditFlags_NoBorder) == 0) {
		if (g.Style.FrameBorderSize > 0.0f)
			...
			window->DrawList->AddRect(
				...
				rounding);	// Color buttons are often in need of some sort of
							// border // FIXME-DPI
	}
...
bool ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open,
	...
	if (g.DragDropActive &&
		!g.DragDropPayload.IsDataType(
			IMGUI_PAYLOAD_TYPE_WINDOW))	 // FIXME: May be an opt-in property of
										 // the payload to disable this
	...
	if (held && single_floating_window_node && IsMouseDragging(0, 0.0f)) {
		...
		if (can_undock) {
								 ...
								 // g.IO.KeyShift)
			{
				...
				float distance_from_edge_y = ImMax(bb.Min.y - g.IO.MousePos.y,
