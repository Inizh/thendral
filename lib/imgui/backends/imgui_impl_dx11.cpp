...
//  2025-06-11: DirectX11: Added support for
//  ImGuiBackendFlags_RendererHasTextures, for dynamic font atlas. 2025-05-07:
//  DirectX11: Honor draw_data->FramebufferScale to allow for custom backends
...
//  [Docking] Added undocumented ImGui_ImplDX11_SetSwapChainDescs() to configure
//  swap chain creation for secondary viewports. 2025-01-06: DirectX11: Expose
...
static void ImGui_ImplDX11_DestroyTexture(ImTextureData* tex) {
	if (ImGui_ImplDX11_Texture* backend_tex =
			(ImGui_ImplDX11_Texture*)tex->BackendUserData) {
...
void ImGui_ImplDX11_UpdateTexture(ImTextureData* tex) {
	...
	if (tex->Status == ImTextureStatus_WantCreate) {
		...
		bd->pd3dDevice->CreateShaderResourceView(
			backend_tex->pTexture, &srvDesc, &backend_tex->pTextureView);
		...
		// region.
		ImGui_ImplDX11_Texture* backend_tex =
			(ImGui_ImplDX11_Texture*)tex->BackendUserData;
		IM_ASSERT(backend_tex->pTextureView ==
				  (ID3D11ShaderResourceView*)(intptr_t)tex->TexID);
		for (ImTextureRect& r : tex->Updates) {
			...
			bd->pd3dDeviceContext->UpdateSubresource(
				backend_tex->pTexture, 0, &box, tex->GetPixelsAt(r.x, r.y),
				(UINT)tex->GetPitch(), 0);
		}
		tex->SetStatus(ImTextureStatus_OK);
	}
	...
	// Create the vertex shader
	{
		...
		D3D11_INPUT_ELEMENT_DESC local_layout[] = {
			{"POSITION", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
			 (UINT)offsetof(ImDrawVert, pos), D3D11_INPUT_PER_VERTEX_DATA, 0},
			{"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
			 (UINT)offsetof(ImDrawVert, uv), D3D11_INPUT_PER_VERTEX_DATA, 0},
			{"COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0,
			 (UINT)offsetof(ImDrawVert, col), D3D11_INPUT_PER_VERTEX_DATA, 0},
		};
...
// Multi-Viewports: configure templates used when creating swapchains for
// secondary viewports. Will try them in order. This is intentionally not
// declared in the .h file yet, so you will need to copy this declaration:
void ImGui_ImplDX11_SetSwapChainDescs(
	const DXGI_SWAP_CHAIN_DESC* desc_templates, int desc_templates_count);
void ImGui_ImplDX11_SetSwapChainDescs(
	const DXGI_SWAP_CHAIN_DESC* desc_templates, int desc_templates_count) {
	ImGui_ImplDX11_Data* bd = ImGui_ImplDX11_GetBackendData();
...
static void ImGui_ImplDX11_CreateWindow(ImGuiViewport* viewport) {
	...
	for (const DXGI_SWAP_CHAIN_DESC& sd_template :
		...
		IM_ASSERT(sd_template.BufferDesc.Width == 0 &&
