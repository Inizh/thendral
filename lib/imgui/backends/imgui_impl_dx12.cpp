...
//  texture identifier. Read the FAQ about ImTextureID/ImTextureRef! [X]
//  Renderer: Large meshes support (64k+ vertices) even with 16-bit indices
//  (ImGuiBackendFlags_RendererHasVtxOffset). [X] Renderer: Texture updates
//  support for dynamic font atlas (ImGuiBackendFlags_RendererHasTextures). [X]
...
//      FIXME: The transition from removing a viewport and moving the window in
...
//  ImGuiPlatformIO interface. 2025-10-23: [Docking] DirectX12: Fixed an issue
//  in synchronization logic improving rendering throughput for secondary
//  viewports. (#8961, #9025) 2025-10-11: DirectX12: Reuse texture upload buffer
//  and grow it only when necessary. (#9002) 2025-09-29: DirectX12: Rework
//  synchronization logic. (#8961) 2025-09-29: DirectX12: Enable swapchain
//  tearing to eliminate viewports framerate throttling. (#8965) 2025-09-29:
//  DirectX12: Reuse a command list and allocator for texture uploads instead of
//  recreating them each time. (#8963) 2025-09-18: Call
//  platform_io.ClearRendererHandlers() on shutdown. 2025-06-19: Fixed build on
//  MinGW. (#8702, #4594) 2025-06-11: DirectX12: Added support for
//  ImGuiBackendFlags_RendererHasTextures, for dynamic font atlas. 2025-05-07:
//  DirectX12: Honor draw_data->FramebufferScale to allow for custom backends
//  and experiment using it (consistently with other renderer backends, even
//  though in normal condition it is not set under Windows). 2025-02-24:
//  DirectX12: Fixed an issue where ImGui_ImplDX12_Init() signature change from
//  2024-11-15 combined with change from 2025-01-15 made legacy
//  ImGui_ImplDX12_Init() crash. (#8429) 2025-01-15: DirectX12: Texture upload
//  use the command queue provided in ImGui_ImplDX12_InitInfo instead of
//  creating its own. 2024-12-09: DirectX12: Let user specifies the
//  DepthStencilView format by setting ImGui_ImplDX12_InitInfo::DSVFormat.
//  2024-11-15: DirectX12: *BREAKING CHANGE* Changed ImGui_ImplDX12_Init()
//  signature to take a ImGui_ImplDX12_InitInfo struct. Legacy
//  ImGui_ImplDX12_Init() signature is still supported (will obsolete).
//  2024-11-15: DirectX12: *BREAKING CHANGE* User is now required to pass
//  function pointers to allocate/free SRV Descriptors. We provide convenience
//  legacy fields to pass a single descriptor, matching the old API, but
//  upcoming features will want multiple. 2024-10-23: DirectX12: Unmap() call
//  specify written range. The range is informational and may be used by debug
//  tools. 2024-10-07: DirectX12: Changed default texture sampler to Clamp
//  instead of Repeat/Wrap. 2024-10-07: DirectX12: Expose selected render state
//  in ImGui_ImplDX12_RenderState, which you can access in 'void*
//  platform_io.Renderer_RenderState' during draw callbacks. 2024-10-07:
//  DirectX12: Compiling with '#define ImTextureID=ImU64' is unnecessary now
//  that dear imgui defaults ImTextureID to u64 instead of void*. 2022-10-11:
//  Using 'nullptr' instead of 'NULL' as per our switch to C++11. 2021-06-29:
//  Reorganized backend to pull data from a single structure to facilitate usage
//  with multiple-contexts (all g_XXXX access changed to bd->XXXX). 2021-05-19:
//  DirectX12: Replaced direct access to ImDrawCmd::TextureId with a call to
//  ImDrawCmd::GetTexID(). (will become a requirement) 2021-02-18: DirectX12:
//  Change blending equation to preserve alpha in output buffer. 2021-01-11:
//  DirectX12: Improve Windows 7 compatibility (for D3D12On7) by loading
//  d3d12.dll dynamically. 2020-09-16: DirectX12: Avoid rendering calls with
//  zero-sized scissor rectangle since it generates a validation layer warning.
//  2020-09-08: DirectX12: Clarified support for building on 32-bit systems by
//  redefining ImTextureID. 2019-10-18: DirectX12: *BREAKING CHANGE* Added extra
//  ID3D12DescriptorHeap parameter to ImGui_ImplDX12_Init() function.
//  2019-05-29: DirectX12: Added support for large mesh (64K+ vertices), enable
...
typedef decltype(D3D12SerializeRootSignature)*
...
// ImGuiViewport to easily retrieve our backend data. Main viewport created by
// application will only use the Resources field. Secondary viewports created by
// this backend will use all the fields (including Window fields),
struct ImGui_ImplDX12_ViewportData {
	...
	ImGui_ImplDX12_ViewportData(UINT num_frames_in_flight) {
		...
		FrameIndex = 0;
		FrameRenderBuffers =
			new ImGui_ImplDX12_RenderBuffers[NumFramesInFlight];
	...
	~ImGui_ImplDX12_ViewportData() {
		...
		for (UINT i = 0; i < NumFramesInFlight; ++i) {
			IM_ASSERT(FrameCtx[i].CommandAllocator == nullptr &&
					  FrameCtx[i].RenderTarget == nullptr);
			IM_ASSERT(FrameRenderBuffers[i].IndexBuffer == nullptr &&
					  FrameRenderBuffers[i].VertexBuffer == nullptr);
		}
...
// FIXME-WIP: Allow user to forward declare those two, for until we come up with
// a backend agnostic API to do this. (#9173)
void ImGui_ImplDX12_SetupSamplerLinear(ID3D12GraphicsCommandList* command_list);
void ImGui_ImplDX12_SetupSamplerNearest(
	ID3D12GraphicsCommandList* command_list);

// Functions
void ImGui_ImplDX12_SetupSamplerLinear(
	ID3D12GraphicsCommandList* command_list) {
...
static void ImGui_ImplDX12_SetupRenderState(
	ImDrawData* draw_data, ID3D12GraphicsCommandList* command_list,
	...
	command_list->SetGraphicsRoot32BitConstants(0, 16, &vertex_constant_buffer,
												0);
...
void ImGui_ImplDX12_RenderDrawData(ImDrawData* draw_data,
	...
	ImGui_ImplDX12_RenderBuffers* fr =
		&vd->FrameRenderBuffers[vd->FrameIndex % bd->numFramesInFlight];
	vd->FrameIndex++;
		...
		for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
			...
			if (pcmd->UserCallback != nullptr) {
				...
				if (pcmd->UserCallback == ImDrawCallback_ResetRenderState)
					ImGui_ImplDX12_SetupRenderState(draw_data, command_list,
													fr);
				else
...
static void ImGui_ImplDX12_DestroyTexture(ImTextureData* tex) {
	if (ImGui_ImplDX12_Texture* backend_tex =
			(ImGui_ImplDX12_Texture*)tex->BackendUserData) {
...
void ImGui_ImplDX12_UpdateTexture(ImTextureData* tex) {
	...
	if (tex->Status == ImTextureStatus_WantCreate) {
		...
		bd->InitInfo.SrvDescriptorAllocFn(
			...
			&backend_tex
				 ->hFontSrvGpuDescHandle);	// Allocate a desctriptor handle
		...
		bd->pd3dDevice->CreateShaderResourceView(
			pTexture, &srvDesc, backend_tex->hFontSrvCpuDescHandle);
		SafeRelease(backend_tex->pTextureResource);
		...
		need_barrier_before_copy =
			false;	// Because this is a newly-created texture it will be in
					// D3D12_RESOURCE_STATE_COMMON and thus we don't need a
					// barrier
		// We don't set tex->Status to ImTextureStatus_OK to let the code
		// fallthrough below.
	}

	if (tex->Status == ImTextureStatus_WantCreate ||
		tex->Status == ImTextureStatus_WantUpdates) {
		ImGui_ImplDX12_Texture* backend_tex =
			(ImGui_ImplDX12_Texture*)tex->BackendUserData;
		IM_ASSERT(tex->Format == ImTextureFormat_RGBA32);

		// We could use the smaller rect on _WantCreate but using the full rect
		// allows us to clear the texture.
		// FIXME-OPT: Uploading single box even when using
		// ImTextureStatus_WantUpdates. Could use tex->Updates[]
		// - Copy all blocks contiguously in upload buffer.
		// - Barrier before copy, submit all CopyTextureRegion(), barrier after
		// copy.
		const int upload_x =
		...
		// upload individual regions.
		UINT upload_pitch_src = upload_w * tex->BytesPerPixel;
		UINT upload_pitch_dst =
			(upload_pitch_src + D3D12_TEXTURE_DATA_PITCH_ALIGNMENT - 1u) &
			~(D3D12_TEXTURE_DATA_PITCH_ALIGNMENT - 1u);
		UINT upload_size = upload_pitch_dst * upload_h;

		if (bd->pTexUploadBuffer == nullptr ||
			upload_size > bd->pTexUploadBufferSize) {
			...
			hr = bd->pTexUploadBuffer->Map(0, &range,
		...
		for (int y = 0; y < upload_h; y++)
			memcpy((void*)((uintptr_t)bd->pTexUploadBufferMapped +
						   y * upload_pitch_dst),
		...
		cmdList->CopyTextureRegion(&dstLocation, upload_x, upload_y, 0,
								   &srcLocation, nullptr);
		...
		// - To remove this may need to create NumFramesInFlight x
		// ImGui_ImplDX12_FrameContext in backend data (mimick docking version)
		// - Store per-frame in flight: upload buffer?
		// - Where do cmdList and cmdAlloc fit?
		bd->Fence->SetEventOnCompletion(bd->FenceLastSignaledValue,
										bd->FenceEvent);
...
bool ImGui_ImplDX12_CreateDeviceObjects() {
	...
	if (bd->pPipelineStateLinear) ImGui_ImplDX12_InvalidateDeviceObjects();

	HRESULT hr = ::CreateDXGIFactory1(IID_PPV_ARGS(&bd->pdxgiFactory));
	IM_ASSERT(hr == S_OK);

	BOOL allow_tearing = FALSE;
	bd->pdxgiFactory->CheckFeatureSupport(DXGI_FEATURE_PRESENT_ALLOW_TEARING,
										  &allow_tearing,
	...
	// Create the root signature
	{
		...
		_PFN_D3D12_SERIALIZE_ROOT_SIGNATURE D3D12SerializeRootSignatureFn =
			(_PFN_D3D12_SERIALIZE_ROOT_SIGNATURE)(void*)::GetProcAddress(
		...
		bd->pd3dDevice->CreateRootSignature(
			0, blob->GetBufferPointer(), blob->GetBufferSize(),
			IID_PPV_ARGS(&bd->pRootSignatureLinear));
		blob->Release();
		...
		bd->pd3dDevice->CreateRootSignature(
			0, blob->GetBufferPointer(), blob->GetBufferSize(),
			IID_PPV_ARGS(&bd->pRootSignatureNearest));
		blob->Release();
	}
	...
	//  code and assign them to psoDesc.VS/PS [preferred solution] 2) use code
	//  to detect any version of the DLL and grab a pointer to D3DCompile from
	//  the DLL.
	...
	HRESULT result_pipeline_state = bd->pd3dDevice->CreateGraphicsPipelineState(
		&psoDesc, IID_PPV_ARGS(&bd->pPipelineStateLinear));
	...
	result_pipeline_state = bd->pd3dDevice->CreateGraphicsPipelineState(
		&psoDesc, IID_PPV_ARGS(&bd->pPipelineStateNearest));
	vertexShaderBlob->Release();
	pixelShaderBlob->Release();
	if (result_pipeline_state != S_OK) return false;

	// Create command allocator and command list for
	// ImGui_ImplDX12_UpdateTexture()
	hr = bd->pd3dDevice->CreateCommandAllocator(
		D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&bd->pTexCmdAllocator));
	IM_ASSERT(SUCCEEDED(hr));
	hr = bd->pd3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT,
										   bd->pTexCmdAllocator, nullptr,
										   IID_PPV_ARGS(&bd->pTexCmdList));
	IM_ASSERT(SUCCEEDED(hr));
...
static void ImGui_ImplDX12_DestroyRenderBuffers(
	ImGui_ImplDX12_RenderBuffers* render_buffers) {
...
static void ImGui_ImplDX12_InitLegacySingleDescriptorMode(
		...
		[](ImGui_ImplDX12_InitInfo*,
		   ...
		   D3D12_GPU_DESCRIPTOR_HANDLE* out_gpu_handle) {
			...
			IM_ASSERT(bd->LegacySingleDescriptorUsed == false &&
					  "Only 1 simultaneous texture allowed with legacy "
					  "ImGui_ImplDX12_Init() signature!");
...
bool ImGui_ImplDX12_Init(ImGui_ImplDX12_InitInfo* init_info) {
	...
	// Since this is created and managed by the application, we will only use
	// the ->Resources[] fields.
	ImGuiViewport* main_viewport = ImGui::GetMainViewport();
	main_viewport->RendererUserData =
		IM_NEW(ImGui_ImplDX12_ViewportData)(bd->numFramesInFlight);
...
bool ImGui_ImplDX12_Init(ID3D12Device* device, int num_frames_in_flight,
	...
	ImGuiIO& io = ImGui::GetIO();
	io.BackendFlags &=
		~ImGuiBackendFlags_RendererHasTextures;	 // Using legacy
												 // ImGui_ImplDX12_Init() call
												 // with 1 SRV descriptor we
...
void ImGui_ImplDX12_Shutdown() {
	...
	// Manually delete main viewport render resources in-case we haven't
	// initialized for viewports
	ImGuiViewport* main_viewport = ImGui::GetMainViewport();
	if (ImGui_ImplDX12_ViewportData* vd =
			(ImGui_ImplDX12_ViewportData*)main_viewport->RendererUserData) {
		// We could just call ImGui_ImplDX12_DestroyWindow(main_viewport) as a
		// convenience but that would be misleading since we only use
		// data->Resources[]
...
static void ImGui_ImplDX12_CreateWindow(ImGuiViewport* viewport) {
	...
	// be a higher-level handle (e.g. GLFWWindow*, SDL's WindowID). Some
	// backends will leave PlatformHandleRaw == 0, in which case we assume
	...
	if (bd->tearingSupport) sd1.Flags |= DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING;

	IDXGISwapChain1* swap_chain = nullptr;
	res = bd->pdxgiFactory->CreateSwapChainForHwnd(
		vd->CommandQueue, hwnd, &sd1, nullptr, nullptr, &swap_chain);
	IM_ASSERT(res == S_OK);
	res = bd->pdxgiFactory->MakeWindowAssociation(
		hwnd, DXGI_MWA_NO_ALT_ENTER |
				  DXGI_MWA_NO_WINDOW_CHANGES);	// Disable e.g. Alt+Enter
	IM_ASSERT(res == S_OK);
	...
	if (vd->SwapChain) {
		...
		D3D12_CPU_DESCRIPTOR_HANDLE rtv_handle =
			vd->RtvDescHeap->GetCPUDescriptorHandleForHeapStart();
		for (UINT i = 0; i < bd->numFramesInFlight; i++) {
		...
		for (UINT i = 0; i < bd->numFramesInFlight; i++) {
			...
			bd->pd3dDevice->CreateRenderTargetView(
				back_buffer, nullptr,
				vd->FrameCtx[i].RenderTargetCpuDescriptors);
			vd->FrameCtx[i].RenderTarget = back_buffer;
		}

		hr = vd->SwapChain->SetMaximumFrameLatency(bd->numFramesInFlight);
		IM_ASSERT(hr == S_OK);
		vd->SwapChainWaitableObject =
			vd->SwapChain->GetFrameLatencyWaitableObject();
	}
...
static void ImGui_WaitForPendingOperations(ImGui_ImplDX12_ViewportData* vd) {
	ImGui_ImplDX12_Data* bd = ImGui_ImplDX12_GetBackendData();
	HRESULT hr =
		vd->CommandQueue->Signal(bd->Fence, ++bd->FenceLastSignaledValue);
	IM_ASSERT(hr == S_OK);

	hr = bd->Fence->SetEventOnCompletion(bd->FenceLastSignaledValue,
										 bd->FenceEvent);
	IM_ASSERT(hr == S_OK);
	::WaitForSingleObject(bd->FenceEvent, INFINITE);
}

static ImGui_ImplDX12_FrameContext* ImGui_WaitForNextFrameContext(
	ImGui_ImplDX12_ViewportData* vd) {
	ImGui_ImplDX12_Data* bd = ImGui_ImplDX12_GetBackendData();
	ImGui_ImplDX12_FrameContext* frame_context =
		&vd->FrameCtx[vd->FrameIndex % vd->NumFramesInFlight];
	if (bd->Fence->GetCompletedValue() < frame_context->FenceValue) {
		HRESULT hr = bd->Fence->SetEventOnCompletion(frame_context->FenceValue,
													 bd->FenceEvent);
		IM_ASSERT(hr == S_OK);
		HANDLE waitableObjects[] = {vd->SwapChainWaitableObject,
									bd->FenceEvent};
...
static void ImGui_ImplDX12_DestroyWindow(ImGuiViewport* viewport) {
	...
	ImGui_ImplDX12_Data* bd = ImGui_ImplDX12_GetBackendData();
	if (ImGui_ImplDX12_ViewportData* vd =
			(ImGui_ImplDX12_ViewportData*)viewport->RendererUserData) {
...
static void ImGui_ImplDX12_SetWindowSize(ImGuiViewport* viewport, ImVec2 size) {
	...
	if (vd->SwapChain) {
		...
		vd->SwapChain->ResizeBuffers(0, (UINT)size.x, (UINT)size.y, desc.Format,
									 desc.Flags);
		for (UINT i = 0; i < bd->numFramesInFlight; i++) {
			vd->SwapChain->GetBuffer(i, IID_PPV_ARGS(&back_buffer));
			bd->pd3dDevice->CreateRenderTargetView(
...
static void ImGui_ImplDX12_RenderWindow(ImGuiViewport* viewport, void*) {
	...
	ImGui_ImplDX12_FrameContext* frame_context =
		ImGui_WaitForNextFrameContext(vd);
	...
	cmd_list->OMSetRenderTargets(
		1, &vd->FrameCtx[back_buffer_idx].RenderTargetCpuDescriptors, FALSE,
		nullptr);
	if (!(viewport->Flags & ImGuiViewportFlags_NoRendererClear))
		cmd_list->ClearRenderTargetView(
			vd->FrameCtx[back_buffer_idx].RenderTargetCpuDescriptors,
			(const float*)&clear_color, 0, nullptr);
	...
	vd->CommandQueue->ExecuteCommandLists(1,
...
static void ImGui_ImplDX12_SwapBuffers(ImGuiViewport* viewport, void*) {
	...
	vd->SwapChain->Present(0,
						   bd->tearingSupport ? DXGI_PRESENT_ALLOW_TEARING : 0);
}
