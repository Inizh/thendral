...
//  [!] Renderer: User texture binding. Use 'VkDescriptorSet' as texture
//  identifier. Call ImGui_ImplVulkan_AddTexture() to register one. Read the FAQ
//  about ImTextureID/ImTextureRef + https://github.com/ocornut/imgui/pull/914
//  for discussions. [X] Renderer: Large meshes support (64k+ vertices) even
//  with 16-bit indices (ImGuiBackendFlags_RendererHasVtxOffset). [X] Renderer:
//  Texture updates support for dynamic font atlas
//  (ImGuiBackendFlags_RendererHasTextures). [X] Renderer: Expose selected
//  render state for draw callbacks to use. Access in
//  '(ImGui_ImplXXXX_RenderState*)GetPlatformIO().Renderer_RenderState'. [x]
//  Renderer: Multi-viewport / platform windows. With issues (flickering when
...
//  ImGuiPlatformIO interface. 2025-09-26: [Helpers] *BREAKING CHANGE*: Vulkan:
//  Helper ImGui_ImplVulkanH_DestroyWindow() does not call
//  vkDestroySurfaceKHR(): as surface is created by caller of
//  ImGui_ImplVulkanH_CreateOrResizeWindow(), it is more consistent that we
//  don't destroy it. (#9163) 2026-01-05: [Helpers] *BREAKING CHANGE*: Vulkan:
//  Helper for creating render pass uses
//  ImGui_ImplVulkanH_Window::AttachmentDesc to create render pass. Removed
//  ClearEnabled. (#9152) 2025-11-24: [Helpers] Vulkan: Helper for creating a
//  swap-chain (used by examples and multi-viewports) selects
//  VkSwapchainCreateInfoKHR's compositeAlpha based on
//  cap.supportedCompositeAlpha. (#8784) 2025-11-13: [Docking] Handle viewport
//  surface creation failure without crashing. (#9068) 2025-10-15: Vulkan: Added
//  IMGUI_IMPL_VULKAN_VOLK_FILENAME to configure path to volk.h header. (#9008)
//  2025-09-26: *BREAKING CHANGE*: moved some fields in
//  ImGui_ImplVulkan_InitInfo: init_info.RenderPass -->
//  init_info.PipelineInfoMain.RenderPass, init_info.Subpass -->
//  init_info.PipelineInfoMain.Subpass, init_info.MSAASamples -->
//  init_info.PipelineInfoMain.MSAASamples,
//  init_info.PipelineRenderingCreateInfo -->
//  init_info.PipelineInfoMain.PipelineRenderingCreateInfo. 2025-09-26:
//  *BREAKING CHANGE*: renamed ImGui_ImplVulkan_MainPipelineCreateInfo to
//  ImGui_ImplVulkan_PipelineInfo. Introduced very recently so shouldn't affect
//  many users. 2025-09-26: [Helpers] *BREAKING CHANGE*: Helper
//  ImGui_ImplVulkanH_CreateOrResizeWindow() added a VkImageUsageFlags
//  image_usage` argument, default to VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT if 0.
//  2025-09-26: Vulkan: Added a way to customize shaders by filling
//  ImGui_ImplVulkan_InitInfo::CustomShaderVertCreateInfo/CustomShaderFragCreateInfo.
//  (#8585) 2025-09-22: [Docking] Added
//  ImGui_ImplVulkanH_GetWindowDataFromViewport() accessor/helper. (#8946,
//  #8940) 2025-09-18: Call platform_io.ClearRendererHandlers() on shutdown.
//  2025-09-04: Vulkan: Added ImGui_ImplVulkan_CreateMainPipeline(). (#8110,
//  #8111) 2025-07-27: Vulkan: Fixed texture update corruption introduced on
//  2025-06-11. (#8801, #8755, #8840) 2025-07-07: Vulkan: Fixed texture
//  synchronization issue introduced on 2025-06-11. (#8772) 2025-06-27: Vulkan:
//  Fixed validation errors during texture upload/update by aligning upload size
//  to 'nonCoherentAtomSize'. (#8743, #8744) 2025-06-11: Vulkan: Added support
//  for ImGuiBackendFlags_RendererHasTextures, for dynamic font atlas. Removed
//  ImGui_ImplVulkan_CreateFontsTexture() and
//  ImGui_ImplVulkan_DestroyFontsTexture(). 2025-05-07: Vulkan: Fixed validation
//  errors during window detach in multi-viewport mode. (#8600, #8176)
//  2025-05-07: Vulkan: Load dynamic rendering functions using
//  vkGetDeviceProcAddr() + try both non-KHR and KHR versions. (#8600, #8326,
//  #8365) 2025-04-07: Vulkan: Deep-copy
//  ImGui_ImplVulkan_InitInfo::PipelineRenderingCreateInfo's
//  pColorAttachmentFormats buffer when set, in order to reduce common
//  user-error of specifying a pointer to data that gets out of scope. (#8282)
//  2025-02-14: *BREAKING CHANGE*: Added uint32_t api_version to
//  ImGui_ImplVulkan_LoadFunctions(). 2025-02-13: Vulkan: Added ApiVersion field
//  in ImGui_ImplVulkan_InitInfo. Default to header version if unspecified.
//  Dynamic rendering path loads "vkCmdBeginRendering/vkCmdEndRendering"
//  (without -KHR suffix) on API 1.3. (#8326) 2025-01-09: Vulkan: Added
//  IMGUI_IMPL_VULKAN_MINIMUM_IMAGE_SAMPLER_POOL_SIZE to clarify how many image
//  sampler descriptors are expected to be available in descriptor pool. (#6642)
//  2025-01-06: [Helpers] Vulkan: Added more ImGui_ImplVulkanH_XXXX helper
//  functions to simplify our examples. 2024-12-11: [Helpers] Vulkan: Fixed
//  setting VkSwapchainCreateInfoKHR::preTransform for platforms not supporting
//  VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR. (#8222) 2024-11-27: Vulkan: Make
//  user-provided descriptor pool optional. As a convenience, when setting
//  init_info->DescriptorPoolSize the backend will create one itself. (#8172,
//  #4867) 2024-10-07: Vulkan: Changed default texture sampler to Clamp instead
//  of Repeat/Wrap. 2024-10-07: Vulkan: Expose selected render state in
//  ImGui_ImplVulkan_RenderState, which you can access in 'void*
//  platform_io.Renderer_RenderState' during draw callbacks. 2024-10-07: Vulkan:
//  Compiling with '#define ImTextureID=ImU64' is unnecessary now that dear
//  imgui defaults ImTextureID to u64 instead of void*. 2024-04-19: Vulkan:
//  Added convenience support for Volk via IMGUI_IMPL_VULKAN_USE_VOLK define
//  (you can also use IMGUI_IMPL_VULKAN_NO_PROTOTYPES + wrap Volk via
...
//  filling PipelineRenderingCreateInfo structure. 2024-01-19: [Helpers] Vulkan:
//  Fixed vkAcquireNextImageKHR() validation errors in VulkanSDK 1.3.275 by
//  allocating one extra semaphore than in-flight frames. (#7236) 2024-01-11:
//  Vulkan: Fixed vkMapMemory() calls unnecessarily using full buffer size
//  (#3957). Fixed MinAllocationSize handing (#7189). 2024-01-03: Vulkan: Added
...
//              unnecessary as we create and destroy those objects in the
...
//  ImGui_ImplVulkan_RemoveTexture() for api symmetry. (#914, #5738).
//  2022-01-20: Vulkan: Added support for ImTextureID as VkDescriptorSet. User
//  need to call ImGui_ImplVulkan_AddTexture(). Building for 32-bit targets
//  requires '#define ImTextureID ImU64'. (#914). 2021-10-15: Vulkan: Call
//  vkCmdSetScissor() at the end of render a full-viewport to reduce likelihood
//  of issues with people using VK_DYNAMIC_STATE_SCISSOR in their app without
//  calling vkCmdSetScissor() explicitly every frame. 2021-06-29: Reorganized
//  backend to pull data from a single structure to facilitate usage with
//  multiple-contexts (all g_XXXX access changed to bd->XXXX). 2021-03-22:
//  Vulkan: Fix mapped memory validation error when buffer sizes are not
//  multiple of VkPhysicalDeviceLimits::nonCoherentAtomSize. 2021-02-18: Vulkan:
//  Change blending equation to preserve alpha in output buffer. 2021-01-27:
//  Vulkan: Added support for custom function load and
...
//  during VkPipeline creation. 2020-09-07: Vulkan: Added VkPipeline parameter
//  to ImGui_ImplVulkan_RenderDrawData (default to one passed to
...
//  ImGui_ImplVulkan_SetMinImageCount(). 2019-04-04: [Helpers] Vulkan: Added
//  VkInstance argument to ImGui_ImplVulkanH_CreateWindow() optional helper.
//  2019-04-04: Vulkan: Avoid passing negative coordinates to vkCmdSetScissor,
//  which debug validation layers do not like. 2019-04-01: Vulkan: Support for
//  32-bit index buffer (#define ImDrawIdx unsigned int). 2019-02-16: Vulkan:
//  Viewport and clipping rectangles correctly using draw_data->FramebufferScale
//  to allow retina display. 2018-11-30: Misc: Setting up io.BackendRendererName
//  so it can be displayed in the About Window. 2018-08-25: [Helpers] Vulkan:
//  Fixed mishandled VkSurfaceCapabilitiesKHR::maxImageCount=0 case. 2018-06-22:
//  Inverted the parameters to ImGui_ImplVulkan_RenderDrawData() to be
//  consistent with other backends. 2018-06-08: Misc: Extracted
//  imgui_impl_vulkan.cpp/.h away from the old combined GLFW+Vulkan example.
//  2018-06-08: Vulkan: Use draw_data->DisplayPos and draw_data->DisplaySize to
//  setup projection matrix and clipping rectangle. 2018-03-03: [Helpers]
//  Vulkan: Various refactor, created a couple of ImGui_ImplVulkanH_XXX helper
//  that the example can use and that viewport support will use. 2018-03-01:
//  Vulkan: Renamed ImGui_ImplVulkan_Init_Info to ImGui_ImplVulkan_InitInfo and
//  fields to match more closely Vulkan terminology. 2018-02-16: Misc: Obsoleted
//  the io.RenderDrawListsFn callback, ImGui_ImplVulkan_Render() calls
...
void ImGui_ImplVulkanH_CreateWindowSwapChain(
	...
	int h, uint32_t min_image_count, VkImageUsageFlags image_usage);
...
struct ImGui_ImplVulkan_ViewportData {
	...
	ImGui_ImplVulkan_WindowRenderBuffers
		RenderBuffers;	// Used by all viewports
	bool WindowOwned;
...
struct ImGui_ImplVulkan_Data {
	...
	VkPipeline PipelineForViewports;  // pipeline for secondary viewports
									  // (created by backend)
	VkShaderModule ShaderModuleVert;
	VkShaderModule ShaderModuleFrag;
	VkDescriptorPool DescriptorPool;
	ImVector<VkFormat>
		PipelineRenderingCreateInfoColorAttachmentFormats;	// Deep copy of
															// format array
...
static void CreateOrResizeBuffer(VkBuffer& buffer,
	...
	err =
		vkAllocateMemory(v->Device, &alloc_info, v->Allocator, &buffer_memory);
	check_vk_result(err);
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data,
	...
	// element with an OK status, aka nothing to do. (This almost always points
	// to ImGui::GetPlatformIO().Textures[] but is part of ImDrawData to allow
	...
	if (wrb->FrameRenderBuffers.Size == 0) {
		...
		memset((void*)wrb->FrameRenderBuffers.Data, 0,
	...
	// Note: at this point both vkCmdSetViewport() and vkCmdSetScissor() have
	// been called. Our last values will leak into user/application rendering
	// IF:
	// - Your app uses a pipeline with VK_DYNAMIC_STATE_VIEWPORT or
	// VK_DYNAMIC_STATE_SCISSOR dynamic state
	// - And you forgot to call vkCmdSetViewport() and vkCmdSetScissor()
	// yourself to explicitly set that state. If you use
	// VK_DYNAMIC_STATE_VIEWPORT or VK_DYNAMIC_STATE_SCISSOR you are responsible
	// for setting the values before rendering. In theory we should aim to
	// backup/restore those values but I am not sure this is possible. We
	// perform a call to vkCmdSetScissor() to set back a full viewport which is
	// likely to fix things for 99% users but technically this is not perfect.
	// (See github #4644)
...
static void ImGui_ImplVulkan_DestroyTexture(ImTextureData* tex) {
	if (ImGui_ImplVulkan_Texture* backend_tex =
			(ImGui_ImplVulkan_Texture*)tex->BackendUserData) {
		...
		// Clear identifiers and mark as destroyed (in order to allow e.g.
		// calling InvalidateDeviceObjects while running)
...
void ImGui_ImplVulkan_UpdateTexture(ImTextureData* tex) {
	...
	if (tex->Status == ImTextureStatus_WantCreate) {
		...
		ImGui_ImplVulkan_Texture* backend_tex =
			IM_NEW(ImGui_ImplVulkan_Texture)();
		...
		// Create the Image:
		{
			...
			err = vkAllocateMemory(v->Device, &alloc_info, v->Allocator,
								   &backend_tex->Memory);
			check_vk_result(err);
			err = vkBindImageMemory(v->Device, backend_tex->Image,
									backend_tex->Memory, 0);
			check_vk_result(err);
		}
		...
		// Create the Image View:
		{
			...
			err = vkCreateImageView(v->Device, &info, v->Allocator,
		...
		backend_tex->DescriptorSet = ImGui_ImplVulkan_AddTexture(
			bd->TexSamplerLinear, backend_tex->ImageView,
	...
	if (tex->Status == ImTextureStatus_WantCreate ||
		...
		// Update full texture or selected blocks. We only ever write to
		// textures regions which have never been used before! This backend
		// choose to use tex->UpdateRect but you can use tex->Updates[] to
		// upload individual regions. We could use the smaller rect on
		// _WantCreate but using the full rect allows us to clear the texture.
		const int upload_x =
		...
		VkDeviceSize upload_size =
			AlignBufferSize(upload_h * upload_pitch, bd->NonCoherentAtomSize);
		{
			VkBufferCreateInfo buffer_info = {};
			buffer_info.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
			...
			check_vk_result(err);
			err = vkBindBufferMemory(v->Device, upload_buffer,
		...
		{
			char* map = nullptr;
			err = vkMapMemory(v->Device, upload_buffer_memory, 0, upload_size,
							  0, (void**)(&map));
			check_vk_result(err);
		...
		// Copy to Image:
		{
			...
			vkCmdPipelineBarrier(bd->TexCommandBuffer,
								 ...
								 1, upload_barrier, 1, copy_barrier);
			...
			vkCmdCopyBufferToImage(
				bd->TexCommandBuffer, upload_buffer, backend_tex->Image,
...
static void ImGui_ImplVulkan_CreateShaderModules(
	VkDevice device, const VkAllocationCallbacks* allocator) {
	ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
	ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
	if (bd->ShaderModuleVert == VK_NULL_HANDLE) {
		...
		VkShaderModuleCreateInfo* p_vert_info =
			(v->CustomShaderVertCreateInfo.sType ==
			 VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO)
				? &v->CustomShaderVertCreateInfo
				: &default_vert_info;
		VkResult err = vkCreateShaderModule(device, p_vert_info, allocator,
											&bd->ShaderModuleVert);
		check_vk_result(err);
	}
	if (bd->ShaderModuleFrag == VK_NULL_HANDLE) {
		...
		VkShaderModuleCreateInfo* p_frag_info =
			(v->CustomShaderFragCreateInfo.sType ==
			 VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO)
				? &v->CustomShaderFragCreateInfo
				: &default_frag_info;
		VkResult err = vkCreateShaderModule(device, p_frag_info, allocator,
											&bd->ShaderModuleFrag);
		check_vk_result(err);
	}
}
...
static VkPipeline ImGui_ImplVulkan_CreatePipeline(
	VkDevice device, const VkAllocationCallbacks* allocator,
	VkPipelineCache pipelineCache, const ImGui_ImplVulkan_PipelineInfo* info) {
	ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
	...
	if (bd->VulkanInitInfo.UseDynamicRendering) {
		...
		create_info.renderPass =
			VK_NULL_HANDLE;	 // Just make sure it's actually nullptr.
	}
#endif
	VkPipeline pipeline;
	VkResult err = vkCreateGraphicsPipelines(
		device, pipelineCache, 1, &create_info, allocator, &pipeline);
	check_vk_result(err);
	return pipeline;
}
...
bool ImGui_ImplVulkan_CreateDeviceObjects() {
	...
	if (v->DescriptorPoolSize != 0) {
		IM_ASSERT(v->DescriptorPoolSize >=
				  IMGUI_IMPL_VULKAN_MINIMUM_IMAGE_SAMPLER_POOL_SIZE);
		VkDescriptorPoolSize pool_size = {
			VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, v->DescriptorPoolSize};
		...
		err = vkCreateDescriptorPool(v->Device, &pool_info, v->Allocator,
									 &bd->DescriptorPool);
		check_vk_result(err);
	}
	...
	// Create pipeline
	bool create_main_pipeline =
		(v->PipelineInfoMain.RenderPass != VK_NULL_HANDLE);
#ifdef IMGUI_IMPL_VULKAN_HAS_DYNAMIC_RENDERING
	create_main_pipeline |=
		(v->UseDynamicRendering &&
		 v->PipelineInfoMain.PipelineRenderingCreateInfo.sType ==
			 VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR);
#endif
	...
	if (!bd->TexCommandPool) {
		...
		err = vkCreateCommandPool(v->Device, &info, v->Allocator,
								  &bd->TexCommandPool);
		check_vk_result(err);
	}
...
void ImGui_ImplVulkan_CreateMainPipeline(
	const ImGui_ImplVulkan_PipelineInfo* pipeline_info_in) {
	ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
	ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
	if (bd->Pipeline) {
	...
	if (v->UseDynamicRendering &&
		...
		formats.resize(
			(int)pipeline_rendering_create_info->colorAttachmentCount);
		memcpy(formats.Data,
...
#endif
	bd->Pipeline = ImGui_ImplVulkan_CreatePipeline(
		v->Device, v->Allocator, v->PipelineCache, pipeline_info);
}
...
void ImGui_ImplVulkan_DestroyDeviceObjects() {
	...
	if (bd->TexCommandBuffer) {
		vkFreeCommandBuffers(v->Device, bd->TexCommandPool, 1,
							 &bd->TexCommandBuffer);
...
static void ImGui_ImplVulkan_LoadDynamicRenderingFunctions(
	...
	// - Try loading core (non-KHR) versions first (this will work for
	// Vulkan 1.3+ and the device supports dynamic rendering)
	...
	// - Fallback to KHR versions if core not available (this will work if KHR
	// extension is available and enabled and also the device supports dynamic
	// rendering)
...
// We don't care about other versions than 1.3 for our checks, so don't need to
// make this exhaustive (e.g. with all #ifdef VK_VERSION_1_X checks)
...
bool ImGui_ImplVulkan_LoadFunctions(
	...
	//      ImGui_ImplVulkan_LoadFunctions(VK_API_VERSION_1_3, [](const char*
	//      function_name, void*) { return
...
#ifdef IMGUI_IMPL_VULKAN_HAS_DYNAMIC_RENDERING
	ImGui_ImplVulkan_LoadDynamicRenderingFunctions(api_version, loader_func,
												   user_data);
#endif
#else
...
bool ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info) {
	...
	io.BackendFlags |=
		ImGuiBackendFlags_RendererHasViewports;	 // We can create
	...
	if (info->DescriptorPool !=
		VK_NULL_HANDLE)	 // Either DescriptorPool or DescriptorPoolSize must be
						 // set, not both!
	...
	if (info->UseDynamicRendering)
		IM_ASSERT(info->PipelineInfoMain.RenderPass == VK_NULL_HANDLE &&
				  info->PipelineInfoForViewports.RenderPass == VK_NULL_HANDLE);
	...
	if (!ImGui_ImplVulkan_CreateDeviceObjects())
		IM_ASSERT(
			0 &&
			"ImGui_ImplVulkan_CreateDeviceObjects() failed!");	// <- Can't be
																// hit yet.

	// Our render function expect RendererUserData to be storing the window
	// render buffer we need (for the main viewport we won't use ->Window)
...
VkDescriptorSet ImGui_ImplVulkan_AddTexture(VkSampler sampler,
	...
	VkDescriptorPool pool =
		bd->DescriptorPool ? bd->DescriptorPool : v->DescriptorPool;
...
// about Vulkan, but it is recommended you use your own custom tailored code to
...
VkSurfaceFormatKHR ImGui_ImplVulkanH_SelectSurfaceFormat(
	...
	// default behavior is without setting this bit, there is no need for
	// separate Swapchain image and image view format Additionally several new
	// color spaces were introduced with Vulkan Spec v1.0.40, hence we must make
	// sure that a format with the mostly available color space,
	...
	if (avail_count == 1) {
		...
		for (int request_i = 0; request_i < request_formats_count; request_i++)
			for (uint32_t avail_i = 0; avail_i < avail_count; avail_i++)
				if (avail_format[avail_i].format ==
						request_formats[request_i] &&
...
void ImGui_ImplVulkanH_CreateWindowCommandBuffers(
	...
	for (uint32_t i = 0; i < wd->ImageCount; i++) {
		ImGui_ImplVulkanH_Frame* fd = &wd->Frames[i];
		{
			...
			err =
				vkCreateCommandPool(device, &info, allocator, &fd->CommandPool);
			check_vk_result(err);
		}
		{
			VkCommandBufferAllocateInfo info = {};
...
void ImGui_ImplVulkanH_CreateWindowSwapChain(
	...
	// We don't use ImGui_ImplVulkanH_DestroyWindow() because we want to
	// preserve the old swapchain to create the new one. Destroy old Framebuffer
	for (uint32_t i = 0; i < wd->ImageCount; i++)
		ImGui_ImplVulkanH_DestroyFrame(device, &wd->Frames[i], allocator);
	for (uint32_t i = 0; i < wd->SemaphoreCount; i++)
		ImGui_ImplVulkanH_DestroyFrameSemaphores(
			device, &wd->FrameSemaphores[i], allocator);
	wd->Frames.clear();
	...
	// Create Swapchain
	{
		...
		info.imageSharingMode =
			VK_SHARING_MODE_EXCLUSIVE;	// Assume that graphics family ==
										// present family
		info.preTransform =
		...
		else if (cap.supportedCompositeAlpha &
		...
		memset(wd->FrameSemaphores.Data, 0,
	...
	if (wd->UseDynamicRendering == false) {
		...
		// We do not create a pipeline by default as this is also used by
		// examples' main.cpp, but secondary viewport in multi-viewport mode may
		// want to create one with:
	...
	// Create The Image Views
	{
		...
		VkImageSubresourceRange image_range = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1,
											   0, 1};
		info.subresourceRange = image_range;
		for (uint32_t i = 0; i < wd->ImageCount; i++) {
			ImGui_ImplVulkanH_Frame* fd = &wd->Frames[i];
			info.image = fd->Backbuffer;
			err = vkCreateImageView(device, &info, allocator,
									&fd->BackbufferView);
			check_vk_result(err);
		}
	}
	...
	if (wd->UseDynamicRendering == false) {
		...
		for (uint32_t i = 0; i < wd->ImageCount; i++) {
			...
			err =
				vkCreateFramebuffer(device, &info, allocator, &fd->Framebuffer);
			check_vk_result(err);
		}
	}
}
...
// - 2025/09/26: v1.92.4 added a trailing 'VkImageUsageFlags image_usage'
// parameter which is usually VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT.
void ImGui_ImplVulkanH_CreateOrResizeWindow(
	...
	ImGui_ImplVulkanH_CreateWindowSwapChain(physical_device, device, wd,
	...
	// FIXME: to submit the command buffer, we need a queue. In the examples
	// folder, the ImGui_ImplVulkanH_CreateOrResizeWindow function is called
	// before the ImGui_ImplVulkan_Init function, so we don't have access to the
	// queue yet. Here we have the queue_family that we can use to grab a queue
	// from the device and submit the command buffer. It would be better to have
...
void ImGui_ImplVulkanH_DestroyWindow(VkInstance instance, VkDevice device,
	...
	IM_UNUSED(instance);
	vkDeviceWaitIdle(
		device);  // FIXME: We could wait on the Queue if we had the queue in
				  // wd-> (otherwise VulkanH functions can't use globals)
	...
	wd->FrameIndex = wd->ImageCount = wd->SemaphoreCount = wd->SemaphoreIndex =
		0;
	// vkDestroySurfaceKHR(instance, wd->Surface, allocator); // v1.92.6
	// (~2026-01-16): because wd->Surface is user provided we don't attempt to
	// destroy it ourself.
}
	...
	for (int n = 0; n < platform_io.Viewports.Size; n++)
		if (ImGui_ImplVulkan_ViewportData* vd =
				(ImGui_ImplVulkan_ViewportData*)platform_io.Viewports[n]
					->RendererUserData)
			ImGui_ImplVulkan_DestroyWindowRenderBuffers(
				device, &vd->RenderBuffers, allocator);
}

ImGui_ImplVulkanH_Window* ImGui_ImplVulkanH_GetWindowDataFromViewport(
...
static void ImGui_ImplVulkan_CreateWindow(ImGuiViewport* viewport) {
	...
	vkGetPhysicalDeviceSurfaceSupportKHR(v->PhysicalDevice, v->QueueFamily,
										 wd->Surface, &res);
	if (res != VK_TRUE) {
		vkDestroySurfaceKHR(v->Instance, wd->Surface,
							v->Allocator);	// Error: no WSI support on physical
											// device, clean up and return
		IM_DELETE(vd);
		return;
	}
...
#ifdef IMGUI_IMPL_VULKAN_HAS_DYNAMIC_RENDERING
	for (uint32_t n = 0;
		 n < pipeline_info->PipelineRenderingCreateInfo.colorAttachmentCount;
		 n++)
		requestSurfaceImageFormats.push_back(
			pipeline_info->PipelineRenderingCreateInfo
				.pColorAttachmentFormats[n]);
#endif
	...
	// Select Present Mode
	// FIXME-VULKAN: Even thought mailbox seems to get us maximum framerate with
	// a single window, it halves framerate with a second window etc. (w/ Nvidia
	// and SDK 1.82.1)
	...
	ImGui_ImplVulkanH_CreateOrResizeWindow(
		...
		v->MinImageCount, pipeline_info->SwapChainImageUsage);
	vd->WindowOwned = true;
	...
	if (bd->PipelineForViewports == VK_NULL_HANDLE) {
		...
		if (wd->UseDynamicRendering) {
			pipeline_info->PipelineRenderingCreateInfo.sType =
				VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO;
			pipeline_info->PipelineRenderingCreateInfo.colorAttachmentCount = 1;
			pipeline_info->PipelineRenderingCreateInfo.pColorAttachmentFormats =
		...
		bd->PipelineForViewports = ImGui_ImplVulkan_CreatePipeline(
			v->Device, v->Allocator, VK_NULL_HANDLE,
...
static void ImGui_ImplVulkan_SetWindowSize(ImGuiViewport* viewport,
	...
	ImGui_ImplVulkan_ViewportData* vd =
		(ImGui_ImplVulkan_ViewportData*)viewport->RendererUserData;
	if (vd == nullptr)	// This is nullptr for the main viewport (which is left
						// to the user/app to handle)
		return;
	ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
	ImGui_ImplVulkanH_Window* wd = &vd->Window;
	wd->AttachmentDesc.loadOp =
...
static void ImGui_ImplVulkan_RenderWindow(ImGuiViewport* viewport, void*) {
	...
	if (vd->SwapChainNeedRebuild || vd->SwapChainSuboptimal) {
		ImGui_ImplVulkanH_CreateOrResizeWindow(
			v->Instance, v->PhysicalDevice, v->Device, wd, v->QueueFamily,
			v->Allocator, (int)viewport->Size.x, (int)viewport->Size.y,
			v->MinImageCount, v->PipelineInfoForViewports.SwapChainImageUsage);
	...
	{
		{
			...
			if (err == VK_ERROR_OUT_OF_DATE_KHR) {
				vd->SwapChainNeedRebuild =
					true;  // Since we are not going to swap this frame anyway,
						   // it's ok that recreation happens on next frame.
				return;
			}
		...
		}
		{
			...
			memcpy(&wd->ClearValue.color.float32[0], &clear_color,
				   4 * sizeof(float));
		}
#ifdef IMGUI_IMPL_VULKAN_HAS_DYNAMIC_RENDERING
		if (v->UseDynamicRendering) {
