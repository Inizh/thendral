...
// (Please note that WebGPU is a recent API, may not be supported by all
// browser, and its ecosystem is generally a mess)

// Implemented features:
//  [X] Renderer: User texture binding. Use 'WGPUTextureView' as ImTextureID.
//  Read the FAQ about ImTextureID/ImTextureRef! [X] Renderer: Large meshes
...
//  Renderer: Texture updates support for dynamic font system
//  (ImGuiBackendFlags_RendererHasTextures).

// Read imgui_impl_wgpu.h about how to use the IMGUI_IMPL_WEBGPU_BACKEND_WGPU or
// IMGUI_IMPL_WEBGPU_BACKEND_DAWN flags.
...
//  2025-10-16: Update to compile with Dawn and Emscripten's 4.0.10+
//  '--use-port=emdawnwebgpu' ports. (#8381, #8898) 2025-09-18: Call
//  platform_io.ClearRendererHandlers() on shutdown. 2025-06-12: Added support
//  for ImGuiBackendFlags_RendererHasTextures, for dynamic font atlas. (#8465)
//  2025-02-26: Recreate image bind groups during render. (#8426, #8046, #7765,
//  #8027) + Update for latest webgpu-native changes. 2024-10-14: Update Dawn
//  support for change of string usages. (#8082, #8083) 2024-10-07: Expose
//  selected render state in ImGui_ImplWGPU_RenderState, which you can access in
//  'void* platform_io.Renderer_RenderState' during draw callbacks. 2024-10-07:
//  Changed default texture sampler to Clamp instead of Repeat/Wrap. 2024-09-16:
//  Added support for optional IMGUI_IMPL_WEBGPU_BACKEND_DAWN /
//  IMGUI_IMPL_WEBGPU_BACKEND_WGPU define to handle ever-changing native
//  implementations. (#7977) 2024-01-22: Added configurable
//  PipelineMultisampleState struct. (#7240) 2024-01-22: (Breaking)
...
// One of IMGUI_IMPL_WEBGPU_BACKEND_DAWN or IMGUI_IMPL_WEBGPU_BACKEND_WGPU must
// be provided. See imgui_impl_wgpu.h for more details.
#if defined(IMGUI_IMPL_WEBGPU_BACKEND_DAWN) == \
	defined(IMGUI_IMPL_WEBGPU_BACKEND_WGPU)
#error Exactly one of IMGUI_IMPL_WEBGPU_BACKEND_DAWN or IMGUI_IMPL_WEBGPU_BACKEND_WGPU must be defined!
#endif

// This condition is true when it's built with EMSCRIPTEN using -sUSE_WEBGPU=1
// flag  (deprecated from 4.0.10) This condition is false for all other 3 cases:
// WGPU-Native, DAWN-Native or DAWN-EMSCRIPTEN (using --use-port=emdawnwebgpu
// flag)
...
// https://github.com/webgpu-native/webgpu-headers/pull/413) Using type alias
// until WGPU adopts the same naming convention (#8369)
...
#define MEMALIGN(_SIZE, _ALIGN)   \
	(((_SIZE) + ((_ALIGN) - 1)) & \
	 ~((_ALIGN) - 1))  // Memory align (copied from IM_ALIGN() macro).
...
struct RenderResources {
	...
	WGPUBindGroup CommonBindGroup =
		nullptr;  // Resources bind-group to bind the common resources to
				  // pipeline
	ImGuiStorage
		ImageBindGroups;  // Resources bind-group to bind the font/image
						  // resources to pipeline (this is a key->value map)
	WGPUBindGroupLayout ImageBindGroupLayout =
		nullptr;  // Cache layout used for the image bind group. Avoids
				  // allocating unnecessary JS objects when working with WebASM
};
...
static ImGui_ImplWGPU_Data* ImGui_ImplWGPU_GetBackendData() {
	return ImGui::GetCurrentContext()
			   ? (ImGui_ImplWGPU_Data*)ImGui::GetIO().BackendRendererUserData
			   : nullptr;
}
...
static void ImGui_ImplWGPU_SetupRenderState(ImDrawData* draw_data,
	...
	wgpuRenderPassEncoderSetBindGroup(
		ctx, 0, bd->renderResources.CommonBindGroup, 0, nullptr);
...
void ImGui_ImplWGPU_RenderDrawData(ImDrawData* draw_data,
		...
		for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
			...
			if (pcmd->UserCallback != nullptr) {
				...
				WGPUBindGroup bind_group =
					(WGPUBindGroup)bd->renderResources.ImageBindGroups
						.GetVoidPtr(tex_id_hash);
				if (!bind_group) {
					bind_group = ImGui_ImplWGPU_CreateImageBindGroup(
				...
				// Clamp to viewport as wgpuRenderPassEncoderSetScissorRect()
				// won't accept values that are off bounds
				if (clip_min.x < 0.0f) {
					clip_min.x = 0.0f;
				}
				if (clip_min.y < 0.0f) {
	...
	for (int i = 0; i < image_bind_groups.Data.Size; i++) {
		WGPUBindGroup bind_group =
			(WGPUBindGroup)image_bind_groups.Data[i].val_p;
...
static void ImGui_ImplWGPU_DestroyTexture(ImTextureData* tex) {
	if (ImGui_ImplWGPU_Texture* backend_tex =
			(ImGui_ImplWGPU_Texture*)tex->BackendUserData) {
		IM_ASSERT(backend_tex->TextureView ==
				  (WGPUTextureView)(intptr_t)tex->TexID);
...
void ImGui_ImplWGPU_UpdateTexture(ImTextureData* tex) {
	...
	if (tex->Status == ImTextureStatus_WantCreate) {
		...
		backend_tex->TextureView =
			wgpuTextureCreateView(backend_tex->Texture, &tex_view_desc);
	...
	if (tex->Status == ImTextureStatus_WantCreate ||
		...
		ImGui_ImplWGPU_Texture* backend_tex =
			(ImGui_ImplWGPU_Texture*)tex->BackendUserData;
		IM_ASSERT(tex->Format == ImTextureFormat_RGBA32);
		...
		wgpuQueueWriteTexture(
			bd->defaultQueue, &dst_view, tex->GetPixelsAt(upload_x, upload_y),
...
bool ImGui_ImplWGPU_CreateDeviceObjects() {
	...
	graphics_pipeline_desc.primitive.stripIndexFormat =
...
#ifdef IMGUI_IMPL_WEBGPU_BACKEND_DAWN
		{nullptr, WGPUVertexFormat_Float32x2,
		 (uint64_t)offsetof(ImDrawVert, pos), 0},
		{nullptr, WGPUVertexFormat_Float32x2,
		 (uint64_t)offsetof(ImDrawVert, uv), 1},
		{nullptr, WGPUVertexFormat_Unorm8x4,
		 (uint64_t)offsetof(ImDrawVert, col), 2},
#else
...
bool ImGui_ImplWGPU_Init(ImGui_ImplWGPU_InitInfo* init_info) {
	...
	io.BackendRendererName =
		"imgui_impl_wgpu (Dawn, Emscripten)";  // compiled & linked using
											   // EMSCRIPTEN with
...
#if defined(__EMSCRIPTEN__)
	io.BackendRendererName =
		"imgui_impl_wgpu (WGPU, Emscripten)";  // linked using EMSCRIPTEN with
											   // "-sUSE_WEBGPU=1" flag,
											   // deprecated from
											   // EMSCRIPTEN 4.0.10
#else
...
#else
	return (status == WGPUSurfaceGetCurrentTextureStatus_OutOfMemory ||
...
WGPUSurface ImGui_ImplWGPU_CreateWGPUSurfaceHelper(
	...
	if (strcmp(info->System, "cocoa") == 0) {
		...
		surface =
			wgpuInstanceCreateSurface(info->Instance, &surface_descriptor);
	}
