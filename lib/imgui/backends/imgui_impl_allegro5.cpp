...
//  [X] Renderer: User texture binding. Use 'ALLEGRO_BITMAP*' as texture
//  identifier. Read the FAQ about ImTextureID/ImTextureRef! [X] Renderer:
//  Texture updates support for dynamic font atlas
//  (ImGuiBackendFlags_RendererHasTextures). [X] Platform: Keyboard support.
//  Since 1.87 we are using the io.AddKeyEvent() function. Pass ImGuiKey values
//  to all key functions e.g. ImGui::IsKeyPressed(ImGuiKey_Space). [Legacy
...
//  2025-09-18: Call platform_io.ClearRendererHandlers() and
//  platform_io.ClearPlatformHandlers() on shutdown. 2025-08-12: Inputs: fixed
//  missing support for ImGuiKey_PrintScreen under Windows, as raw Allegro 5
//  does not receive it. 2025-08-12: Added ImGui_ImplAllegro5_SetDisplay()
//  function to change current ALLEGRO_DISPLAY, as Allegro applications often
//  need to do that. 2025-07-07: Fixed texture update broken on some platforms
//  where ALLEGRO_LOCK_WRITEONLY needed all texels to be rewritten. 2025-06-11:
//  Added support for ImGuiBackendFlags_RendererHasTextures, for dynamic font
//  atlas. Removed ImGui_ImplSDLGPU3_CreateFontsTexture() and
//  ImGui_ImplSDLGPU3_DestroyFontsTexture(). 2025-02-18: Added
...
static ImGui_ImplAllegro5_Data* ImGui_ImplAllegro5_GetBackendData() {
	return ImGui::GetCurrentContext() ? (ImGui_ImplAllegro5_Data*)ImGui::GetIO()
...
void ImGui_ImplAllegro5_RenderDrawData(ImDrawData* draw_data) {
	...
	for (const ImDrawList* draw_list : draw_data->CmdLists) {
		...
		if (sizeof(ImDrawIdx) == 2) {
			...
			// You can '#define ImDrawIdx int' in imconfig.h to request Dear
			// ImGui to output 32-bit indices. Otherwise, we convert them from
			// 16-bit to 32-bit at runtime here, which works perfectly but is a
			// little wasteful.
		...
		// until 5.2.5. Unindex buffers ourselves while converting vertex
		// format.
		vertices.resize(draw_list->IdxBuffer.Size);
		for (int i = 0; i < draw_list->IdxBuffer.Size; i++) {
			const ImDrawVert* src_v =
				&draw_list->VtxBuffer[draw_list->IdxBuffer[i]];
			ImDrawVertAllegro* dst_v = &vertices[i];
			DRAW_VERT_IMGUI_TO_ALLEGRO(dst_v, src_v);
		}
#endif
...
#else
				al_draw_prim(&vertices[0], bd->VertexDecl, texture,
							 pcmd->IdxOffset, pcmd->IdxOffset + pcmd->ElemCount,
							 ALLEGRO_PRIM_TRIANGLE_LIST);
#endif
			}
		}
	...
	al_set_clipping_rectangle(last_clip_x, last_clip_y, last_clip_w,
							  last_clip_h);
...
void ImGui_ImplAllegro5_UpdateTexture(ImTextureData* tex) {
	if (tex->Status == ImTextureStatus_WantCreate) {
		...
		IM_ASSERT(locked_region != nullptr &&
		...
		// region.
		ImTextureRect r = tex->UpdateRect;	// Bounding box encompassing all
											// individual updates
		ALLEGRO_BITMAP* gpu_bitmap = (ALLEGRO_BITMAP*)(intptr_t)tex->TexID;
		ALLEGRO_LOCKED_REGION* locked_region = al_lock_bitmap_region(
			gpu_bitmap, r.x, r.y, r.w, r.h, al_get_bitmap_format(gpu_bitmap),
			ALLEGRO_LOCK_WRITEONLY);
		IM_ASSERT(locked_region && "Backend failed to update texture!");
		for (int y = 0; y < r.h; y++)
			memcpy(
				(unsigned char*)locked_region->data + locked_region->pitch * y,
				tex->GetPixelsAt(r.x, r.y + y),
				r.w * tex->BytesPerPixel);	// dst, src, block pitch
...
static void ImGui_ImplAllegro5_SetClipboardText(ImGuiContext*,
												const char* text) {
	ImGui_ImplAllegro5_Data* bd = ImGui_ImplAllegro5_GetBackendData();
...
bool ImGui_ImplAllegro5_Init(ALLEGRO_DISPLAY* display) {
	...
	platform_io.Platform_SetClipboardTextFn =
		ImGui_ImplAllegro5_SetClipboardText;
	platform_io.Platform_GetClipboardTextFn =
		ImGui_ImplAllegro5_GetClipboardText;
#endif
...
void ImGui_ImplAllegro5_SetDisplay(ALLEGRO_DISPLAY* display) {
	...
	if (bd->Display && !bd->VertexDecl) {
		...
		// Unfortunately Allegro doesn't support 32-bits packed colors so we
		// have to convert them to 4 floats. We still use a custom declaration
		// to use 'ALLEGRO_PRIM_TEX_COORD' instead of
		// 'ALLEGRO_PRIM_TEX_COORD_PIXEL' else we can't do a reliable
		// conversion.
		ALLEGRO_VERTEX_ELEMENT elems[] = {
			{ALLEGRO_PRIM_POSITION, ALLEGRO_PRIM_FLOAT_2,
			 offsetof(ImDrawVertAllegro, pos)},
			{ALLEGRO_PRIM_TEX_COORD, ALLEGRO_PRIM_FLOAT_2,
			 offsetof(ImDrawVertAllegro, uv)},
			{ALLEGRO_PRIM_COLOR_ATTR, 0, offsetof(ImDrawVertAllegro, col)},
			{0, 0, 0}};
		bd->VertexDecl =
			al_create_vertex_decl(elems, sizeof(ImDrawVertAllegro));
	}
}
...
static void ImGui_ImplAllegro5_UpdateKeyModifiers() {
	...
	io.AddKeyEvent(ImGuiMod_Ctrl, al_key_down(&keys, ALLEGRO_KEY_LCTRL) ||
									  al_key_down(&keys, ALLEGRO_KEY_RCTRL));
	io.AddKeyEvent(ImGuiMod_Shift, al_key_down(&keys, ALLEGRO_KEY_LSHIFT) ||
									   al_key_down(&keys, ALLEGRO_KEY_RSHIFT));
	io.AddKeyEvent(ImGuiMod_Alt, al_key_down(&keys, ALLEGRO_KEY_ALT) ||
									 al_key_down(&keys, ALLEGRO_KEY_ALTGR));
	io.AddKeyEvent(ImGuiMod_Super, al_key_down(&keys, ALLEGRO_KEY_LWIN) ||
	...
	switch (ev->type) {
		...
		case ALLEGRO_EVENT_KEY_UP:
			if (ev->keyboard.display == bd->Display) {
				...
				ImGuiKey key =
					ImGui_ImplAllegro5_KeyCodeToImGuiKey(ev->keyboard.keycode);
...
#ifdef _WIN32
	io.AddKeyEvent(ImGuiKey_PrintScreen,
