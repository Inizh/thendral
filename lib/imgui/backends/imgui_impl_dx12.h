...
struct ImGui_ImplDX12_InitInfo {
	ID3D12Device* Device;
	ID3D12CommandQueue*
		CommandQueue;  // Command queue used for queuing texture uploads.
	...
	LegacySingleSrvCpuDescriptor;  // To facilitate transition from single
								   // descriptor to allocator callback, you
								   // may use those.
...
// - font_srv_cpu_desc_handle and font_srv_gpu_desc_handle are handles to a
// single SRV descriptor to use for the internal font texture, they must be in
// 'srv_descriptor_heap'
// - When we introduced the ImGui_ImplDX12_InitInfo struct we also added a
// 'ID3D12CommandQueue* CommandQueue' field.
IMGUI_IMPL_API bool ImGui_ImplDX12_Init(
	ID3D12Device* device, int num_frames_in_flight, DXGI_FORMAT rtv_format,
...
// ImGui_ImplDX12_RenderDrawData() call. (Please open an issue if you feel you
