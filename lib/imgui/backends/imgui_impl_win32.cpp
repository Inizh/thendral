...
//  functions e.g. ImGui::IsKeyPressed(ImGuiKey_Space). [Legacy VK_* values are
//  obsolete since 1.87 and not supported since 1.91.5] [X] Platform: Gamepad
//  support. [X] Platform: Mouse cursor shape and visibility
...
//  ImGuiPlatformIO interface. 2026-01-26: [Docking] Fixed an issue from 1.90.5
//  where newly appearing windows that are not parented to the main viewport
//  don't have task bar icon appear before the windows was explicited refocused.
//  (#7354, #8669) 2025-12-03: Inputs: handle WM_IME_CHAR/WM_IME_COMPOSITION
//  messages to support Unicode inputs on MBCS (non-Unicode) Windows. (#9099,
//  #3653, #5961) 2025-10-19: Inputs: Revert previous change to allow for
//  io.ClearInputKeys() on focus-out not losing gamepad state. 2025-09-23:
//  Inputs: Minor optimization not submitting gamepad input if packet number has
//  not changed. 2025-09-18: Call platform_io.ClearPlatformHandlers() on
//  shutdown. 2025-06-02: [Docking] WM_DPICHANGED also apply
//  io.ConfigDpiScaleViewports for main viewport instead of letting it be done
//  by application code. 2025-04-30: Inputs: Fixed an issue where externally
//  losing mouse capture (due to e.g. focus loss) would fail to claim it again
//  the next subsequent click. (#8594) 2025-03-26: [Docking] Viewports: fixed an
//  issue when closing a window from the OS close button (with
//  io.ConfigViewportsNoDecoration = false) while user code was discarding the
//  'bool* p_open = false' output from Begin(). Because we allowed the Win32
//  window to close early, Windows destroyed it and our imgui window became not
//  visible even though user code was still submitting it. 2025-03-10: When
//  dealing with OEM keys, use scancodes instead of translated keycodes to
//  choose ImGuiKey values. (#7136, #7201, #7206, #7306, #7670, #7672, #8468)
//  2025-02-21: [Docking] WM_SETTINGCHANGE's SPI_SETWORKAREA message also
//  triggers a refresh of monitor list. (#8415) 2025-02-18: Added
//  ImGuiMouseCursor_Wait and ImGuiMouseCursor_Progress mouse cursor support.
//  2024-11-21: [Docking] Fixed a crash when multiple processes are running with
//  multi-viewports, caused by misusage of GetProp(). (#8162, #8069) 2024-10-28:
//  [Docking] Rely on property stored inside HWND to retrieve context/viewport,
//  should facilitate attempt to use this for parallel contexts. (#8069)
//  2024-09-16: [Docking] Inputs: fixed an issue where a viewport destroyed
//  while clicking would hog mouse tracking and temporary lead to incorrect
//  update of HoveredWindow. (#7971) 2024-07-08: Inputs: Fixed ImGuiMod_Super
//  being mapped to VK_APPS instead of VK_LWIN||VK_RWIN. (#7768) 2023-10-05:
//  Inputs: Added support for extra ImGuiKey values: F13 to F24 function keys,
//  app back/forward keys. 2023-09-25: Inputs: Synthesize key-down event on
//  key-up for VK_SNAPSHOT / ImGuiKey_PrintScreen as Windows doesn't emit it
//  (same behavior as GLFW/SDL). 2023-09-07: Inputs: Added support for keyboard
//  codepage conversion for when application is compiled in MBCS mode and using
//  a non-Unicode window. 2023-04-19: Added ImGui_ImplWin32_InitForOpenGL() to
//  facilitate combining raw Win32/Winapi with OpenGL. (#3218) 2023-04-04:
...
static bool ImGui_ImplWin32_InitEx(void* hwnd, bool platform_has_own_dc) {
	...
	io.BackendFlags |=
		ImGuiBackendFlags_HasMouseHoveredViewport;	// We can call
													...
													// (optional)
	io.BackendFlags |=
		ImGuiBackendFlags_HasParentViewport;  // We can honor
											  // viewport->ParentViewportId by
											  // applying the corresponding
											  // parent/child relationship at
											  // platform levle (optional)
	...
	// So as we store a pointer in IMGUI_CONTEXT we need to make sure we only
	// call GetPropA() on windows owned by our process.
...
static void ImGui_ImplWin32_AddKeyEvent(ImGuiIO& io, ImGuiKey key, bool down,
										int native_keycode,
...
static ImGuiViewport* ImGui_ImplWin32_FindViewportByPlatformHandle(
	ImGuiPlatformIO& platform_io, HWND hwnd) {
	// We cannot use ImGui::FindViewportByPlatformHandle() because it doesn't
	// take a context. When called from
	// ImGui_ImplWin32_WndProcHandler_PlatformWindow() we don't assume that
	// context is bound.
	// return ImGui::FindViewportByPlatformHandle((void*)hwnd);
	for (ImGuiViewport* viewport : platform_io.Viewports)
		if (viewport->PlatformHandle == hwnd) return viewport;
	return nullptr;
}

// This code supports multi-viewports (multiple OS Windows mapped into different
// Dear ImGui viewports) Because of that, it is a little more complicated than
...
static void ImGui_ImplWin32_UpdateMouseData(ImGuiIO& io,
	...
	const bool is_app_focused =
		(focused_window &&
		 (focused_window == bd->hWnd || ::IsChild(focused_window, bd->hWnd) ||
		  ImGui_ImplWin32_FindViewportByPlatformHandle(platform_io,
													   focused_window)));
	if (is_app_focused) {
		// (Optional) Set OS mouse position from Dear ImGui if requested (rarely
		// used, only when io.ConfigNavMoveSetMousePos is enabled by user) When
		// multi-viewports are enabled, all Dear ImGui positions are same as OS
		// positions.
		...
		// (WM_MOUSEMOVE already provides this when hovered or captured) This
		// also fills a short gap when clicking non-client area: WM_NCMOUSELEAVE
		// -> modal OS move -> gap -> WM_NCMOUSEMOVE
		if (!io.WantSetMousePos && bd->MouseTrackedArea == 0 &&
			...
			// of the app window) (This is the position you can get with
			// ::GetCursorPos() + ::ScreenToClient() or WM_MOUSEMOVE.)
			...
			// primary monitor) (This is the position you can get with
			// ::GetCursorPos() or WM_MOUSEMOVE + ::ClientToScreen(). In theory
			// adding viewport->Pos to a client position would also be the
			// same.)
	...
	// (here using ::WindowFromPoint with WM_NCHITTEST + HTTRANSPARENT in
	// WndProc does that)
...
static void ImGui_ImplWin32_UpdateGamepads(ImGuiIO& io) {
	...
	// unfortunately too slow. Instead we refresh gamepad availability by
	// calling XInputGetCapabilities() _only_ after receiving WM_DEVICECHANGE.
...
static BOOL CALLBACK ImGui_ImplWin32_UpdateMonitors_EnumFunc(HMONITOR monitor,
															 HDC, LPRECT,
															 LPARAM) {
	MONITORINFO info = {};
	...
	imgui_monitor.MainPos =
		ImVec2((float)info.rcMonitor.left, (float)info.rcMonitor.top);
	...
	imgui_monitor.WorkPos =
		ImVec2((float)info.rcWork.left, (float)info.rcWork.top);
	...
	if (imgui_monitor.DpiScale <= 0.0f)
		return TRUE;  // Some accessibility applications are declaring virtual
...
static void ImGui_ImplWin32_UpdateMonitors() {
	...
	::EnumDisplayMonitors(nullptr, nullptr,
						  ImGui_ImplWin32_UpdateMonitors_EnumFunc, 0);
...
ImGuiKey ImGui_ImplWin32_KeyEventToImGuiKey(WPARAM wParam, LPARAM lParam) {
	...
	switch (scancode) {
		case 41:
			return ImGuiKey_GraveAccent;  // VK_OEM_8 in EN-UK, VK_OEM_3 in
										  // EN-US, VK_OEM_7 in FR, VK_OEM_5 in
										  // DE, etc.
		case 12:
...
// your application based on those two flags. PS: We treat DBLCLK messages as
// regular mouse down messages, so this code will work on windows classes that
...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hwnd, UINT msg,
	...
	// because WndProc are called early in CreateWindow(). We silently allow
	// both context or just only backend data to be nullptr.
	if (ImGui::GetCurrentContext() == nullptr) return 0;
	return ImGui_ImplWin32_WndProcHandlerEx(hwnd, msg, wParam, lParam,
											ImGui::GetIO());
}

// This version is in theory thread-safe in the sense that no path should access
// ImGui::GetCurrentContext().
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandlerEx(HWND hwnd, UINT msg,
														WPARAM wParam,
														LPARAM lParam,
														ImGuiIO& io) {
	ImGui_ImplWin32_Data* bd = ImGui_ImplWin32_GetBackendData(io);
	...
	switch (msg) {
		case WM_MOUSEMOVE:
		case WM_NCMOUSEMOVE: {
			// We need to call TrackMouseEvent in order to receive WM_MOUSELEAVE
			// events
			...
			if (bd->MouseTrackedArea != area) {
				TRACKMOUSEEVENT tme_cancel = {sizeof(tme_cancel), TME_CANCEL,
											  hwnd, 0};
				TRACKMOUSEEVENT tme_track = {
					sizeof(tme_track),
					(DWORD)((area == 2) ? (TME_LEAVE | TME_NONCLIENT)
										: TME_LEAVE),
					hwnd, 0};
			...
			if (msg == WM_MOUSEMOVE &&
				want_absolute_pos)	// WM_MOUSEMOVE are client-relative
									// coordinates.
		...
		case WM_XBUTTONDBLCLK: {
			...
			if (bd->MouseButtonsDown == 0 && hwnd_with_capture == nullptr)
				::SetCapture(
					hwnd);	// Allow us to read mouse coordinates when dragging
							// mouse outside of our window bounds.
		...
		case WM_SYSKEYUP: {
			...
			if (wParam < 256) {
				...
				const ImGuiKey key =
					ImGui_ImplWin32_KeyEventToImGuiKey(wParam, lParam);
				const int vk = (int)wParam;
				const int scancode = (int)LOBYTE(HIWORD(lParam));

				// Special behavior for VK_SNAPSHOT / ImGuiKey_PrintScreen as
				// Windows doesn't emit the key down event.
				if (key == ImGuiKey_PrintScreen && !is_key_down)
					ImGui_ImplWin32_AddKeyEvent(io, key, true, vk, scancode);

				// Submit key event
				if (key != ImGuiKey_None)
					ImGui_ImplWin32_AddKeyEvent(io, key, is_key_down, vk,
												scancode);

				// Submit individual left/right modifier events
				if (vk == VK_SHIFT) {
					// Important: Shift keys tend to get stuck when pressed
					// together, missing key-up events are corrected in
					// ImGui_ImplWin32_ProcessKeyEventsWorkarounds()
					if (IsVkDown(VK_LSHIFT) == is_key_down) {
						ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_LeftShift,
													is_key_down, VK_LSHIFT,
													scancode);
					}
					if (IsVkDown(VK_RSHIFT) == is_key_down) {
						ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_RightShift,
													is_key_down, VK_RSHIFT,
													scancode);
					}
				} else if (vk == VK_CONTROL) {
					if (IsVkDown(VK_LCONTROL) == is_key_down) {
						ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_LeftCtrl,
													is_key_down, VK_LCONTROL,
													scancode);
					}
					if (IsVkDown(VK_RCONTROL) == is_key_down) {
						ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_RightCtrl,
													is_key_down, VK_RCONTROL,
													scancode);
					}
				} else if (vk == VK_MENU) {
					if (IsVkDown(VK_LMENU) == is_key_down) {
						ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_LeftAlt,
													is_key_down, VK_LMENU,
													scancode);
					}
					if (IsVkDown(VK_RMENU) == is_key_down) {
						ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey_RightAlt,
		...
		case WM_IME_COMPOSITION: {
			// Handling WM_IME_COMPOSITION ensure that WM_IME_CHAR value is
			// correct even for MBCS apps. (see #9099, #3653 and
			// https://stackoverflow.com/questions/77450354 topics)
		...
		case WM_IME_CHAR:
			...
			return 0;
		case WM_SETCURSOR:
			// This is required to restore cursor when transitioning from e.g
			// resize borders to client area.
			if (LOWORD(lParam) == HTCLIENT &&
				ImGui_ImplWin32_UpdateMouseCursor(io, bd->LastMouseCursor))
				return 1;
			return 0;
		...
		case WM_SETTINGCHANGE:
			if (wParam == SPI_SETWORKAREA) bd->WantUpdateMonitors = true;
			return 0;
		case WM_DPICHANGED: {
			const RECT* suggested_rect = (RECT*)lParam;
			if (io.ConfigDpiScaleViewports)
				::SetWindowPos(hwnd, nullptr, suggested_rect->left,
							   suggested_rect->top,
...
//   at compile time and Windows 8/10+ at runtime, neither of which we want to
//   require the user to have. So we dynamically select and load those functions
	...
	if (_IsWindows8Point1OrGreater()) {
		...
		if (GetDpiForMonitorFn != nullptr) {
			GetDpiForMonitorFn((HMONITOR)monitor, MDT_EFFECTIVE_DPI, &xdpi,
							   &ydpi);
			IM_ASSERT(xdpi ==
					  ydpi);  // Please contact me if you hit this assert!
			return xdpi / 96.0f;
		}
	}
#ifndef NOGDI
...
static void ImGui_ImplWin32_GetWin32StyleFromViewportFlags(
...
static HWND ImGui_ImplWin32_GetHwndFromViewport(ImGuiViewport* viewport) {
	if (viewport != nullptr) return (HWND)viewport->PlatformHandle;
	return nullptr;
}
...
static void ImGui_ImplWin32_CreateWindow(ImGuiViewport* viewport) {
	...
	ImGui_ImplWin32_GetWin32StyleFromViewportFlags(
		viewport->Flags, &vd->DwStyle, &vd->DwExStyle);
	vd->HwndParent =
		ImGui_ImplWin32_GetHwndFromViewport(viewport->ParentViewport);
	...
	vd->Hwnd =
		::CreateWindowExW(vd->DwExStyle, L"ImGui Platform", L"Untitled",
...
static void ImGui_ImplWin32_DestroyWindow(ImGuiViewport* viewport) {
	...
	if (ImGui_ImplWin32_ViewportData* vd =
		...
		if (::GetCapture() == vd->Hwnd) {
			// Transfer capture so if we started dragging from a window that
			// later disappears, we'll still receive the MOUSEUP event.
			::ReleaseCapture();
			::SetCapture(bd->hWnd);
		}
...
static void ImGui_ImplWin32_ShowWindow(ImGuiViewport* viewport) {
	...
	// ShowParent() even with SW_SHOWNA also brings parent to front, which is
	// not always desirable, so we temporarily disable parenting. (#7354, #8669)
	bool avoid_bringing_parent_to_front =
		vd->HwndParent != NULL &&
		(viewport->Flags & (ImGuiViewportFlags_NoFocusOnAppearing |
							ImGuiViewportFlags_NoTaskBarIcon)) != 0;
...
static void ImGui_ImplWin32_UpdateWindow(ImGuiViewport* viewport) {
	...
	// Unlike style settings derived from configuration flags, this is more
	// likely to change for advanced apps that are manipulating ParentViewportID
	// manually.
	HWND new_parent =
		ImGui_ImplWin32_GetHwndFromViewport(viewport->ParentViewport);
	if (new_parent != vd->HwndParent) {
		...
		// Dear Imgui-side concept of parenting only mostly care about what
		// Win32 call "Owner". The parent parameter of CreateWindowEx() sets up
		// Parent OR Owner depending on WS_CHILD flag. In our case an Owner as
		// we never use WS_CHILD. Calling ::SetParent() here would be incorrect:
		// it will create a full child relation, alter coordinate system and
		// clipping. Calling ::SetWindowLongPtr() with GWLP_HWNDPARENT seems
		// correct although poorly documented.
	...
	// advanced uses (such as manually rewriting viewport flags) make this
	// useful.
	DWORD new_style;
	DWORD new_ex_style;
	ImGui_ImplWin32_GetWin32StyleFromViewportFlags(viewport->Flags, &new_style,
												   &new_ex_style);
	...
	if (vd->DwStyle != new_style || vd->DwExStyle != new_ex_style) {
		...
		HWND insert_after =
			top_most_changed ? ((viewport->Flags & ImGuiViewportFlags_TopMost)
									? HWND_TOPMOST
									: HWND_NOTOPMOST)
							 : 0;
...
static void ImGui_ImplWin32_UpdateWin32StyleFromWindow(
...
static void ImGui_ImplWin32_SetWindowPos(ImGuiViewport* viewport, ImVec2 pos) {
	...
	if (viewport->Flags & ImGuiViewportFlags_OwnedByApp)
		ImGui_ImplWin32_UpdateWin32StyleFromWindow(
			viewport);	// Not our window, poll style before using
	::AdjustWindowRectEx(&rect, vd->DwStyle, FALSE, vd->DwExStyle);
...
static void ImGui_ImplWin32_SetWindowTitle(ImGuiViewport* viewport,
										   const char* title) {
	// ::SetWindowTextA() doesn't properly handle UTF-8 so we explicitely
	// convert our string.
	ImGui_ImplWin32_ViewportData* vd =
		(ImGui_ImplWin32_ViewportData*)viewport->PlatformUserData;
	IM_ASSERT(vd->Hwnd != 0);
	...
	// Calling SetWindowTextW() in a project where UNICODE is not set doesn't
	// work but there's a trick which is to pass it directly to the
	// DefWindowProcW() handler. See:
	// https://stackoverflow.com/questions/9410681/setwindowtextw-in-an-ansi-project
...
static void ImGui_ImplWin32_SetWindowAlpha(ImGuiViewport* viewport,
	...
	if (alpha < 1.0f) {
		DWORD ex_style =
			::GetWindowLongW(vd->Hwnd, GWL_EXSTYLE) | WS_EX_LAYERED;
		::SetWindowLongW(vd->Hwnd, GWL_EXSTYLE, ex_style);
		::SetLayeredWindowAttributes(vd->Hwnd, 0, (BYTE)(255 * alpha),
									 LWA_ALPHA);
	} else {
		DWORD ex_style =
			::GetWindowLongW(vd->Hwnd, GWL_EXSTYLE) & ~WS_EX_LAYERED;
...
static LRESULT CALLBACK ImGui_ImplWin32_WndProcHandler_PlatformWindow(
	HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	// Allow secondary viewport WndProc to be called regardless of current
	// context
	ImGuiContext* ctx = (ImGuiContext*)::GetPropA(hWnd, "IMGUI_CONTEXT");
	if (ctx == NULL)
		return ::DefWindowProcW(
			hWnd, msg, wParam,
			lParam);  // unlike ImGui_ImplWin32_WndProcHandler() we are called
	...
	if (ImGui_ImplWin32_WndProcHandlerEx(hWnd, msg, wParam, lParam, io))
		result = 1;
	else if (ImGuiViewport* viewport =
				 ImGui_ImplWin32_FindViewportByPlatformHandle(platform_io,
															  hWnd)) {
		switch (msg) {
			case WM_CLOSE:
				viewport->PlatformRequestClose = true;
				return 0;  // 0 = Operating system will ignore the message and
						   // not destroy the window. We close ourselves.
			case WM_MOVE:
			...
			case WM_MOUSEACTIVATE:
				...
				break;
			case WM_NCHITTEST:
				// Let mouse pass-through the window. This will allow the
				// backend to call io.AddMouseViewportEvent() correctly. (which
				// is optional). The ImGuiViewportFlags_NoInputs flag is set
				// while dragging a viewport, as want to detect the window
				// behind the one we are dragging. If you cannot easily access
				// those viewport flags from your windowing/event code: you may
				// manually synchronize its state e.g. in your main loop after
				// calling UpdatePlatformWindows(). Iterate all
				// viewports/platform windows and pass the flag to your
				// windowing system.
	...
	if (result == 0) result = ::DefWindowProcW(hWnd, msg, wParam, lParam);
	return result;
}
...
static void ImGui_ImplWin32_InitMultiViewportSupport(bool platform_has_own_dc) {
	...
	platform_io.Platform_GetWindowMinimized =
		ImGui_ImplWin32_GetWindowMinimized;
