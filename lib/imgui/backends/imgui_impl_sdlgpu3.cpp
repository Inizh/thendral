...
//  [X] Renderer: User texture binding. Use 'SDL_GPUTexture*' as texture
//  identifier. Read the FAQ about ImTextureID/ImTextureRef! **IMPORTANT**
//  Before 2025/08/08, ImTextureID was a reference to a
//  SDL_GPUTextureSamplerBinding struct. [X] Renderer: Large meshes support
//  (64k+ vertices) even with 16-bit indices
...
// The aim of imgui_impl_sdlgpu3.h/.cpp is to be usable in your engine without
...
// Important note to the reader who wish to integrate imgui_impl_sdlgpu3.cpp/.h
// in their own engine/app.
// - Unlike other backends, the user must call the function
// ImGui_ImplSDLGPU3_PrepareDrawData() BEFORE issuing a SDL_GPURenderPass
// containing ImGui_ImplSDLGPU3_RenderDrawData.
//   Calling the function is MANDATORY, otherwise the ImGui will not upload
//   neither the vertex nor the index buffer for the GPU. See
//   imgui_impl_sdlgpu3.cpp for more info.

// CHANGELOG
//  2026-XX-XX: Platform: Added support for multiple windows via the
//  ImGuiPlatformIO interface. 2025-11-26: macOS version can use MSL shaders in
//  order to support macOS 10.14+ (vs Metallib shaders requiring macOS 14+).
...
//  ImGui_ImplSDLGPU3_InitInfo::PresentMode to configure how secondary viewports
//  are created. 2025-08-08: *BREAKING* Changed ImTextureID type from
//  SDL_GPUTextureSamplerBinding* to SDL_GPUTexture*, which is more natural and
//  easier for user to manage. If you need to change the current sampler, you
//  can access the ImGui_ImplSDLGPU3_RenderState struct. (#8866, #8163, #7998,
//  #7988) 2025-08-08: Expose SamplerDefault and SamplerCurrent in
//  ImGui_ImplSDLGPU3_RenderState. Allow callback to change sampler. 2025-06-25:
//  Mapping transfer buffer for texture update use cycle=true. Fixes artifacts
//  e.g. on Metal backend. 2025-06-11: Added support for
//  ImGuiBackendFlags_RendererHasTextures, for dynamic font atlas. Removed
//  ImGui_ImplSDLGPU3_CreateFontsTexture() and
//  ImGui_ImplSDLGPU3_DestroyFontsTexture(). 2025-04-28: Added support for
//  special ImDrawCallback_ResetRenderState callback to reset render state.
//  2025-03-30: Made ImGui_ImplSDLGPU3_PrepareDrawData() reuse GPU Transfer
//  Buffers which were unusually slow to recreate every frame. Much faster now.
//  2025-03-21: Fixed typo in function name Imgui_ImplSDLGPU3_PrepareDrawData()
//  -> ImGui_ImplSDLGPU3_PrepareDrawData(). 2025-01-16: Renamed
//  ImGui_ImplSDLGPU3_InitInfo::GpuDevice to Device. 2025-01-09: SDL_GPU: Added
//  the SDL_GPU3 backend.
...
static void ImGui_ImplSDLGPU3_SetupRenderState(
	ImDrawData* draw_data, ImGui_ImplSDLGPU3_RenderState* render_state,
	SDL_GPUGraphicsPipeline* pipeline, SDL_GPUCommandBuffer* command_buffer,
	SDL_GPURenderPass* render_pass, ImGui_ImplSDLGPU3_FrameData* fd,
...
static void CreateOrResizeBuffers(SDL_GPUBuffer** buffer,
	...
	*old_size = new_size;
	IM_ASSERT(*buffer != nullptr &&
			  "Failed to create GPU Buffer, call SDL_GetError() for more "
			  "information");
	...
	*transferbuffer =
		SDL_CreateGPUTransferBuffer(v->Device, &transferbuffer_info);
	IM_ASSERT(*transferbuffer != nullptr &&
			  "Failed to create GPU Transfer Buffer, call SDL_GetError() for "
			  "more information");
}

// SDL_GPU doesn't allow copy passes to occur while a render or compute pass is
// bound! The only way to allow a user to supply their own RenderPass (to render
// to a texture instead of the window for example), is to split the upload part
// of ImGui_ImplSDLGPU3_RenderDrawData() to another function that needs to be
// called by the user before rendering.
void ImGui_ImplSDLGPU3_PrepareDrawData(ImDrawData* draw_data,
	...
	if (fd->VertexBuffer == nullptr || fd->VertexBufferSize < vertex_size)
		CreateOrResizeBuffers(&fd->VertexBuffer, &fd->VertexTransferBuffer,
							  &fd->VertexBufferSize, vertex_size,
							  SDL_GPU_BUFFERUSAGE_VERTEX);
	if (fd->IndexBuffer == nullptr || fd->IndexBufferSize < index_size)
		CreateOrResizeBuffers(&fd->IndexBuffer, &fd->IndexTransferBuffer,
							  &fd->IndexBufferSize, index_size,
							  SDL_GPU_BUFFERUSAGE_INDEX);

	ImDrawVert* vtx_dst = (ImDrawVert*)SDL_MapGPUTransferBuffer(
		v->Device, fd->VertexTransferBuffer, true);
	ImDrawIdx* idx_dst = (ImDrawIdx*)SDL_MapGPUTransferBuffer(
		v->Device, fd->IndexTransferBuffer, true);
	...
	SDL_UploadToGPUBuffer(copy_pass, &vertex_buffer_location,
						  &vertex_buffer_region, true);
	SDL_UploadToGPUBuffer(copy_pass, &index_buffer_location,
...
void ImGui_ImplSDLGPU3_RenderDrawData(ImDrawData* draw_data,
	...
	render_state.SamplerLinear = render_state.SamplerCurrent =
		bd->TexSamplerLinear;
	render_state.SamplerNearest = bd->TexSamplerNearest;
	platform_io.Renderer_RenderState = &render_state;

	ImGui_ImplSDLGPU3_SetupRenderState(draw_data, &render_state, pipeline,
									   command_buffer, render_pass, fd,
									   fb_width, fb_height);
		...
		for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
			...
			if (pcmd->UserCallback != nullptr) {
				...
				if (pcmd->UserCallback == ImDrawCallback_ResetRenderState)
					ImGui_ImplSDLGPU3_SetupRenderState(
						draw_data, &render_state, pipeline, command_buffer,
				...
				// Clamp to viewport as SDL_SetGPUScissor() won't accept values
				// that are off bounds
				if (clip_min.x < 0.0f) {
					clip_min.x = 0.0f;
				}
				...
				texture_sampler_binding.texture =
					(SDL_GPUTexture*)(intptr_t)pcmd->GetTexID();
				texture_sampler_binding.sampler = render_state.SamplerCurrent;
				SDL_BindGPUFragmentSamplers(render_pass, 0,
											&texture_sampler_binding, 1);

				// Draw
				// **IF YOU GET A CRASH HERE** In 1.92.2 on 2025/08/08 we have
				// changed ImTextureID to store 'SDL_GPUTexture*' instead of
				// storing 'SDL_GPUTextureSamplerBinding'. Any code loading
				// custom texture using this backend needs to be updated.
				SDL_DrawGPUIndexedPrimitives(
					render_pass, pcmd->ElemCount, 1,
	...
	// Note: at this point both SDL_SetGPUViewport() and SDL_SetGPUScissor()
	// have been called. Our last values will leak into user/application
	// rendering if you forgot to call SDL_SetGPUViewport() and
	// SDL_SetGPUScissor() yourself to explicitly set that state In theory we
	// should aim to backup/restore those values but I am not sure this is
	// possible. We perform a call to SDL_SetGPUScissor() to set back a full
	// viewport which is likely to fix things for 99% users but technically this
	// is not perfect. (See github #4644)
...
void ImGui_ImplSDLGPU3_UpdateTexture(ImTextureData* tex) {
	...
	if (tex->Status == ImTextureStatus_WantCreate) {
		...
		SDL_GPUTexture* raw_tex =
			SDL_CreateGPUTexture(v->Device, &texture_info);
		IM_ASSERT(
			raw_tex != nullptr &&
			"Failed to create texture, call SDL_GetError() for more info");
	...
	if (tex->Status == ImTextureStatus_WantCreate ||
		...
		if (bd->TexTransferBufferSize < upload_size) {
			...
			bd->TexTransferBuffer =
				SDL_CreateGPUTransferBuffer(v->Device, &transferbuffer_info);
			IM_ASSERT(bd->TexTransferBuffer != nullptr &&
					  "Failed to create transfer buffer, call SDL_GetError() "
					  "for more information");
		}

		// Copy to transfer buffer
		{
			void* texture_ptr = SDL_MapGPUTransferBuffer(
				v->Device, bd->TexTransferBuffer, true);
		...
		// Upload
		{
			...
			SDL_UploadToGPUTexture(copy_pass, &transfer_info, &texture_region,
								   false);
			SDL_EndGPUCopyPass(copy_pass);
...
static void ImGui_ImplSDLGPU3_CreateShaders() {
	...
	if (strcmp(driver, "vulkan") == 0) {
		...
		SDL_GPUShaderFormat supported_formats =
			SDL_GetGPUShaderFormats(v->Device);
	...
	IM_ASSERT(bd->VertexShader != nullptr &&
			  "Failed to create vertex shader, call SDL_GetError() for more "
			  "information");
	IM_ASSERT(bd->FragmentShader != nullptr &&
			  "Failed to create fragment shader, call SDL_GetError() for more "
			  "information");
}
...
static void ImGui_ImplSDLGPU3_CreateGraphicsPipeline() {
	...
	IM_ASSERT(bd->Pipeline != nullptr &&
			  "Failed to create graphics pipeline, call SDL_GetError() for "
			  "more information");
}
...
void ImGui_ImplSDLGPU3_CreateDeviceObjects() {
	...
	if (bd->TexSamplerLinear == nullptr) {
		...
		IM_ASSERT(bd->TexSamplerLinear != nullptr &&
				  "Failed to create sampler, call SDL_GetError() for more "
				  "information");
		...
		IM_ASSERT(bd->TexSamplerNearest != nullptr &&
				  "Failed to create sampler, call SDL_GetError() for more "
				  "information");
	}
...
static void ImGui_ImplSDLGPU3_CreateWindow(ImGuiViewport* viewport) {
	...
	SDL_Window* window =
		SDL_GetWindowFromID((SDL_WindowID)(intptr_t)viewport->PlatformHandle);
	SDL_ClaimWindowForGPUDevice(data->InitInfo.Device, window);
	SDL_SetGPUSwapchainParameters(data->InitInfo.Device, window,
								  data->InitInfo.SwapchainComposition,
								  data->InitInfo.PresentMode);
...
static void ImGui_ImplSDLGPU3_RenderWindow(ImGuiViewport* viewport, void*) {
	...
	SDL_GPUCommandBuffer* command_buffer =
		SDL_AcquireGPUCommandBuffer(data->InitInfo.Device);
	...
	if (swapchain_texture != nullptr) {
		ImGui_ImplSDLGPU3_PrepareDrawData(
			draw_data, command_buffer);	 // FIXME-OPT: Not optimal, may this be
										 // done earlier?
