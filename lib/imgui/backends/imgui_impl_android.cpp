	...
	switch (event_type) {
		case AINPUT_EVENT_TYPE_KEY: {
			...
			io.AddKeyEvent(ImGuiMod_Ctrl,
						   (event_meta_state & AMETA_CTRL_ON) != 0);
			io.AddKeyEvent(ImGuiMod_Shift,
						   (event_meta_state & AMETA_SHIFT_ON) != 0);
			io.AddKeyEvent(ImGuiMod_Alt,
						   (event_meta_state & AMETA_ALT_ON) != 0);
			io.AddKeyEvent(ImGuiMod_Super,
						   (event_meta_state & AMETA_META_ON) != 0);

			switch (event_action) {
				// FIXME: AKEY_EVENT_ACTION_DOWN and AKEY_EVENT_ACTION_UP occur
				// at once as soon as a touch pointer goes up from a key. We use
				// a simple key event queue/ and process one event per key per
				// frame in ImGui_ImplAndroid_NewFrame()...or consider using IO
				// queue, if suitable:
				// https://github.com/ocornut/imgui/issues/2787
				case AKEY_EVENT_ACTION_DOWN:
				case AKEY_EVENT_ACTION_UP: {
					ImGuiKey key =
						ImGui_ImplAndroid_KeyCodeToImGuiKey(event_key_code);
					if (key != ImGuiKey_None) {
						io.AddKeyEvent(key,
									   event_action == AKEY_EVENT_ACTION_DOWN);
						io.SetKeyEventNativeData(key, event_key_code,
												 event_scan_code);
					}
		...
		case AINPUT_EVENT_TYPE_MOTION: {
			...
			switch (event_action) {
				...
				case AMOTION_EVENT_ACTION_UP: {
					// Physical mouse buttons (and probably other physical
					// devices) also invoke the actions
					// AMOTION_EVENT_ACTION_DOWN/_UP, but we have to process
					// them separately to identify the actual button pressed.
					// This is done below via
					// AMOTION_EVENT_ACTION_BUTTON_PRESS/_RELEASE. Here, we only
					// process "FINGER" input (and "UNKNOWN", as a fallback).
					int tool_type = AMotionEvent_getToolType(
						input_event, event_pointer_index);
					if (tool_type == AMOTION_EVENT_TOOL_TYPE_FINGER ||
						...
						io.AddMousePosEvent(
							...
							AMotionEvent_getY(input_event,
											  event_pointer_index));
						io.AddMouseButtonEvent(
							0, event_action == AMOTION_EVENT_ACTION_DOWN);
					}
					break;
				}
				...
				case AMOTION_EVENT_ACTION_BUTTON_RELEASE: {
					int32_t button_state =
						AMotionEvent_getButtonState(input_event);
					io.AddMouseButtonEvent(
						0, (button_state & AMOTION_EVENT_BUTTON_PRIMARY) != 0);
					io.AddMouseButtonEvent(
						1,
						(button_state & AMOTION_EVENT_BUTTON_SECONDARY) != 0);
					io.AddMouseButtonEvent(
						2, (button_state & AMOTION_EVENT_BUTTON_TERTIARY) != 0);
					break;
				}
				case AMOTION_EVENT_ACTION_HOVER_MOVE:  // Hovering: Tool moves
													   // while NOT pressed
													   // (such as a physical
													   // mouse)
				case AMOTION_EVENT_ACTION_MOVE:	 // Touch pointer moves while
												 // DOWN
					io.AddMousePosEvent(
						AMotionEvent_getX(input_event, event_pointer_index),
				...
				case AMOTION_EVENT_ACTION_SCROLL:
					io.AddMouseWheelEvent(
						AMotionEvent_getAxisValue(input_event,
