...
//  platform_io.ClearRendererHandlers() on shutdown. 2025-07-15: OpenGL: Set
//  GL_UNPACK_ALIGNMENT to 1 before updating textures. (#8802) 2025-06-11:
//  OpenGL: Added support for ImGuiBackendFlags_RendererHasTextures, for dynamic
//  font atlas. Removed ImGui_ImplOpenGL2_CreateFontsTexture() and
//  ImGui_ImplOpenGL2_DestroyFontsTexture(). 2024-10-07: OpenGL: Changed default
//  texture sampler to Clamp instead of Repeat/Wrap. 2024-06-28: OpenGL:
//  ImGui_ImplOpenGL2_NewFrame() recreates font texture if it has been destroyed
//  by ImGui_ImplOpenGL2_DestroyFontsTexture(). (#7748) 2022-10-11: Using
//  'nullptr' instead of 'NULL' as per our switch to C++11. 2021-12-08: OpenGL:
//  Fixed mishandling of the ImDrawCmd::IdxOffset field! This is an old bug but
//  it never had an effect until some internal rendering changes in 1.86.
...
//  bd->XXXX). 2021-05-19: OpenGL: Replaced direct access to
//  ImDrawCmd::TextureId with a call to ImDrawCmd::GetTexID(). (will become a
//  requirement) 2021-01-03: OpenGL: Backup, setup and restore GL_SHADE_MODEL
//  state, disable GL_STENCIL_TEST and disable GL_NORMAL_ARRAY client state to
//  increase compatibility with legacy OpenGL applications. 2020-01-23: OpenGL:
//  Backup, setup and restore GL_TEXTURE_ENV to increase compatibility with
//  legacy OpenGL applications. 2019-04-30: OpenGL: Added support for special
//  ImDrawCallback_ResetRenderState callback to reset render state. 2019-02-11:
...
//  2018-06-08: Misc: Extracted imgui_impl_opengl2.cpp/.h away from the old
...
//  callback and exposed ImGui_ImplOpenGL2_RenderDrawData() in the .h file so
...
static void ImGui_ImplOpenGL2_SetupRenderState(ImDrawData* draw_data,
	...
	// GL_ONE_MINUS_SRC_ALPHA); // In order to composite our output buffer we
	// need to preserve alpha
		...
		for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
			...
			if (pcmd->UserCallback) {
				...
				if (pcmd->UserCallback == ImDrawCallback_ResetRenderState)
					ImGui_ImplOpenGL2_SetupRenderState(draw_data, fb_width,
													   fb_height);
				else
