...
// ImGui_ImplVulkan_LoadFunctions().) (When using Volk from directory outside
// your include directories list you can specify full path to the volk.h header,
//  for example when using Volk from VulkanSDK and using
//  include_directories(${Vulkan_INCLUDE_DIRS})' from 'find_package(Vulkan
//  REQUIRED)')
...
struct ImGui_ImplVulkan_PipelineInfo {
	...
	VkImageUsageFlags
		SwapChainImageUsage;  // Extra flags for vkCreateSwapchainKHR() calls
							  // for secondary viewports. We automatically add
							  // VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT. You can
							  // add e.g. VK_IMAGE_USAGE_TRANSFER_SRC_BIT if you
							  // need to capture from viewports.
};
...
struct ImGui_ImplVulkan_InitInfo {
	...
	ImGui_ImplVulkan_PipelineInfo
		PipelineInfoForViewports;  // Infos for Secondary Viewports (created by
								   // backend)
...
// ImGui_ImplVulkan_Init() if possible (== RenderPass xor (UseDynamicRendering
// && PipelineRenderingCreateInfo->sType ==
// VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR)) Else, the pipeline can
...
IMGUI_IMPL_API ImGui_ImplVulkanH_Window*
ImGui_ImplVulkanH_GetWindowDataFromViewport(
	ImGuiViewport* viewport);  // Access to Vulkan objects associated with a
							   // viewport (e.g to export a screenshot)
...
struct ImGui_ImplVulkanH_Window {
	...
	uint32_t
		SemaphoreIndex;	 // Current set of swapchain wait semaphores we're using
						 // (needs to be distinct from per frame data)
