...
// Note that SDL_Renderer is an _optional_ component of SDL3, which IMHO is now
// largely obsolete. For a multi-platform app consider using other technologies:
// - SDL3+SDL_GPU: SDL_GPU is SDL3 new graphics abstraction API.
// - SDL3+DirectX, SDL3+OpenGL, SDL3+Vulkan: combine SDL with dedicated
// renderers. If your application wants to render any non trivial amount of
// graphics other than UI, please be aware that SDL_Renderer currently offers a
// limited graphic API to the end-user and it might be difficult to step out of
// those boundaries.

// Implemented features:
//  [X] Renderer: User texture binding. Use 'SDL_Texture*' as texture
...
// only build the backends you need. Learn about Dear ImGui:
...
//  2025-06-11: Added support for ImGuiBackendFlags_RendererHasTextures, for
//  dynamic font atlas. Removed ImGui_ImplSDLRenderer3_CreateFontsTexture() and
//  ImGui_ImplSDLRenderer3_DestroyFontsTexture(). 2025-01-18: Use
//  endian-dependent RGBA32 texture format, to match SDL_Color. 2024-10-09:
//  Expose selected render state in ImGui_ImplSDLRenderer3_RenderState, which
//  you can access in 'void* platform_io.Renderer_RenderState' during draw
//  callbacks. 2024-07-01: Update for SDL3 api changes: SDL_RenderGeometryRaw()
//  uint32 version was removed (SDL#9009). 2024-05-14: *BREAKING CHANGE*
//  ImGui_ImplSDLRenderer3_RenderDrawData() requires SDL_Renderer* passed as
//  parameter. 2024-02-12: Amend to query SDL_RenderViewportSet() and restore
//  viewport accordingly. 2023-05-30: Initial version.
...
static ImGui_ImplSDLRenderer3_Data* ImGui_ImplSDLRenderer3_GetBackendData() {
	return ImGui::GetCurrentContext()
			   ? (ImGui_ImplSDLRenderer3_Data*)ImGui::GetIO()
...
static int SDL_RenderGeometryRaw8BitColor(
	SDL_Renderer* renderer, ImVector<SDL_FColor>& colors_out,
	...
	return SDL_RenderGeometryRaw(renderer, texture, xy, xy_stride, color3,
								 sizeof(*color3), uv, uv_stride, num_vertices,
...
void ImGui_ImplSDLRenderer3_RenderDrawData(ImDrawData* draw_data,
		...
		for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
			...
			if (pcmd->UserCallback) {
				...
				SDL_RenderGeometryRaw8BitColor(
					renderer, bd->ColorBuffer, tex, xy, (int)sizeof(ImDrawVert),
	...
	SDL_SetRenderViewport(renderer,
						  old.ViewportEnabled ? &old.Viewport : nullptr);
...
void ImGui_ImplSDLRenderer3_UpdateTexture(ImTextureData* tex) {
	...
	if (tex->Status == ImTextureStatus_WantCreate) {
		...
		SDL_Texture* sdl_texture = SDL_CreateTexture(
			bd->Renderer, SDL_PIXELFORMAT_RGBA32, SDL_TEXTUREACCESS_STATIC,
			tex->Width, tex->Height);
		IM_ASSERT(sdl_texture != nullptr &&
				  "Backend failed to create texture!");
		SDL_UpdateTexture(sdl_texture, nullptr, tex->GetPixels(),
						  tex->GetPitch());
		...
		for (ImTextureRect& r : tex->Updates) {
			...
			SDL_UpdateTexture(sdl_texture, &sdl_r, tex->GetPixelsAt(r.x, r.y),
							  tex->GetPitch());
		}
		tex->SetStatus(ImTextureStatus_OK);
