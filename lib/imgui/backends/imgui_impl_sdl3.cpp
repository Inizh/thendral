...
//  2026-01-15: Changed GetClipboardText() handler to return nullptr on error
//  aka clipboard contents is not text. Consistent with other backends. (#9168)
//  2025-11-05: Fixed an issue with missing characters events when an already
//  active text field changes viewports. (#9054) 2025-10-22: Fixed
//  Platform_OpenInShellFn() return value (unused in core). 2025-09-24: Skip
//  using the SDL_GetGlobalMouseState() state when one of our window is hovered,
//  as the SDL_EVENT_MOUSE_MOTION data is reliable. Fix macOS notch mouse
//  coordinates issue in fullscreen mode + better perf on X11. (#7919, #7786)
//  2025-09-18: Call platform_io.ClearPlatformHandlers() on shutdown.
//  2025-09-15: Use SDL_GetWindowDisplayScale() on Mac to output
...
//  SDL_StartTextInput() again if already active. (#8727) 2025-04-22: IME: honor
...
//  #3650) 2025-02-24: Avoid calling SDL_GetGlobalMouseState() when mouse is in
//  relative mode. 2025-02-18: Added ImGuiMouseCursor_Wait and
//  ImGuiMouseCursor_Progress mouse cursor support. 2025-02-10: Using
//  SDL_OpenURL() in platform_io.Platform_OpenInShellFn handler. 2025-01-20:
//  Made ImGui_ImplSDL3_SetGamepadMode(ImGui_ImplSDL3_GamepadMode_Manual) accept
//  an empty array. 2024-10-24: Emscripten: SDL_EVENT_MOUSE_WHEEL event doesn't
//  require dividing by 100.0f on Emscripten. 2024-09-03: Update for SDL3 api
//  changes: SDL_GetGamepads() memory ownership revert. (#7918, #7898, #7807)
...
bool ImGui_ImplSDL3_ProcessEvent(const SDL_Event* event) {
	...
	switch (event->type) {
		...
		case SDL_EVENT_KEY_UP: {
			if (ImGui_ImplSDL3_GetViewportForWindowID(event->key.windowID) ==
				nullptr)
				return false;
			ImGui_ImplSDL3_UpdateKeyModifiers((SDL_Keymod)event->key.mod);
			// IMGUI_DEBUG_LOG("SDL_EVENT_KEY_%s : key=%d ('%s'), scancode=%d
			// ('%s'), mod=%X\n",
