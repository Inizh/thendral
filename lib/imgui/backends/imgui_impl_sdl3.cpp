...
// This needs to be used along with a Renderer (e.g. SDL_GPU, DirectX11,
// OpenGL3, Vulkan..) (Info: SDL3 is a cross-platform general purpose library
// for handling windows, inputs, graphics context creation, etc.)
...
//  ImGuiPlatformIO interface. 2026-01-15: Changed GetClipboardText() handler to
//  return nullptr on error aka clipboard contents is not text. Consistent with
//  other backends. (#9168) 2025-11-05: Fixed an issue with missing characters
//  events when an already active text field changes viewports. (#9054)
//  2025-10-22: Fixed Platform_OpenInShellFn() return value (unused in core).
//  2025-09-24: Skip using the SDL_GetGlobalMouseState() state when one of our
//  window is hovered, as the SDL_EVENT_MOUSE_MOTION data is reliable. Fix macOS
//  notch mouse coordinates issue in fullscreen mode + better perf on X11.
//  (#7919, #7786) 2025-09-18: Call platform_io.ClearPlatformHandlers() on
//  shutdown. 2025-09-15: Use SDL_GetWindowDisplayScale() on Mac to output
//  DisplayFrameBufferScale. The function is more reliable during resolution
//  changes e.g. going fullscreen. (#8703, #4414) 2025-06-27: IME: avoid calling
//  SDL_StartTextInput() again if already active. (#8727) 2025-05-15: [Docking]
//  Add Platform_GetWindowFramebufferScale() handler, to allow varying Retina
//  display density on multiple monitors. 2025-05-06: [Docking] macOS: fixed
//  secondary viewports not appearing on other monitors before of parenting.
//  2025-04-09: [Docking] Revert update monitors and work areas information
//  every frame. Only do it on Windows. (#8415, #8558) 2025-04-22: IME: honor
//  ImGuiPlatformImeData->WantTextInput as an alternative way to call
//  SDL_StartTextInput(), without IME being necessarily visible. 2025-04-09:
//  Don't attempt to call SDL_CaptureMouse() on drivers where we don't call
//  SDL_GetGlobalMouseState(). (#8561) 2025-03-30: Update for SDL3 api changes:
//  Revert SDL_GetClipboardText() memory ownership change. (#8530, #7801)
//  2025-03-21: Fill gamepad inputs and set ImGuiBackendFlags_HasGamepad
//  regardless of ImGuiConfigFlags_NavEnableGamepad being set. 2025-03-10: When
...
//  2025-02-26: Only start SDL_CaptureMouse() when mouse is being dragged, to
//  mitigate issues with e.g.Linux debuggers not claiming capture back. (#6410,
//  #3650) 2025-02-25: [Docking] Revert to use SDL_GetDisplayBounds() for
//  WorkPos/WorkRect if SDL_GetDisplayUsableBounds() failed. 2025-02-24: Avoid
//  calling SDL_GetGlobalMouseState() when mouse is in relative mode.
...
//  2025-02-18: Added ImGuiMouseCursor_Wait and ImGuiMouseCursor_Progress mouse
//  cursor support. 2025-02-10: Using SDL_OpenURL() in
//  platform_io.Platform_OpenInShellFn handler. 2025-01-20: Made
//  ImGui_ImplSDL3_SetGamepadMode(ImGui_ImplSDL3_GamepadMode_Manual) accept an
//  empty array. 2024-10-24: Emscripten: SDL_EVENT_MOUSE_WHEEL event doesn't
//  require dividing by 100.0f on Emscripten. 2024-09-11: (Docking) Added
//  support for viewport->ParentViewportId field to support parenting at OS
//  level. (#7973) 2024-09-03: Update for SDL3 api changes: SDL_GetGamepads()
//  memory ownership revert. (#7918, #7898, #7807) 2024-08-22: moved some
//  OS/backend related function pointers from ImGuiIO to ImGuiPlatformIO:
...
static void ImGui_ImplSDL3_InitMultiViewportSupport(SDL_Window* window,
													void* sdl_gl_context);
...
static ImGuiViewport* ImGui_ImplSDL3_GetViewportForWindowID(
...
// We discard viewport passed via ImGuiPlatformImeData and always call
// SDL_StartTextInput() on SDL_GetKeyboardFocus().
static void ImGui_ImplSDL3_UpdateIme() {
	ImGui_ImplSDL3_Data* bd = ImGui_ImplSDL3_GetBackendData();
	...
	if (data->WantVisible) {
		...
		if (ImGuiViewport* viewport =
				ImGui_ImplSDL3_GetViewportForWindowID(SDL_GetWindowID(window)))
...
// undocumented)
ImGuiKey ImGui_ImplSDL3_KeyEventToImGuiKey(SDL_Keycode keycode,
...
bool ImGui_ImplSDL3_ProcessEvent(const SDL_Event* event) {
	...
	switch (event->type) {
		case SDL_EVENT_MOUSE_MOTION: {
			if (ImGui_ImplSDL3_GetViewportForWindowID(event->motion.windowID) ==
				nullptr)
				return false;
		...
		case SDL_EVENT_MOUSE_WHEEL: {
			if (ImGui_ImplSDL3_GetViewportForWindowID(event->wheel.windowID) ==
				nullptr)
				return false;
		...
		case SDL_EVENT_MOUSE_BUTTON_UP: {
			if (ImGui_ImplSDL3_GetViewportForWindowID(event->button.windowID) ==
				nullptr)
				return false;
			int mouse_button = -1;
		...
		case SDL_EVENT_TEXT_INPUT: {
			if (ImGui_ImplSDL3_GetViewportForWindowID(event->text.windowID) ==
				nullptr)
				return false;
		...
		case SDL_EVENT_KEY_UP: {
			ImGuiViewport* viewport =
				ImGui_ImplSDL3_GetViewportForWindowID(event->key.windowID);
			if (viewport == nullptr) return false;
			// IMGUI_DEBUG_LOG("SDL_EVENT_KEY_%s : key=0x%08X ('%s'),
			// scancode=%d ('%s'), mod=%X, windowID=%d, viewport=%08X\n",
			...
			io.SetKeyEventNativeData(
				...
				(int)event->key
					.scancode);	 // To support legacy indexing (<1.87 user
		...
		case SDL_EVENT_WINDOW_MOUSE_ENTER: {
			if (ImGui_ImplSDL3_GetViewportForWindowID(event->window.windowID) ==
				nullptr)
				return false;
		...
		case SDL_EVENT_WINDOW_FOCUS_LOST: {
			ImGuiViewport* viewport =
				ImGui_ImplSDL3_GetViewportForWindowID(event->window.windowID);
			if (viewport == nullptr) return false;
			// IMGUI_DEBUG_LOG("%s: windowId %d, viewport: %08X\n", (event->type
			// == SDL_EVENT_WINDOW_FOCUS_GAINED) ?
			// "SDL_EVENT_WINDOW_FOCUS_GAINED" : "SDL_WINDOWEVENT_FOCUS_LOST",
...
static void ImGui_ImplSDL3_SetupPlatformHandles(ImGuiViewport* viewport,
												SDL_Window* window) {
...
static bool ImGui_ImplSDL3_Init(SDL_Window* window, SDL_Renderer* renderer,
	...
	snprintf(bd->BackendPlatformName, sizeof(bd->BackendPlatformName),
			 "imgui_impl_sdl3 (%d.%d.%d; %d.%d.%d)", SDL_MAJOR_VERSION,
			 SDL_MINOR_VERSION, SDL_MICRO_VERSION,
			 SDL_VERSIONNUM_MAJOR(ver_linked), SDL_VERSIONNUM_MINOR(ver_linked),
	...
	// (ImGuiBackendFlags_PlatformHasViewports and
	// ImGuiBackendFlags_HasParentViewport may be set just below)
	// (ImGuiBackendFlags_HasMouseHoveredViewport is set dynamically in our
	// _NewFrame function)
	...
	const char* capture_and_global_state_whitelist[] = {"windows", "cocoa",
														"x11", "DIVE", "VMAN"};
	...
	if (bd->MouseCanUseGlobalState) {
		...
		io.BackendFlags |=
			ImGuiBackendFlags_HasParentViewport;  // We can honor
												  ...
												  // by applying the
												  // corresponding parent/child
												  // relationship at platform
												  // level (optional)
	}
	...
	bd->MouseCursors[ImGuiMouseCursor_Wait] =
...
bool ImGui_ImplSDL3_InitForVulkan(SDL_Window* window) {
	if (!ImGui_ImplSDL3_Init(window, nullptr, nullptr)) return false;
	ImGui_ImplSDL3_Data* bd = ImGui_ImplSDL3_GetBackendData();
	bd->UseVulkan = true;
	return true;
}
...
bool ImGui_ImplSDL3_InitForSDLRenderer(SDL_Window* window,
									   SDL_Renderer* renderer) {
...
static void ImGui_ImplSDL3_UpdateMouseData() {
...
#if SDL_HAS_CAPTURE_AND_GLOBAL_MOUSE
	// - SDL_CaptureMouse() let the OS know e.g. that our drags can extend
	// outside of parent boundaries (we want updated position) and shouldn't
	// trigger other operations outside.
	// - Debuggers under Linux tends to leave captured mouse on break, which may
	// be very inconvenient, so to mitigate the issue we wait until mouse has
	// moved to begin capture.
	if (bd->MouseCanUseCapture) {
		bool want_capture = false;
		for (int button_n = 0;
			 button_n < ImGuiMouseButton_COUNT && !want_capture; button_n++)
	...
	const bool is_app_focused =
		(focused_window && (bd->Window == focused_window ||
							ImGui_ImplSDL3_GetViewportForWindowID(
								SDL_GetWindowID(focused_window)) != NULL));
#else
	...
	if (is_app_focused) {
		...
		// (Optional) Fallback to provide unclamped mouse position when focused
		// but not hovered (SDL_EVENT_MOUSE_MOTION already provides this when
		// hovered or captured) Note that SDL_GetGlobalMouseState() is in theory
		// slow on X11, but this only runs on rather specific cases. If a
		// problem we may provide a way to opt-out this feature.
		SDL_Window* hovered_window = SDL_GetMouseFocus();
		const bool is_relative_mouse_mode =
			SDL_GetWindowRelativeMouseMode(bd->Window);
		if (hovered_window == nullptr && bd->MouseCanUseGlobalState &&
			bd->MouseButtonsDown == 0 && !is_relative_mouse_mode) {
...
void ImGui_ImplSDL3_SetGamepadMode(ImGui_ImplSDL3_GamepadMode mode,
	...
	if (mode == ImGui_ImplSDL3_GamepadMode_Manual) {
		IM_ASSERT(manual_gamepads_array != nullptr ||
...
static void ImGui_ImplSDL3_UpdateGamepadButton(ImGui_ImplSDL3_Data* bd,
...
static void ImGui_ImplSDL3_UpdateGamepadAnalog(ImGui_ImplSDL3_Data* bd,
											   ImGuiIO& io, ImGuiKey key,
											   SDL_GamepadAxis axis_no,
											   float v0, float v1) {
	float merged_value = 0.0f;
	for (SDL_Gamepad* gamepad : bd->Gamepads) {
		float vn = Saturate((float)(SDL_GetGamepadAxis(gamepad, axis_no) - v0) /
							(float)(v1 - v0));
...
static void ImGui_ImplSDL3_UpdateGamepads() {
	...
	const int thumb_dead_zone =
		8000;  // SDL_gamepad.h suggests using this value.
	ImGui_ImplSDL3_UpdateGamepadButton(bd, io, ImGuiKey_GamepadStart,
									   SDL_GAMEPAD_BUTTON_START);
	ImGui_ImplSDL3_UpdateGamepadButton(bd, io, ImGuiKey_GamepadBack,
	...
	ImGui_ImplSDL3_UpdateGamepadButton(bd, io, ImGuiKey_GamepadDpadUp,
	...
	ImGui_ImplSDL3_UpdateGamepadAnalog(
		bd, io, ImGuiKey_GamepadL2, SDL_GAMEPAD_AXIS_LEFT_TRIGGER, 0.0f, 32767);
	ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadR2,
									   SDL_GAMEPAD_AXIS_RIGHT_TRIGGER, 0.0f,
									   32767);
	ImGui_ImplSDL3_UpdateGamepadButton(bd, io, ImGuiKey_GamepadL3,
									   SDL_GAMEPAD_BUTTON_LEFT_STICK);
	ImGui_ImplSDL3_UpdateGamepadButton(bd, io, ImGuiKey_GamepadR3,
									   SDL_GAMEPAD_BUTTON_RIGHT_STICK);
	ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadLStickLeft,
									   SDL_GAMEPAD_AXIS_LEFTX, -thumb_dead_zone,
									   -32768);
	ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadLStickRight,
									   SDL_GAMEPAD_AXIS_LEFTX, +thumb_dead_zone,
									   +32767);
	ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadLStickUp,
									   SDL_GAMEPAD_AXIS_LEFTY, -thumb_dead_zone,
									   -32768);
	ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadLStickDown,
									   SDL_GAMEPAD_AXIS_LEFTY, +thumb_dead_zone,
									   +32767);
	ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadRStickLeft,
									   SDL_GAMEPAD_AXIS_RIGHTX,
									   -thumb_dead_zone, -32768);
	ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadRStickRight,
									   SDL_GAMEPAD_AXIS_RIGHTX,
									   +thumb_dead_zone, +32767);
	ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadRStickUp,
...
static void ImGui_ImplSDL3_UpdateMonitors() {
	...
	for (int n = 0; n < display_count; n++) {
		...
		monitor.DpiScale = SDL_GetDisplayContentScale(
			display_id);  // See https://wiki.libsdl.org/SDL3/README-highdpi for
						  // details.
...
static void ImGui_ImplSDL3_GetWindowSizeAndFramebufferScale(
	SDL_Window* window, ImVec2* out_size, ImVec2* out_framebuffer_scale) {
	int w, h;
	SDL_GetWindowSize(window, &w, &h);
	if (SDL_GetWindowFlags(window) & SDL_WINDOW_MINIMIZED) w = h = 0;

#if defined(__APPLE__)
	float fb_scale_x = SDL_GetWindowDisplayScale(
		window);  // Seems more reliable during resolution change (#8703)
...
void ImGui_ImplSDL3_NewFrame() {
	...
	bd->WantUpdateMonitors =
		true;  // Keep polling under Windows to handle changes of work area when
			   // resizing task-bar (#8415)
#endif
	if (bd->WantUpdateMonitors) ImGui_ImplSDL3_UpdateMonitors();
...
struct ImGui_ImplSDL3_ViewportData {
	...
	Uint32 WindowID;  // Stored in ImGuiViewport::PlatformHandle. Use
					  // SDL_GetWindowFromID() to get SDL_Window* from Uint32
					  // WindowID.
	bool WindowOwned;
...
static SDL_Window* ImGui_ImplSDL3_GetSDLWindowFromViewport(
	ImGuiViewport* viewport) {
	if (viewport != nullptr) {
		SDL_WindowID window_id =
...
static void ImGui_ImplSDL3_CreateWindow(ImGuiViewport* viewport) {
	...
	vd->ParentWindow =
		ImGui_ImplSDL3_GetSDLWindowFromViewport(viewport->ParentViewport);
	...
	sdl_flags |= use_opengl ? SDL_WINDOW_OPENGL
...
#ifndef __APPLE__  // On Mac, SDL3 Parenting appears to prevent viewport from
				   // appearing in another monitor
...
static void ImGui_ImplSDL3_ShowWindow(ImGuiViewport* viewport) {
	ImGui_ImplSDL3_ViewportData* vd =
		(ImGui_ImplSDL3_ViewportData*)viewport->PlatformUserData;
#if defined(_WIN32) &&                                                       \
	...
	SDL_SetHint(SDL_HINT_WINDOW_ACTIVATE_WHEN_SHOWN,
				"1");  // Otherwise new window appear under
#else
	SDL_SetHint(
...
static ImVec2 ImGui_ImplSDL3_GetWindowFramebufferScale(
	ImGuiViewport* viewport) {
	ImGui_ImplSDL3_ViewportData* vd =
		(ImGui_ImplSDL3_ViewportData*)viewport->PlatformUserData;
	ImVec2 framebuffer_scale;
	ImGui_ImplSDL3_GetWindowSizeAndFramebufferScale(vd->Window, nullptr,
													&framebuffer_scale);
	return framebuffer_scale;
}

static void ImGui_ImplSDL3_SetWindowTitle(ImGuiViewport* viewport,
										  const char* title) {
	ImGui_ImplSDL3_ViewportData* vd =
		(ImGui_ImplSDL3_ViewportData*)viewport->PlatformUserData;
...
static void ImGui_ImplSDL3_SetWindowAlpha(ImGuiViewport* viewport,
										  float alpha) {
	ImGui_ImplSDL3_ViewportData* vd =
		(ImGui_ImplSDL3_ViewportData*)viewport->PlatformUserData;
...
static int ImGui_ImplSDL3_CreateVkSurface(ImGuiViewport* viewport,
										  ImU64 vk_instance,
	...
	(void)vk_allocator;
	bool ret =
		SDL_Vulkan_CreateSurface(vd->Window, (VkInstance)vk_instance,
...
static void ImGui_ImplSDL3_InitMultiViewportSupport(SDL_Window* window,
	...
	platform_io.Platform_GetWindowFramebufferScale =
		ImGui_ImplSDL3_GetWindowFramebufferScale;
