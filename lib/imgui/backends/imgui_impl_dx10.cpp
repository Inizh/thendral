...
//  ImGuiPlatformIO interface. 2026-01-19: DirectX11: Added 'SamplerNearest' in
//  ImGui_ImplDX11_RenderState. Renamed 'SamplerDefault' to 'SamplerLinear'.
//  2025-09-18: Call platform_io.ClearRendererHandlers() on shutdown.
//  2025-06-11: DirectX10: Added support for
//  ImGuiBackendFlags_RendererHasTextures, for dynamic font atlas. 2025-05-07:
//  DirectX10: Honor draw_data->FramebufferScale to allow for custom backends
//  and experiment using it (consistently with other renderer backends, even
//  though in normal condition it is not set under Windows). 2025-01-06:
//  DirectX10: Expose selected render state in ImGui_ImplDX10_RenderState, which
//  you can access in 'void* platform_io.Renderer_RenderState' during draw
//  callbacks. 2024-10-07: DirectX10: Changed default texture sampler to Clamp
//  instead of Repeat/Wrap. 2022-10-11: Using 'nullptr' instead of 'NULL' as per
//  our switch to C++11. 2021-06-29: Reorganized backend to pull data from a
//  single structure to facilitate usage with multiple-contexts (all g_XXXX
//  access changed to bd->XXXX). 2021-05-19: DirectX10: Replaced direct access
//  to ImDrawCmd::TextureId with a call to ImDrawCmd::GetTexID(). (will become a
...
static void ImGui_ImplDX10_DestroyTexture(ImTextureData* tex) {
	if (ImGui_ImplDX10_Texture* backend_tex =
			(ImGui_ImplDX10_Texture*)tex->BackendUserData) {
		IM_ASSERT(backend_tex->pTextureView ==
				  (ID3D10ShaderResourceView*)(intptr_t)tex->TexID);
...
void ImGui_ImplDX10_UpdateTexture(ImTextureData* tex) {
	...
	if (tex->Status == ImTextureStatus_WantCreate) {
		...
		bd->pd3dDevice->CreateTexture2D(&desc, &subResource,
										&backend_tex->pTexture);
		IM_ASSERT(backend_tex->pTexture != nullptr &&
		...
		bd->pd3dDevice->CreateShaderResourceView(
			backend_tex->pTexture, &srv_desc, &backend_tex->pTextureView);
		IM_ASSERT(backend_tex->pTextureView != nullptr &&
				  "Backend failed to create texture!");
		...
		// region.
		ImGui_ImplDX10_Texture* backend_tex =
			(ImGui_ImplDX10_Texture*)tex->BackendUserData;
		IM_ASSERT(backend_tex->pTextureView ==
				  (ID3D10ShaderResourceView*)(intptr_t)tex->TexID);
		for (ImTextureRect& r : tex->Updates) {
			...
			bd->pd3dDevice->UpdateSubresource(backend_tex->pTexture, 0, &box,
...
bool ImGui_ImplDX10_CreateDeviceObjects() {
	...
	// Create the vertex shader
	{
		...
		D3D10_INPUT_ELEMENT_DESC local_layout[] = {
			{"POSITION", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
			 (UINT)offsetof(ImDrawVert, pos), D3D10_INPUT_PER_VERTEX_DATA, 0},
			{"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
			 (UINT)offsetof(ImDrawVert, uv), D3D10_INPUT_PER_VERTEX_DATA, 0},
			{"COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0,
			 (UINT)offsetof(ImDrawVert, col), D3D10_INPUT_PER_VERTEX_DATA, 0},
		};
	...
	// Create the pixel shader
	{
		...
		if (bd->pd3dDevice->CreatePixelShader(
				...
				pixelShaderBlob->GetBufferSize(), &bd->pPixelShader) != S_OK) {
...
static void ImGui_ImplDX10_CreateWindow(ImGuiViewport* viewport) {
	...
	bd->pFactory->MakeWindowAssociation(
...
static void ImGui_ImplDX10_DestroyWindow(ImGuiViewport* viewport) {
	...
	if (ImGui_ImplDX10_ViewportData* vd =
		...
		if (vd->RTView) vd->RTView->Release();
		vd->RTView = nullptr;
		IM_DELETE(vd);
	}
...
static void ImGui_ImplDX10_SwapBuffers(ImGuiViewport* viewport, void*) {
	ImGui_ImplDX10_ViewportData* vd =
		(ImGui_ImplDX10_ViewportData*)viewport->RendererUserData;
	if (vd->SwapChain) vd->SwapChain->Present(0, 0);  // Present without vsync
}
