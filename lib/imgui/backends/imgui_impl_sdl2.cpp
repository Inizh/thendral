...
//  ImGuiConfigFlags_NoMouseCursorChange'. [X] Platform: Basic IME support. App
//  needs to call 'SDL_SetHint(SDL_HINT_IME_SHOW_UI, "1");' before
//  SDL_CreateWindow()!. [X] Platform: Multi-viewport support (multiple
...
//  events (at least under Windows). [ ] Platform: Multi-viewport: Missing
...
//  other backends. (#9168) 2025-09-24: Skip using the SDL_GetGlobalMouseState()
//  state when one of our window is hovered, as the SDL_MOUSEMOTION data is
//  reliable. Fix macOS notch mouse coordinates issue in fullscreen mode +
//  better perf on X11. (#7919, #7786) 2025-09-18: Call
//  platform_io.ClearPlatformHandlers() on shutdown. 2025-09-15: Content Scales
//  are always reported as 1.0 on Wayland. (#8921) 2025-07-08: Made
//  ImGui_ImplSDL2_GetContentScaleForWindow(),
//  ImGui_ImplSDL2_GetContentScaleForDisplay() helpers return 1.0f on Emscripten
//  and Android platforms, matching macOS logic. (#8742, #8733) 2025-06-11:
//  Added ImGui_ImplSDL2_GetContentScaleForWindow(SDL_Window* window) and
//  ImGui_ImplSDL2_GetContentScaleForDisplay(int display_index) helper to
//  facilitate making DPI-aware apps. 2025-05-15: [Docking] Add
//  Platform_GetWindowFramebufferScale() handler, to allow varying Retina
//  display density on multiple monitors. 2025-04-09: [Docking] Revert update
//  monitors and work areas information every frame. Only do it on Windows.
//  (#8415, #8558) 2025-04-09: Don't attempt to call SDL_CaptureMouse() on
//  drivers where we don't call SDL_GetGlobalMouseState(). (#8561) 2025-03-21:
//  Fill gamepad inputs and set ImGuiBackendFlags_HasGamepad regardless of
//  ImGuiConfigFlags_NavEnableGamepad being set. 2025-03-10: When dealing with
//  OEM keys, use scancodes instead of translated keycodes to choose ImGuiKey
//  values. (#7136, #7201, #7206, #7306, #7670, #7672, #8468) 2025-02-26: Only
//  start SDL_CaptureMouse() when mouse is being dragged, to mitigate issues
//  with e.g.Linux debuggers not claiming capture back. (#6410, #3650)
//  2025-02-25: [Docking] Revert to use SDL_GetDisplayBounds() for
...
//  ImGui_ImplSDL2_SetGamepadMode(ImGui_ImplSDL2_GamepadMode_Manual) accept an
//  empty array. 2024-10-24: Emscripten: from SDL 2.30.9, SDL_EVENT_MOUSE_WHEEL
//  event doesn't require dividing by 100.0f. 2024-09-09: use
//  SDL_Vulkan_GetDrawableSize() when available. (#7967, #3190) 2024-08-22:
...
//  2024-08-19: Storing SDL's Uint32 WindowID inside
//  ImGuiViewport::PlatformHandle instead of SDL_Window*. 2024-08-19:
//  ImGui_ImplSDL2_ProcessEvent() now ignores events intended for other SDL
//  windows. (#7853) 2024-07-02: Emscripten: Added io.PlatformOpenInShellFn()
//  handler for Emscripten versions. 2024-07-02: Update for
//  io.SetPlatformImeDataFn() -> io.PlatformSetImeDataFn() renaming in main
//  library. 2024-02-14: Inputs: Handle gamepad disconnection. Added
//  ImGui_ImplSDL2_SetGamepadMode(). 2023-10-05: Inputs: Added support for extra
//  ImGuiKey values: F13 to F24 function keys, app back/forward keys.
...
static void ImGui_ImplSDL2_InitMultiViewportSupport(SDL_Window* window,
													void* sdl_gl_context);
...
static void ImGui_ImplSDL2_PlatformSetImeData(ImGuiContext*,
...
bool ImGui_ImplSDL2_ProcessEvent(const SDL_Event* event) {
	...
	switch (event->type) {
		case SDL_MOUSEMOTION: {
			if (ImGui_ImplSDL2_GetViewportForWindowID(event->motion.windowID) ==
				nullptr)
				return false;
		...
		case SDL_MOUSEWHEEL: {
			if (ImGui_ImplSDL2_GetViewportForWindowID(event->wheel.windowID) ==
				nullptr)
				return false;
		...
		case SDL_MOUSEBUTTONUP: {
			if (ImGui_ImplSDL2_GetViewportForWindowID(event->button.windowID) ==
				nullptr)
				return false;
			int mouse_button = -1;
		...
		case SDL_TEXTINPUT: {
			if (ImGui_ImplSDL2_GetViewportForWindowID(event->text.windowID) ==
				nullptr)
				return false;
		...
		case SDL_KEYUP: {
			ImGuiViewport* viewport =
				ImGui_ImplSDL2_GetViewportForWindowID(event->key.windowID);
			if (viewport == nullptr) return false;
			// IMGUI_DEBUG_LOG("SDL_KEY%s : key=0x%08X ('%s'), scancode=%d
			// ('%s'), mod=%X, windowID=%d, viewport=%08X\n",
			//     (event->type == SDL_KEYDOWN) ? "DOWN" : "UP  ",
			//     event->key.keysym.sym, SDL_GetKeyName(event->key.keysym.sym),
			//     event->key.keysym.scancode,
			//     SDL_GetScancodeName(event->key.keysym.scancode),
			//     event->key.keysym.mod, event->key.windowID, viewport ?
			//     viewport->ID : 0);
		...
		case SDL_DISPLAYEVENT: {
			...
			// so change of DPI/Scaling are not reflected in this event. (SDL3
			// has it)
		...
		case SDL_WINDOWEVENT: {
			ImGuiViewport* viewport =
				ImGui_ImplSDL2_GetViewportForWindowID(event->window.windowID);
			if (viewport == NULL) return false;

			// - When capturing mouse, SDL will send a bunch of conflicting
			// LEAVE/ENTER event on every mouse move, but the final ENTER tends
			// to be right.
			// - However we won't get a correct LEAVE event for a captured
			// window.
			// - In some cases, when detaching a window from main viewport SDL
			// may send SDL_WINDOWEVENT_ENTER one frame too late,
			//   causing SDL_WINDOWEVENT_LEAVE on previous frame to interrupt
			//   drag operation by clear mouse position. This is why we delay
			//   process the SDL_WINDOWEVENT_LEAVE events by one frame. See
			...
			// IMGUI_DEBUG_LOG("SDL_WINDOWEVENT_FOCUS_GAINED: windowId %d,
			// viewport: %08X\n", event->window.windowID, viewport ?
			// viewport->ID : 0); } if (window_event ==
			// SDL_WINDOWEVENT_FOCUS_LOST) {
			// IMGUI_DEBUG_LOG("SDL_WINDOWEVENT_FOCUS_LOST: windowId %d,
			// viewport: %08X\n", event->window.windowID, viewport ?
			// viewport->ID : 0); }
			if (window_event == SDL_WINDOWEVENT_FOCUS_GAINED)
...
static bool ImGui_ImplSDL2_Init(SDL_Window* window, SDL_Renderer* renderer,
	...
	snprintf(bd->BackendPlatformName, sizeof(bd->BackendPlatformName),
			 "imgui_impl_sdl2 (%u.%u.%u, %u.%u.%u)", ver_compiled.major,
			 ver_compiled.minor, ver_compiled.patch, ver_runtime.major,
			 ver_runtime.minor, ver_runtime.patch);
	...
	// can ignore SDL_MOUSEBUTTONDOWN events coming right after a
	...
	// only affects secondary windows, if any. For the main window to be
	// affected, your application needs to call this manually before calling
	// SDL_CreateWindow().
	...
	// ImGui_ImplSDL2_InitMultiViewportSupport() outside of #ifdef to avoid
...
static void ImGui_ImplSDL2_UpdateMouseData() {
	...
	if (is_app_focused) {
		...
		// but not hovered (SDL_MOUSEMOTION already provides this when hovered
		// or captured) Note that SDL_GetGlobalMouseState() is in theory slow on
		// X11, but this only runs on rather specific cases. If a problem we may
		// provide a way to opt-out this feature.
	...
	if (io.BackendFlags & ImGuiBackendFlags_HasMouseHoveredViewport) {
		...
		if (ImGuiViewport* mouse_viewport =
				ImGui_ImplSDL2_GetViewportForWindowID(bd->MouseWindowID))
...
float ImGui_ImplSDL2_GetContentScaleForWindow(SDL_Window* window) {
	return ImGui_ImplSDL2_GetContentScaleForDisplay(
...
static void ImGui_ImplSDL2_UpdateGamepadButton(
	ImGui_ImplSDL2_Data* bd, ImGuiIO& io, ImGuiKey key,
...
static void ImGui_ImplSDL2_UpdateGamepads() {
	...
	ImGui_ImplSDL2_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadL2,
									   SDL_CONTROLLER_AXIS_TRIGGERLEFT, 0.0f,
									   32767);
...
static void ImGui_ImplSDL2_GetWindowSizeAndFramebufferScale(
	SDL_Window* window, SDL_Renderer* renderer, ImVec2* out_size,
	...
	if (out_framebuffer_scale != nullptr)
		*out_framebuffer_scale = (w > 0 && h > 0) ? ImVec2((float)display_w / w,
														   (float)display_h / h)
												  : ImVec2(1.0f, 1.0f);
}
...
void ImGui_ImplSDL2_NewFrame() {
	...
	ImGui_ImplSDL2_GetWindowSizeAndFramebufferScale(
		bd->Window, bd->Renderer, &io.DisplaySize, &io.DisplayFramebufferScale);
...
#endif
	if (bd->WantUpdateMonitors) ImGui_ImplSDL2_UpdateMonitors();
...
static void ImGui_ImplSDL2_ShowWindow(ImGuiViewport* viewport) {
	...
	SDL_SetHint(
		...
		(viewport->Flags & ImGuiViewportFlags_NoFocusOnAppearing) ? "1" : "0");
#elif defined(_WIN32)
...
static ImVec2 ImGui_ImplSDL2_GetWindowFramebufferScale(
	ImGuiViewport* viewport) {
	...
	ImGui_ImplSDL2_GetWindowSizeAndFramebufferScale(
		vd->Window, nullptr, nullptr, &framebuffer_scale);
...
static void ImGui_ImplSDL2_InitMultiViewportSupport(SDL_Window* window,
	...
	platform_io.Platform_GetWindowFramebufferScale =
		ImGui_ImplSDL2_GetWindowFramebufferScale;
