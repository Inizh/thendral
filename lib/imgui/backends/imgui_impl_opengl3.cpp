...
//  [X] Renderer: User texture binding. Use 'GLuint' OpenGL texture as texture
...
//  (ImGuiBackendFlags_RendererHasVtxOffset) [Desktop OpenGL only!] [X]
//  Renderer: Texture updates support for dynamic font atlas
//  (ImGuiBackendFlags_RendererHasTextures). [X] Renderer: Multi-viewport
...
//  ImGuiPlatformIO interface. 2025-12-11: OpenGL: Fixed embedded loader
//  multiple init/shutdown cycles broken on some platforms. (#8792, #9112)
//  2025-09-18: Call platform_io.ClearRendererHandlers() on shutdown.
//  2025-07-22: OpenGL: Add and call embedded loader shutdown during
//  ImGui_ImplOpenGL3_Shutdown() to facilitate multiple init/shutdown cycles in
//  same process. (#8792) 2025-07-15: OpenGL: Set GL_UNPACK_ALIGNMENT to 1
//  before updating textures (#8802) + restore non-WebGL/ES update path that
//  doesn't require a CPU-side copy. 2025-06-11: OpenGL: Added support for
//  ImGuiBackendFlags_RendererHasTextures, for dynamic font atlas. Removed
//  ImGui_ImplOpenGL3_CreateFontsTexture() and
//  ImGui_ImplOpenGL3_DestroyFontsTexture(). 2025-06-04: OpenGL: Made GLES 3.20
//  contexts not access GL_CONTEXT_PROFILE_MASK nor GL_PRIMITIVE_RESTART.
//  (#8664) 2025-02-18: OpenGL: Lazily reinitialize embedded GL loader for when
//  calling backend from e.g. other DLL boundaries. (#8406) 2024-10-07: OpenGL:
//  Changed default texture sampler to Clamp instead of Repeat/Wrap. 2024-06-28:
//  OpenGL: ImGui_ImplOpenGL3_NewFrame() recreates font texture if it has been
//  destroyed by ImGui_ImplOpenGL3_DestroyFontsTexture(). (#7748) 2024-05-07:
//  OpenGL: Update loader for Linux to support EGL/GLVND. (#7562) 2024-04-16:
//  OpenGL: Detect ES3 contexts on desktop based on version string, to e.g.
//  avoid calling glPolygonMode() on them. (#7447) 2024-01-09: OpenGL: Update
//  GL3W based imgui_impl_opengl3_loader.h to load "libGL.so" and variants,
//  fixing regression on distros missing a symlink. 2023-11-08: OpenGL: Update
//  GL3W based imgui_impl_opengl3_loader.h to load "libGL.so" instead of
...
//  loader crash when GL_VERSION returns nullptr. (#6154, #4445, #3530)
//  2023-03-06: OpenGL: Fixed restoration of a potentially deleted OpenGL
//  program, by calling glIsProgram(). (#6220, #6224) 2022-11-09: OpenGL:
//  Reverted use of glBufferSubData(), too many corruptions issues + old issues
//  seemingly can't be reproed with Intel drivers nowadays (revert 2021-12-15
//  and 2022-05-23 changes). 2022-10-11: Using 'nullptr' instead of 'NULL' as
//  per our switch to C++11. 2022-09-27: OpenGL: Added ability to '#define
...
//   ../imgui/backends/imgui_impl_opengl3_loader.h --ref
...
bool ImGui_ImplOpenGL3_InitLoader() {
	...
	// (to facilitate handling multiple DLL boundaries and multiple context
	// shutdowns we call this from all main entry points)
...
bool ImGui_ImplOpenGL3_Init(const char* glsl_version) {
	...
	*/
	...
	printf(
		...
		"'%s'\n",
		bd->GlVersion, gl_version_str, bd->GlProfileIsCompat, bd->GlProfileMask,
	...
	IM_ASSERT((int)strlen(glsl_version) + 2 <
			  IM_COUNTOF(bd->GlslVersionString));
	...
	for (const ImDrawList* draw_list : draw_data->CmdLists) {
		...
		if (bd->UseBufferSubData) {
			if (bd->VertexBufferSize < vtx_buffer_size) {
				...
				GL_CALL(glBufferData(GL_ARRAY_BUFFER, bd->VertexBufferSize,
									 nullptr, GL_STREAM_DRAW));
			}
			if (bd->IndexBufferSize < idx_buffer_size) {
				bd->IndexBufferSize = idx_buffer_size;
				GL_CALL(glBufferData(GL_ELEMENT_ARRAY_BUFFER,
									 bd->IndexBufferSize, nullptr,
									 GL_STREAM_DRAW));
			}
...
void ImGui_ImplOpenGL3_UpdateTexture(ImTextureData* tex) {
	...
	if (tex->Status == ImTextureStatus_WantCreate) {
		...
		GL_CALL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,
								GL_CLAMP_TO_EDGE));
		GL_CALL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, tex->Width, tex->Height,
		...
		for (ImTextureRect& r : tex->Updates)
			GL_CALL(glTexSubImage2D(GL_TEXTURE_2D, 0, r.x, r.y, r.w, r.h,
									GL_RGBA, GL_UNSIGNED_BYTE,
									tex->GetPixelsAt(r.x, r.y)));
		GL_CALL(glPixelStorei(GL_UNPACK_ROW_LENGTH, 0));
#else
		// GL ES doesn't have GL_UNPACK_ROW_LENGTH, so we need to (A) copy to a
		// contiguous buffer or (B) upload line by line.
		ImGui_ImplOpenGL3_Data* bd = ImGui_ImplOpenGL3_GetBackendData();
		for (ImTextureRect& r : tex->Updates) {
